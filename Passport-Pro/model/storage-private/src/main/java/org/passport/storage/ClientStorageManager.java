...
import org.passport.storage.client.ClientLookupProvider;
import org.passport.storage.client.ClientStorageProvider;
import org.passport.storage.client.ClientStorageProviderFactory;
import org.passport.storage.client.ClientStorageProviderModel;
import org.passport.utils.ServicesUtils;
...
public class ClientStorageManager implements ClientProvider {
    ...
    public static ClientStorageProvider getStorageProvider(PassportSession session, RealmModel realm, String componentId) {
        ComponentModel model = realm.getComponent(componentId);
        if (model == null) return null;
        ClientStorageProviderModel storageModel = new ClientStorageProviderModel(model);
        ClientStorageProviderFactory factory = (ClientStorageProviderFactory)session.getPassportSessionFactory().getProviderFactory(ClientStorageProvider.class, model.getProviderId());
        if (factory == null) {
    ...
    private static <T> Stream<ClientStorageProviderModel> getStorageProviders(RealmModel realm, PassportSession session, Class<T> type) {
    ...
    public static ClientStorageProvider getStorageProviderInstance(PassportSession session, ClientStorageProviderModel model, ClientStorageProviderFactory factory) {
    ...
    private static ClientStorageProviderFactory getClientStorageProviderFactory(ClientStorageProviderModel model, PassportSession session) {
        return (ClientStorageProviderFactory) session.getPassportSessionFactory()
    ...
    public static boolean hasEnabledStorageProviders(PassportSession session, RealmModel realm, Class<?> type) {
        return getStorageProviders(realm, session, type).anyMatch(ClientStorageProviderModel::isEnabled);
    }


    public ClientStorageManager(PassportSession session, long clientStorageProviderTimeout) {
    ...
    protected Stream<ClientModel> query(PaginatedQuery paginatedQuery, RealmModel realm, Integer firstResult, Integer maxResults) {
        ...
        if (hasEnabledStorageProviders(session, realm, ClientLookupProvider.class)) {
            ...
            /*
              ...
              the {@link org.passport.services.DefaultPassportSessionFactory#getClientStorageProviderTimeout()} property is used.
              Default value is 3000 milliseconds and it's configurable.
              See {@link org.passport.services.DefaultPassportSessionFactory} for details.
             */
