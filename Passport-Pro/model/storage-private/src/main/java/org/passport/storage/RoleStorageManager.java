...
import org.passport.storage.role.RoleLookupProvider;
import org.passport.storage.role.RoleStorageProvider;
import org.passport.storage.role.RoleStorageProviderFactory;
import org.passport.storage.role.RoleStorageProviderModel;
...
public class RoleStorageManager implements RoleProvider {
    ...
    public RoleStorageManager(PassportSession session, long roleStorageProviderTimeout) {
    ...
    public static RoleStorageProvider getStorageProvider(PassportSession session, RealmModel realm, String componentId) {
        ComponentModel model = realm.getComponent(componentId);
        if (model == null) return null;
        RoleStorageProviderModel storageModel = new RoleStorageProviderModel(model);
        RoleStorageProviderFactory factory = (RoleStorageProviderFactory)session.getPassportSessionFactory().getProviderFactory(RoleStorageProvider.class, model.getProviderId());
        if (factory == null) {
    ...
    public static <T> Stream<RoleStorageProviderModel> getStorageProviders(RealmModel realm, PassportSession session, Class<T> type) {
    ...
    private static RoleStorageProviderFactory getRoleStorageProviderFactory(RoleStorageProviderModel model, PassportSession session) {
        return (RoleStorageProviderFactory) session.getPassportSessionFactory()
                .getProviderFactory(RoleStorageProvider.class, model.getProviderId());
    }

    public static RoleStorageProvider getStorageProviderInstance(PassportSession session, RoleStorageProviderModel model, RoleStorageProviderFactory factory) {
     ...
     * the <code>org.passport.services.DefaultPassportSessionFactory#getRoleStorageProviderTimeout()</code> property is used.
     * Default value is 3000 milliseconds and it's configurable.
     * See <code>org.passport.services.DefaultPassportSessionFactory</code> for details.
     */
    @Override
     ...
     * Obtaining roles from an external role storage is time-bounded. In case the external role storage
     * isn't available at least roles from a local storage are returned. For this purpose
     * the <code>org.passport.services.DefaultPassportSessionFactory#getRoleStorageProviderTimeout()} property is used.
     * Default value is 3000 milliseconds and it's configurable.
     * See <code>org.passport.services.DefaultPassportSessionFactory</code> for details.
     */
    @Override
