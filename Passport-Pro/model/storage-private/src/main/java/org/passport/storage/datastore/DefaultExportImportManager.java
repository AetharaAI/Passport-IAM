...
import org.passport.deployment.DeployedConfigurationsManager;
import org.passport.exportimport.ExportAdapter;
import org.passport.exportimport.ExportOptions;
import org.passport.exportimport.util.ExportUtils;
...
import org.passport.migration.migrators.MigrateTo8_0_0;
...
import org.passport.models.ClaimMask;
...
import org.passport.models.WebAuthnPolicyPasswordlessDefaults;
import org.passport.models.WebAuthnPolicyTwoFactorDefaults;
import org.passport.models.utils.ComponentUtil;
...
import org.passport.organization.validation.OrganizationsValidation;
import org.passport.partialimport.PartialImportResults;
import org.passport.protocol.oidc.OIDCConfigAttributes;
import org.passport.representations.idm.ApplicationRepresentation;
import org.passport.representations.idm.AuthenticationExecutionExportRepresentation;
import org.passport.representations.idm.AuthenticationFlowRepresentation;
import org.passport.representations.idm.AuthenticatorConfigRepresentation;
import org.passport.representations.idm.ClaimRepresentation;
...
import org.passport.representations.idm.ClientTemplateRepresentation;
...
import org.passport.representations.idm.MemberRepresentation;
import org.passport.representations.idm.MembershipType;
import org.passport.representations.idm.OAuthClientRepresentation;
import org.passport.representations.idm.OrganizationRepresentation;
...
import org.passport.representations.idm.RequiredActionProviderRepresentation;
import org.passport.representations.idm.RoleRepresentation;
import org.passport.representations.idm.ScopeMappingRepresentation;
import org.passport.representations.idm.SocialLinkRepresentation;
import org.passport.representations.idm.UserConsentRepresentation;
import org.passport.representations.idm.UserFederationMapperRepresentation;
import org.passport.representations.idm.UserFederationProviderRepresentation;
import org.passport.representations.idm.UserRepresentation;
import org.passport.storage.ExportImportManager;
import org.passport.storage.ImportRealmFromRepresentationEvent;
import org.passport.storage.PartialImportRealmFromRepresentationEvent;
...
import org.passport.validation.ValidationUtil;

import org.jboss.logging.Logger;

import static org.passport.models.utils.DefaultRequiredActions.getDefaultRequiredActionCaseInsensitively;
import static org.passport.models.utils.ModelToRepresentation.stripRealmAttributesIncludedAsFields;
import static org.passport.models.utils.RepresentationToModel.createCredentials;
import static org.passport.models.utils.RepresentationToModel.createFederatedIdentities;
import static org.passport.models.utils.RepresentationToModel.createGroups;
import static org.passport.models.utils.RepresentationToModel.createRoleMappings;
import static org.passport.models.utils.RepresentationToModel.importGroup;
import static org.passport.models.utils.RepresentationToModel.importRoles;
import static org.passport.models.utils.StripSecretsUtils.stripSecrets;
...
public class DefaultExportImportManager implements ExportImportManager {
    ...
    public static class Batcher implements Consumer<PassportSession> {
        private int count;

        @Override
        public void accept(PassportSession session) {
    ...
    public DefaultExportImportManager(PassportSession session) {
    ...
    public void importRealm(RealmRepresentation rep, RealmModel newRealm, Runnable userImport) {
        ...
        // PASSPORT-7688 Offline Session Max for Offline Token
        ...
        if (rep.getGroups() != null) {
            ...
            if (rep.getDefaultGroups() != null) {
                PassportModelUtils.setDefaultGroups(session, newRealm, rep.getDefaultGroups().stream());
            }
        }
    ...
    private static Map<String, ClientModel> createClients(PassportSession session, ModelVersion version, RealmRepresentation rep, RealmModel realm, Map<String, String> mappedFlows) {
    ...
    private static void importIdentityProviders(RealmRepresentation rep, RealmModel newRealm, PassportSession session) {
    ...
    private static void importIdentityProviderMappers(RealmRepresentation rep, PassportSession session) {
    ...
    private static ScopeContainerModel getScopeContainerHavingScope(RealmModel realm, ScopeMappingRepresentation scope) {
        if (scope.getClient() != null) {
            ...
            ClientScopeModel clientScope = PassportModelUtils.getClientScopeByName(realm, scope.getClientScope());
            if (clientScope == null) {
            ...
            String templateName = PassportModelUtils.convertClientScopeName(scope.getClientTemplate());
            ClientScopeModel clientTemplate = PassportModelUtils.getClientScopeByName(realm, templateName);
    ...
    public static void renameRealm(RealmModel realm, String name) {
        ...
        masterApp.setClientId(PassportModelUtils.getMasterRealmAdminManagementClientId(name));
        realm.setName(name);
    ...
    public void updateRealm(RealmRepresentation rep, RealmModel realm) {
        ...
        if (rep.getDefaultGroups() != null) {
            PassportModelUtils.setDefaultGroups(session, realm, rep.getDefaultGroups().stream());
        }
    ...
    private static void convertDeprecatedApplications(PassportSession session, RealmRepresentation realm) {
    ...
    public static void importUserFederationProvidersAndMappers(PassportSession session, RealmRepresentation rep, RealmModel newRealm) {
        ...
        mapperConvertSet.put(LDAPConstants.LDAP_PROVIDER, "org.passport.storage.ldap.mappers.LDAPStorageMapper");
    ...
    public static Map<String, String> importAuthenticationFlows(PassportSession session, RealmModel newRealm, RealmRepresentation rep) {
        ...
        if (rep.getAuthenticatorConfig() != null) {
            for (AuthenticatorConfigRepresentation configRep : rep.getAuthenticatorConfig()) {
                if (configRep.getAlias() == null) {
                    // this can happen only during import json files from passport 3.4.0 and older
    ...
    private static AuthenticationExecutionModel toModel(PassportSession session, RealmModel realm, AuthenticationFlowModel parentFlow, AuthenticationExecutionExportRepresentation rep) {
    ...
    public static void importRealmAuthorizationSettings(RealmRepresentation rep, RealmModel newRealm, PassportSession session) {
        if (rep.getClients() != null) {
    ...
    public static void importFederatedUser(PassportSession session, RealmModel newRealm, UserRepresentation userRep) {
        ...
        if (userRep.getGroups() != null) {
            for (String path : userRep.getGroups()) {
                GroupModel group = PassportModelUtils.findGroupByPath(session, newRealm, path);
                if (group == null) {
