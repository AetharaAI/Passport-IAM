...
/**
 * Use this to revoke a token, so they will be available even after the restart of Passport.
 * The store can be optimized in a way that entries are only added and are only removed by expiry.
 *
 * The first Passport instance starting up will re-load all expired tokens from it.
 *
