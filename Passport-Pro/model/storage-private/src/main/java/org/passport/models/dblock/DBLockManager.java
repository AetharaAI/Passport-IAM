...
package org.passport.models.dblock;
...
public class DBLockManager {
    ...
    public DBLockManager(PassportSession session) {
        this.session = session;
    }

    public void checkForcedUnlock() {
        if (Boolean.getBoolean("passport.dblock.forceUnlock")) {
    ...
    public DBLockProviderFactory getDBLockFactory() {
        return (DBLockProviderFactory) session.getPassportSessionFactory().getProviderFactory(DBLockProvider.class);
    }
}
