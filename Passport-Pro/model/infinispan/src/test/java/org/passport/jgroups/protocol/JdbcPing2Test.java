package org.passport.jgroups.protocol;
...
import org.passport.infinispan.health.impl.JdbcPingClusterHealthImpl;
...
/**
 * Misc tests for {@link PASSPORT_JDBC_PING2}, running against H2
 * @author Bela Ban
...
public class JdbcPing2Test {
    ...
    static {
        ClassConfigurator.addProtocol((short) 1026, PASSPORT_JDBC_PING2_FOR_TESTING.class);
    }
    ...
    public void testClusterHealth() {
        ...
        assertEquals(PASSPORT_JDBC_PING2.HealthStatus.ERROR, ping.healthStatus());
        // A database exception do not change the cluster health.
        // It relies on Quarkus database health check to mark the Passport instance as not ready.
        ...
        assertEquals(PASSPORT_JDBC_PING2.HealthStatus.NO_COORDINATOR, ping.healthStatus());
        clusterHealth.triggerClusterHealthCheck();
        assertFalse(clusterHealth.isHealthy());

        // test member in the view / single coordinator
        ping.setPingData(List.of(addresses[0]));
        assertEquals(PASSPORT_JDBC_PING2.HealthStatus.HEALTHY, ping.healthStatus());
        clusterHealth.triggerClusterHealthCheck();
        assertTrue(clusterHealth.isHealthy());
        ...
        assertEquals(PASSPORT_JDBC_PING2.HealthStatus.UNHEALTHY, ping.healthStatus());
        clusterHealth.triggerClusterHealthCheck();
        assertFalse(clusterHealth.isHealthy());
