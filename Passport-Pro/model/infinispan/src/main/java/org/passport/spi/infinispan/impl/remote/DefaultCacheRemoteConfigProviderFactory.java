...
package org.passport.spi.infinispan.impl.remote;
...
import static org.passport.connections.infinispan.InfinispanConnectionProvider.skipSessionsCacheIfRequired;
import static org.passport.spi.infinispan.impl.Util.copyFromOption;
...
public class DefaultCacheRemoteConfigProviderFactory implements CacheRemoteConfigProviderFactory, CacheRemoteConfigProvider, EnvironmentDependentProviderFactory {
    ...
    private volatile Config.Scope passportConfiguration;
    ...
    @Override
    public CacheRemoteConfigProvider create(PassportSession session) {
        lazyInit();
        return this;
    }
    ...
    public void init(Config.Scope config) {
        this.passportConfiguration = config;
    }
    ...
    protected ConfigurationBuilder createConfigurationBuilder() throws IOException {
        ...
        builder.clientIntelligence(ClientIntelligence.valueOf(passportConfiguration.get(CLIENT_INTELLIGENCE, CLIENT_INTELLIGENCE_DEFAULT)));
    ...
    private void loadProperties(ConfigurationBuilder builder) throws IOException {
        var path = passportConfiguration.get(PROPERTIES_FILE);
        if (path == null) {
    ...
    private void configureHostname(ConfigurationBuilder builder) {
        var host = passportConfiguration.get(HOSTNAME);
        if (host == null) {
            logger.debug("Hot Rod hostname not configured.");
            return;
        }
        var port = passportConfiguration.getInt(PORT, ConfigurationProperties.DEFAULT_HOTROD_PORT);
    ...
    private void configureConnectionPool(ConfigurationBuilder builder) {
        builder.connectionPool()
                .maxActive(passportConfiguration.getInt(CONNECTION_POOL_MAX_ACTIVE, CONNECTION_POOL_MAX_ACTIVE_DEFAULT))
                .exhaustedAction(ExhaustedAction.valueOf(passportConfiguration.get(CONNECTION_POOL_EXHAUSTED_ACTION, CONNECTION_POOL_EXHAUSTED_ACTION_DEFAULT)));
    }

    private void configureTls(ConfigurationBuilder builder) {
        if (!passportConfiguration.getBoolean(TLS_ENABLED, Boolean.FALSE)) {
            logger.debug("Hot Rod TLS not enabled.");
            return;
        }
        var sniHostName = passportConfiguration.get(TLS_SNI_HOSTNAME);
        if (sniHostName == null) {
            sniHostName = passportConfiguration.get(HOSTNAME);
        }
    ...
    private void configureAuthentication(ConfigurationBuilder builder) {
        var username = passportConfiguration.get(USERNAME);
        var password = passportConfiguration.get(PASSWORD);
        ...
        builder.security().authentication()
                ...
                .username(username)
                .password(password)
                .realm(passportConfiguration.get(AUTH_REALM, AuthenticationConfigurationBuilder.DEFAULT_REALM))
                .saslMechanism(passportConfiguration.get(SASL_MECHANISM, SASL_MECHANISM_DEFAULT));
    }

    private static SSLContext createSSLContext() {
        try {
            // uses the default Java Runtime TrustStore, or the one generated by Passport (see org.passport.truststore.TruststoreBuilder)
    ...
    private void configureRemoteCaches(ConfigurationBuilder builder) {
        var sites = passportConfiguration.getArray(BACKUP_SITES);

        // hijack the embedded cache configuration :)
        var embeddedPassportConfig = Config.scope(CacheEmbeddedConfigProviderSpi.SPI_NAME, DefaultCacheRemoteConfigProviderFactory.PROVIDER_ID);
        skipSessionsCacheIfRequired(Arrays.stream(CLUSTERED_CACHE_NAMES))
                .forEach(name -> {
                    var cacheConfig = CacheConfigurator.getRemoteCacheConfiguration(name, embeddedPassportConfig, sites);
                    if (cacheConfig == null) {
                        return;
                    }
