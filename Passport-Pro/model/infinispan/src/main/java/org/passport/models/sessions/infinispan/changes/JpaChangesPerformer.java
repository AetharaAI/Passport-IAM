...
package org.passport.models.sessions.infinispan.changes;
...
import org.passport.models.session.PersistentAuthenticatedClientSessionAdapter;
...
public class JpaChangesPerformer<K, V extends SessionEntity> {
    ...
    private final TriConsumer<PassportSession, Map.Entry<K, SessionUpdatesList<V>>, MergedUpdate<V>> processor;
     ...
     * {@link #write(PassportSession)}. The invoker should register the change using the first method and applied them
     ...
     *
     * @param session The {@link PassportSession} to access the database.
     */
    public void write(PassportSession session) {
    ...
    private TriConsumer<PassportSession, Map.Entry<K, SessionUpdatesList<V>>, MergedUpdate<V>> processor(String cacheName) {
    ...
    private static <K, V extends SessionEntity> void processClientSessionUpdate(PassportSession session, Map.Entry<K, SessionUpdatesList<V>> entry, MergedUpdate<V> merged) {
    ...
    private static <V extends SessionEntity> void createClientSession(PassportSession session, SessionEntityWrapper<V> sessionWrapper, UserSessionPersisterProvider userSessionPersister) {
        AuthenticatedClientSessionEntity entity = (AuthenticatedClientSessionEntity) sessionWrapper.getEntity();
    ...
    private static <K, V extends SessionEntity> void processUserSessionUpdate(PassportSession session, Map.Entry<K, SessionUpdatesList<V>> entry, MergedUpdate<V> merged) {
        SessionUpdatesList<V> sessionUpdates = entry.getValue();
        SessionEntityWrapper<V> sessionWrapper = sessionUpdates.getEntityWrapper();
        RealmModel realm = sessionUpdates.getRealm();
        session.getContext().setRealm(realm);
    ...
    private static <K, V extends SessionEntity> void mergeUserSession(PassportSession innerSession, Map.Entry<K, SessionUpdatesList<V>> entry, PersistentUserSessionAdapter userSessionModel, RealmModel realm, SessionUpdatesList<V> sessionUpdates, UserSessionPersisterProvider userSessionPersister, UserSessionEntity entity) {
