...
package org.passport.cluster.infinispan.remote;
...
import org.passport.cluster.infinispan.InfinispanClusterProvider;
...
public class RemoteInfinispanClusterProviderFactory implements ClusterProviderFactory, RemoteInfinispanClusterProvider.SharedData, EnvironmentDependentProviderFactory {
    ...
    public ClusterProvider create(PassportSession session) {
        if (workCache == null) {
            // Passport does not ensure postInit() is invoked before create()
            lazyInit(session);
        }
    ...
    private synchronized void lazyInit(PassportSession session) {
        if (workCache != null) {
            return;
        }
        var provider = session.getProvider(InfinispanConnectionProvider.class);
        executor = provider.getExecutor("cluster-provider");
        clusterStartupTime = initClusterStartupTime(provider.getRemoteCache(WORK_CACHE_NAME), (int) (session.getPassportSessionFactory().getServerStartupTimestamp() / 1000));
