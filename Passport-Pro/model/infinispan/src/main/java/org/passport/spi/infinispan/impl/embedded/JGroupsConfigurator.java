...
import org.passport.config.Option;
...
import org.passport.connections.jpa.JpaConnectionProviderFactory;
import org.passport.connections.jpa.util.JpaUtils;
import org.passport.infinispan.util.InfinispanUtils;
import org.passport.jgroups.header.TracerHeader;
import org.passport.jgroups.protocol.PASSPORT_JDBC_PING2;
import org.passport.jgroups.protocol.OPEN_TELEMETRY;
...
import org.passport.storage.configuration.ServerConfigStorageProvider;
...
import static org.passport.config.CachingOptions.CACHE_EMBEDDED_PREFIX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.JBOSS_NODE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.JBOSS_SITE_NAME;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.MACHINE_NAME;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.NODE_NAME;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.PROVIDER_ID;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.RACK_NAME;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.SITE_NAME;
import static org.passport.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.TRACING;
...
/**
 * Utility class to configure JGroups based on the Passport configuration.
 */
public final class JGroupsConfigurator {
    ...
    static {
        // Use custom Passport JDBC_PING implementation that workarounds issue https://issues.redhat.com/browse/JGRP-2870
        ...
        ClassConfigurator.addProtocol((short) 1025, PASSPORT_JDBC_PING2.class);
    ...
    /**
     * Configures JGroups based on the Passport configuration.
     *
     * @param config  The Passport configuration.
     * @param holder  The {@link ConfigurationBuilderHolder} where the transport is configured.
     * @param session The {@link PassportSession} sessions for Database access.
     */
    public static void configureJGroups(Config.Scope config, ConfigurationBuilderHolder holder, PassportSession session) {
        if (isLocal(holder)) {
            return;
        }
    ...
    private static void configureTls(ConfigurationBuilderHolder holder, PassportSession session) {
    ...
    private static void configureDiscovery(ConfigurationBuilderHolder holder, PassportSession session, boolean tracingEnabled) {
        ...
        var connectionFactory = (JpaConnectionProviderFactory) session.getPassportSessionFactory().getProviderFactory(JpaConnectionProvider.class);

        String clusterName = transportOf(holder).attributes().attribute(CLUSTER_NAME).get();

        Address address = Retry.call(ignored -> PassportModelUtils.runJobInTransactionWithResult(session.getPassportSessionFactory(),
     ...
     */
    private static Address prepareJGroupsAddress(PassportSession session, String clusterName) {
        ...
        long mySequence = getNextSequence(session.getPassportSessionFactory(), highestSequence);
    ...
    private static long getNextSequence(PassportSessionFactory sf, long highestSequence) {
        ...
        return PassportModelUtils.runJobInTransactionWithResult(sf, session -> {
    ...
    private static List<ProtocolConfiguration> getProtocolConfigurations(String tableName, boolean udp, boolean tracingEnabled) {
        ...
        list.add(new ProtocolConfiguration(PASSPORT_JDBC_PING2.class.getName(),
              Map.of(
                    // Leave initialize_sql blank as table is already created by Passport
                    "initialize_sql", "",
    ...
    private static class JpaFactoryAwareJGroupsChannelConfigurator extends EmbeddedJGroupsChannelConfigurator {
        ...
        public void afterCreation(Protocol protocol) {
            ...
            if (protocol instanceof PASSPORT_JDBC_PING2 kcPing) {
