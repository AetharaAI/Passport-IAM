...
package org.passport.models.sessions.infinispan.util;
...
public class InfinispanKeyGenerator {
    ...
    public String generateKeyString(PassportSession session, Cache<String, ?> cache) {
        return generateKey(session, cache, new StringKeyGenerator());
    }


    public UUID generateKeyUUID(PassportSession session, Cache<UUID, ?> cache) {
        return generateKey(session, cache, new UUIDKeyGenerator());
    }


    protected <K> K generateKey(PassportSession session, Cache<K, ?> cache, KeyGenerator<K> keyGenerator) {
