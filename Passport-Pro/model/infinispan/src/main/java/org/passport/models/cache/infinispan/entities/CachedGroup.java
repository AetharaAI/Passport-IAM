...
public class CachedGroup extends AbstractRevisioned implements InRealm {
    ...
    public MultivaluedHashMap<String, String> getAttributes(PassportSession session, Supplier<GroupModel> group) {
        return attributes.get(session, group);
    }

    public Set<String> getRoleMappings(PassportSession session, Supplier<GroupModel> group) {
    ...
    public Set<String> getSubGroups(PassportSession session, Supplier<GroupModel> group) {
