 ...
 * <p>
 * Each Passport instance will add itself, periodically, to the remote cache, and it relies on the Hot Rod client
 * listener to keep the membership up to date.
 * <p>
 * During network partitions, it has a probability of two or more Passport instances to be assigned to the same realm.
 * In this scenario, we rely on the database lock to keep data consistent.
 * <p>
 * Passport instances starting and stopping information may not be available in real time, and it is possible some
...
class ConsistentHash {
    ...
    public void onPassportConnected(ClientCacheEntryCreatedEvent<String> event) {
        addPassportNode(event.getKey());
    }
    ...
    public void onMissingHeartbeat(ClientCacheEntryExpiredEvent<String> event) {
        removePassportNode(event.getKey());
    }

    @ClientCacheEntryRemoved
    public void onPassportDisconnect(ClientCacheEntryRemovedEvent<String> event) {
        removePassportNode(event.getKey());
    }

    private void addPassportNode(String uuid) {
        if (uuid.startsWith(MEMBER_KEY_PREFIX)) {
            log.debugf("Adding a passport instance with ID: %s", uuid);
            membership.add(uuid);
        }
    }

    private void removePassportNode(String uuid) {
        if (uuid.startsWith(MEMBER_KEY_PREFIX)) {
            log.debugf("Removing passport instance with ID: %s", uuid);
