...
public class GroupAdapter implements GroupModel {
    ...
    public GroupAdapter(CachedGroup cached, RealmCacheSession cacheSession, PassportSession passportSession, RealmModel realm) {
        this.cached = cached;
    ...
    public String getFirstAttribute(String name) {
        ...
        return cached.getAttributes(passportSession, modelSupplier).getFirst(name);
    }

    @Override
    public Stream<String> getAttributeStream(String name) {
        if (isUpdated()) return updated.getAttributeStream(name);
        List<String> values = cached.getAttributes(passportSession, modelSupplier).get(name);
    ...
    public boolean hasRole(RoleModel role) {
        ...
        if (cached.getRoleMappings(passportSession, modelSupplier).contains(role.getId())) return true;
    ...
    public GroupModel getParent() {
        ...
        return passportSession.groups().getGroupById(realm, cached.getParentId());
    }
    ...
    public Stream<GroupModel> getSubGroupsStream() {
        ...
        for (String id : cached.getSubGroups(passportSession, modelSupplier)) {
            GroupModel subGroup = passportSession.groups().getGroupById(realm, id);
            if (subGroup == null) {
                // chance that role was removed, so just delegate to persistence and get user invalidated
                getDelegateForUpdate();
    ...
    public boolean escapeSlashesInGroupPath() {
        return PassportModelUtils.escapeSlashesInGroupPath(passportSession);
    }
