...
import static org.passport.authorization.fgap.AdminPermissionsSchema.runWithoutAuthorization;

/**
 * Default implementation of {@link DefaultLazyLoader} that only fetches data once. This implementation is thread-safe
 * as cached data is used in instanced of {@link org.passport.models.cache.infinispan.entities.CachedRealm} which are shared
 * between multiple threads within a Passport instance.
 *
...
public class DefaultLazyLoader<S, D> implements LazyLoader<S, D> {
    ...
    @Override
    public D get(PassportSession session, Supplier<S> sourceSupplier) {
        if (data == null) {
            synchronized (this) {
                if (data == null) {
