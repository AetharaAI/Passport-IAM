...
import static org.passport.jgroups.certificates.JGroupsCertificate.fromJson;
import static org.passport.jgroups.certificates.JGroupsCertificate.toJson;

/**
 * A {@link JGroupsCertificateProvider} implementation that stores the certificates in the database.
 * <p>
 * The generated certificate is self-signed, and the database is used to share the certificate amongst the Passport
...
public class DatabaseJGroupsCertificateProvider implements JGroupsCertificateProvider {
    ...
    private DatabaseJGroupsCertificateProvider(PassportSessionFactory sessionFactory, Duration rotationPeriod) {
    ...
    public static DatabaseJGroupsCertificateProvider create(PassportSessionFactory factory, Duration rotationPeriod) {
    ...
    private void init() {
        ...
        var cert = Retry.call(ignored -> PassportModelUtils.runJobInTransactionWithResult(sessionFactory, this::loadOrCreateCertificate), STARTUP_RETRIES, STARTUP_RETRY_SLEEP_MILLIS);
        useCertificate(cert);
    ...
    public void rotateCertificate() {
        PassportModelUtils.runJobInTransaction(sessionFactory, this::replaceCertificateFromDatabase);
    }
    ...
    private void doReload() {
        PassportModelUtils.runJobInTransactionWithResult(sessionFactory, DatabaseJGroupsCertificateProvider::loadCertificateFromDatabase)
    ...
    private static Optional<String> loadCertificateFromDatabase(PassportSession session) {
        return session.getProvider(ServerConfigStorageProvider.class).find(CERTIFICATE_ID);
    }

    private void replaceCertificateFromDatabase(PassportSession session) {
    ...
    private JGroupsCertificate loadOrCreateCertificate(PassportSession session) {
        var storage = session.getProvider(ServerConfigStorageProvider.class);
