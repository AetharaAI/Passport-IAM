 ...
 * <p>
 * It takes advantage of the {@link DistributionManager} to assign Passport instances to distinct {@link RealmModel},
 * allowing a distributed check of expired session through the cluster.
 * <p>
 * A consistent hash is not updated at the same time in all Passport cluster members. It is possible some realms are not
 * checked during an iteration, or the same realms are checked by multiple Passport instances. In the latter case, we rely on
 * database locking.
 */
class DistributionAwareExpirationTask extends BaseExpirationTask implements Predicate<RealmModel> {
    ...
    DistributionAwareExpirationTask(PassportSessionFactory factory, ScheduledExecutorService scheduledExecutorService, int intervalSeconds, Consumer<Duration> onTaskExecuted, DistributionManager distributionManager) {
