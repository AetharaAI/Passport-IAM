...
package org.passport.models.sessions.infinispan.changes.sessions;
...
public abstract class AbstractLastSessionRefreshStore {
    ...
    public void putLastSessionRefresh(PassportSession kcSession, String sessionId, String realmId, int lastSessionRefresh) {
    ...
    void checkSendingMessage(PassportSession kcSession, int currentTime) {
    ...
    /**
     * Bulk update the underlying store with all the user sessions, which were refreshed by Passport since the last call of this method
     *
     * @param kcSession
     * @param refreshesToSend Key is userSession ID, SessionData are data about the session
     */
    protected abstract void sendMessage(PassportSession kcSession, Map<String, SessionData> refreshesToSend);
}
