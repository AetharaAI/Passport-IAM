...
import org.passport.models.sessions.infinispan.changes.ClientSessionPersistentChangelogBasedTransaction;
...
import org.passport.models.sessions.infinispan.changes.PersistentUpdate;
import org.passport.models.sessions.infinispan.changes.UserSessionPersistentChangelogBasedTransaction;
import org.passport.models.sessions.infinispan.changes.sessions.PersisterLastSessionRefreshStore;
import org.passport.models.sessions.infinispan.changes.sessions.PersisterLastSessionRefreshStoreFactory;
...
import org.passport.models.sessions.infinispan.expiration.ExpirationTask;
import org.passport.models.sessions.infinispan.expiration.ExpirationTaskFactory;
import org.passport.models.sessions.infinispan.listeners.EmbeddedUserSessionExpirationListener;
...
public class InfinispanUserSessionProviderFactory implements UserSessionProviderFactory<UserSessionProvider>, ServerInfoAwareProviderFactory, EnvironmentDependentProviderFactory {
    ...
    @Override
    public UserSessionProvider create(PassportSession session) {
    ...
    public void postInit(final PassportSessionFactory factory) {
        factory.register(event -> {
            if (event instanceof PostMigrationEvent) {
                if (useCaches) {
                    PassportModelUtils.runJobInTransaction(factory, (PassportSession session) -> {
                ...
                UserSessionProvider provider1 = userRemovedEvent.getPassportSession().getProvider(UserSessionProvider.class, getId());
        ...
        if (MultiSiteUtils.isPersistentSessionsEnabled()) {
            ...
            try (var session = factory.create()) {
                ...
                expirationListener = new EmbeddedUserSessionExpirationListener(session.getPassportSessionFactory(), blockingManager);
            }
    ...
    public void initializePersisterLastSessionRefreshStore(final PassportSessionFactory sessionFactory) {
        PassportModelUtils.runJobInTransaction(sessionFactory, session -> {
    ...
    public List<ProviderConfigProperty> getConfigMetadata() {
        ...
        builder.property()
                ...
                .type("int")
                .helpText("Override how long offline client sessions should be kept in memory in seconds (deprecated, to be removed in Passport 27)")
                .add();

        builder.property()
                .name(CONFIG_OFFLINE_SESSION_CACHE_ENTRY_LIFESPAN_OVERRIDE)
                .type("int")
                .helpText("Override how long offline user sessions should be kept in memory in seconds (deprecated, to be removed in Passport 27)")
                .add();
    ...
    private VolatileTransactions createVolatileTransaction(PassportSession session) {
    ...
    private PersistentTransaction createPersistentTransaction(PassportSession session) {
