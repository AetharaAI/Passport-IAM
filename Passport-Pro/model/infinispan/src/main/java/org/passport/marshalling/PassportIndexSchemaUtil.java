...
public class PassportIndexSchemaUtil {
    ...
    public static void uploadAndReindexCaches(RemoteCacheManager remoteCacheManager, GeneratedSchema schema, List<IndexedEntity> indexedEntities) {
        ...
        if (stored == null) {
            if (protostreamMetadataCache.putIfAbsent(key, current) == null) {
                ...
                logger.info("Failed to update Infinispan ProtoStream schema. Assumed it was updated by other Passport server.");
            }
    ...
    private static void reindexCaches(RemoteCacheManager remoteCacheManager, String oldSchema, String newSchema, List<IndexedEntity> indexedEntities) {
        ...
        var oldPS = PassportModelSchema.parseProtoSchema(oldSchema);
        var newPS = PassportModelSchema.parseProtoSchema(newSchema);
    ...
    private static boolean isEntityChanged(FileDescriptor oldSchema, FileDescriptor newSchema, String entity) {
        var v1 = PassportModelSchema.findEntity(oldSchema, entity);
        var v2 = PassportModelSchema.findEntity(newSchema, entity);
        return v1.isPresent() && v2.isPresent() && PassportIndexSchemaUtil.isIndexSchemaChanged(v1.get(), v2.get());
    }
