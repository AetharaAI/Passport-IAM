...
import static org.passport.connections.infinispan.InfinispanConnectionProvider.AUTHORIZATION_CACHE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.AUTHORIZATION_REVISIONS_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.AUTHORIZATION_REVISIONS_CACHE_NAME;
...
import static org.passport.connections.infinispan.InfinispanConnectionProvider.CRL_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.CRL_CACHE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.KEYS_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.KEYS_CACHE_MAX_IDLE_SECONDS;
...
import static org.passport.connections.infinispan.InfinispanConnectionProvider.REALM_CACHE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.REALM_REVISIONS_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.REALM_REVISIONS_CACHE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.SESSIONS_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.USER_CACHE_NAME;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.USER_REVISIONS_CACHE_DEFAULT_MAX;
import static org.passport.connections.infinispan.InfinispanConnectionProvider.USER_REVISIONS_CACHE_NAME;
...
public final class CacheConfigurator {
    ...
    /**
     * Configures the Infinispan local caches used by Passport (e.g., for realm or user data) using the provided
     * Passport configuration.
     *
     * @param passportConfig The Passport configuration.
     ...
     *                               configuration.
     */
    public static void configureLocalCaches(Config.Scope passportConfig, ConfigurationBuilderHolder holder) {
        logger.debug("Configuring embedded local caches");
        // configure local caches except revision caches
        configureCacheMaxCount(passportConfig, holder, Arrays.stream(LOCAL_MAX_COUNT_CACHES));
    ...
    public static void validateWorkCacheConfiguration(ConfigurationBuilderHolder holder) {
        ...
        if (!isClustered(holder)) {
            // non-clustered, Passport started in dev mode?
            return;
        }
        var cacheMode = cacheBuilder.clustering().cacheMode();
        if (!cacheMode.isReplicated()) {
            throw new RuntimeException("Unable to start Passport. '%s' cache must be replicated but is %s".formatted(WORK_CACHE_NAME, cacheMode.friendlyCacheModeString().toLowerCase()));
        }
    }
     ...
     *
     * @param passportConfig The Passport configuration, which provides the maximum entry counts for the caches.
     * @param holder         The {@link ConfigurationBuilderHolder} where the caches are configured.
     ...
     */
    public static void configureCacheMaxCount(Config.Scope passportConfig, ConfigurationBuilderHolder holder, Stream<String> caches) {
        for (var it = caches.iterator(); it.hasNext(); ) {
            var name = it.next();
            ...
            var maxCount = passportConfig.getLong(maxCountConfigKey(name));
            if (maxCount != null) {
                if (maxCount < 0) {
     ...
     *                               the {@code holder}. This could indicate a missing or incorrect configuration.
     */
    public static void configureSessionsCachesForPersistentSessions(Config.Scope passportConfig, ConfigurationBuilderHolder holder) {
        ...
        for (var name : CLUSTERED_MAX_COUNT_CACHES) {
            ...
            setMemoryMaxCount(passportConfig, name, builder);
     ...
     *                               the {@code holder}. This could indicate a missing or incorrect configuration.
     */
    public static void configureSessionsCachesForVolatileSessions(Config.Scope passportConfig, ConfigurationBuilderHolder holder) {
     ...
     * input.
     *
     * @param passportConfig The Passport configuration, which provides the number owners value for the caches.
     * @param holder         The {@link ConfigurationBuilderHolder} where the caches are configured.
     */
    public static void configureNumOwners(Config.Scope passportConfig, ConfigurationBuilderHolder holder) {
        for (var name : CLUSTERED_CACHE_NUM_OWNERS) {
            var builder = holder.getNamedConfigurationBuilders().get(name);
            if (builder == null) {
                throw cacheNotFound(name);
            }
            var owners = passportConfig.getInt(numOwnerConfigKey(name));
            if (owners != null) {
    ...
    private static void setMemoryMaxCount(Config.Scope passportConfig, String name, ConfigurationBuilder builder) {
        var maxCount = passportConfig.getLong(maxCountConfigKey(name));
        if (maxCount != null) {
            builder.memory().maxCount(maxCount);
        }
