...
package org.passport.infinispan.health.impl;
...
import org.passport.jgroups.protocol.PASSPORT_JDBC_PING2.HealthStatus;
...
/**
 * A {@link ClusterHealth} implementation that makes use of {@link PASSPORT_JDBC_PING2}.
 * <p>
 * Since each node is registered in the database, it is possible to detect if a partition is happening.
 * <p>
 * The method {@link PASSPORT_JDBC_PING2#healthStatus()} contains the algorithm description. If it returns
 ...
 *
 * @see PASSPORT_JDBC_PING2#healthStatus()
 */
@Scope(Scopes.GLOBAL)
public class JdbcPingClusterHealthImpl implements ClusterHealth {
    ...
    public void inject(Transport transport, BlockingManager blockingManager) {
        ...
        PASSPORT_JDBC_PING2 ping = jgrp.getChannel().getProtocolStack().findProtocol(PASSPORT_JDBC_PING2.class);
        if (ping == null) {
    ...
    public void init(PASSPORT_JDBC_PING2 discovery, Executor executor) {
        runner = new HealthRunner(discovery, executor, this::checkHealth);
    }

    private void checkHealth(PASSPORT_JDBC_PING2 ping) {
        assert ping != null;
        if (!lock.tryLock()) {
            // check in progress
            return;
    ...
    private record HealthRunner(PASSPORT_JDBC_PING2 discovery, Executor executor, Consumer<PASSPORT_JDBC_PING2> check) {
