...
package org.passport.connections.jpa.updater.liquibase.custom;
 ...
 */
public class JpaUpdate1_2_0_Beta1 extends CustomPassportTask {
            ...
            try {
                ...
                while (resultSet.next()) {
                    ...
                    if (!providerInProgress) {
                        ...
                        String internalId = PassportModelUtils.generateId();
    ...
    private String getAdminRoleId() throws SQLException, DatabaseException {
        PreparedStatement statement = jdbcConnection.prepareStatement("select ID from " + getTableName("PASSPORT_ROLE") + " where NAME = ? AND REALM = ?");
    ...
    protected void addNewRealmAdminRoles() throws SQLException, DatabaseException {
        PreparedStatement statement = jdbcConnection.prepareStatement("select CLIENT.ID REALM_ADMIN_APP_ID, CLIENT.REALM_ID REALM_ID, PASSPORT_ROLE.ID ADMIN_ROLE_ID from " +
                getTableName("CLIENT") + " CLIENT," + getTableName("PASSPORT_ROLE") + " PASSPORT_ROLE where PASSPORT_ROLE.APPLICATION = CLIENT.ID AND CLIENT.NAME = 'realm-management' AND PASSPORT_ROLE.NAME = ?");
    ...
    private void addAdminRole(String roleName, String realmId, String applicationId, String realmAdminAppRoleId) {
        String roleTableName = database.correctObjectName("PASSPORT_ROLE", Table.class);
        String compositeRoleTableName = database.correctObjectName("COMPOSITE_ROLE", Table.class);
        String newRoleId = PassportModelUtils.generateId();
            ...
            try {
                ...
                while (resultSet.next()) {
                    ...
                    for (ProtocolMapperRepresentation protocolMapper : protocolMappers) {
                        String mapperId = PassportModelUtils.generateId();
