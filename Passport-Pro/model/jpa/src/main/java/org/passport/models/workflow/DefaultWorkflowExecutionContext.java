...
import org.passport.models.workflow.WorkflowStateProvider.ScheduledStep;
...
final class DefaultWorkflowExecutionContext implements WorkflowExecutionContext {
     ...
     */
    DefaultWorkflowExecutionContext(PassportSession session, Workflow workflow, WorkflowEvent event) {
     ...
     */
    DefaultWorkflowExecutionContext(PassportSession session, Workflow workflow, WorkflowEvent event, String executionId) {
     ...
     */
    DefaultWorkflowExecutionContext(PassportSession session, Workflow workflow, WorkflowEvent event, ScheduledStep step) {
     ...
     * @param step the scheduled step
     */
    DefaultWorkflowExecutionContext(PassportSession session, Workflow workflow, ScheduledStep step) {
        this(session, workflow, null, step.stepId(), step.executionId(), step.resourceId());
    }

    /**
     * A copy constructor that creates a new execution context based on an existing one but bound to a different {@link PassportSession}.

     * @param session the session
     * @param context the existing context
     */
    DefaultWorkflowExecutionContext(PassportSession session, DefaultWorkflowExecutionContext context) {
    ...
    /**
     * A copy constructor that creates a new execution context based on an existing one but bound to a different {@link PassportSession} and the given {@link WorkflowStep}.

     * @param session the session
     * @param context the existing context
     * @param step the current step
     */
    DefaultWorkflowExecutionContext(PassportSession session, DefaultWorkflowExecutionContext context, WorkflowStep step) {
        this(session, context.getWorkflow(), context.getEvent(), step.getId(), context.getExecutionId(), context.getResourceId());
        completed = context.isCompleted();
    }

    private DefaultWorkflowExecutionContext(PassportSession session, Workflow workflow, WorkflowEvent event, String stepId, String executionId, String resourceId) {
