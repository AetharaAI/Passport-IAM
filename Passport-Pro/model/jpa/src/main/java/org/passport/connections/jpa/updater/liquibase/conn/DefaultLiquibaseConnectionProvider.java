...
package org.passport.connections.jpa.updater.liquibase.conn;
...
import org.passport.connections.jpa.updater.liquibase.LiquibaseJpaUpdaterProvider;
...
public class DefaultLiquibaseConnectionProvider implements LiquibaseConnectionProviderFactory, LiquibaseConnectionProvider {
    ...
    public static final String INDEX_CREATION_THRESHOLD_PARAM = "passport.indexCreationThreshold";
    ...
    @Override
    public LiquibaseConnectionProvider create(PassportSession session) {
    ...
    public void init(Config.Scope config) {
        ...
        String liquibaseType = org.passport.config.database.Database.getVendor(dbAlias).orElseThrow().getLiquibaseType();
    ...
    @Override
    public PassportLiquibase getLiquibase(Connection connection, String defaultSchema) throws LiquibaseException {
        ...
        return new PassportLiquibase(changelog, resourceAccessor, database);
    }

    @Override
    public PassportLiquibase getLiquibaseForCustomUpdate(Connection connection, String defaultSchema, String changelogLocation, ClassLoader classloader, String changelogTableName) throws LiquibaseException {
        Database database = getLiquibaseDatabase(connection);
        if (defaultSchema != null) {
            database.setDefaultSchemaName(defaultSchema);
        }
        ...
        return new PassportLiquibase(changelogLocation, resourceAccessor, database);
    }

    // Similarly to Hibernate, we want to enforce Liquibase to use the same DB as configured in Passport
