...
package org.passport.connections.jpa;
...
import org.passport.ServerStartupError;
import org.passport.common.util.StackUtil;
import org.passport.common.util.StringPropertyReplacer;
import org.passport.connections.jpa.support.EntityManagerProxy;
import org.passport.connections.jpa.updater.JpaUpdaterProvider;
...
import static org.passport.connections.jpa.util.JpaUtils.configureNamedQuery;
import static org.passport.connections.jpa.util.JpaUtils.getDatabaseType;
...
public class DefaultJpaConnectionProviderFactory implements JpaConnectionProviderFactory, ServerInfoAwareProviderFactory {
    ...
    private PassportSessionFactory factory;

    @Override
    public JpaConnectionProvider create(PassportSession session) {
    ...
    private EntityManager createEntityManager(PassportSession session, boolean sessionManaged) {
        EntityManager em;
        if (!jtaEnabled) {
            logger.trace("enlisting EntityManager in JpaPassportTransaction");
        ...
        if (!jtaEnabled) {
            session.getTransactionManager().enlist(new JpaPassportTransaction(em));
        }
        return em;
    }

    private void addSpecificNamedQueries(PassportSession session, Connection connection) {
    ...
    protected void checkJtaEnabled(PassportSessionFactory factory) {
            ...
            synchronized (this) {
                if (emf == null) {
                    PassportModelUtils.suspendJtaTransaction(session.getPassportSessionFactory(), () -> {
                        ...
                        String unitName = "passport-default";
    ...
    private File getDatabaseUpdateFile() {
        String databaseUpdateFile = config.get("migrationExport", "passport-database-update.sql");
    ...
    protected void startGlobalStats(PassportSession session, int globalStatsIntervalSecs) {
    ...
    void migration(MigrationStrategy strategy, boolean initializeEmpty, String schema, File databaseUpdateFile, Connection connection, PassportSession session) {
    ...
    protected void update(Connection connection, String schema, PassportSession session, JpaUpdaterProvider updater) {
        PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), new PassportSessionTask() {
            @Override
            public void run(PassportSession lockSession) {
    ...
    protected void export(Connection connection, String schema, File databaseUpdateFile, PassportSession session, JpaUpdaterProvider updater) {
        PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), new PassportSessionTask() {
            @Override
            public void run(PassportSession lockSession) {
                DBLockManager dbLockManager = new DBLockManager(lockSession);
    ...
    public String getSchema() {
        ...
        if (schema != null && schema.contains("-") && ! Boolean.parseBoolean(System.getProperty(GlobalConfiguration.PRESERVE_SCHEMA_CASE.getKey()))) {
            ...
            logger.warnf("The passed schema '%s' contains a dash. Setting liquibase config option PRESERVE_SCHEMA_CASE to true. See https://github.com/passport/passport/issues/20870 for more information.", schema);
        }
        return schema;
    }
    ...
    private void migrateModel(PassportSession session) {
        ...
        try {
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), MigrationModelManager::migrate);
        } finally {
            dbLock.releaseLock();
        }
    }
    ...
    /**
     * Starting with H2 version 2.x, marking "VALUE" as a non-keyword is necessary as some columns are named "VALUE" in the Passport schema.
     * <p />
    ...
    /**
     * For a PostgreSQL cluster, Passport would need to connect to the primary node that is writable.
