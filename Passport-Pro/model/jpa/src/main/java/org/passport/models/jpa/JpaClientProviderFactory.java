...
import org.passport.models.ClientProviderFactory;
...
import static org.passport.models.jpa.JpaRealmProviderFactory.PROVIDER_ID;
import static org.passport.models.jpa.JpaRealmProviderFactory.PROVIDER_PRIORITY;
...
public class JpaClientProviderFactory implements ClientProviderFactory {
    ...
    public void init(Config.Scope config) {
        ...
        if (searchableAttrsArr == null) {
            String s = System.getProperty("passport.client.searchableAttributes");
            ...
            factory.register(event -> {
                if (event instanceof RealmModel.RealmAttributeUpdateEvent attrUpdateEvent) {
                    if (Objects.equals(attrUpdateEvent.getAttributeName(), RealmAttributes.ADMIN_PERMISSIONS_ENABLED) && Boolean.parseBoolean(attrUpdateEvent.getAttributeValue())) {
                        PassportSession passportSession = attrUpdateEvent.getPassportSession();
                        RealmModel realm = attrUpdateEvent.getRealm();
                        AdminPermissionsSchema.SCHEMA.init(passportSession, realm);
                    }
                }
            });
        }
    ...
    @Override
    public ClientProvider create(PassportSession session) {
