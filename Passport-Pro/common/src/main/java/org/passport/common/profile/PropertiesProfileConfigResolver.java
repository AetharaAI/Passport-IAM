...
public class PropertiesProfileConfigResolver implements ProfileConfigResolver {
    ...
    public Profile.ProfileName getProfileName() {
        String profile = getter.apply("passport.profile");

        if (profile != null) {
            try {
                return Profile.ProfileName.valueOf(profile.toUpperCase());
            } catch (IllegalArgumentException e) {
                throw new ProfileException(String.format("Invalid profile '%s' specified via 'passport.profile' property", profile));
            }
        }
        return null;
    }
    ...
    public static String getPropertyKey(String feature) {
        return "passport.profile.feature." + feature.replaceAll("[-:]", "_");
    }
}
