...
public class EventBuilder {
    ...
    public EventBuilder(RealmModel realm, PassportSession session, ClientConnection clientConnection) {
        this(realm, session);
        ipAddress(clientConnection.getRemoteHost());
    }

    public EventBuilder(RealmModel realm, PassportSession session) {
    ...
    private static EventStoreProvider getEventStoreProvider(PassportSession session) {
    ...
    private static List<EventListenerProvider> getEventListeners(PassportSession session, RealmModel realm) {
        HashSet<String> realmListeners = new HashSet<>(realm.getEventsListenersStream().toList());
        List<EventListenerProvider> result = session.getPassportSessionFactory().getProviderFactoriesStream(EventListenerProvider.class)
    ...
    private EventBuilder(PassportSession session, EventStoreProvider store, List<EventListenerProvider> listeners, RealmModel realm, Event event) {
    ...
    private void send(boolean sendImmediately) {
        ...
        if (sendImmediately) {
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), session.getContext(), innerSession -> {
