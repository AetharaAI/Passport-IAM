...
public interface BruteForceProtector extends Provider {
    ...
    boolean isTemporarilyDisabled(PassportSession session, RealmModel realm, UserModel user);

    boolean isPermanentlyLockedOut(PassportSession session, RealmModel realm, UserModel user);
     ...
     * @param user
     */
    void cleanUpPermanentLockout(PassportSession session, RealmModel realm, UserModel user);
}
