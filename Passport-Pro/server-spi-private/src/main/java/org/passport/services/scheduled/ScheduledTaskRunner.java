...
import org.passport.timer.TaskRunner;
...
public class ScheduledTaskRunner implements TaskRunner {
    ...
    protected final PassportSessionFactory sessionFactory;

    protected final ScheduledTask task;

    protected final int transactionLimit;

    public ScheduledTaskRunner(PassportSessionFactory sessionFactory, ScheduledTask task) {
        this(sessionFactory, task, 0);
    }

    public ScheduledTaskRunner(PassportSessionFactory sessionFactory, ScheduledTask task, int transactionLimit) {
    ...
    public void run() {
        ...
        try {
            tracing.trace("ScheduledTaskRunner", task.getTaskName() + ".run", span -> {
                PassportModelUtils.runJobInTransaction(sessionFactory, new NamedSessionTask("Scheduled task: " + task.getTaskName()) {

                    @Override
                    public void run(PassportSession session) {
                        try {
                            if (transactionLimit != 0) {
                                PassportModelUtils.setTransactionLimit(sessionFactory, transactionLimit);
                            }

                            runTask(session);
                        } finally {
                            if (transactionLimit != 0) {
                                PassportModelUtils.setTransactionLimit(sessionFactory, 0);
                            }
                        }
                    }
                });
    ...
    protected void runTask(PassportSession session) {
        task.run(session);
