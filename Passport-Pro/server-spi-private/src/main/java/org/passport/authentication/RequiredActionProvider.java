...
public interface RequiredActionProvider extends Provider {
     ...
     *
     */
    default void initiatedActionCanceled(PassportSession session, AuthenticationSessionModel authSession) {
        return;
    }
    ...
    /**
     * @deprecated in favor of {@link #getMaxAuthAge(PassportSession)} to support individual configuration of max auth age for all required actions. This method has no effect anymore.
     *
     ...
     * On default uses configured max_auth_age value from the required action config. If not configured, it uses the default max_auth_age value from the PassportConstants class.
     */
     ...
     * On default uses configured max_auth_age value from the required action config. If not configured, it uses the default max_auth_age value from the PassportConstants class.
     */
    default int getMaxAuthAge(PassportSession session) {
        if (session == null) {
        ...
        AuthenticationSessionModel authSession = passportContext.getAuthenticationSession();
        if (authSession != null) {
