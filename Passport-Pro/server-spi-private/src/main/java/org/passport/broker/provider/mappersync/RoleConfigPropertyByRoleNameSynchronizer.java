...
package org.passport.broker.provider.mappersync;
...
public class RoleConfigPropertyByRoleNameSynchronizer implements ConfigSynchronizer<RoleModel.RoleNameChangeEvent> {
    ...
    public void handleEvent(RoleModel.RoleNameChangeEvent event) {
        ...
        String currentRoleValue = PassportModelUtils.buildRoleQualifier(event.getClientId(), event.getPreviousName());
        event.getPassportSession().identityProviders().getMappersStream(Map.of(ConfigConstants.ROLE, currentRoleValue), null, null)
                .forEach(idpMapper -> {
                    String newRoleValue = PassportModelUtils.buildRoleQualifier(event.getClientId(), event.getNewName());
                    ...
                    event.getPassportSession().identityProviders().updateMapper(idpMapper);
                });
