...
public class RoleConfigPropertyByClientIdSynchronizer implements ConfigSynchronizer<ClientModel.ClientIdChangeEvent> {
    ...
    public void handleEvent(ClientModel.ClientIdChangeEvent event) {
        ...
        event.getPassportSession().identityProviders().getMappersStream(Map.of(ConfigConstants.ROLE, event.getPreviousClientId() + ".*"), null, null)
                .forEach(idpMapper -> {
                    String currentRoleValue = idpMapper.getConfig().get(ConfigConstants.ROLE);
                    String configuredRoleName = PassportModelUtils.parseRole(currentRoleValue)[1];
                    String newRoleValue = PassportModelUtils.buildRoleQualifier(event.getNewClientId(), configuredRoleName);
