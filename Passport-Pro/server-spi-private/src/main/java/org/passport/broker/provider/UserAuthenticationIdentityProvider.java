...
public interface UserAuthenticationIdentityProvider<C extends IdentityProviderModel> extends IdentityProvider<C> {
    ...
    interface AuthenticationCallback {
         ...
         * Assumption is that authenticationSession is set in the {@link org.passport.models.PassportContext} when this method is called
         *
    ...
    void preprocessFederatedIdentity(PassportSession session, RealmModel realm, BrokeredIdentityContext context);
    void authenticationFinished(AuthenticationSessionModel authSession, BrokeredIdentityContext context);
    void importNewUser(PassportSession session, RealmModel realm, UserModel user, BrokeredIdentityContext context);
    void updateBrokeredUser(PassportSession session, RealmModel realm, UserModel user, BrokeredIdentityContext context);

    /**
     * JAXRS callback endpoint for when the remote IDP wants to callback to passport.
     *
     * @return
     */
     ...
     * @return
     */
    Response retrieveToken(PassportSession session, FederatedIdentityModel identity);

    void backchannelLogout(PassportSession session, UserSessionModel userSession, UriInfo uriInfo, RealmModel realm);

    /**
     * Called when a Passport application initiates a logout through the browser.  This is expected to do a logout
     * with the IDP
     *
     * @param userSession
     * @param uriInfo
     ...
     */
    Response passportInitiatedBrowserLogout(PassportSession session, UserSessionModel userSession, UriInfo uriInfo, RealmModel realm);
