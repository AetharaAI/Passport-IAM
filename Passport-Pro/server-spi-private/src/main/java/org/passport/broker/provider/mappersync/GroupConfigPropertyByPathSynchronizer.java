...
public class GroupConfigPropertyByPathSynchronizer implements ConfigSynchronizer<GroupModel.GroupPathChangeEvent> {
    ...
    public void handleEvent(GroupModel.GroupPathChangeEvent event) {
        ...
        event.getPassportSession().identityProviders().getMappersStream(Map.of(ConfigConstants.GROUP, event.getPreviousPath()), null, null)
                .forEach(idpMapper -> {
        ...
        event.getPassportSession().identityProviders().getMappersStream(
