...
public abstract class OCSPProvider {
     ...
     */
    public OCSPRevocationStatus check(PassportSession session, X509Certificate cert, X509Certificate issuerCertificate, URI responderURI, X509Certificate responderCert, Date date) throws CertPathValidatorException {
        if (cert == null)
     ...
     * @return revocation status
     */
    public OCSPRevocationStatus check(PassportSession session, X509Certificate cert, X509Certificate issuerCertificate, Date date, X509Certificate responderCert) throws CertPathValidatorException {
    ...
    protected byte[] getEncodedOCSPResponse(PassportSession session, byte[] encodedOCSPReq, URI responderUri) throws IOException {
     ...
     * @return revocation status
     */
    public OCSPRevocationStatus check(PassportSession session, X509Certificate cert, X509Certificate issuerCertificate) throws CertPathValidatorException {
     ...
     */
    protected abstract OCSPRevocationStatus check(PassportSession session, X509Certificate cert,
