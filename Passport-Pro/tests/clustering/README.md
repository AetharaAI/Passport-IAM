...
KC_TEST_DATABASE_INTERNAL=true -> configure passport with the internal database container IP instead of localhost (configured by default in clustering module)

Example, 2 node cluster, the first using the distribution archive and the second the nightly image
KC_TEST_DATABASE=postgres KC_TEST_SERVER_IMAGES=-,quay.io/passport/passport:nightly mvn verify -pl tests/clustering/  -Dtest=MixedVersionClusterTest

Using a mixed cluster with 26.2.3 and 26.2.4
KC_TEST_DATABASE=postgres KC_TEST_SERVER_IMAGES=quay.io/passport/passport:26.2.3,quay.io/passport/passport:26.2.4 mvn verify -pl tests/clustering/  -Dtest=MixedVersionClusterTest
