...
import org.passport.testframework.events.EventMatchers;
...
import org.passport.testsuite.util.CredentialBuilder;
 ...
 */
@PassportIntegrationTest(config = ImpersonationTest.ImpersonationTestServerConfig.class)
public class ImpersonationTest {
    ...
    public void testImpersonationWorksWhenAuthenticationSessionExists() throws Exception {
        // Open the URL for the client (will redirect to Passport server AuthorizationEndpoint and create authenticationSession)
        oauth.openLoginForm();
    ...
    private Set<Cookie> testSuccessfulImpersonation(String admin, String adminRealm) {
        // Login adminClient
        try (Passport client = login(admin, adminRealm)) {
            // Impersonate
            return impersonate(client, admin, adminRealm);
        }
    }

    private Set<Cookie> impersonate(Passport adminClient, String admin, String adminRealm) {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().setDefaultCookieStore(cookieStore).build()) {
            ...
            HttpUriRequest req = RequestBuilder.post()
                    .setUri(passportUrls.getBase() + "/admin/realms/test/users/" + managedUser.getId() + "/impersonation")
    ...
    private void testForbiddenImpersonation(String admin, String adminRealm) {
        try (Passport client = createAdminClient(adminRealm, establishClientId(adminRealm), admin)) {
    ...
    private Passport createAdminClient(String realm, String clientId, String username) {
    ...
    private Passport login(String username, String realm) {
        String clientId = establishClientId(realm);
        Passport client = createAdminClient(realm, clientId, username);
    ...
    private Set<Cookie> testSuccessfulServiceAccountImpersonation(UserRepresentation serviceAccount, String serviceAccountRealm) {
        // Login adminClient
        try (Passport client = loginServiceAccount(serviceAccount, serviceAccountRealm)) {
            // Impersonate test-user with service account
            return impersonateServiceAccount(client);
        }
    }

    private Passport loginServiceAccount(UserRepresentation serviceAccount, String serviceAccountRealm) {
        Passport client = createServiceAccountClient(serviceAccountRealm, serviceAccount);
        // get token
        client.tokenManager().getAccessToken();
        return client;
    }

    private Passport createServiceAccountClient(String serviceAccountRealm, UserRepresentation serviceAccount) {
    ...
    private Set<Cookie> impersonateServiceAccount(Passport adminClient) {
        BasicCookieStore cookieStore = new BasicCookieStore();
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().setDefaultCookieStore(cookieStore).build()) {
    ...
    public static class ImpersonationTestServerConfig implements PassportServerConfig {

        @Override
        public PassportServerConfigBuilder configure(PassportServerConfigBuilder server) {
