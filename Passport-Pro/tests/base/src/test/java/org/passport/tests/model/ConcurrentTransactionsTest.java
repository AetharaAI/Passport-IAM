...
public class ConcurrentTransactionsTest {
    ...
    @TestOnServer
    public void persistClient(PassportSession session) {
        final ClientModel[] client = {null};
        AtomicReference<String> clientDBIdAtomic = new AtomicReference<>();
        AtomicReference<Exception> exceptionHolder = new AtomicReference<>();

        try {
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), session.getContext(), (PassportSession sessionSetup) -> {
            ...
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession session1) -> {
                ...
                final PassportSessionFactory sessionFactory = session1.getPassportSessionFactory();
                ...
                Thread thread1 = new Thread(() -> {
                    PassportModelUtils.runJobInTransaction(sessionFactory, session11 -> {
                        try {
                            PassportSession currentSession = session11;
                ...
                Thread thread2 = new Thread(() -> {
                    PassportModelUtils.runJobInTransaction(sessionFactory, session22 -> {
                        try {
                            PassportSession currentSession = session22;
                            // Wait until transaction in both threads started
                            transactionsCounter.countDown();
    ...
    // PASSPORT-3296 , PASSPORT-3494
    @TestOnServer
    public void removeUserAttribute(PassportSession session) throws Exception {

        try {
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession sessionSet) -> {
            ...
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession session2) -> {
                ...
                final PassportSessionFactory sessionFactory = session2.getPassportSessionFactory();
                    ...
                    try {
                        PassportModelUtils.runJobInTransaction(sessionFactory, session1 -> {
                            try {
                                ...
                                // PASSPORT-3296 : Remove user attribute in both threads
                                john.removeAttribute("foo");

                                // PASSPORT-3494 : Set single attribute in both threads
    ...
    private void tearDownRealm(PassportSession session, String realmName) {
