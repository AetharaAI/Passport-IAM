...
package org.passport.tests.admin.authentication;
...
import org.passport.authentication.forms.RegistrationRecaptcha;
import org.passport.authentication.forms.RegistrationRecaptchaEnterprise;
...
import org.passport.tests.utils.KerberosUtils;
...
public class ProvidersTest extends AbstractAuthenticationTest {
    ...
    public void testAuthenticatorConfigDescription() {
        ...
        Assertions.assertEquals("Detect if there is existing Passport account with same email like identity provider. If no, create new user", infoRep.getHelpText());
        ...
        Assert.assertProviderConfigProperty(infoRep.getProperties().get(0), "require.password.update.after.registration", "Require Password Update After Registration",
                null, "If this option is true and new user is successfully imported from Identity Provider to Passport (there is no duplicated email or username detected in Passport DB), then this user is required to update his password",
                "boolean");
    }
    ...
    private List<Map<String, Object>> expectedAuthProviders() {
        ...
        addProviderInfo(result, "idp-create-user-if-unique", "Create User If Unique", "Detect if there is existing Passport account " +
                "with same email like identity provider. If no, create new user");
        addProviderInfo(result, "idp-email-verification", "Verify existing account by Email", "Email verification of existing Passport " +
        ...
        addProviderInfo(result, "idp-detect-existing-broker-user", "Detect existing broker user",
                "Detect if there is an existing Passport account with same email like identity provider. If no, throw an error.");
