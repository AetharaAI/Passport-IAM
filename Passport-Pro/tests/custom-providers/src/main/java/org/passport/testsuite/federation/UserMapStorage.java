...
import org.passport.models.credential.PasswordUserCredentialModel;
...
import org.passport.storage.adapter.AbstractUserAdapterFederatedStorage;
import org.passport.storage.federated.UserGroupMembershipFederatedStorage;
...
public class UserMapStorage implements UserLookupProvider, UserStorageProvider, UserRegistrationProvider, CredentialInputUpdater,
    ...
    public UserMapStorage(PassportSession session, ComponentModel model, Map<String, String> userPasswords, ConcurrentMap<String, Set<String>> userGroups) {
    ...
    public boolean removeUser(RealmModel realm, UserModel user) {
        if (editMode == EditMode.READ_ONLY || editMode == EditMode.UNSYNCED) {
            log.warnf("User '%s' can't be deleted in LDAP as editMode is '%s'. Deleting user just from Passport DB, but he will be re-imported from LDAP again once searched in Passport", user.getUsername(), editMode.toString());
    ...
    public boolean removeUserByName(String userName) {
        if (editMode == EditMode.READ_ONLY || editMode == EditMode.UNSYNCED) {
            log.warnf("User '%s' can't be deleted in LDAP as editMode is '%s'. Deleting user just from Passport DB, but he will be re-imported from LDAP again once searched in Passport", userName, editMode.toString());
