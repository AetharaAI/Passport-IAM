...
public abstract class AbstractSessionCacheCommand extends AbstractCommand {
    ...
    @Override
    protected void doRunCommand(PassportSession session) {
    ...
    protected abstract void doRunCacheCommand(PassportSession session, Cache<String, SessionEntityWrapper> cache);
    ...
    public static class PutCommand extends AbstractSessionCacheCommand {
        ...
        @Override
        protected void doRunCacheCommand(PassportSession session, Cache<String, SessionEntityWrapper> cache) {
    ...
    public static class CreateManySessionsCommand extends AbstractSessionCacheCommand {
        ...
        protected void doRunCacheCommand(PassportSession session, Cache<String, SessionEntityWrapper> cache) {
            ...
            BatchTaskRunner.runInBatches(0, count, batchCount, session.getPassportSessionFactory(), (PassportSession batchSession, int firstInIteration, int countInIteration) -> {
                for (int i=0 ; i<countInIteration ; i++) {
                    UserSessionEntity userSession = new UserSessionEntity(PassportModelUtils.generateId());
