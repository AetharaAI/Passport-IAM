...
public class LoadPersistentSessionsCommand extends AbstractCommand {
    ...
    private static class MyWorker implements Runnable {
        ...
        public MyWorker(int workerId, String lastSessionId, int limit, PassportSessionFactory sessionFactory) {
        ...
        @Override
        public void run() {
            PassportModelUtils.runJobInTransaction(sessionFactory, (passportSession) -> {
                int offset = workerId * limit;

                UserSessionPersisterProvider persister = passportSession.getProvider(UserSessionPersisterProvider.class);
