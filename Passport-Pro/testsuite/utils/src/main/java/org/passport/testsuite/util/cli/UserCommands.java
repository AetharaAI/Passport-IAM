...
public class UserCommands {
    ...
    public static class Create extends AbstractCommand {
        ...
        protected void doRunCommand(PassportSession session) {
            ...
            BatchTaskRunner.runInBatches(first, count, batchCount, session.getPassportSessionFactory(), this::createUsersInBatch);

            log.infof("Command finished. All users from %s to %s created", usernamePrefix + first, usernamePrefix + (first + count - 1));
        }

        private void createUsersInBatch(PassportSession session, int first, int count) {
            ...
            for (int counter = first; counter < last; counter++) {
                ...
                user.setEmail(username + "@passport-pro.ai");
