...
public class GroupCommands {
    ...
    public static class Create extends AbstractCommand {
        ...
        protected void doRunCommand(PassportSession session) {
            ...
            BatchTaskRunner.runInBatches(first, count, batchCount, session.getPassportSessionFactory(), this::createGroupsInBatch);

            log.infof("Command finished. All groups from %s to %s created", groupPrefix + first, groupPrefix
                    + (first + count - 1));
        }

        private void createGroupsInBatch(PassportSession session, int first, int count) {
