...
import org.passport.utils.JsonConfigProvider;
...
public class JsonConfigProviderFactory implements ConfigProviderFactory {
    ...
    public Optional<Config.ConfigProvider> create() {
        ...
        try {
            ...
            if (configDir != null) {
                File f = new File(configDir + File.separator + "passport-server.json");
                if (f.isFile()) {
            ...
            if (node == null) {
                URL resource = Thread.currentThread().getContextClassLoader().getResource("META-INF/passport-server.json");
                if (resource != null) {
