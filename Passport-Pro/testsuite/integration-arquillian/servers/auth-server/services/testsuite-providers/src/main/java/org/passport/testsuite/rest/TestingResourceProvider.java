...
import org.passport.common.enums.HostnameVerificationPolicy;
import org.passport.common.profile.PropertiesProfileConfigResolver;
import org.passport.common.util.HtmlUtils;
...
import org.passport.events.EventQuery;
import org.passport.events.EventStoreProvider;
import org.passport.events.EventType;
import org.passport.events.admin.AdminEvent;
import org.passport.events.admin.AdminEventQuery;
...
import org.passport.protocol.oid4vc.issuance.OID4VCIssuerWellKnownProvider;
...
import org.passport.storage.datastore.PeriodicEventInvalidation;
import org.passport.testsuite.components.amphibian.TestAmphibianProvider;
import org.passport.testsuite.events.TestEventsListenerProvider;
import org.passport.testsuite.federation.DummyUserFederationProviderFactory;
import org.passport.testsuite.forms.PassThroughAuthenticator;
import org.passport.testsuite.forms.PassThroughClientAuthenticator;
import org.passport.testsuite.model.infinispan.InfinispanTestUtil;
import org.passport.testsuite.rest.representation.AuthenticatorState;
import org.passport.testsuite.rest.resource.TestCacheResource;
import org.passport.testsuite.rest.resource.TestLDAPResource;
import org.passport.testsuite.rest.resource.TestingExportImportResource;
import org.passport.testsuite.runonserver.FetchOnServer;
import org.passport.testsuite.runonserver.RunOnServer;
import org.passport.testsuite.runonserver.SerializationUtil;
import org.passport.testsuite.util.FeatureDeployerUtil;
import org.passport.timer.TimerProvider;
import org.passport.truststore.FileTruststoreProvider;
import org.passport.truststore.FileTruststoreProviderFactory;
...
public class TestingResourceProvider implements RealmResourceProvider {
    ...
    public TestingResourceProvider(PassportSession session, TestingResourceProviderFactory factory, Map<String, TimerProvider.TimerTaskContext> suspendedTimerTasks) {
    ...
    public UserRepresentation getUserByUsernameFromFedProviderFactory(@QueryParam("realmName") String realmName,
                                                                      @QueryParam("userName") String userName) {
        RealmModel realm = getRealmByName(realmName);
        DummyUserFederationProviderFactory factory = (DummyUserFederationProviderFactory) session.getPassportSessionFactory().getProviderFactory(UserStorageProvider.class, "dummy");
    ...
    public String runModelTestOnServer(@QueryParam("testClassName") String testClassName,
        ...
        try {
            ...
            Method testMethod = testClass.getDeclaredMethod(testMethodName, PassportSession.class);
    ...
    public void reinitializeProviderFactoryWithSystemPropertiesScope(@QueryParam("provider-type") String providerType, @QueryParam("provider-id") String providerId,
        ...
        ProviderFactory<?> factory = session.getPassportSessionFactory().getProviderFactory(providerClass, providerId);
    ...
    public void disableTruststoreSpi() {
        FileTruststoreProviderFactory factory = (FileTruststoreProviderFactory) session.getPassportSessionFactory().getProviderFactory(TruststoreProvider.class);
    ...
    public void modifyTruststoreSpiHostnamePolicy(@QueryParam("hostnamePolicy") final HostnameVerificationPolicy hostnamePolicy) {
        FileTruststoreProviderFactory fact = (FileTruststoreProviderFactory) session.getPassportSessionFactory().getProviderFactory(TruststoreProvider.class);
    ...
    public void addEventsToEmailEventListenerProvider(List<EventType> events) {
        if (events != null && !events.isEmpty()) {
            EmailEventListenerProviderFactory prov = (EmailEventListenerProviderFactory) session.getPassportSessionFactory()
