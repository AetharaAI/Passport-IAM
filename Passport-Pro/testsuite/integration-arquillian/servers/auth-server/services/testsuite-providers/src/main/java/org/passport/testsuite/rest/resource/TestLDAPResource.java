...
package org.passport.testsuite.rest.resource;
...
public class TestLDAPResource {
    ...
    public TestLDAPResource(PassportSession session, RealmModel realm) {
    ...
    public void prepareGroupsLDAPTest() {
        ...
        // Sync LDAP groups to Passport DB
        ...
        realm.addDefaultGroup(PassportModelUtils.findGroupByPath(session, realm, "/defaultGroup1/defaultGroup11"));
        realm.addDefaultGroup(PassportModelUtils.findGroupByPath(session, realm, "/defaultGroup1/defaultGroup12"));

        // Delete all LDAP users
        LDAPTestUtils.removeAllLDAPUsers(ldapFedProvider, realm);

        // Add some LDAP users for testing
        LDAPObject john = LDAPTestUtils.addLDAPUser(ldapFedProvider, realm, "johnpassport", "John", "Doe", "john@email.org", null, "1234");
        LDAPTestUtils.updateLDAPPassword(ldapFedProvider, john, "Password1");

        LDAPObject mary = LDAPTestUtils.addLDAPUser(ldapFedProvider, realm, "marypassport", "Mary", "Kelly", "mary@email.org", null, "5678");
        LDAPTestUtils.updateLDAPPassword(ldapFedProvider, mary, "Password1");

        LDAPObject rob = LDAPTestUtils.addLDAPUser(ldapFedProvider, realm, "robpassport", "Rob", "Brown", "rob@email.org", null, "8910");
        LDAPTestUtils.updateLDAPPassword(ldapFedProvider, rob, "Password1");

        LDAPObject james = LDAPTestUtils.addLDAPUser(ldapFedProvider, realm, "jamespassport", "James", "Brown", "james@email.org", null, "8910");
    ...
    public void prepareRolesLDAPTest() {
        ...
        // Sync LDAP groups to Passport DB roles
        ComponentModel mapperModel = LDAPTestUtils.getSubcomponentByName(realm, ldapModel, "rolesMapper");
        new RoleLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(realm);
    }
