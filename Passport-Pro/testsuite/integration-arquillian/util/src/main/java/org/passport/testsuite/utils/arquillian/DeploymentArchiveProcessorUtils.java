 ...
 */
package org.passport.testsuite.utils.arquillian;
...
import static org.passport.testsuite.util.ServerURLs.getAppServerContextRoot;
...
import static org.passport.testsuite.utils.io.IOUtil.modifyDocElementValue;
    ...
    public static void modifyOIDCAdapterConfig(Archive<?> archive, String adapterConfigPath) {
        try {
            ...
            if (AUTH_SERVER_SSL_REQUIRED) {
                String trustStorePathInDeployment = "passport.truststore";
                if (adapterConfigPath.contains("WEB-INF")) {
                    // This is a Java adapter, we can use classpath
                    trustStorePathInDeployment = "classpath:passport.truststore";
                }
                adapterConfig.setTruststore(trustStorePathInDeployment);
                adapterConfig.setTruststorePassword(TRUSTSTORE_PASSWORD);

                String truststoreUrl = System.getProperty("dependency.keystore.root", "") + "/passport.truststore";
                File truststore = new File(truststoreUrl);

                if (!truststore.exists()) {
                    truststore = new File(DeploymentArchiveProcessorUtils.class.getResource("/keystore/passport.truststore").getFile());
                }
