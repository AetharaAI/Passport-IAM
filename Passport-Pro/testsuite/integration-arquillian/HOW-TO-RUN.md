...
### Running tests in the development mode (Passport on embedded undertow)
...
By default, the development setup is used with the Passport server deployed on
...
### Running tests in the production mode (Passport on Quarkus)

For the "production" testing, it is possible to run the Passport server deployed on real Quarkus server.
...
For test driven development, it is possible to run the Passport server deployed on real Quarkus server.
...
For debugging, the best is to run the test from IDE and debug it directly. When you use embedded Undertow (which is by default), then JUnit test, Passport server
...
It is configured in `testsuite/integration-arquillian/tests/base/src/test/resources/logging.properties` . You can see that logging of testsuite itself (category `org.passport.testsuite`) is DEBUG by default.

When you run tests with undertow (which is by default), there is logging for Passport server and adapter (category `org.passport` ) in `INFO` when you run tests from IDE, but `OFF` when
you run tests with maven. The reason is that, we don't want huge logs when running mvn build. However using system property `passport.logging.level` will override it. This can be used for both IDE or maven.
So for example using `-Dpassport.logging.level=DEBUG` will enable debug logging for passport server and adapter.
...
When using Passport on Wildfly/EAP, there is INFO logging level enabled by default for most of the java packages.
...
* `-Dorg.passport.services.scheduled=DEBUG` - for package `org.passport.services.scheduled`
...
Run tests with SAML applications deployed on Wildfly:
    ...
    mvn -f testsuite/integration-arquillian/tests/base/pom.xml \
       clean install \
       -Papp-server-wildfly \
       -Dtest=org.passport.testsuite.adapter.**
...
- Start Passport old version 19.0.3.
- Import realm and add some data to the database.
- Stop Passport 19.0.3.
- Start latest Passport, which automatically updates DB from 19.0.3.
...
The first version of Passport on Quarkus is version `17.0.0`, but the initial versions have a complete different set of boot options that make co-existence impossible.
Therefore the first version that can be tested is `19.0.3`.
You can execute those tests as follows:
```
export OLD_PASSPORT_VERSION=19.0.3
...
mvn -B -f testsuite/integration-arquillian/pom.xml \
  ...
  -Dmigrated.auth.server.version=$OLD_PASSPORT_VERSION \
  -Dmigration.import.file.name=migration-realm-$OLD_PASSPORT_VERSION.json \
  -Dauth.server.ssl.required=false \
  -Dauth.server.db.host=localhost
```

The `DATABASE` variable can be: `mariadb`, `mysql`, `postgres`, `mssql` or `oracle`.
As commented `OLD_PASSPORT_VERSION` can only be `19.0.3` right now.

For the available versions of old passport server, you can take a look to [this directory](tests/base/src/test/resources/migration-test) .

### DB migration test with manual mode

Same test as above, but it uses manual migration mode. During startup of the new Passport server, Liquibase won't automatically perform DB update, but it
just exports the needed SQL into the script. This SQL script then needs to be manually executed against the DB.
Then there is another startup of the new Passport server against the DB, which already has SQL changes applied and
...
## Disabling features
Some features in Passport can be disabled. To run the testsuite with a specific feature disabled use the `auth.server.feature` system property. For example to run the tests with authorization disabled run:
```
mvn -f testsuite/integration-arquillian/tests/base/pom.xml clean test -Pauth-server-wildfly -Dauth.server.feature=-Dpassport.profile.feature.authorization=disabled
```
## WebAuthN tests
...
Cluster tests use 2 backend servers (Passport on Quarkus or Passport on Undertow), 1 frontend loadbalancer server node and one shared DB. Invalidation tests don't use loadbalancer.
...
*  two clustered nodes of Passport server on Wildfly/EAP or on embedded undertow
*  shared DB
    
### Cluster tests with Passport on Quarkus
...
To run tests using a different database such as PostgreSQL, add the following properties into the `testsuite/integration-arquillian/servers/auth-server/quarkus/src/main/content/conf/passport.conf` configuration file:

```
# HA using PostgreSQL
%ha.datasource.dialect=org.hibernate.dialect.PostgreSQLDialect
%ha.datasource.driver = org.postgresql.xa.PGXADataSource
%ha.datasource.url = jdbc:postgresql://localhost/passport
%ha.datasource.username = passport
...
### Cluster tests with Passport on embedded undertow
    ...
    -Dtest=org.passport.testsuite.cluster.**.*Test clean install
...
The test uses Undertow loadbalancer on `http://localhost:8180` and two embedded backend Undertow servers with Passport on `http://localhost:8181` and `http://localhost:8182` .
You can use any cluster test (eg. AuthenticationSessionFailoverClusterTest) and run from IDE with those system properties (replace with your DB settings):
    ...
    -Dpassport.connectionsJpa.url=jdbc:mysql://localhost/passport -Dpassport.connectionsJpa.driver=com.mysql.jdbc.Driver
    -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport -Dresources
    -Dsession.cache.owners=2
...
This mode is useful for develop/manual tests of clustering features. You will need to manually run passport backend nodes and loadbalancer.

1) Run PassportServer server1 with:

    -Dpassport.connectionsJpa.url=jdbc:mysql://localhost/passport -Dpassport.connectionsJpa.driver=com.mysql.jdbc.Driver
    -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport
    -Dresources

and argument: `-p 8181`

2) Run PassportServer server2 with same parameters but argument: `-p 8182`

3) Run loadbalancer (class `SimpleUndertowLoadBalancer`) without arguments and system properties. Loadbalancer runs on port 8180, so you can access Passport on `http://localhost:8180/auth`
...
By default, the test will run against Undertow based embedded Passport Server, thus no distribution build is required beforehand.
...
If docker daemon doesn't run locally, or if you're not running on Linux, you may need
 to determine the IP of the bridge interface or local interface that Docker daemon can use to connect to Passport Server.
...
Then, run the test passing `-Ddocker.io-prefix-explicit=true`:
    ...
    mvn -f testsuite/integration-arquillian/tests/base/pom.xml \
        ...
        -Dpassport.profile.feature.docker=enabled \
...
### Running Docker test against Passport Server distribution

Make sure to build the distribution:

    mvn clean install -f distribution

Then, before running the test, setup Passport Server distribution for the tests:
...
## Hostname Tests 
For changing the hostname in the hostname tests (e.g. [DefaultHostnameTest](https://github.com/passport/passport/blob/main/testsuite/integration-arquillian/tests/base/src/test/java/org/passport/testsuite/url/DefaultHostnameTest.java)),
...
```
java.lang.RuntimeException: java.net.UnknownHostException: passport.localtest.me: nodename nor servname provided, 
or not known at org.passport.testsuite.util.OAuthClient.doWellKnownRequest(OAuthClient.java:1032)
at org.passport.testsuite.url.DefaultHostnameTest.assertBackendForcedToFrontendWithMatchingHostname(
...
On the FIPS enabled platform with FIPS enabled OpenJDK 21, you can run this to test against a Passport server on Quarkus
...
most important part is that Passport itself is running on the JVM with FIPS enabled. You can check log from server startup and
there should be messages similar to those:
```
2022-10-11 19:34:29,521 DEBUG [org.passport.common.crypto.CryptoIntegration] (main) Using the crypto provider: org.passport.crypto.fips.FIPS1402Provider
2022-10-11 19:34:31,072 TRACE [org.passport.common.crypto.CryptoIntegration] (main) Java security providers: [ 
 KC(BCFIPS version 1.000203, FIPS-JVM: enabled) version 1.0 - class org.passport.crypto.fips.PassportFipsSecurityProvider, 
...
```
The log should contain `PassportFipsSecurityProvider` mentioning "Approved mode". Something like:
```
KC(BCFIPS version 1.000203 Approved Mode, FIPS-JVM: enabled) version 1.0 - class org.passport.crypto.fips.PassportFipsSecurityProvider,
```
...
mvn test -Pauth-server-quarkus -Pdb-aurora-postgres -Dtest=$TESTS  -Dauth.server.db.host=$AURORA_ENDPOINT -Dpassport.connectionsJpa.password=$AURORA_PASSWORD -pl testsuite/integration-arquillian/tests/base
```
