# Passport Arquillian Integration Testsuite
...
Passport server is automatically started by the testsuite on the `BeforeSuite` event and stopped on `AfterSuite` event.
...
- two clustered nodes of Passport server on Quarkus
...
The app-servers with installed Passport adapter are prepared in `servers/app-server` submodules, activated by `-Papp-server-MODULE` or `-Dapp.server=MODULE`
...
These objects are injected into `AbstractPassportTest` class so they can be used everywhere.
...
The `AbstractPassportTest` has an initialized instance of AdminClient. Subclasses can use it to access any REST subresources.
...
2. Using *example demo apps* from `passport/examples` modules.
...
2. **Bundled** - In the deployed war in `/WEB-INF/passport.json`. **Default.**
 ...
 * `CustomUndertowContainer` - Custom container controller for JAX-RS-enabled Undertow with Passport Server.
