...
public class LDAPSpecialCharsTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        ...
        testingClient.server().run(session -> {
            ...
            // Resync LDAP groups to Passport DB
    ...
    public void test01_userSearch() {
        ...
        assertContainsUsername(users, "jamespassport");
        assertContainsUsername(users, "johnpassport");
    }
    ...
    public void test03_specialCharUserJoiningSpecialCharGroup() {
        testingClient.server().run(session -> {
            ...
            GroupModel specialGroup = PassportModelUtils.findGroupByPath(session, appRealm, "/group-spec,ia*l_characžter)s");
            Assert.assertNotNull(specialGroup);

            specialUser.joinGroup(specialGroup);

            GroupModel groupWithSlashes = PassportModelUtils.findGroupByPath(session, appRealm, "/group/with/three/slashes");
