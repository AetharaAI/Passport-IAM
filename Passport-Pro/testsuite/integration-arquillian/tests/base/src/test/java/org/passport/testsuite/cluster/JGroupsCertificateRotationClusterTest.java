...
import org.passport.jgroups.certificates.DatabaseJGroupsCertificateProvider;
...
public class JGroupsCertificateRotationClusterTest extends AbstractClusterTest {
    ...
    private static CertificateReloadManager certificateReloadManager(PassportSession session) {
        return GlobalComponentRegistry.componentOf(cacheManager(session), CertificateReloadManager.class);
    }

    private static DatabaseJGroupsCertificateProvider databaseJGroupsCertificateProvider(PassportSession session) {
        return (DatabaseJGroupsCertificateProvider) session.getProvider(JGroupsCertificateProvider.class);
    }

    private static EmbeddedCacheManager cacheManager(PassportSession session) {
        return session.getProvider(InfinispanConnectionProvider.class)
                .getCache(InfinispanConnectionProvider.USER_CACHE_NAME)
                .getCacheManager();
    }

    private static String currentCertificateAlias(PassportSession session) {
        return databaseJGroupsCertificateProvider(session)
                .getCurrentCertificate()
                .getAlias();
    }

    private static void rotateCertificate(PassportSession session) {
