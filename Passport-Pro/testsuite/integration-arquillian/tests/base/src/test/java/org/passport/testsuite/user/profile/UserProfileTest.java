...
import org.passport.authentication.requiredactions.TermsAndConditions;
...
import org.passport.representations.idm.AbstractUserRepresentation;
...
import org.passport.representations.userprofile.config.UPAttribute;
import org.passport.representations.userprofile.config.UPAttributePermissions;
import org.passport.representations.userprofile.config.UPAttributeRequired;
import org.passport.representations.userprofile.config.UPAttributeSelector;
...
import org.passport.representations.userprofile.config.UPGroup;
...
import org.passport.testsuite.util.ClientScopeBuilder;
import org.passport.testsuite.util.PassportModelUtils;
import org.passport.userprofile.AttributeGroupMetadata;
import org.passport.userprofile.Attributes;
import org.passport.userprofile.UserProfile;
import org.passport.userprofile.UserProfileConstants;
import org.passport.userprofile.UserProfileContext;
import org.passport.userprofile.UserProfileProvider;
import org.passport.userprofile.UserProfileUtil;
import org.passport.userprofile.ValidationException;
import org.passport.userprofile.config.UPConfigUtils;
import org.passport.userprofile.validator.MultiValueValidator;
import org.passport.userprofile.validator.PersonNameProhibitedCharactersValidator;
import org.passport.userprofile.validator.UsernameIDNHomographValidator;
...
import static org.passport.userprofile.config.UPConfigUtils.ROLE_ADMIN;
import static org.passport.userprofile.config.UPConfigUtils.ROLE_USER;
import static org.passport.userprofile.config.UPConfigUtils.parseSystemDefaultConfig;
...
public class UserProfileTest extends AbstractUserProfileTest {
    ...
    public void configureTestRealm(RealmRepresentation testRealm) {
        ...
        ClientRepresentation client = PassportModelUtils.createClient(testRealm, "client-a");
        ...
        PassportModelUtils.createClient(testRealm, "client-b");
    }
    ...
    public void testReadOnlyAllowed() throws Exception {
        ...
        userRep.setEmail(org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
    ...
    private static void testIdempotentProfile(PassportSession session) {
    ...
    private static void testCustomAttributeInAnyContext(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, org.passport.models.utils.PassportModelUtils.generateId());
        attributes.put(UserModel.FIRST_NAME, "John");
        attributes.put(UserModel.LAST_NAME, "Doe");
        attributes.put(UserModel.EMAIL, org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
    ...
    private static void testEmptyAttributeRemoved(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testResolveProfile(PassportSession session) {
        configureAuthenticationSession(session);

        Map<String, Object> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
        attributes.put(UserModel.FIRST_NAME, "John");
        attributes.put(UserModel.LAST_NAME, "Doe");
        attributes.put(UserModel.EMAIL, org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
    ...
    public void testValidation() throws IOException {
        ...
        getTestingClient().server(TEST_REALM_NAME).run((PassportSession session) -> testNonAsciiEmailValidator(session, false));
        try (RealmAttributeUpdater updater = new RealmAttributeUpdater(testRealm())
            ...
            getTestingClient().server(TEST_REALM_NAME).run((PassportSession session) -> testNonAsciiEmailValidator(session, true));
        }
    }

    private static void failValidationWhenEmptyAttributes(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
        ...
        try {
            attributes.clear();
            attributes.put(UserModel.EMAIL, "profile-user@passport-pro.ai");
            profile = provider.create(UserProfileContext.UPDATE_PROFILE, attributes);
            profile.validate();
            Assert.fail("Should fail validation");
        } catch (ValidationException ve) {
    ...
    private static void testAttributeValidation(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
    ...
    private static void testEmailAsUsernameValidation(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
        provider.setConfiguration(null);
        UserProfile profile;
    ...
    private static void testNonAsciiEmailValidator(PassportSession session, boolean success) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
        List<ValidationError> errors = new ArrayList<>();
    ...
    private static void testValidateComplianceWithUserProfile(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
        ...
        user.setSingleAttribute(UserModel.EMAIL, "jd@passport-pro.ai");
    ...
    private static void testGetProfileAttributes(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
        UserModel user = session.users().addUser(realm, org.passport.models.utils.PassportModelUtils.generateId());
        user.setFirstName("John");
        user.setLastName("John");
        user.setEmail(org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
    ...
    private static void testGetProfileAttributeGroups(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
        UserModel user = session.users().addUser(realm, org.passport.models.utils.PassportModelUtils.generateId());
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCreateAndUpdateUser(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        ...
        Map<String, Object> attributes = new HashMap<>();
        String userName = org.passport.models.utils.PassportModelUtils.generateId();

        attributes.put(UserModel.USERNAME, userName);
        attributes.put(UserModel.EMAIL, "user@passport-pro.ai");
        attributes.put(UserModel.FIRST_NAME, "Joe");
        attributes.put(UserModel.LAST_NAME, "Doe");
        ...
        attributes.put(UserModel.EMAIL, "alice@passport-pro.ai");
        ...
        attributesUpdatedOldValues.put(UserModel.EMAIL, "user@passport-pro.ai");
    ...
    private static void testReadonlyUpdates(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testReadonlyEmailCannotBeUpdated(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testUpdateEmail(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private static void testDoNotUpdateUndefinedAttributes(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testComponentModelId(PassportSession session) {
    ...
    private static void testInvalidConfiguration(PassportSession session) {
        try {
    ...
    private static void testResetConfiguration(PassportSession session) {
    ...
    private static void testDefaultConfig(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testCustomValidationForUsername(PassportSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testRemoveDefaultValidationFromUsername(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        ...
        attributes.put(UserModel.EMAIL, "test@passport-pro.ai");
    ...
    private static void testHomographValidator(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testOptionalAttributes(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    public void testPersonNameProhibitedCharsValidator(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    @Test
    @ModelTest(realmName = "test")
    public void testUriValidator(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCustomAttributeRequired(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute attribute = new UPAttribute();
    ...
    private static void testNullAttributesInConfig(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCustomAttributeOptional(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute attribute = new UPAttribute();
    ...
    private static void testRequiredIfUser(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testRequiredIfAdmin(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    public void testEmailRequired(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private static void testNoValidationsIfUserReadOnly(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testNoValidationsIfAdminReadOnly(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testIgnoreReadOnlyAttribute(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testReadOnlyInternalAttributeValidation(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    @Test
    @ModelTest
    public void testRequiredByClientScope(PassportSession session) {
    ...
    @Test
    @ModelTest
    public void testRequiredByOptionalClientScope(PassportSession session) {
        RealmModel realm = session.realms().getRealmByName("test");
        session.getContext().setRealm(realm);
    ...
    private static void testConfigurationInvalidScope(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testUsernameAndEmailPermissionNotSetIfEmpty(PassportSession session){
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = provider.getConfiguration();
        ...
        attributes.put(UserModel.EMAIL, "test@passport.com");
        ...
        assertEquals("test@passport.com", user.getEmail());
    }
    ...
    private static void testDoNotRemoveAttributes(PassportSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testRemoveEmptyRootAttribute(PassportSession session) {
        Map<String, List<String>> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, List.of(org.passport.models.utils.PassportModelUtils.generateId()));
    ...
    private static void testRemoveOptionalAttributesFromDefaultConfigIfNotSet(PassportSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testUnmanagedPolicy(PassportSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testOptionalRootAttributesAsUnmanagedAttribute(PassportSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UserProfileProvider provider = getUserProfileProvider(session);
        provider.setConfiguration(config);
        Map<String, String> rawAttributes = new HashMap<>();
        rawAttributes.put(UserModel.USERNAME, org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
        rawAttributes.put(UserModel.EMAIL, org.passport.models.utils.PassportModelUtils.generateId() + "@passport-pro.ai");
    ...
    private static void testAttributeNormalization(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testEmailAttributeInUpdateEmailContext(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        String userName = org.passport.models.utils.PassportModelUtils.generateId();
        Map<String, String> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, userName);
        attributes.put(UserModel.EMAIL, userName + "@passport-pro.ai");
        attributes.put(UserModel.FIRST_NAME, "Joe");
        attributes.put(UserModel.LAST_NAME, "Doe");
    ...
    private static void testEmailAnnotationsInAccountContext(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        String userName = org.passport.models.utils.PassportModelUtils.generateId();
        Map<String, String> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, userName);
        String originalEmail = userName + "@passport-pro.ai";
    ...
    private static void testEmailFieldHiddenWhenEmptyAndReadOnlyWithUpdateEmailEnabled(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        String userName = org.passport.models.utils.PassportModelUtils.generateId();
        Map<String, String> attributes = new HashMap<>();
    ...
    private static void testInvalidConfigDefaultValue(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testDefaultValue(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute foo = new UPAttribute("foo", new UPAttributePermissions(Set.of(), Set.of(UserProfileConstants.ROLE_ADMIN)));
        foo.setDefaultValue("def");
    ...
    private static void testNoDefaultValueForRootAttributes(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testMultivalued(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
        provider.setConfiguration(upConfig);
        String userName = org.passport.models.utils.PassportModelUtils.generateId();
        ...
        attributes.put(UserModel.USERNAME, List.of(userName));
        attributes.put(UserModel.EMAIL, List.of(userName + "@passport-pro.ai"));
    ...
    private static void testDefaultConfigWhenComponentConfigIsNotSet(PassportSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
