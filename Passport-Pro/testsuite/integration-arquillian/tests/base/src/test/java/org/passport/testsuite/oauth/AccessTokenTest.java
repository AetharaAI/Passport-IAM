...
import org.passport.crypto.ECDSAAlgorithm;
...
import org.passport.keys.GeneratedEddsaKeyProviderFactory;
...
import static org.passport.testsuite.admin.ApiUtil.findClientByClientId;
import static org.passport.testsuite.admin.ApiUtil.findUserByUsername;
import static org.passport.testsuite.admin.ApiUtil.findUserByUsernameId;
import static org.passport.testsuite.util.ProtocolMapperUtil.createRoleNameMapper;
 ...
 */
public class AccessTokenTest extends AbstractPassportTest {
    ...
    @Test
    public void testPassport2221() throws Exception {
        Client client = AdminClientUtil.createResteasyClient();
        UriBuilder builder = UriBuilder.fromUri(AUTH_SERVER_ROOT);
        URI grantUri = OIDCLoginProtocolService.tokenUrl(builder).build("test");
        WebTarget grantTarget = client.target(grantUri);
    ...
    // PASSPORT-1595 Assert that public client is able to retrieve token even if header "Authorization: Negotiate something" was used (parameter client_id has preference in this case)
    @Test
    ...
    private IDToken getIdToken(org.passport.representations.AccessTokenResponse tokenResponse) throws JWSInputException {
        JWSInput input = new JWSInput(tokenResponse.getIdToken());
        return input.readJsonContent(IDToken.class);
    }

    private AccessToken getAccessToken(org.passport.representations.AccessTokenResponse tokenResponse) throws JWSInputException {
