...
import org.passport.authentication.authenticators.browser.RecoveryAuthnCodesFormAuthenticatorFactory;
import org.passport.authentication.authenticators.browser.UsernamePasswordFormFactory;
import org.passport.authentication.authenticators.conditional.ConditionalLoaAuthenticator;
import org.passport.authentication.authenticators.conditional.ConditionalLoaAuthenticatorFactory;
...
import org.passport.models.AuthenticationFlowBindings;
...
import org.passport.testsuite.util.RealmRepUtil;
...
import static org.passport.testsuite.actions.AppInitiatedActionDeleteCredentialTest.getKcActionParamForDeleteCredential;
...
public class LevelOfAssuranceFlowTest extends AbstractChangeImportedUserPasswordsTest {
    ...
    public static void configureStepUpFlow(PassportTestingClient testingClient) {
        configureStepUpFlow(testingClient, ConditionalLoaAuthenticator.DEFAULT_MAX_AGE, 0, 0);
    }

    private static void configureStepUpFlow(PassportTestingClient testingClient, int maxAge1, int maxAge2, int maxAge3) {
    ...
    private static void configureFlowsWithRecoveryCodes(PassportTestingClient testingClient) {
        testingClient.server(TEST_REALM_NAME)
                .run(session -> {
    ...
    // Backwards compatibility with Passport 17 when condition was configured with option "Store Loa in User Session"
    @Test
    public void testBackwardsCompatibilityForLoaConditionConfig() {
        // Reconfigure to the format of Passport 17 with option "Store Loa in User Session"
        BrowserFlowTest.revertFlows(testRealm(), FLOW_ALIAS);
        testingClient.server(TEST_REALM_NAME).run(session -> FlowUtil.inCurrentRealm(session).copyBrowserFlow(FLOW_ALIAS));
        testingClient.server(TEST_REALM_NAME)
                .run(session -> FlowUtil.inCurrentRealm(session).selectFlow(FLOW_ALIAS).inForms(forms -> forms.clear()
