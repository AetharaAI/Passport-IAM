...
import org.passport.testsuite.runonserver.FetchOnServerWrapper;
...
/**
 * Test for simulating token refresh with the offline tokens created in older Passport versions.
 *
 * Passport supports refresh of the offline tokens, which were created in older Passport versions than current Passport version. But
 * testing real migration is sometimes hard to achieve as it requires running of the old Passport server, which is sometimes not feasible.
 *
 * This test just simulates the refresh with the old offline-token by manually converting offline-token to the offline-token format, which was used by the specified old Passport version
 *
 * @author <a href="mailto:mposolda@redhat.com">Marek Posolda</a>
 */
public class OfflineTokenMigrationTest extends AbstractTestRealmPassportTest {
    ...
    // Issue 31224
    // Test refresh with the offline-token created in Passport 14 works
    @Test
    public void testOfflineTokenMigrationFromPassport14() throws Exception {
        OfflineTokenConverter convertOfflineTokenToPassport14Format = (session, oldOfflineToken) -> {
            try {
        ...
        testOfflineTokenMigration(convertOfflineTokenToPassport14Format);
    }
    ...
    private void testOfflineTokenMigration(OfflineTokenConverter offlineTokenConverter) throws Exception {
        ...
        // Convert offline token to the format of some old Passport version
    ...
    public interface OfflineTokenConverter extends Serializable, BiFunction<PassportSession, String, String> {
    }
}
