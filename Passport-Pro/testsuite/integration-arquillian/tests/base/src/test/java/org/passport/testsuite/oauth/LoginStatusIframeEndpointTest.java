...
import org.passport.testsuite.runonserver.ServerVersion;
 ...
 */
public class LoginStatusIframeEndpointTest extends AbstractPassportTest {
        ...
        try (CloseableHttpClient client = HttpClients.custom().setDefaultCookieStore(cookieStore).build()) {
            ...
            for (Header h : response.getAllHeaders()) {
                if (h.getName().equals("Set-Cookie")) {
                    if (h.getValue().contains("PASSPORT_SESSION")) {
                        setSessionCookieHeader = h;

                    } else if (h.getValue().contains("PASSPORT_IDENTITY")) {
            ...
            for (Cookie cookie : cookieStore.getCookies()) {
                if (cookie.getName().equals("PASSPORT_SESSION")) {
