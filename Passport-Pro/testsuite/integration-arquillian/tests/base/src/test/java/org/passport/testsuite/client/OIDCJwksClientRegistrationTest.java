...
public class OIDCJwksClientRegistrationTest extends AbstractClientRegistrationTest {
    ...
    private KeyPair getKeyPairFromGeneratedPems(Map<String, String> generatedKeys) {
        ...
        PrivateKey privateKey = PassportModelUtils.getPrivateKey(privateKeyPem);
        PublicKey publicKey = PassportModelUtils.getPublicKey(publicKeyPem);
    ...
    private String getClientSignedJWT(String clientId, KeyPair keyPair, final String kid) {
        String realmInfoUrl = PassportUriBuilder.fromUri(getAuthServerRoot()).path(ServiceUrlConstants.REALM_INFO_PATH).build(REALM_NAME).toString();
