...
public class KerberosLdapCrossRealmTrustTest extends AbstractKerberosTest {
    ...
    public void test01SpnegoLoginCRTSuccess() throws Exception {
        // Login as user from realm KC2.COM . Realm PASSPORT.ORG will trust us
    ...
    public void test02SpnegoLoginCorrectKerberosPrincipalUserFound() throws Exception {
        // Login as kerberos user jduke@KC2.COM. Ensure I am logged as user "jduke2" from realm KC2.COM (not as user jduke@PASSPORT.ORG)
    ...
    public void test04SpnegoLoginWithoutKerberosPrincipalAttrConfigured() throws Exception {
        ...
        // Passport will lookup user just based on 1st part of kerberos principal. Hence for "jduke@KC2.COM", it will lookup user "jduke"
        AccessTokenResponse tokenResponse = assertSuccessfulSpnegoLogin("jduke@KC2.COM", "jduke", "theduke2");
        AccessToken token = oauth.verifyToken(tokenResponse.getAccessToken());

        Assert.assertEquals(token.getEmail(), "jduke@passport-pro.ai");
        assertUser("jduke", "jduke@passport-pro.ai", "Java", "Duke", null, false);
        ...
        tokenResponse = assertSuccessfulSpnegoLogin("jduke@PASSPORT.ORG", "jduke", "theduke");
        token = oauth.verifyToken(tokenResponse.getAccessToken());
    ...
    public void test05DisableTrust() throws Exception {
        // Remove the LDAP entry corresponding to the Kerberos principal krbtgt/PASSPORT.ORG@KC2.COM
