...
import org.passport.testsuite.arquillian.annotation.EnableFeatures;
...
public class ThemeResourceProviderTest extends AbstractTestRealmPassportTest {
    ...
    public void testThemeFallback() {
        testingClient.server().run(session -> {
            try {
                ...
                Assert.assertEquals("passport.v2", theme.getName());
            } catch (IOException e) {
                Assert.fail(e.getMessage());
            }
        });
    ...
    public void getMessages() {
        testingClient.server().run(session -> {
            try {
                Theme theme = session.theme().getTheme("base", Theme.Type.LOGIN);
                Assert.assertNotNull(theme.getMessages("messages", Locale.ENGLISH).get("test.passport-8818"));
    ...
    public void gzipEncoding() throws IOException {
        ...
        boolean filesNotExistsInTmp = testingClient.server().fetch(session -> {
            ...
            File file1 = Paths.get(System.getProperty("java.io.tmpdir"), "kc-gzip-cache", resourcesVersion, "welcome", "passport", "css", "welcome.css.gz").toFile();
            if (file1.isFile()) {
        ...
        assertEncoded(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + resourcesVersion + "/welcome/passport/css/welcome.css", ".pf-v5-c-background-image");
        ...
        if (filesNotExistsInTmp) {
            testingClient.server().run(session -> {
                assertFalse(Paths.get(System.getProperty("java.io.tmpdir"), "kc-gzip-cache", resourcesVersion, "welcome", "passport", "css", "welcome.css.gz").toFile().isFile());
            });
        }

        testingClient.server().run(session -> {
            String serverTmpDir = Platform.getPlatform().getTmpDirectory().toString();
            assertTrue(Paths.get(serverTmpDir, "kc-gzip-cache", resourcesVersion, "welcome", "passport", "css", "welcome.css.gz").toFile().isFile());
        });
    }
    ...
    public void notFoundOnInvalidThemeType() throws IOException {
        final String resourcesVersion = testingClient.server().fetch(session -> Version.RESOURCES_VERSION, String.class);
        assertNotFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + resourcesVersion + "/invalid-theme-type/passport/css/welcome.css");
    }
    ...
    public void fetchStaticResourceShouldRedirectOnUnknownVersion() throws IOException {
        final String resourcesVersion = testingClient.server().fetch(session -> Version.RESOURCES_VERSION, String.class);
        assertFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + resourcesVersion + "/login/passport.v2/css/styles.css");
        assertFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + resourcesVersion + "/login/passport.v2/css%2Fstyles.css");
        assertNotFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + "unkno" + "/login/passport.v2/css%2Fstyles.css");
        assertNotFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + "unkn%2F" + "/login/passport.v2/css/styles.css");
        assertNotFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + "unkno" + "/login/passport.v2/css/unknown.css");
        ...
        // assertNotFound(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + "unkno" + "/login/passport.v2/css/../css/styles.css");
        assertRedirectAndValidateRedirect(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + "unkno" + "/login/passport.v2/css/styles.css?name=%2Fvalue",
                suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth/resources/" + resourcesVersion + "/login/passport.v2/css/styles.css");
    }
    ...
    public void getMessagesLocaleResolving() {
        testingClient.server().run(session -> {
            try {
                Theme theme = session.theme().getTheme("base", Theme.Type.LOGIN);
                assertEquals("Test en_US_variant", theme.getMessages("messages", new Locale("en", "US", "variant")).get("test.passport-12926"));
                assertEquals("Test en_US", theme.getMessages("messages", new Locale("en", "US")).get("test.passport-12926"));
                assertEquals("Test en", theme.getMessages("messages", Locale.ENGLISH).get("test.passport-12926"));
                assertEquals("Test en_US", theme.getMessages("messages", new Locale("en", "US")).get("test.passport-12926"));
                assertEquals("Test en", theme.getMessages("messages", Locale.ENGLISH).get("test.passport-12926"));

                assertEquals("only de_AT_variant", theme.getMessages("messages", new Locale("de", "AT", "variant")).get("test.passport-12926-resolving1"));
                assertNull(theme.getMessages("messages", new Locale("de", "AT")).get("test.passport-12926-resolving1"));

                assertEquals("only de_AT", theme.getMessages("messages", new Locale("de", "AT", "variant")).get("test.passport-12926-resolving2"));
                assertNull(theme.getMessages("messages", new Locale("de")).get("test.passport-12926-resolving2"));

                assertEquals("only de", theme.getMessages("messages", new Locale("de", "AT", "variant")).get("test.passport-12926-only_de"));
                assertNull(theme.getMessages("messages", Locale.ENGLISH).get("test.passport-12926-only_de"));

                assertEquals("fallback en", theme.getMessages("messages", new Locale("de", "AT", "variant")).get("test.passport-12926-resolving3"));
                assertEquals("fallback en", theme.getMessages("messages", new Locale("de", "AT")).get("test.passport-12926-resolving3"));
                assertEquals("fallback en", theme.getMessages("messages", new Locale("de")).get("test.passport-12926-resolving3"));
