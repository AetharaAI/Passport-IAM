...
import org.passport.storage.ldap.mappers.msad.UserAccountControl;
...
public class LDAPMSADMapperTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        testingClient.server().run(session -> {
            ...
            LDAPObject john = LDAPTestUtils.addLDAPUser(ldapFedProvider, appRealm, "johnpassport", "John", "Doe", "john@email.org", null, "1234");
    ...
    // TODO: Currently we can't rely on it. See PASSPORT-4276
    @Ignore
    @Test
    ...
    public void test02UpdatePasswordTest() {
        // Add required action to user johnpassport through Passport admin API
        UserResource john = ApiUtil.findUserByUsernameId(adminClient.realm("test"), "johnpassport");
        ...
        // Check in LDAP, that johnpassport has pwdLastSet set to 0 in LDAP
        Assert.assertEquals(0, getPwdLastSetOfJohn());

        // Login as johnpassport and update password after login
        loginPage.open();
        ...
        // Check in LDAP, that johnpassport does not have pwdLastSet set to 0
        assertThat(getPwdLastSetOfJohn(), Matchers.greaterThan(0L));

        // Check in admin REST API, that johnpassport does not have required action on him
    ...
    public void test03UpdatePasswordWithLDAPDirectly() {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject ldapJohn = ctx.getLdapProvider().loadLDAPUserByUsername(appRealm, "johnpassport");
    ...
    public void test05UpdatePasswordUnsyncedMode() {
        ...
        // Add required action to user johnpassport through Passport admin API. Due UNSYNCED mode, this should update Passport DB, but not MSAD
        UserResource john = ApiUtil.findUserByUsernameId(adminClient.realm("test"), "johnpassport");
        UserRepresentation johnRep = john.toRepresentation();
        johnRep.setRequiredActions(Collections.singletonList(UserModel.RequiredAction.UPDATE_PASSWORD.name()));
        john.update(johnRep);

        // Check in LDAP, that johnpassport has pwdLastSet set attribute set in MSAD to bigger value than 0. Previous update of requiredAction did not update LDAP
        ...
        // Check in LDAP, that pwdLastSet attribute of MSAD user johnpassport did not change in MSAD
    ...
    @Test
    public void test07DisabledUserInMSADSwitchedToEnabledInPassport() {
        ...
        // Login as johnpassport, but user disabled
        loginPage.open();
        loginPage.login("johnpassport", "Password1");
    ...
    public void test08DisabledUserUnsyncedMode() {
        ...
        // Disable user johnpassport through Passport admin API. Due UNSYNCED mode, this should update Passport DB, but not MSAD
        UserResource john = ApiUtil.findUserByUsernameId(adminClient.realm("test"), "johnpassport");
        UserRepresentation johnRep = john.toRepresentation();
        johnRep.setEnabled(false);
        john.update(johnRep);

        // Check in LDAP, that johnpassport is still enabled in MSAD
        Assert.assertTrue(isJohnEnabledInMSAD());

        // Login as johnpassport and see the user is disabled
        loginPage.open();
        loginPage.login("johnpassport", "Password1");
        Assert.assertEquals("Account is disabled, contact your administrator.", loginPage.getError());

        // Enable johnpassport in admin REST API
        johnRep = john.toRepresentation();
        johnRep.setEnabled(true);
        john.update(johnRep);
    ...
    public void test09DisableUserImportDisabled() {
        ...
        // disable user johnpassport - it should disable both locally and on MSAD.
        johnRep.setEnabled(false);
        john.update(johnRep);
    ...
    public void test10DisabledUserSwitchedToEnabledOnMSAD() {
        // disable user johnpassport via REST API - should be disabled in MSAD as well.
        UserResource john = ApiUtil.findUserByUsernameId(adminClient.realm("test"), "johnpassport");
        UserRepresentation johnRep = john.toRepresentation();
        johnRep.setEnabled(false);
        john.update(johnRep);
        ...
        // enable user johnpassport in MSAD only
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            RealmModel appRealm = ctx.getRealm();
    ...
    private long getPwdLastSetOfJohn() {
        ...
        if (pwdLastSett == null) {
            Assert.fail("LDAP user johnpassport does not have pwdLastSet on him");
        }
    ...
    private boolean isJohnEnabledInMSAD() {
        ...
        if (userAccountControls == null) {
            Assert.fail("LDAP user johnpassport does not have userAccountControl attribute on him");
        }
