...
package org.passport.testsuite.events;
...
public class EventMetricsProviderTest extends AbstractPassportTest {
    ...
    public void shouldCountSingleEventWithTagsAndFilter() {
        ...
        testingClient.server().run(session -> {
            MatcherAssert.assertThat("Searching for metrics match in " + Metrics.globalRegistry.find("passport.user").meters(),
                    Metrics.globalRegistry.counter("passport.user", "event", "login", "realm", TEST).count() == 1);
        });
        ...
        testingClient.server().run(session -> {
            MatcherAssert.assertThat("Two metrics recorded",
                    Metrics.globalRegistry.find("passport.user").meters().size(), Matchers.equalTo(2));
            MatcherAssert.assertThat("Searching for login error metric",
                    Metrics.globalRegistry.counter("passport.user", "event", "login", "error", "ERROR", "realm", TEST, "client.id", CLIENT_ID, "idp", "IDENTITY_PROVIDER").count() == 1);
            MatcherAssert.assertThat("Searching for refresh with unknown client",
                    Metrics.globalRegistry.counter("passport.user", "event", "refresh_token", "error", "client_not_found", "realm", TEST, "client.id", "unknown").count() == 1);
        });
