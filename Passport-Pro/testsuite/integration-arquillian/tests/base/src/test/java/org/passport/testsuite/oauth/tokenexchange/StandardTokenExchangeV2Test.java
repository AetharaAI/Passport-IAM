...
import org.passport.services.clientpolicy.executor.JWTClaimEnforcerExecutor;
import org.passport.services.clientpolicy.executor.JWTClaimEnforcerExecutorFactory;
...
import org.passport.testsuite.pages.ConsentPage;
...
import org.passport.testsuite.util.oauth.TokenExchangeRequest;
...
public class StandardTokenExchangeV2Test extends AbstractClientPoliciesTest {
    ...
    public void testTransientSessionWithAdminApi() throws Exception {
        ...
        try (RoleScopeUpdater roleScopeUpdater = UserAttributeUpdater.forUserByUsername(realm, "john")
            ...
            try (Passport passport = Passport.getInstance(ServerURLs.getAuthServerContextRoot() + "/auth",
                    TEST, Constants.ADMIN_CLI_CLIENT_ID, response.getAccessToken(), TLSUtils.initializeTLS())) {
                assertEquals(TEST, passport.realm(TEST).toRepresentation().getRealm());
                setTimeOffset(10);
                assertEquals(TEST, passport.realm(TEST).toRepresentation().getRealm());
                realm.deleteSession(exchangedToken.getSessionId(), false);
                assertThrows(NotAuthorizedException.class, () -> passport.realm(TEST).toRepresentation().getRealm());
            }
        }
    }
