...
import org.passport.testsuite.cluster.AuthenticationSessionFailoverClusterTest;
...
import org.passport.testsuite.util.UserActionTokenBuilder;
...
import static org.passport.authentication.requiredactions.VerifyEmail.EMAIL_RESEND_COOLDOWN_KEY_PREFIX;
 ...
 */
public class RequiredActionEmailVerificationTest extends AbstractTestRealmPassportTest {
    ...
    public void verifyEmailConfig() throws MessagingException {
        ...
        Assert.assertEquals("<auto+bounces@passport-pro.ai>", message.getHeader("Return-Path")[0]);
        // displayname <email@example.org>
        Assert.assertEquals("Passport SSO <auto@passport-pro.ai>", message.getHeader("From")[0]);
        Assert.assertEquals("Passport no-reply <reply-to@passport-pro.ai>", message.getHeader("Reply-To")[0]);
    }
    ...
    public void verifyEmailFromAnotherAccountWhenUserIsAuthenticated() throws Exception {
        loginPage.open();
        loginPage.clickRegister();
        String username1 = PassportModelUtils.generateId();
        ...
        loginPage.open();
        loginPage.clickRegister();
        String username2 = PassportModelUtils.generateId();
    ...
    public void verifyEmailInvalidKeyInVerficationLink() throws IOException {
        ...
        verificationUrl = PassportUriBuilder.fromUri(verificationUrl).replaceQueryParam(Constants.KEY, "foo").build().toString();
    ...
    // https://issues.jboss.org/browse/PASSPORT-5861
    @Test
