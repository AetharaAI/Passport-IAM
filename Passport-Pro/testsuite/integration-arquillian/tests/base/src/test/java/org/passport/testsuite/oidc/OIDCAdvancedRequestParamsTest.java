...
import org.passport.protocol.oidc.mappers.ClaimsParameterTokenMapper;
 ...
 */
public class OIDCAdvancedRequestParamsTest extends AbstractTestRealmPassportTest {
    ...
    protected void afterAbstractPassportTestRealmImport() {
        ...
        keys.getConfig().putSingle("algorithm", org.passport.crypto.Algorithm.RS256);
        ...
        keys.getConfig().putSingle(Attributes.KEY_USE, KeyUse.ENC.getSpecName());
        keys.getConfig().putSingle("algorithm", org.passport.crypto.Algorithm.PS256);
    ...
    public void promptLogin() {
        ...
        // SSO login first WITHOUT prompt=login ( Tests PASSPORT-5248 )
        oauth.openLoginForm();
        Assert.assertEquals(AppPage.RequestType.AUTH_RESPONSE, appPage.getRequestType());
    ...
    public void requestObjectClientIdAndResponseTypeTest() {
        ...
        oauth.loginForm().request(request).state("some-state").open();
        appPage.assertCurrent();
        oauth.responseMode("query"); // Passport falls back to query in this case
