...
import org.passport.authentication.authenticators.browser.WebAuthnAuthenticatorFactory;
import org.passport.authentication.authenticators.browser.WebAuthnPasswordlessAuthenticatorFactory;
import org.passport.authentication.requiredactions.DeleteCredentialAction;
import org.passport.authentication.requiredactions.WebAuthnPasswordlessRegisterFactory;
import org.passport.authentication.requiredactions.WebAuthnRegisterFactory;
import org.passport.common.enums.AccountRestApiVersion;
...
import org.passport.representations.account.ClientRepresentation;
import org.passport.representations.account.ConsentRepresentation;
import org.passport.representations.account.ConsentScopeRepresentation;
import org.passport.representations.account.DeviceRepresentation;
import org.passport.representations.account.SessionRepresentation;
import org.passport.representations.account.UserRepresentation;
...
import org.passport.representations.idm.RequiredActionProviderSimpleRepresentation;
import org.passport.representations.idm.UserProfileAttributeMetadata;
...
import org.passport.services.resources.account.AccountCredentialResource;
import org.passport.services.util.ResolveRelative;
import org.passport.testsuite.AbstractAuthenticationTest;
...
import org.passport.testsuite.util.TokenUtil;
import org.passport.testsuite.util.UserBuilder;
import org.passport.testsuite.util.oauth.AccessTokenResponse;
import org.passport.testsuite.util.userprofile.UserProfileUtil;
...
import static org.passport.testsuite.util.userprofile.UserProfileUtil.PERMISSIONS_ALL;
...
public class AccountRestServiceTest extends AbstractRestServiceTest {
    ...
    public void testEditUsernameAllowed() throws IOException {
        ...
        try {
            ...
            user.setEmail("changed-email@passport-pro.ai");
            user = updateAndGet(user);
            assertEquals("changed-username", user.getUsername());
            assertEquals("changed-email@passport-pro.ai", user.getEmail());
            ...
            user.setEmail("user@passport-pro.ai");
            user = updateAndGet(user);
            assertEquals("user@passport-pro.ai", user.getUsername());
            assertEquals("user@passport-pro.ai", user.getEmail());
            ...
            user.setUsername("should-be-the-email");
            user.setEmail("should-not-change@passport-pro.ai");
            user = updateAndGet(user);
            assertEquals("user@passport-pro.ai", user.getUsername());
            assertEquals("user@passport-pro.ai", user.getEmail());
    ...
    /**
     * Reproducer for bugs PASSPORT-17424 and PASSPORT-17582
     */
    @Test
    public void testUpdateProfileEmailChangeSetsEmailVerified() throws IOException {
        ...
        try {
            ...
            org.passport.representations.idm.UserRepresentation ur = userResource.toRepresentation();
    ...
    public void testRemoveCredentialWithNonOtpCredentialTriggeringNoEvent() throws IOException {
        ...
        org.passport.representations.idm.UserRepresentation userRep = UserBuilder.edit(user.toRepresentation())
    ...
    public void testCredentialsGetWithDisabledOtpRequiredAction() throws IOException {
        ...
        org.passport.representations.idm.UserRepresentation userRep = UserBuilder.edit(adminUserResource.toRepresentation())
    ...
    public void testAudience() throws Exception {
        ...
        org.passport.representations.idm.ClientRepresentation clientRep = testRealm().clients().findByClientId("custom-audience").get(0);
