...
package org.passport.testsuite.adapter;
...
import org.passport.testsuite.arquillian.AppServerTestEnricher;
...
public abstract class AbstractServletsAdapterTest extends AbstractAdapterTest {
    ...
    public static WebArchive samlServletDeployment(String name, String customArchiveName, String webXMLPath, Integer clockSkewSec, Class... servletClasses) {
        String baseSAMLPath = "/adapter-test/passport-saml/";
        String webInfPath = baseSAMLPath + name + "/WEB-INF/";

        URL passportSAMLConfig = AbstractServletsAdapterTest.class.getResource(webInfPath + "passport-saml.xml");
        Assert.assertNotNull("passport-saml.xml should be in " + webInfPath, passportSAMLConfig);

        URL webXML = AbstractServletsAdapterTest.class.getResource(baseSAMLPath + webXMLPath);
        Assert.assertNotNull("web.xml should be in " + baseSAMLPath + webXMLPath, passportSAMLConfig);
        ...
        try {
            ...
            if (clockSkewSec != null) {
                String passportSamlXMLContent = IOUtils.toString(passportSAMLConfig.openStream(), StandardCharsets.UTF_8)
                    .replace("%CLOCK_SKEW%", "${allowed.clock.skew:" + String.valueOf(clockSkewSec) + "}");
                deployment.addAsWebInfResource(new StringAsset(passportSamlXMLContent), "passport-saml.xml");
            } else {
                deployment.addAsWebInfResource(passportSAMLConfig, "passport-saml.xml");
            }
