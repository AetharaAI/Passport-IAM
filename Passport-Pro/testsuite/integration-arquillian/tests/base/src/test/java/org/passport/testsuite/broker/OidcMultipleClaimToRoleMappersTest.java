...
import org.passport.broker.oidc.mappers.AdvancedClaimToRoleMapper;
import org.passport.broker.oidc.mappers.ClaimToRoleMapper;
 ...
 * to the same {@code Passport} role.
 * <p/>
 * This class aims to test the fix for {@code PASSPORT-8730}. When configuring two or more mappers that map different IDP
 * attributes to the same {@code Passport} role, the user would sometimes not be granted the expected {@code Passport} role
 * depending on the order in which the mappers would run. For example, consider a scenario where mapper A maps IDP role 'x'
 * to the role 'passport' and mapper B maps IDP role 'y' to the same role 'passport'. The user only has role 'x' in the IDP,
 * so when updating the brokered user the following could happen:
 * <ul>
 *     <li>mapper A runs, checks user has role 'x', therefore role 'passport' is granted to user</li>
 *     <li>mapper B runs, checks users doesn't have role 'y', so it removes role 'passport' from user even if the previous
 *     mapper has already verified that the role should have been granted.</li>
 * </ul>
 * This test configures three different OIDC claim mappers that all map to the same {@code Passport} role. Only the first
...
public class OidcMultipleClaimToRoleMappersTest extends OidcClaimToRoleMapperTest {
    ...
    protected void createClaimToRoleMapper(String claimValue, IdentityProviderMapperSyncMode syncMode,
        ...
        secondOidcClaimToRoleMapper.setName("external-passport-role-mapper");
        secondOidcClaimToRoleMapper.setIdentityProviderMapper(ExternalPassportRoleToRoleMapper.PROVIDER_ID);
