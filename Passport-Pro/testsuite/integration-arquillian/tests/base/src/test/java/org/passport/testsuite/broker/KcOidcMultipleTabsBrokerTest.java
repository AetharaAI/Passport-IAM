...
public class KcOidcMultipleTabsBrokerTest  extends AbstractInitializedBaseBrokerTest {
    ...
    public void testAuthenticationExpiredWithMoreBrowserTabs_loginExpiredInProvider() throws Exception {
        ...
        try (BrowserTabUtil tabUtil = BrowserTabUtil.getInstanceAndSetEnv(driver);
            ...
            org.passport.testsuite.Assert.assertNotNull(authzResponse.getCode());
            org.passport.testsuite.Assert.assertNull(authzResponse.getError());
        }
    }
    ...
    private void assertOnAppPageWithAlreadyLoggedInError() {
        ...
        AuthorizationEndpointResponse authzResponse = oauth.parseLoginResponse();
        org.passport.testsuite.Assert.assertEquals(OAuthErrorException.TEMPORARILY_UNAVAILABLE, authzResponse.getError());
        org.passport.testsuite.Assert.assertEquals(Constants.AUTHENTICATION_EXPIRED_MESSAGE, authzResponse.getErrorDescription());
    }
