...
import org.passport.saml.processing.core.parsers.saml.xmldsig.XmlDSigQNames;
import org.passport.saml.processing.core.parsers.util.HasQName;
...
import org.passport.testsuite.updaters.IdentityProviderCreator;
import org.passport.testsuite.util.IdentityProviderBuilder;
import org.passport.testsuite.util.SamlClientBuilder;
import org.passport.testsuite.util.saml.SamlBackchannelArtifactResolveReceiver;
...
import static org.passport.saml.SignatureAlgorithm.RSA_SHA1;
    ...
    public void testNoNameIDAndPrincipalFromAttribute() throws IOException {
        ...
        try (IdentityProviderCreator idp = new IdentityProviderCreator(realm, rep)) {
            new SamlClientBuilder()
                    ...
                    .assertResponse(org.passport.testsuite.util.Matchers.statusCodeIsHC(200))
                    .execute();
        }
        ...
        try (IdentityProviderCreator idp = new IdentityProviderCreator(realm, addIdentityProvider("https://saml.idp/"))) {
            new SamlClientBuilder()
              ...
              .processSamlResponse(REDIRECT)
                ...
                .targetBinding(POST)
                .build()
              .assertResponse(org.passport.testsuite.util.Matchers.statusCodeIsHC(Status.OK))
              .execute();
        }
    }
    ...
    private void assertExpired(XMLGregorianCalendar notBefore, XMLGregorianCalendar notOnOrAfter, boolean shouldPass) throws Exception {
        ...
        try (IdentityProviderCreator idp = new IdentityProviderCreator(realm, addIdentityProvider("https://saml.idp/"))) {
            new SamlClientBuilder()
              ...
              .targetUri(getSamlBrokerUrl(REALM_NAME))
              .build()
              .assertResponse(org.passport.testsuite.util.Matchers.statusCodeIsHC(expectedStatus))
              .execute();
        }
    }
    ...
    public void testResolveArtifactBindingAsSp() {
        ...
        try (SamlBackchannelArtifactResolveReceiver samlBackchannelArtifactResolveReceiver = new SamlBackchannelArtifactResolveReceiver(
            ...
            try (IdentityProviderCreator idp = new IdentityProviderCreator(realm, rep)) {
                ...
                samlClientBuilder.assertResponse(org.passport.testsuite.util.Matchers.statusCodeIsHC(Status.OK));
