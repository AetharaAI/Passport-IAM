...
package org.passport.testsuite.oauth.tokenexchange;
...
public class ClientTokenExchangeSAML2Test extends AbstractPassportTest {
    ...
    public static void setupRealm(PassportSession session) {
    ...
    public void testExchangeToSAML2SignedAssertion() throws Exception {
        ...
        org.passport.testsuite.util.oauth.AccessTokenResponse response = oauth.doPasswordGrantRequest("user", "password");
    ...
    public void testBadImpersonator() throws Exception {
        ...
        org.passport.testsuite.util.oauth.AccessTokenResponse response = oauth.doPasswordGrantRequest("bad-impersonator", "password");
        String accessToken = response.getAccessToken();
        TokenVerifier<AccessToken> accessTokenVerifier = TokenVerifier.create(accessToken, AccessToken.class);
        AccessToken token = accessTokenVerifier.parse().getToken();
    ...
    private static void addTargetClients(PassportSession session) {
        RealmModel realm = session.realms().getRealmByName(TEST);
    ...
    private static void addDirectExchanger(PassportSession session) {
        RealmModel realm = session.realms().getRealmByName(TEST);
    ...
    private PublicKey publicKeyFromString() {
        ...
        return org.passport.testsuite.util.KeyUtils.publicKeyFromString(keyRep.getPublicKey());
    }

    private PrivateKey privateKeyFromString(String privateKey) {
        return org.passport.testsuite.util.KeyUtils.privateKeyFromString(privateKey);
    }
}
