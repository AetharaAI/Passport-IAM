...
import org.passport.storage.ldap.LDAPUtils;
import org.passport.storage.ldap.idm.model.LDAPDn;
...
import org.passport.storage.ldap.mappers.membership.MembershipType;
import org.passport.storage.ldap.mappers.membership.group.GroupLDAPStorageMapper;
...
import static org.passport.testsuite.util.LDAPTestUtils.getGroupDescriptionLDAPAttrName;
...
public class LDAPGroupMapperTest extends AbstractLDAPTest {
    ...
    protected void test01_ldapOnlyGroupMappings(boolean importEnabled) {
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            UserModel mary = session.users().getUserByUsername(appRealm, "marypassport");

            // 1 - Grant some groups in LDAP

            // This group should already exists as it was imported from LDAP
            GroupModel group1 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1");
            john.joinGroup(group1);

            // This group should already exists as it was imported from LDAP
            GroupModel group11 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1/group11");
            mary.joinGroup(group11);

            // This group should already exists as it was imported from LDAP
            GroupModel group12 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1/group12");
            john.joinGroup(group12);
            mary.joinGroup(group12);

            // This group should already exists as it was imported from LDAP
            GroupModel groupWithSlashesInName = PassportModelUtils.findGroupByPath(session, appRealm, "Team 2016/2017");
            john.joinGroup(groupWithSlashesInName);
            mary.joinGroup(groupWithSlashesInName);

            // This group should already exists as it was imported from LDAP
            GroupModel groupChildWithSlashesInName = PassportModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019");
            john.joinGroup(groupChildWithSlashesInName);
            mary.joinGroup(groupChildWithSlashesInName);

            Assert.assertEquals("Team SubChild 2020/2021", PassportModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019/Team SubChild 2020/2021").getName());
            Assert.assertEquals("defaultGroup14", PassportModelUtils.findGroupByPath(session, appRealm, "defaultGroup13/Team SubChild 2022/2023/A/B/C/D/E/defaultGroup14").getName());
            Assert.assertEquals("Team SubChild 2026/2027", PassportModelUtils.findGroupByPath(session, appRealm, "Team Root 2024/2025/A/B/C/D/defaultGroup15/Team SubChild 2026/2027").getName());
        });


        // 2 - Check that group mappings are not in local Passport DB (They are in LDAP).
        if (importEnabled) {
            testingClient.server().run(session -> {
                LDAPTestContext ctx = LDAPTestContext.init(session);
                RealmModel appRealm = ctx.getRealm();

                UserModel johnDb = UserStoragePrivateUtil.userLocalStorage(session).getUserByUsername(appRealm, "johnpassport");
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel group11 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1/group11");
            GroupModel group12 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1/group12");
            GroupModel groupTeam20162017 = PassportModelUtils.findGroupByPath(session, appRealm, "Team 2016/2017");
            GroupModel groupTeamChild20182019 = PassportModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019");
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            UserModel mary = session.users().getUserByUsername(appRealm, "marypassport");
            ...
            Assert.assertEquals("johnpassport", group1Members.get(0).getUsername());
            Assert.assertEquals(1, group11Members.size());
            Assert.assertEquals("marypassport", group11Members.get(0).getUsername());
    ...
    protected void test02_readOnlyGroupMappings(boolean importEnabled) {
        testingClient.server().run(session -> {
            ...
            LDAPObject maryLdap = ctx.getLdapProvider().loadLDAPUserByUsername(appRealm, "marypassport");
        ...
        if (importEnabled) {
            testingClient.server().run(session -> {
                ...
                UserModel maryDB = UserStoragePrivateUtil.userLocalStorage(session).getUserByUsername(appRealm, "marypassport");
                ...
                Assert.assertEquals("marypassport", group12Members.get(0).getUsername());
        ...
        testingClient.server().run(session -> {
            ...
            List<UserModel> group11Members = session.users().getGroupMembersStream(appRealm, group11, 0, 10)
                    .collect(Collectors.toList());
            Assert.assertEquals(1, group1Members.size());
            Assert.assertEquals("marypassport", group1Members.get(0).getUsername());
            Assert.assertEquals(1, group11Members.size());
            Assert.assertEquals("marypassport", group11Members.get(0).getUsername());
    ...
    public void test03_importGroupMappings() {
        testingClient.server().run(session -> {
            ...
            LDAPObject robLdap = ldapProvider.loadLDAPUserByUsername(appRealm, "robpassport");
            ...
            UserModel rob = session.users().getUserByUsername(appRealm, "robpassport");
            ...
            Assert.assertEquals(1, group11Members.size());
            Assert.assertEquals("robpassport", group11Members.get(0).getUsername());
            Assert.assertEquals(1, group12Members.size());
            Assert.assertEquals("robpassport", group12Members.get(0).getUsername());
    ...
    public void test04_groupReferencingNonExistentMember() {
        testingClient.server().run(session -> {
            ...
            // Ignoring this test on ActiveDirectory as it's not allowed to have LDAP group referencing nonexistent member. PASSPORT-2682 was related to OpenLDAP TODO: Better solution than programmatic...
            ...
            LDAPObject jamesLdap = ldapProvider.loadLDAPUserByUsername(appRealm, "jamespassport");
            ...
            groupMapper.syncDataFromFederationProviderToPassport(appRealm);
            GroupModel kcGroup2 = PassportModelUtils.findGroupByPath(session, appRealm, "/group2");
            ...
            Assert.assertEquals("jamespassport", rob.getUsername());
    ...
    public void test05_getGroupsFromUserMemberOfStrategyTest() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject carlos = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "carlospassport", "Carlos", "Doel", "carlos.doel@email.org", ldapGroupDN, "1234");
        ...
        testingClient.server().run(session -> {
            ...
            // Get user in Passport. Ensure that he is member of requested group
            UserModel carlos = session.users().getUserByUsername(appRealm, "carlospassport");
            ...
            assertThat(group1Members.get(0).getUsername(), equalTo("carlospassport"));
        });
    ...
    // PASSPORT-5017
    @Test
    public void test06_addingUserToNewPassportGroup() throws Exception {
        // Add some groups to Passport
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            RealmModel appRealm = ctx.getRealm();
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel group14 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1/group14");
            GroupModel group31 = PassportModelUtils.findGroupByPath(session, appRealm, "/group3/group31");
            GroupModel group32 = PassportModelUtils.findGroupByPath(session, appRealm, "/group3/group32");
            GroupModel group4 = PassportModelUtils.findGroupByPath(session, appRealm, "/group4");
        ...
        testingClient.server().run(session -> {
            ...
            Assert.assertEquals(0, john.getGroupsStream("Passport", 0, 10).count());
        });
    }
    ...
    public void test07_newUserDefaultGroupsImportModeTest() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            UserModel david = session.users().addUser(appRealm, "davidpassport");

            GroupModel defaultGroup11 = PassportModelUtils.findGroupByPath(session, appRealm, "/defaultGroup1/defaultGroup11");
            Assert.assertNotNull(defaultGroup11);

            GroupModel defaultGroup12 = PassportModelUtils.findGroupByPath(session, appRealm, "/defaultGroup1/defaultGroup12");
    ...
    public void test08_ldapOnlyGroupMappingsRanged() {
        testingClient.server().run(session -> {
            ...
            GroupModel kcBigGroup = PassportModelUtils.findGroupByPath(session, appRealm, "/biggroup");
    ...
    public void test09_emptyMemberOnDeletionWorks() {
        testingClient.server().run(session -> {
            ...
            // Ignoring this test on ActiveDirectory (same for rhds) as it's not allowed to have LDAP group referencing nonexistent member. PASSPORT-2682 was related to OpenLDAP TODO: Better solution than programmatic...
            LDAPConfig ldapConfig = ctx.getLdapProvider().getLdapIdentityStore().getConfig();
            if (ldapConfig.isActiveDirectory() || LDAPConstants.VENDOR_RHDS.equals(ldapConfig.getVendor())) {
                return;
            }
            ...
            // import into passport
            LDAPStorageProvider ldapProvider = LDAPTestUtils.getLdapProvider(session, ctx.getLdapModel());
            GroupLDAPStorageMapper groupMapper = LDAPTestUtils.getGroupMapper(mapperModel, ldapProvider, appRealm);
            groupMapper.syncDataFromFederationProviderToPassport(appRealm);

            // check everything is OK
            GroupModel kcDeleteGroup = PassportModelUtils.findGroupByPath(session, appRealm, "/deletegroup");
            UserModel mary = session.users().getUserByUsername(appRealm, "marypassport");
            ...
            Assert.assertEquals("marypassport", groupMembers.get(0).getUsername());
            Set<GroupModel> maryGroups = mary.getGroupsStream().collect(Collectors.toSet());
    ...
    public void test10_multipleGroupMappersInDifferentPaths() {
        ...
        testingClient.server().run(session -> {
            ...
            // 3- Add a couple of group mappers that map groups from different OUs into different group paths in Passport.
            ...
            // 4- Sync LDAP groups from both mappers to Passport DB
            ComponentModel mapperModel = LDAPTestUtils.getSubcomponentByName(realm, ldapModel, "group-mapper-1");
            new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(realm);
        ...
        testingClient.server(TEST_REALM_NAME).run(session -> {
            ...
            GroupModel groupA = PassportModelUtils.findGroupByPath(session, realm, "/ldap-groups-1/GroupA");
            GroupModel groupB = PassportModelUtils.findGroupByPath(session, realm, "/ldap-groups-1/GroupB");
            GroupModel groupC = PassportModelUtils.findGroupByPath(session, realm, "/ldap-groups-2/GroupC");

            UserModel john = session.users().getUserByUsername(realm, "johnpassport");
            UserModel mary = session.users().getUserByUsername(realm, "marypassport");
            ...
            Assert.assertEquals("johnpassport", groupAMembers.get(0));
            Assert.assertEquals(1, groupBMembers.size());
            Assert.assertEquals("marypassport", groupBMembers.get(0));
            Assert.assertEquals(2, groupCMembers.size());
            Assert.assertTrue(groupCMembers.contains("johnpassport"));
            Assert.assertTrue(groupCMembers.contains("marypassport"));
        });
