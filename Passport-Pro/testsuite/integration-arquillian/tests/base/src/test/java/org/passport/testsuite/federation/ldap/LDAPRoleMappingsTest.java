...
public class LDAPRoleMappingsTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject mary = LDAPTestUtils.addLDAPUser(ldapFedProvider, appRealm, "marypassport", "Mary", "Kelly", "mary@email.org", null, "5678");
            LDAPTestUtils.updateLDAPPassword(ldapFedProvider, mary, "Password1");

            LDAPObject rob = LDAPTestUtils.addLDAPUser(ldapFedProvider, appRealm, "robpassport", "Rob", "Brown", "rob@email.org", null, "8910");
            ...
            // Sync LDAP roles to Passport DB
    ...
    public void test01_ldapOnlyRoleMappings() {
        testingClient.server().run(session -> {
            ...
            // 2 - Check that role mappings are not in local Passport DB (They are in LDAP).
    ...
    public void test02_readOnlyRoleMappings() {
        testingClient.server().run(session -> {
            ...
            LDAPObject maryLdap = ldapProvider.loadLDAPUserByUsername(appRealm, "marypassport");
    ...
    public void test04_syncRoleMappings() {
        testingClient.server().run(session -> {
            ...
            LDAPTestUtils.addOrUpdateRoleLDAPMappers(appRealm, ctx.getLdapModel(), LDAPGroupMapperMode.LDAP_ONLY);
            SynchronizationResult syncResult = UserStoragePrivateUtil.runPeriodicSync(session.getPassportSessionFactory(), new UserStorageProviderModel(ctx.getLdapModel()));
            syncResult.getAdded();
        });
        ...
        testingClient.server().run(session -> {
            ...
            UserStoragePrivateUtil.runPeriodicSync(session.getPassportSessionFactory(), new UserStorageProviderModel(ctx.getLdapModel()));
        });
    ...
    public void test05_getRolesFromUserMemberOfStrategyTest() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject carlos = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "carlospassport", "Carlos", "Doel", "carlos.doel@email.org", ldapRoleDN, "1234");
