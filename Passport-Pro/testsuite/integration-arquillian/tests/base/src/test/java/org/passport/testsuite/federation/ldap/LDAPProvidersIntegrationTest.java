...
import org.passport.models.cache.CachedUserModel;
...
import org.passport.storage.ldap.mappers.HardcodedLDAPAttributeMapper;
import org.passport.storage.ldap.mappers.HardcodedLDAPAttributeMapperFactory;
import org.passport.storage.ldap.mappers.HardcodedLDAPGroupStorageMapper;
import org.passport.storage.ldap.mappers.HardcodedLDAPGroupStorageMapperFactory;
import org.passport.storage.ldap.mappers.HardcodedLDAPRoleStorageMapper;
import org.passport.storage.ldap.mappers.HardcodedLDAPRoleStorageMapperFactory;
...
import org.passport.testsuite.AbstractAuthTest;
...
public class LDAPProvidersIntegrationTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        testingClient.server().run(session -> {
            ...
            LDAPTestUtils.addLocalUser(session, appRealm, "marypassport", "mary@test.com", "password-app");
            ...
            LDAPObject john = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "johnpassport", "John", "Doe", "john@email.org", null, "1234");
    ...
    public void testSyncRegistrationWithCreateDNRelativeToBaseDN() {
        testingClient.server().run(session -> {
            ...
            ctx.getLdapModel().put(LDAPConstants.USERS_DN, "dc=passport,dc=org");
        ...
        MatcherAssert.assertThat(newUser.firstAttribute(LDAPConstants.LDAP_ENTRY_DN), Matchers.containsString("=newuser0,ou=People,dc=passport,dc=org"));
        ...
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            ctx.getLdapModel().put(LDAPConstants.USERS_DN, "ou=People,dc=passport,dc=org");
    ...
    public void testSyncRegistrationEmailRDNNoDefault() {
        testingClient.server().run(session -> {
            ...
            ComponentModel hardcodedMapperModel = PassportModelUtils.createComponentModel("hardcodedAttr-description", ctx.getLdapModel().getId(), HardcodedLDAPAttributeMapperFactory.PROVIDER_ID, LDAPStorageMapper.class.getName(),
        ...
        try {
            ...
            newUser1 = AbstractAuthTest.createUserRepresentation("newuser1", "newuser1@passport-pro.ai", "newuser1", "newuser1", true);
            String userId;
            try (Response resp = testRealm().users().create(newUser1)) {
                userId = ApiUtil.getCreatedId(resp);
            }
            ...
            Assert.assertEquals("newuser1@passport-pro.ai", newUser1.getEmail());
            ...
            Assert.assertEquals("newuser1@passport-pro.ai", emailValueInLdap);
    ...
    public void testSyncRegistrationEmailRDNDefaultValue() {
        testingClient.server().run(session -> {
            ...
            ComponentModel emailMapper = LDAPTestUtils.getSubcomponentByName(ctx.getRealm(), ldapModel, "email");
            emailMapper.getConfig().putSingle(UserAttributeLDAPStorageMapper.IS_MANDATORY_IN_LDAP, "true");
            emailMapper.getConfig().putSingle(UserAttributeLDAPStorageMapper.ATTRIBUTE_DEFAULT_VALUE, "empty@passport-pro.ai");
            ctx.getRealm().updateComponent(emailMapper);
        });
        try {
            ...
            Assert.assertEquals("empty@passport-pro.ai", emailValueInLdap);
    ...
    public void testRemoveImportedUsers() {
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            UserModel user = session.users().getUserByUsername(ctx.getRealm(), "johnpassport");
        ...
        testingClient.server().run(session -> {
            ...
            UserModel user = UserStoragePrivateUtil.userLocalStorage(session).getUserByUsername(appRealm, "johnpassport");
    ...
    public void deleteFederationLink() throws Exception {
        // PASSPORT-4789: Login in client, which requires consent
        oauth.clientId("third-party");
        loginPage.open();
        loginPage.login("johnpassport", "Password1");
    ...
    public void loginClassic() {
        loginPage.open();
        loginPage.login("marypassport", "password-app");
    ...
    public void loginLdap() {
        ...
        UserRepresentation userRepresentation = AccountHelper.getUserRepresentation(adminClient.realm(TEST_REALM_NAME), "johnpassport");
    ...
    public void loginLdapWithDirectGrant() throws Exception {
        AccessTokenResponse response = oauth.doPasswordGrantRequest("johnpassport", "Password1");
        Assert.assertEquals(200, response.getStatusCode());
        AccessToken accessToken = oauth.verifyToken(response.getAccessToken());

        response = oauth.doPasswordGrantRequest("johnpassport", "");
    ...
    public void ldapPasswordChangeWithAccountConsole() throws Exception {
        Assert.assertTrue(AccountHelper.updatePassword(testRealm(), "johnpassport", "New-password1"));

        loginPage.open();
        loginPage.login("johnpassport", "Bad-password1");
        Assert.assertEquals("Invalid username or password.", loginPage.getInputError());

        loginPage.open();
        loginPage.login("johnpassport", "New-password1");
        Assert.assertEquals(AppPage.RequestType.AUTH_RESPONSE, appPage.getRequestType());

        // Change password back to previous value
        Assert.assertTrue(AccountHelper.updatePassword(testRealm(), "johnpassport", "Password1"));
    }
    ...
    public void testCommaInUsername() {
        ...
        if (!skipTest) {
            // Try to import the user with comma in username into Passport
    ...
    public void testHardcodedRoleMapper() {
        testingClient.server().run(session -> {
            ...
            // assert that user "johnpassport" doesn't have hardcoded role
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            Assert.assertFalse(john.hasRole(hardcodedRole));

            ComponentModel hardcodedMapperModel = PassportModelUtils.createComponentModel("hardcoded role", ctx.getLdapModel().getId(),
        ...
        testingClient.server().run(session -> {
            ...
            // Assert user is successfully imported in Passport DB now with correct firstName and lastName
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
    ...
    public void testHardcodedGroupMapper() {
        ...
        testingClient.server().run(session -> {
            ...
            // assert that user "johnpassport" doesn't have hardcoded group
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            Assert.assertFalse(john.isMemberOf(hardcodedGroup));

            ComponentModel hardcodedMapperModel = PassportModelUtils.createComponentModel("hardcoded group",
    ...
    public void testImportExistingUserFromLDAP() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "marypassport", "Mary1", "Kelly1", "mary1@email.org", null, "123");
        ...
        // Try to import the duplicated LDAP user into Passport
        loginPage.open();
    ...
    public void testReadonly() {
        ...
        UserRepresentation userRep = ApiUtil.findUserByUsername(testRealm(), "johnpassport");
    ...
    public void testUnsynced() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            try {
                LDAPObject ldapUser = ctx.getLdapProvider().loadLDAPUserByUsername(appRealm, "johnpassport");
        ...
        UserResource userResource = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
        ...
        testingClient.server().run(session -> {
            ...
            Assert.assertNull(UserStoragePrivateUtil.userLocalStorage(session).getUserByUsername(appRealm, "johnpassport"));
            Assert.assertNotNull(session.users().getUserByUsername(appRealm, "johnpassport"));
        });
        ...
        testingClient.server().run(session -> {
            ...
            user.setUsername("johnpassport-renamed");
        });

        // check user is found just once
        List<UserRepresentation> users = testRealm().users().search("johnpassport", 0, 2);
    ...
    public void testSearchWithPartiallyCachedUser() {
        ...
        List<UserRepresentation> users = adminClient.realm("test").users().search("johnpassport", 0, 10, false);
        Assert.assertTrue(users.stream().anyMatch(userRepresentation -> "johnpassport".equals(userRepresentation.getUsername())));
    }
    ...
    public void testAlwaysReadValueFromLdapCached() throws Exception {
        try {
            testingClient.testing().setTestingInfinispanTimeService();
            // import user from the ldap johnpassport and cache it reading it by id
            List<UserRepresentation> users = testRealm().users().search("johnpassport", true);
            Assert.assertEquals(1, users.size());
            ...
            testingClient.server().run(session -> {
                LDAPTestContext ctx = LDAPTestContext.init(session);
                LDAPObject johnLdapObject = ctx.getLdapProvider().loadLDAPUserByUsername(ctx.getRealm(), "johnpassport");
            ...
            users = testRealm().users().search("johnpassport", true);
            Assert.assertEquals(1, users.size());
    ...
    public void testAlwaysReadValueFromLdapNoCache() throws Exception {
        ...
        try {
            // import user from the ldap johnpassport
            List<UserRepresentation> users = testRealm().users().search("johnpassport", true);
            Assert.assertEquals(1, users.size());
            UserRepresentation john = users.iterator().next();
            Assert.assertEquals("Doe", john.getLastName());
            ...
            testingClient.server().run(session -> {
                LDAPTestContext ctx = LDAPTestContext.init(session);
                RealmModel appRealm = ctx.getRealm();
                LDAPObject johnLdapObject = ctx.getLdapProvider().loadLDAPUserByUsername(appRealm, "johnpassport");
                johnLdapObject.setSingleAttribute(LDAPConstants.SN, "sn-modified");
                ctx.getLdapProvider().getLdapIdentityStore().update(johnLdapObject);
            });
    ...
    public void testUserAttributeLDAPStorageMapperHandlingUsernameLowercasing() {
        ...
        testingClient.server().run(session -> {
            ...
            UserModel johnpassport = session.users().getUserByUsername(appRealm, "johnpassport");
            // If the username was case sensitive in the username-cn mapper, then this would throw an exception
            johnpassport.setSingleAttribute(UserModel.USERNAME, "JohnPassport");
        });
    ...
    public void updateLDAPUsernameTest() {
        ...
        String origPassportUserId = testingClient.server().fetchString(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
        ...
        testingClient.server().run(session -> {
            ...
            Assert.assertNotNull(user);
            String newPassportUserId = user.getId();
            // Need to remove double quotes from server response
            Assert.assertEquals(origPassportUserId.replace("\"",""), newPassportUserId);
        });
    }
