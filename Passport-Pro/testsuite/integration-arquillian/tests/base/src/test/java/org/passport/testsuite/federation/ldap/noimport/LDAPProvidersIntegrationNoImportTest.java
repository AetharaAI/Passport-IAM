...
import org.passport.admin.client.resource.ComponentResource;
...
import org.passport.testsuite.federation.ldap.LDAPProvidersIntegrationTest;
...
public class LDAPProvidersIntegrationNoImportTest extends LDAPProvidersIntegrationTest {
    ...
    public void testFullNameMapperWriteOnly() {
        ComponentRepresentation firstNameMapperRep = testingClient.server().fetch(session -> {
            ...
            ComponentModel fullNameMapperModel = PassportModelUtils.createComponentModel("full name", ldapModel.getId(), FullNameLDAPStorageMapperFactory.PROVIDER_ID, LDAPStorageMapper.class.getName(),
        ...
        // Assert changed user available in Passport, but his firstName is null (due the fullnameMapper is write-only and firstName mapper is removed)
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            RealmModel appRealm = ctx.getRealm();
    ...
    public void testImpossibleToChangeNonLDAPMappedAttributes() {
        UserResource john = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
    ...
    public void testCannotUpdateReadOnlyUserImportDisabled() {
        UserResource user = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
        ...
        try {
            ...
            Assert.assertEquals("The user is read-only. Not possible to write 'required action VERIFY_EMAIL' when updating user 'johnpassport'.", bde.getResponse().readEntity(ErrorRepresentation.class).getErrorMessage());
        }
    }
