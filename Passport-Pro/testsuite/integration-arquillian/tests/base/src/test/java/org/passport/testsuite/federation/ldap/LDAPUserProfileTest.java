...
import static org.passport.userprofile.UserProfileUtil.USER_METADATA_GROUP;
...
public class LDAPUserProfileTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        testingClient.server().run(session -> {
            ...
            UserModel user = LDAPTestUtils.addLocalUser(session, appRealm, "marypassport", "mary@test.com", "Password1");
            ...
            LDAPObject john2 = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "johnpassport2", "John", "Doe", "john2@email.org", null, "1234");
    ...
    public void testUserProfile() {
        // Test user profile of user johnpassport in admin API
        UserResource johnResource = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
        UserRepresentation john = johnResource.toRepresentation(true);

        assertUser(john, "johnpassport", "john@email.org", "John", "Doe", "1234");
    ...
    public void testUserProfileWithReadOnlyLdap() {
        // Test user profile of user johnpassport in admin console as well as account console. Check attributes are writable.
        setLDAPReadOnly();
        try {
            UserResource johnResource = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
            UserRepresentation john = johnResource.toRepresentation(true);
    ...
    public void testUserProfileWithReadOnlyLdapLocalUser() {
        ...
        setLDAPReadOnly();
        try {
            UserResource maryResource = ApiUtil.findUserByUsernameId(testRealm(), "marypassport");
            ...
            loginPage.open();
            loginPage.login("marypassport", "Password1");
            updateProfilePage.assertCurrent();
    ...
    public void testUserProfileWithoutImport() {
        ...
        try {
            ...
            // Test local user is writable and has only attributes defined explicitly in user-profile
            // Test user profile of user johnpassport in admin API
            UserResource johnResource = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport2");
            UserRepresentation john = johnResource.toRepresentation(true);

            assertUser(john, "johnpassport2", "john2@email.org", "John", "Doe", "1234");
            assertProfileAttributes(john, null, false, "username", "email", "firstName", "lastName", "postal_code");
            assertProfileAttributes(john, USER_METADATA_GROUP, true, LDAPConstants.LDAP_ID, LDAPConstants.LDAP_ENTRY_DN);
        } finally {
    ...
    public void testMultipleLDAPProviders() {
        ...
        userResource = ApiUtil.findUserByUsernameId(testRealm(), "johnpassport");
    ...
    public void testUsernameRespectFormatFromExternalStore() {
        ...
        String upperCaseUsername = "JOHNPASSPORT3";
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session, "test-ldap");
            RealmModel appRealm = ctx.getRealm();
