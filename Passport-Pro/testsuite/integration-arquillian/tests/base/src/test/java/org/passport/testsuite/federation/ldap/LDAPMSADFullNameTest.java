...
public class LDAPMSADFullNameTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        testingClient.server().run(session -> {
            ...
            mapperModel = PassportModelUtils.createComponentModel("fullNameWritable", ldapModel.getId(), FullNameLDAPStorageMapperFactory.PROVIDER_ID, LDAPStorageMapper.class.getName(),
...
//    }
    ...
    public void test01_addUserWithoutFullName() {
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().addUser(appRealm, "johnpassport");
            john.setEmail("johnpassport@email.cz");
        });
        ...
        testingClient.server().run(session -> {
            ...
            assertDnStartsWith(session, ctx, john, "cn=johnpassport");
    ...
    public void test02_registerUserWithFullName() {
        ...
        registerPage.register("Johny", "Anthony", "johnyanth@check.cz", "johnpassport", "Password1", "Password1");
        ...
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            assertUser(session, ctx, john, "johnpassport", "Johny", "Anthony", true, "cn=Johny Anthony");
    ...
    public void test03_addUserWithFirstNameOnly() {
        ...
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            assertUser(session, ctx, john, "johnpassport", "Johnyyy", "", true, "cn=Johnyyy");
    ...
    public void test04_addUserWithLastNameOnly() {
        ...
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            assertUser(session, ctx, john, "johnpassport", "", "Anthonyy", true, "cn=Anthonyy");
    ...
    public void test05_registerUserWithFullNameSpecialChars() {
        ...
        registerPage.register("Jož,o", "Baříč", "johnyanth@check.cz", "johnpassport", "Password1", "Password1");
        Assert.assertEquals(AppPage.RequestType.AUTH_RESPONSE, appPage.getRequestType());
        ...
        testingClient.server().run(session -> {
            ...
            UserModel john = session.users().getUserByUsername(appRealm, "johnpassport");
            assertUser(session, ctx, john, "johnpassport", "Jož,o", "Baříč", true, "cn=Jož\\,o Baříč");
    ...
    private static void assertUser(PassportSession session, LDAPTestContext ctx, UserModel user, String expectedUsername, String expectedFirstName, String expectedLastName, boolean expectedEnabled, String expectedDn) {
    ...
    private static void assertDnStartsWith(PassportSession session, LDAPTestContext ctx, UserModel user, String expectedRDn) {
