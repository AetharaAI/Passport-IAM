...
public class LDAPSyncTest extends AbstractLDAPTest {
    ...
    public void test02duplicateUsernameAndEmailSync() {
        ...
        testingClient.server().run(session -> {
            ...
            SynchronizationResult result = UserStoragePrivateUtil.runFullSync(session.getPassportSessionFactory(), ctx.getLdapModel());
        ...
        testingClient.server().run(session -> {
            ...
            result = UserStoragePrivateUtil.runFullSync(session.getPassportSessionFactory(), ctx.getLdapModel());
    ...
    // PASSPORT-10770 user-storage/{id}/sync should return 400 instead of 404
    @Test
    ...
    public void test08LDAPGroupSyncAfterGroupRename() {
        testingClient.server().run(session -> {
            ...
            // sync groups to Passport
            new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(appRealm);
        });
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel kcGroup1 = PassportModelUtils.findGroupByPath(session, appRealm, "/group1");
            String descriptionAttrName = LDAPTestUtils.getGroupDescriptionLDAPAttrName(ctx.getLdapProvider());
        ...
        testingClient.server().run(session -> {
            ...
            // sync to Passport should pass without an error
            SynchronizationResult syncResult = new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(appRealm);
        ...
        testingClient.server().run(session -> {
            ...
            // load previously synced group (a new group has been created in Passport)
            GroupModel kcGroup5 = PassportModelUtils.findGroupByPath(session, appRealm, "/group5");
            String descriptionAttrName = LDAPTestUtils.getGroupDescriptionLDAPAttrName(ctx.getLdapProvider());
    ...
    public void test09MembershipUsingDifferentAttributes() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel user8Group = PassportModelUtils.findGroupByPath(session, appRealm, "/user8group");
