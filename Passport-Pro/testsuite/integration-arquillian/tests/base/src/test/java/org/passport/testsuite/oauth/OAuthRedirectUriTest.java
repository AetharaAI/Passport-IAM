...
import static org.passport.testsuite.util.oauth.OAuthClient.APP_ROOT;
 ...
 */
public class OAuthRedirectUriTest extends AbstractPassportTest {
    ...
    public void afterAbstractPassportTest() throws Exception {
        super.afterAbstractPassportTest();
    ...
    public void addTestRealms(List<RealmRepresentation> testRealms) {
        ...
        ClientBuilder installedAppCustomScheme = ClientBuilder.create().clientId("custom-scheme").name("custom-scheme")
                .redirectUris("android-app://org.passport.examples.cordova/https/passport-cordova-example.github.io/login")
                .secret("password");
    ...
    public void testWithCustomScheme() throws IOException {
        ...
        oauth.redirectUri("android-app://org.passport.examples.cordova/https/passport-cordova-example.github.io/login");
        oauth.openLoginForm();
        ...
        try {
            ...
            response.getStatus();
            assertThat(response.getFirstHeader("Location"), Matchers.startsWith("android-app://org.passport.examples.cordova/https/passport-cordova-example.github.io/login"));
        } finally {
            client.close();
        }
    }
    ...
    private void checkRedirectUri(String redirectUri, boolean expectValid, boolean checkCodeToToken) throws IOException {
        ...
        if (!expectValid) {
            ...
            if (!checkCodeToToken) {
                ...
                // Test that browser URL where Passport redirected user matches with the used redirectUri
                String browserUrlAfterRedirectFromPassport = PassportUriBuilder.fromUri(driver.getCurrentUrl())
                        ...
                        .build().toString();
                if (browserUrlAfterRedirectFromPassport.endsWith("/")) browserUrlAfterRedirectFromPassport = browserUrlAfterRedirectFromPassport.substring(0, browserUrlAfterRedirectFromPassport.length() - 1);
                if (Constants.INSTALLED_APP_URN.equals(redirectUri)) {
                    assertThat(browserUrlAfterRedirectFromPassport, Matchers.endsWith("oauth/oob"));
                } else {
                    Assert.assertEquals(redirectUri, browserUrlAfterRedirectFromPassport);
                }
