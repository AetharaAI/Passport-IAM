...
import static org.passport.common.Profile.Feature.DYNAMIC_SCOPES;
...
import static org.passport.testsuite.util.oauth.OAuthClient.SERVER_ROOT;
...
public class LoginTest extends AbstractChangeImportedUserPasswordsTest {
    ...
    public void loginDifferentUserAfterDisabledUserThrownOut() {
        ...
        try {
            ...
            loginPage.login("passport-user@localhost", getPassword("passport-user@localhost"));
    ...
    public void loginSuccessRealmSigningAlgorithms() throws JWSInputException {
        ...
        String passportIdentity = driver.manage().getCookieNamed("PASSPORT_IDENTITY").getValue();

        // Check identity cookie is signed with HS256
        String algorithm = new JWSInput(passportIdentity).getHeader().getAlgorithm().name();
        ...
        try {
            ...
            driver.navigate().to(getAuthServerContextRoot() + "/auth/realms/test/");
            passportIdentity = driver.manage().getCookieNamed("PASSPORT_IDENTITY").getValue();

            // Check identity cookie is still signed with HS256
            algorithm = new JWSInput(passportIdentity).getHeader().getAlgorithm().name();
            assertEquals(Constants.INTERNAL_SIGNATURE_ALGORITHM, algorithm);
