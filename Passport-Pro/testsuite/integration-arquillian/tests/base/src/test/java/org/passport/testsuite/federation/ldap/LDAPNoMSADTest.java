...
public class LDAPNoMSADTest extends AbstractLDAPTest {
    ...
    public void testUpdateWithUnmappedRdnAttribute() {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject john2 = LDAPTestUtils.addLDAPUser(ldapProvider, appRealm, "johnpassport2", "John2", "Doe2", "john2@email.org", null, "4321");
        ...
        // Try to update johnpassport2 user. It shouldn't try to update DN
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            RealmModel appRealm = ctx.getRealm();

            UserModel johnpassport2 = session.users().getUserByUsername(appRealm, "johnpassport2");
            Assert.assertNotNull(johnpassport2);

            johnpassport2.setFirstName("foo2");
            johnpassport2.setLastName("foo");
        });
    ...
    public void testMultivaluedRDN() {
        testingClient.server().run(session -> {
            ...
            // Create LDAP user with both "uid" and "sn" attribute in RDN. Something like "uid=johnpassport3+sn=Doe3,ou=People,dc=domain,dc=com"
            LDAPStorageProvider ldapProvider = LDAPTestUtils.getLdapProvider(session, ctx.getLdapModel());
            LDAPObject john2 = LDAPTestUtils.addLDAPUser(ldapProvider, appRealm, "johnpassport3", "John3", "Doe3", "john3@email.org", null, "4321");
            ...
            Assert.assertEquals("sn=Doe3+" + rdnAttrName + "=johnpassport3", john2.getDn().getFirstRdn().toString());
        });

        // Update some user attributes not mapped to DN. DN won't be changed
        String userId = testRealm().users().search("johnpassport3").get(0).getId();
        UserResource user = testRealm().users().get(userId);

        UserRepresentation userRep = user.toRepresentation();
        assertFirstRDNEndsWith(userRep, "johnpassport3", "Doe3");
        ...
        assertFirstRDNEndsWith(userRep, "johnpassport3", "Doe3Changed");
