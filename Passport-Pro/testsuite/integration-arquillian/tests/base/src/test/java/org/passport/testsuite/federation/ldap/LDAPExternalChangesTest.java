...
public class LDAPExternalChangesTest extends AbstractLDAPTest {
    ...
    public void failAuthenticationIfEmailDifferentThanExternalStorage() {
        testingClient.server().run((session) -> {
            ...
            LDAPObject john = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), realm, "johnpassport", "John", "Doe", "john@email.org", null, "1234");
        ...
        String updatedEmail = "updatedjohnpassport@email.org";
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
