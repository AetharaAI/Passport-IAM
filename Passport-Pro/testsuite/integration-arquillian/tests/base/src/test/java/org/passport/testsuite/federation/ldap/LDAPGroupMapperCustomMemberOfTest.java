...
public class LDAPGroupMapperCustomMemberOfTest extends AbstractLDAPTest {
    ...
    public void getGroupsFromUserMemberOfStrategyWithFilterTest() throws Exception {
        testingClient.server().run(session -> {
            ...
            LDAPObject carlos = LDAPTestUtils.addLDAPUser(ctx.getLdapProvider(), appRealm, "carlospassport", "Carlos", "Doel", "carlos.doel@email.org", "street", "1234");
            ...
            // Sync LDAP groups to Passport DB - it should bring only group2 due to the defined filter 
            ComponentModel mapperModel = LDAPTestUtils.getSubcomponentByName(appRealm, ctx.getLdapModel(), "groupsMapper");
            SynchronizationResult syncDataResult = new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(appRealm);
        ...
        testingClient.server().run(session -> {
            ...
            // Get user in Passport. It should not load any new groups where Carlos is memberOf due to defined filter
