...
import org.passport.jose.jwk.ECPublicJWK;
import org.passport.jose.jwk.JWK;
import org.passport.jose.jwk.JWKBuilder;
import org.passport.jose.jwk.RSAPublicJWK;
import org.passport.jose.jws.JWSHeader;
import org.passport.keys.AbstractEddsaKeyProviderFactory;
...
import org.passport.services.clientpolicy.executor.DPoPBindEnforcerExecutorFactory;
...
import org.passport.testsuite.broker.util.SimpleHttpDefault;
...
import org.passport.util.DPoPGenerator;
...
import static org.passport.OAuth2Constants.DPOP_HTTP_HEADER;
import static org.passport.OAuth2Constants.DPOP_JWT_HEADER_TYPE;
import static org.passport.OAuthErrorException.INVALID_TOKEN;
import static org.passport.services.util.DPoPUtil.DPOP_SCHEME;
import static org.passport.services.util.DPoPUtil.DPOP_TOKEN_TYPE;
...
import static org.passport.testsuite.util.ClientPoliciesUtil.createDPoPBindEnforcerExecutorConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createEcJwk;
import static org.passport.testsuite.util.ClientPoliciesUtil.createRsaJwk;
import static org.passport.testsuite.util.ClientPoliciesUtil.generateEcdsaKey;
import static org.passport.testsuite.util.ClientPoliciesUtil.generateSignedDPoPProof;
...
public class DPoPTest extends AbstractTestRealmPassportTest {
    ...
    public void beforeDPoPTest() throws Exception {
        ...
        jwsRsaHeader = new JWSHeader(org.passport.jose.jws.Algorithm.PS256, DPOP_JWT_HEADER_TYPE, jwkRsa.getKeyId(), jwkRsa);
        ...
        jwsEcHeader = new JWSHeader(org.passport.jose.jws.Algorithm.ES256, DPOP_JWT_HEADER_TYPE, jwkEc.getKeyId(), jwkEc);
    ...
    public void testDPoPProofByConfidentialClient_EdDSA() throws Exception {
        ...
        // Header
        JWSHeader jwsEdHeader = new JWSHeader(org.passport.jose.jws.Algorithm.valueOf(Algorithm.EdDSA), DPOP_JWT_HEADER_TYPE, jwkEd.getKeyId(), jwkEd);
    ...
    public void testDPoPProofWithoutJwk() throws Exception {
        JWSHeader jwsHeader = new JWSHeader(org.passport.jose.jws.Algorithm.ES256, DPOP_JWT_HEADER_TYPE, jwkEc.getKeyId(), null);
    ...
    public void testDPoPProofInvalidAlgorithm() throws Exception {
        JWSHeader jwsHeader = new JWSHeader(org.passport.jose.jws.Algorithm.none, DPOP_JWT_HEADER_TYPE, jwkEc.getKeyId(), jwkEc);
    ...
    public void testDPoPProofInvalidType() throws Exception {
        JWSHeader jwsEcHeader = new JWSHeader(org.passport.jose.jws.Algorithm.ES256, "jwt+dpop", jwkEc.getKeyId(), jwkEc);
    ...
    public void testDPoPProofInvalidSignature() throws Exception {
        testDPoPProofFailure(generateSignedDPoPProof(UUID.randomUUID().toString(), HttpMethod.POST, oauth.getEndpoints().getToken(), (long) Time.currentTime(),
                Algorithm.PS256, jwsEcHeader, rsaKeyPair.getPrivate(), null, new TestingDPoPGenerator()), "DPoP verification failure: org.passport.exceptions.TokenSignatureInvalidException: Invalid token signature");
    }
    ...
    public void testDPoPProofOnUserInfoWithMissingAcccessTokenHash() throws Exception {
        ...
        JWSHeader jwsRsaHeader1 = new JWSHeader(org.passport.jose.jws.Algorithm.PS256, DPOP_JWT_HEADER_TYPE, jwkRsa1.getKeyId(), jwkRsa1);
        // No ath
    ...
    public void testInvalidDPoPProofOnUserInfo() throws Exception {
        ...
        JWSHeader jwsRsaHeader = new JWSHeader(org.passport.jose.jws.Algorithm.PS256, DPOP_JWT_HEADER_TYPE, jwkRsa.getKeyId(), jwkRsa);
        // invalid "htu" claim
    ...
    public void testDPoPBindEnforcerExecutor() throws Exception {
        ...
        JWSHeader jwsEcHeader = new JWSHeader(org.passport.jose.jws.Algorithm.ES256, DPOP_JWT_HEADER_TYPE, jwkEc.getKeyId(), jwkEc);
        String dpopProofEcEncoded = generateSignedDPoPProof(UUID.randomUUID().toString(), HttpMethod.POST, oauth.getEndpoints().getToken(), (long) Time.currentTime(), Algorithm.ES256, jwsEcHeader, ecKeyPair.getPrivate(), null);
    ...
    public void testDPoPAdminRequestSuccess() throws Exception {
        ...
        try (Passport adminClientDPoP = AdminClientUtil.createAdminClient(false, ServerURLs.getAuthServerContextRoot(), REALM_NAME,
