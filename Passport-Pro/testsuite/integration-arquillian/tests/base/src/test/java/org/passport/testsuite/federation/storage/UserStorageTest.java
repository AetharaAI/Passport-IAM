...
import org.passport.credential.CredentialProviderFactory;
...
import org.passport.storage.CacheableStorageProviderModel.CachePolicy;
...
import org.passport.testsuite.federation.UserMapStorage;
import org.passport.testsuite.federation.UserMapStorageFactory;
import org.passport.testsuite.federation.UserPropertyFileStorageFactory;
...
import org.passport.testsuite.pages.VerifyEmailPage;
...
import org.passport.testsuite.util.TestCleanup;
import org.passport.testsuite.util.userprofile.UserProfileUtil;
import org.passport.userprofile.DefaultAttributes;
...
import static org.passport.models.UserModel.RequiredAction.UPDATE_PROFILE;
import static org.passport.storage.UserStorageProviderModel.CACHE_POLICY;
import static org.passport.storage.UserStorageProviderModel.EVICTION_DAY;
import static org.passport.storage.UserStorageProviderModel.EVICTION_HOUR;
import static org.passport.storage.UserStorageProviderModel.EVICTION_MINUTE;
import static org.passport.storage.UserStorageProviderModel.IMPORT_ENABLED;
import static org.passport.storage.UserStorageProviderModel.MAX_LIFESPAN;
import static org.passport.testsuite.actions.RequiredActionEmailVerificationTest.getEmailLink;
import static org.passport.testsuite.util.URLAssert.assertCurrentUrlDoesntStartWith;
...
public class UserStorageTest extends AbstractAuthTest {
    ...
    public void afterTestCleanUp() throws URISyntaxException, IOException {
        testingClient.server().run(session -> {
            ...
            UserMapStorageFactory userMapStorageFactory = (UserMapStorageFactory) session.getPassportSessionFactory().getProviderFactory(UserStorageProvider.class, UserMapStorageFactory.PROVIDER_ID);
...
//    @Test
    ...
    @Test
    @ModelTest
    public void testCast(PassportSession session) throws Exception {
        session.getPassportSessionFactory().getProviderFactoriesStream(CredentialProvider.class)
    ...
    public void testRegisterWithRequiredEmail() throws Exception {
        try (AutoCloseable c = new RealmAttributeUpdater(testRealmResource())
          .updateWith(r -> {
            ...
            config.put("from", "auto@passport-pro.ai");
    ...
    @Test
    @ModelTest
    public void testCredentialCRUD(PassportSession session) {
        ...
        PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession currentSession) -> {
            ...
            org.passport.testsuite.Assert.assertEquals(0, credentials.count());
        ...
        PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession currentSession) -> {
            ...
            org.passport.testsuite.Assert.assertTrue(user.credentialManager().moveStoredCredentialTo(otp2Id.get(), null));
        });
    ...
    private void assertOrder(List<CredentialModel> creds, String... expectedIds) {
        org.passport.testsuite.Assert.assertEquals(expectedIds.length, creds.size());

        if (creds.size() == 0) return;

        for (int i=0 ; i<expectedIds.length ; i++) {
            org.passport.testsuite.Assert.assertEquals(creds.get(i).getId(), expectedIds[i]);
        }
    }
