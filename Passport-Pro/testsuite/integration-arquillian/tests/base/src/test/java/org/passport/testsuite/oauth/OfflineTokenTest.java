...
import static org.passport.testsuite.admin.ApiUtil.findRealmRoleByName;
 ...
 */
public class OfflineTokenTest extends AbstractPassportTest {
    ...
    public void offlineTokenUserNotAllowed() {
        String userId = findUserByUsername(adminClient.realm("test"), "passport-user@localhost").getId();

        oauth.scope(OAuth2Constants.OFFLINE_ACCESS);
        oauth.client("offline-client", "secret1");
        oauth.redirectUri(offlineClientAppUri);
        oauth.doLogin("passport-user@localhost", "password");
    ...
    public void offlineTokenAdminRESTAccess() {
        ...
        try (Passport offlineTokenAdmin = Passport.getInstance(getAuthServerContextRoot() + "/auth",
    ...
    public void browserOfflineTokenLogoutFollowedByLoginSameSession() {
        ...
        String offlineUserSessionId = testingClient.server().fetch((PassportSession session) ->
