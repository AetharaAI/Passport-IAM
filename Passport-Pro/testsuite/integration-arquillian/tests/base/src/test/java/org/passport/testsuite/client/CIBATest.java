...
import org.passport.models.CibaConfig;
import org.passport.protocol.oidc.OIDCAdvancedConfigWrapper;
import org.passport.protocol.oidc.grants.ciba.CibaGrantType;
import org.passport.protocol.oidc.grants.ciba.channel.AuthenticationChannelRequest;
import org.passport.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse;
import org.passport.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory;
import org.passport.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSessionEnforceExecutorFactory;
import org.passport.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSignedAuthenticationRequestExecutor;
import org.passport.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSignedAuthenticationRequestExecutorFactory;
import org.passport.protocol.oidc.grants.ciba.endpoints.ClientNotificationEndpointRequest;
...
import org.passport.representations.oidc.OIDCClientRepresentation;
...
import org.passport.services.clientpolicy.condition.ClientRolesConditionFactory;
import org.passport.services.clientpolicy.condition.ClientScopesConditionFactory;
import org.passport.services.clientpolicy.condition.ClientUpdaterContextConditionFactory;
import org.passport.services.clientpolicy.executor.ConfidentialClientAcceptExecutorFactory;
import org.passport.services.clientpolicy.executor.HolderOfKeyEnforcerExecutorFactory;
import org.passport.services.clientpolicy.executor.SecureSigningAlgorithmForSignedJwtExecutorFactory;
import org.passport.testsuite.AssertEvents;
import org.passport.testsuite.admin.ApiUtil;
import org.passport.testsuite.client.policies.AbstractClientPoliciesTest;
import org.passport.testsuite.client.resources.TestApplicationResourceUrls;
import org.passport.testsuite.client.resources.TestOIDCEndpointsApplicationResource;
import org.passport.testsuite.rest.representation.TestAuthenticationChannelRequest;
import org.passport.testsuite.rest.resource.TestingOIDCEndpointsApplicationResource.AuthorizationEndpointRequestObject;
import org.passport.testsuite.services.clientpolicy.executor.TestRaiseExceptionExecutorFactory;
import org.passport.testsuite.util.ClientPoliciesUtil;
...
import org.passport.testsuite.util.InfinispanTestTimeServiceRule;
import org.passport.testsuite.util.PassportModelUtils;
import org.passport.testsuite.util.MutualTLSUtils;
import org.passport.testsuite.util.RoleBuilder;
import org.passport.testsuite.util.ServerURLs;
import org.passport.testsuite.util.UserBuilder;
import org.passport.testsuite.util.oauth.AccessTokenResponse;
import org.passport.testsuite.util.oauth.LogoutResponse;
import org.passport.testsuite.util.oauth.ciba.AuthenticationRequestAcknowledgement;
...
import static org.passport.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.CANCELLED;
import static org.passport.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.SUCCEED;
import static org.passport.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.UNAUTHORIZED;
import static org.passport.testsuite.AbstractAdminTest.loadJson;
import static org.passport.testsuite.Assert.assertExpiration;
import static org.passport.testsuite.util.ClientPoliciesUtil.createAnyClientConditionConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createClientRolesConditionConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createClientScopesConditionConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createClientUpdateContextConditionConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig;
import static org.passport.testsuite.util.ClientPoliciesUtil.createTestRaiseExeptionExecutorConfig;
...
public class CIBATest extends AbstractClientPoliciesTest {
    ...
    public void addTestRealms(List<RealmRepresentation> testRealms) {
        ...
        ClientRepresentation confApp = PassportModelUtils.createClient(realm, SECOND_TEST_CLIENT_NAME);
...
//            oauth.ciba().doAuthenticationChannelCallback(SECOND_TEST_CLIENT_NAME, SECOND_TEST_CLIENT_SECRET, usernameAuthenticated, authenticationChannelReq.getBearerToken(), SUCCEEDED);
    ...
    public void testBackchannelAuthenticationFlowWithInvalidSignedAuthenticationRequest() throws Exception {
        ...
        try {
            ...
            requestObject.audience(Urls.realmIssuer(new URI(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth"), REALM_NAME), "https://example.com");
            requestObject.issuer(TEST_CLIENT_NAME);
            requestObject.id(org.passport.models.utils.PassportModelUtils.generateId());
            requestObject.iat(Long.valueOf(Time.currentTime()));
    ...
    public void testSecureCibaAuthenticationRequestSigningAlgorithmEnforceExecutor() throws Exception {
        ...
        String cAppAdminId = createClientByAdmin(generateSuffixedName("App-by-Admin"), (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.passport.crypto.Algorithm.ES256);
            });
        ClientRepresentation cRep = getClientByAdmin(cAppAdminId);
        assertEquals(org.passport.crypto.Algorithm.ES256, cRep.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));
        ...
        assertEquals(org.passport.crypto.Algorithm.PS256, cRep2.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));

        // update by Admin REST API - fail
        try {
            updateClientByAdmin(cAppAdminId, (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.passport.crypto.Algorithm.RS512);
            });
        ...
        updateClientByAdmin(cAppAdminId, (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.passport.crypto.Algorithm.PS384);
        });
        cRep = getClientByAdmin(cAppAdminId);
        assertEquals(org.passport.crypto.Algorithm.PS384, cRep.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));
        ...
        json = (new ClientProfilesBuilder()).addProfile(
                (new ClientProfileBuilder()).createProfile(PROFILE_NAME, "Den Forsta Profilen")
                    .addExecutor(SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory.PROVIDER_ID,
                            createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig(org.passport.crypto.Algorithm.ES256))
                    .toRepresentation()
                ).toString();
        ...
        assertEquals(org.passport.crypto.Algorithm.ES256, cRep2.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));

        // update profiles, fall back to PS256
        json = (new ClientProfilesBuilder()).addProfile(
                (new ClientProfileBuilder()).createProfile(PROFILE_NAME, "Den Forsta Profilen")
                    .addExecutor(SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory.PROVIDER_ID,
                            createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig(org.passport.crypto.Algorithm.RS512))
                    .toRepresentation()
                ).toString();
        updateProfiles(json);
        ...
        try {
            createClientByAdmin(generateSuffixedName("App-in-Dynamic"), (ClientRepresentation clientRep) -> {
                    clientRep.setSecret("secret");
                    clientRep.setAttributes(new HashMap<>());
                    clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.passport.crypto.Algorithm.RS384);
                });
            fail();
        } catch (ClientPolicyException e) {
            assertEquals(OAuthErrorException.INVALID_REQUEST, e.getMessage());
        ...
        String cAppDynamicClientId = createClientDynamically(generateSuffixedName("App-in-Dynamic"), (OIDCClientRepresentation clientRep) -> {
                clientRep.setBackchannelAuthenticationRequestSigningAlg(org.passport.crypto.Algorithm.ES256);
            });
        ...
        try {
            updateClientDynamically(cAppDynamicClientId, (OIDCClientRepresentation clientRep) -> {
                     clientRep.setBackchannelAuthenticationRequestSigningAlg(org.passport.crypto.Algorithm.RS256);
                 });
            fail();
        } catch (ClientRegistrationException e) {
            assertEquals(ERR_MSG_CLIENT_REG_FAIL, e.getMessage());
        }
        assertEquals(org.passport.crypto.Algorithm.ES256, getClientDynamically(cAppDynamicClientId).getBackchannelAuthenticationRequestSigningAlg());

        // update dynamically - success
        updateClientDynamically(cAppDynamicClientId, (OIDCClientRepresentation clientRep) -> {
                clientRep.setBackchannelAuthenticationRequestSigningAlg(org.passport.crypto.Algorithm.ES384);
            });
        assertEquals(org.passport.crypto.Algorithm.ES384, getClientDynamically(cAppDynamicClientId).getBackchannelAuthenticationRequestSigningAlg());
        ...
        assertEquals(org.passport.crypto.Algorithm.PS256, cAppDynamicClient2Rep.getBackchannelAuthenticationRequestSigningAlg());
        ...
        assertEquals(org.passport.crypto.Algorithm.ES256, cAppDynamicClient2Rep.getBackchannelAuthenticationRequestSigningAlg());
    }
    ...
    private TestAuthenticationChannelRequest doAuthenticationChannelRequest(String bindingMessage) {
        // get Authentication Channel Request passport has done on Backchannel Authentication Endpoint from the FIFO queue of testing Authentication Channel Request API
        TestOIDCEndpointsApplicationResource oidcClientEndpointsResource = testingClient.testApp().oidcClientEndpoints();
        TestAuthenticationChannelRequest authenticationChannelReq = oidcClientEndpointsResource.getAuthenticationChannel(bindingMessage);
