...
import org.passport.representations.idm.SynchronizationResultRepresentation;
...
public class LDAPGroupMapperSyncTest extends AbstractLDAPTest {
    ...
    public void test03_syncWithGroupInheritance() throws Exception {
        testingClient.server().run(session -> {
            ...
            // PASSPORT-11415 - This test requires the group mapper to be configured with preserve group inheritance
            ...
            // Assert groups are imported to passport including their inheritance from LDAP
            GroupModel kcGroup1 = PassportModelUtils.findGroupByPath(session, realm, "/group1");
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group11"));
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group12"));
            ...
            syncResult = new GroupLDAPStorageMapperFactory().create(session, mapperModel).syncDataFromFederationProviderToPassport(realm);
            LDAPTestAsserts.assertSyncEquals(syncResult, 0, 3, 0, 0);

            // Assert attributes changed in passport
            kcGroup1 = PassportModelUtils.findGroupByPath(session, realm, "/group1");
            kcGroup12 = PassportModelUtils.findGroupByPath(session, realm, "/group1/group12");
    ...
    public void test04_syncWithDropNonExistingGroups() throws Exception {
        testingClient.server().run(session -> {
            ...
            // Assert groups are imported to passport including their inheritance from LDAP
            GroupModel kcGroup1 = PassportModelUtils.findGroupByPath(session, realm, "/group1");
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group12"));

            Assert.assertEquals(2, kcGroup1.getSubGroupsStream().count());

            // Create some new groups in passport
            ...
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group12"));
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/model1"));
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/model2"));
            ...
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group11"));
            Assert.assertNotNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/group12"));
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/model1"));
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group1/model2"));
        });
    }
    ...
    public void test05_syncNoPreserveGroupInheritanceWithLazySync() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            LDAPObject johnLdap = LDAPTestUtils.addLDAPUser(ldapProvider, realm, "johnpassport", "John", "Doe", "john@email.org", null, "1234");
            ...
            // Assert groups not yet imported to Passport DB
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group1"));
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group11"));
            Assert.assertNull(PassportModelUtils.findGroupByPath(session, realm, "/group12"));

            // Load user from LDAP to Passport DB
            ...
            GroupModel group1 = PassportModelUtils.findGroupByPath(session, realm, "/group1");
            GroupModel group11 = PassportModelUtils.findGroupByPath(session, realm, "/group11");
            GroupModel group12 = PassportModelUtils.findGroupByPath(session, realm, "/group12");
    ...
    public void test06SyncRestAPI() {
        ...
        try {
            // testing PASSPORT-3980 which threw an NPE because I was looking up the factory wrong.
    ...
    // PASSPORT-8253 - Test if synchronization of large number of LDAP groups takes linear time
    ...
    public void test08_flatSynchWithBatchSizeLessThanNumberOfGroups() {
        ...
        testingClient.server().run(session -> {
            ...
            // synch groups a first time - imports all new groups into passport.
            LDAPStorageMapper groupMapper = new GroupLDAPStorageMapperFactory().create(session, mapperModel);
            SynchronizationResult syncResult = groupMapper.syncDataFromFederationProviderToPassport(realm);
            LDAPTestAsserts.assertSyncEquals(syncResult, 3, 0, 0, 0);

            // check all groups were imported as top level groups with no subgroups.
            Stream.of("/group1", "/group11", "/group12").forEach(path -> {
                GroupModel kcGroup = PassportModelUtils.findGroupByPath(session, realm, path);
            ...
            syncResult = groupMapper.syncDataFromFederationProviderToPassport(realm);
            LDAPTestAsserts.assertSyncEquals(syncResult, 0, 3, 0, 0);
        });
