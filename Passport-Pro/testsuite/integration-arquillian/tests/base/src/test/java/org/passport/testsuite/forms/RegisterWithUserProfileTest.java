 ...
 */
public class RegisterWithUserProfileTest extends AbstractTestRealmPassportTest {
    ...
    public void testEmailNotAllowedButEmailAsUsername() {
        ...
        registerPage.registerWithEmailAsUsername("firstName", "lastName", "myusername1@passport-pro.ai", generatePassword());

        assertEquals(RequestType.AUTH_RESPONSE, appPage.getRequestType());
        UserRepresentation user = testRealm().users().search("myusername1@passport-pro.ai").get(0);
        assertEquals("myusername1@passport-pro.ai", user.getEmail());
        UserRepresentation rep = testRealm().users().get(user.getId()).toRepresentation();
        assertEquals("myusername1@passport-pro.ai", rep.getEmail());
    }
