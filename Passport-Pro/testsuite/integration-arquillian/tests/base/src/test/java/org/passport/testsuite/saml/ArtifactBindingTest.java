...
import org.passport.dom.saml.v2.protocol.NameIDMappingResponseType;
...
import org.passport.testsuite.util.SamlUtils;
import org.passport.testsuite.util.saml.HandleArtifactStepBuilder;
import org.passport.testsuite.util.saml.SamlMessageReceiver;
import org.passport.testsuite.util.saml.SessionStateChecker;
...
import static org.passport.testsuite.util.Matchers.isSamlLogoutRequest;
import static org.passport.testsuite.util.Matchers.isSamlResponse;
import static org.passport.testsuite.util.Matchers.isSamlStatusResponse;
import static org.passport.testsuite.util.Matchers.statusCodeIsHC;
import static org.passport.testsuite.util.SamlClient.Binding.ARTIFACT_RESPONSE;
import static org.passport.testsuite.util.SamlClient.Binding.POST;
import static org.passport.testsuite.util.SamlClient.Binding.REDIRECT;
import static org.passport.testsuite.util.SamlUtils.getSPInstallationDescriptor;
...
public class ArtifactBindingTest extends AbstractSamlTest {
    ...
    public void testSessionStateDuringArtifactBindingLogoutWithMoreFrontChannelClients() {
        ...
        SAMLDocumentHolder response = new SamlClientBuilder()
                ...
                // Since sales_post uses frontchannel logout, passport should send LogoutRequest to sales_post first
                .handleArtifact(getAuthServerSamlEndpoint(REALM_NAME), SAML_CLIENT_ID_SALES_POST)
                    .setBeforeStepChecks(new SessionStateChecker(testingClient.server())
                        .expectedUserSession(userSessionId)
                ...
                // Now Passport should finish logout process so it should respond with LogoutResponse
                .handleArtifact(getAuthServerSamlEndpoint(REALM_NAME), SAML_CLIENT_ID_SALES_POST2)
                    .verifyRedirect(true)
                    .setBeforeStepChecks(new SessionStateChecker(testingClient.server())
                        .expectedUserSession(userSessionId)
    ...
    public void testImportClientArtifactResolutionSingleServices() {
        ...
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ARTIFACT_RESOLUTION_SERVICE_URL_ATTRIBUTE), is("https://test.passport.com/auth/login/epd/callback/soap"));
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ASSERTION_CONSUMER_URL_ARTIFACT_ATTRIBUTE), is("https://test.passport.com/auth/login/epd/callback/http-artifact"));
    }
    ...
    public void testImportClientMultipleServices() {
        ...
        ClientRepresentation clientRep = adminClient.realm(REALM_NAME).convertClientDescription(IOUtil.documentToString(doc));
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ARTIFACT_RESOLUTION_SERVICE_URL_ATTRIBUTE), is("https://test.passport.com/auth/login/epd/callback/soap-1"));
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ASSERTION_CONSUMER_URL_ARTIFACT_ATTRIBUTE), Matchers.startsWith("https://test.passport.com/auth/login/epd/callback/http-artifact"));
    }
    ...
    public void testImportClientMultipleServicesWithDefault() {
        ...
        ClientRepresentation clientRep = adminClient.realm(REALM_NAME).convertClientDescription(IOUtil.documentToString(doc));
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ARTIFACT_RESOLUTION_SERVICE_URL_ATTRIBUTE), is("https://test.passport.com/auth/login/epd/callback/soap-9"));
        assertThat(clientRep.getAttributes().get(SamlProtocol.SAML_ASSERTION_CONSUMER_URL_ARTIFACT_ATTRIBUTE), Matchers.startsWith("https://test.passport.com/auth/login/epd/callback/http-artifact"));
    }
