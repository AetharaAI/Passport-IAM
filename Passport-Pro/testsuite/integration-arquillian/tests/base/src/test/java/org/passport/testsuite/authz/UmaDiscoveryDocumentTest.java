...
import org.passport.authorization.config.UmaConfiguration;
import org.passport.authorization.config.UmaWellKnownProviderFactory;
...
public class UmaDiscoveryDocumentTest extends AbstractPassportTest {
    ...
    public void testFetchDiscoveryDocumentUsingFrontEndUrl() {
        ...
        final String frontendUrl = "https://mypassport/auth";
        ...
        try (Response response = oidcDiscoveryTarget.request().get()) {
            ...
            String baseFrontendUri = UriBuilder
                    .fromUri(frontendUrl)
                    .path(RealmsResource.class).path(RealmsResource.class, "getRealmResource").scheme("https").host("mypassport").port(-1).build(realmsResouce().realm("test").toRepresentation().getRealm()).toString();
