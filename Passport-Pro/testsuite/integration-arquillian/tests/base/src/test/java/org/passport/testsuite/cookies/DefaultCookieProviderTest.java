package org.passport.testsuite.cookies;
...
public class DefaultCookieProviderTest extends AbstractPassportTest {

    private PassportTestingClient testing;
    ...
    public void testCookieDefaults() {
        ...
        assertCookie(response, "PASSPORT_IDENTITY", "my-identity", "/auth/realms/master/", 333, true, true, "None", true);
        assertCookie(response, "PASSPORT_LOCALE", "my-locale", "/auth/realms/master/", -1, true, true, "None", false);
        assertCookie(response, "PASSPORT_REMEMBER_ME", "my-username", "/auth/realms/master/", 31536000, true, true, "None", false);
        assertCookie(response, "PASSPORT_SESSION", "my-session", "/auth/realms/master/", 444, true, false, "None", true);
    ...
    public void testCookieDefaultsWithInsecureContext() {
        PassportTestingClient testingInsecure = PassportTestingClient.getInstance("http://localtest.me:8180/auth");
        ...
        assertCookie(response, "PASSPORT_IDENTITY", "my-identity", "/auth/realms/master/", 333, false, true, "Lax", true);
        assertCookie(response, "PASSPORT_LOCALE", "my-locale", "/auth/realms/master/", -1, false, true, "Lax", false);
        assertCookie(response, "PASSPORT_REMEMBER_ME", "my-username", "/auth/realms/master/", 31536000, false, true, "Lax", false);
        assertCookie(response, "PASSPORT_SESSION", "my-session", "/auth/realms/master/", 444, false, false, "Lax", true);
    ...
    public void testSessionCookieValue() {
        ...
        filter.setHeader("Cookie", "PASSPORT_SESSION=\"" + sessionValue + "\";");
    ...
    public void testExpireOldUnused() {
        filter.setHeader("Cookie", "AUTH_SESSION_ID_LEGACY=legacy; PASSPORT_IDENTITY_LEGACY=legacy; PASSPORT_SESSION_LEGACY=ignore");
        ...
        assertCookie(response, "PASSPORT_IDENTITY_LEGACY", "", "/auth/realms/master/", 0, false, false, null, false);
        assertCookie(response, "PASSPORT_SESSION_LEGACY", "", "/auth/realms/master/", 0, false, false, null, false);
    }
    ...
    public void testSafariQuirks() {
        ...
        try (PassportTestingClient testingClient = createTestingClient("http://localhost:8180/auth")) {
    ...
    private PassportTestingClient createTestingClient(String serverUrl) {
        ResteasyClientBuilder restEasyClientBuilder = PassportTestingClient.getRestEasyClientBuilder(serverUrl);
        ResteasyClient resteasyClient = restEasyClientBuilder.build();
        resteasyClient.register(filter);
        return PassportTestingClient.getInstance(serverUrl, resteasyClient);
    }
