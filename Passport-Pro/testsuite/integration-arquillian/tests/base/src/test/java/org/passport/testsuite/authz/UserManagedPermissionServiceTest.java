...
import org.passport.authorization.client.resource.PolicyResource;
...
import static org.passport.authorization.model.Policy.FilterOption.OWNER;
...
public class UserManagedPermissionServiceTest extends AbstractResourceServerTest {
    ...
    private static void testRemovePolicyWhenOwnerDeleted(PassportSession session) {
    ...
    private static void testRemovePoliciesOnResourceDelete(PassportSession session) {
        RealmModel realm = session.realms().getRealmByName("authz-test");
        ClientModel client = realm.getClientByClientId("resource-server-test");
        AuthorizationProvider provider = session.getProvider(AuthorizationProvider.class);
