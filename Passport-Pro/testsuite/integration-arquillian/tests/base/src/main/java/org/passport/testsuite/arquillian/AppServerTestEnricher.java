...
import org.passport.testsuite.arquillian.annotation.AppServerContainers;
import org.passport.testsuite.arquillian.containers.SelfManagedAppContainerLifecycle;
...
import static org.passport.testsuite.arquillian.ServerTestEnricherUtil.reloadOrRestartTimeoutClient;
...
public class AppServerTestEnricher {
    ...
    public static void enableHTTPSForManagementClient(OnlineManagementClient client) throws CommandFailedException, InterruptedException, TimeoutException, IOException, CliException, OperationException {
        ...
        if (!operations.exists(Address.subsystem("elytron").and("server-ssl-context", "KCSslContext"))) {
            ...
            client.execute("/subsystem=elytron/key-store=KCTrustStore:add(relative-to=jboss.server.config.dir,path=passport.truststore,credential-reference={clear-text=secret},type=JKS)");
        ...
        client.execute("/system-property=javax.net.ssl.trustStore:add(value=${jboss.server.config.dir}/passport.truststore)");
