...
import org.passport.testsuite.model.StoreProvider;
 ...
 * @author mhajas
 */
public class PassportQuarkusServerDeployableContainer extends AbstractQuarkusDeployableContainer implements RemoteContainer {

    private static final int DEFAULT_SHUTDOWN_TIMEOUT_SECONDS = 10;

    private static final Logger log = Logger.getLogger(PassportQuarkusServerDeployableContainer.class);
    ...
    private void importRealm() throws IOException, URISyntaxException {
        if (suiteContext.get().isAuthServerMigrationEnabled() && configuration.getImportFile() != null) {
            ...
            Path passportConf = Paths.get(wrkDir.toURI()).getParent().resolve("conf").resolve("passport.conf");

            // there are several issues with import in initial quarkus versions, so better use the passport.conf file
            StoreProvider storeProvider = StoreProvider.getCurrentProvider();
            List<String> storageOptions = storeProvider.getStoreOptionsToPassportConfImport();
            Path passportConfBkp = null;
            try {
                if (!storageOptions.isEmpty()) {
                    passportConfBkp = passportConf.getParent().resolve("passport.conf.bkp");
                    Files.copy(passportConf, passportConfBkp);
                    // write the options to the file
                    try ( BufferedWriter w = new BufferedWriter(new FileWriter(passportConf.toFile(), true))) {
                ...
                // restore initial passport.conf if modified for import
                if (passportConfBkp != null && Files.exists(passportConfBkp)) {
                    Files.move(passportConfBkp, passportConf, StandardCopyOption.REPLACE_EXISTING);
                }
            }
        }
    }
    ...
    protected void checkLiveness() {
        if (!container.isAlive()) {
            throw new IllegalStateException("Passport unexpectedly died :(");
        }
    }
