...
TEST_DIR='../src/test/java/org/passport/testsuite'
BASE_PACKAGE='org.passport.testsuite'
...
for i in `ls -d $TEST_DIR/*/ | sed "s|$TEST_DIR||g" | sed "s|/||g"`; do
  if ( ! cat $TESTSUITE_FILE | grep "^$i," >/dev/null ); then
    echo "Package 'org.passport.testsuite.$i' not defined in base-suite"
    exit 1
  fi
done
...
for i in `echo $PACKAGES`; do
  ...
  if [ ! -d "$TEST_DIR/$PACKAGE" ]; then
    echo "Package 'org.passport.testsuite.$PACKAGE' not found"
    exit 1
  fi
