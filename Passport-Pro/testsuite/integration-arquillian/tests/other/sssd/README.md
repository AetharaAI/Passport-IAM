...
This module contains integration tests for testing the SSSD features of Passport.
...
*All the commands are intended to be run from the root `passport` project directory.*

First build the distribution of passport:
```
mvn clean install -B -DskipTests -Pdistribution
```

It may fail in the end, but it's not a problem as far as it creates a zip distribution of Passport inside
