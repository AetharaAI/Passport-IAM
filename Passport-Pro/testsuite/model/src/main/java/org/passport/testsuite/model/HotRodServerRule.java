...
public class HotRodServerRule extends ExternalResource {
    ...
    public void createEmbeddedHotRodServer(Config.Scope config) {
        ...
        // create remote passport caches
        createPassportCaches(config.getBoolean("async", false) ? CacheMode.REPL_ASYNC : CacheMode.REPL_SYNC);

        // Use Passport time service in remote caches
        InfinispanUtil.setTimeServiceToPassportTime(hotRodCacheManager);
        InfinispanUtil.setTimeServiceToPassportTime(hotRodCacheManager2);
    }

    private void createPassportCaches(CacheMode cacheMode) {
