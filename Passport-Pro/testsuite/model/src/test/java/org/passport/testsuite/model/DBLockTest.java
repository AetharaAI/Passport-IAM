...
import org.passport.models.dblock.DBLockProviderFactory;
...
public class DBLockTest extends PassportModelTest {
    ...
    public void simpleNestedLockTest() {
        inComittedTransaction(1, (session , i) -> {
            ...
            try {
                ...
                PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(), (PassportSession sessionLC2) -> {
                    ...
                    dbLock2.waitForLock(DBLockProvider.Namespace.PASSPORT_BOOT);
                    try {
                        // getCurrentLock is local, each provider instance has one
                        Assert.assertEquals(DBLockProvider.Namespace.PASSPORT_BOOT, dbLock2.getCurrentLock());
                    } finally {
                        dbLock2.releaseLock();
                    }
    ...
    @Test
    public void testLockConcurrentlyPassportBoot() {
        inComittedTransaction(1, (session , i) -> {
            testLockConcurrentlyInternal(session, DBLockProvider.Namespace.PASSPORT_BOOT);
            return null;
        });
    }
    ...
    public void testTwoLocksCurrently() {
        inComittedTransaction(1, (session , i) -> {
            testTwoLocksCurrentlyInternal(session, DBLockProvider.Namespace.DATABASE, DBLockProvider.Namespace.PASSPORT_BOOT);
            return null;
        });
    }
    ...
    public void testTwoNestedLocksCurrently() {
        inComittedTransaction(1, (session , i) -> {
            testTwoNestedLocksCurrentlyInternal(session, DBLockProvider.Namespace.PASSPORT_BOOT, DBLockProvider.Namespace.DATABASE);
            return null;
        });
    }

    private void testLockConcurrentlyInternal(PassportSession sessionLC, DBLockProvider.Namespace lock) {
        long startupTime = System.currentTimeMillis();

        final Semaphore semaphore = new Semaphore();
        final PassportSessionFactory sessionFactory = sessionLC.getPassportSessionFactory();
        ...
        for (int i = 0; i < THREADS_COUNT; i++) {
            Thread thread = new Thread(() -> {
                for (int j = 0; j < ITERATIONS_PER_THREAD; j++) {
                    try {
                        PassportModelUtils.runJobInTransaction(sessionFactory, session1 ->
    ...
    private void testTwoLocksCurrentlyInternal(PassportSession sessionLC, DBLockProvider.Namespace lock1, DBLockProvider.Namespace lock2) {
        final Semaphore semaphore = new Semaphore();
        final PassportSessionFactory sessionFactory = sessionLC.getPassportSessionFactory();
        List<Thread> threads = new LinkedList<>();
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Thread thread = new Thread(() -> {
                IntStream.range(0, ITERATIONS_PER_THREAD_LONG).parallel().forEach(j -> {
                    try {
                        PassportModelUtils.runJobInTransaction(sessionFactory, session1 -> lock(session1, lock, semaphore, SLEEP_TIME_MILLIS_FOR_TWO_LOCKS));
                    } catch (RuntimeException e) {
                        semaphore.setException(e);
                    }
                });
    ...
    private void testTwoNestedLocksCurrentlyInternal(PassportSession sessionLC, DBLockProvider.Namespace lockTop, DBLockProvider.Namespace lockInner) {
        final Semaphore semaphore = new Semaphore();
        final PassportSessionFactory sessionFactory = sessionLC.getPassportSessionFactory();
        List<Thread> threads = new LinkedList<>();
        // launch two threads and expect an error because the locks are different
                ...
                for (int j = 0; j < ITERATIONS_PER_THREAD_MEDIUM; j++) {
                    try {
                        if (nested) {
                            ...
                            PassportModelUtils.runJobInTransaction(sessionFactory,
    ...
    private void lock(PassportSession session, DBLockProvider.Namespace lock, Semaphore semaphore) {
        this.lock(session, lock, semaphore, SLEEP_TIME_MILLIS);
    }

    private void lock(PassportSession session, DBLockProvider.Namespace lock, Semaphore semaphore, long sleepTime) {
        DBLockProvider dbLock = new DBLockManager(session).getDBLock();
    ...
    private void nestedTwoLevelLock(PassportSession session, DBLockProvider.Namespace lockTop,
        ...
        try {
            ...
            PassportModelUtils.runJobInTransaction(session.getPassportSessionFactory(),
