...
package org.passport.testsuite.model.session;
...
import org.passport.models.jpa.session.JpaUserSessionPersisterProvider;
import org.passport.models.jpa.session.JpaUserSessionPersisterProviderFactory;
import org.passport.models.jpa.session.PersistentUserSessionEntity;
...
import org.passport.models.sessions.infinispan.expiration.ExpirationTaskFactory;
import org.passport.models.utils.PassportModelUtils;
import org.passport.models.utils.RealmExpiration;
...
import org.passport.storage.client.ClientStorageProvider;
...
import static org.passport.models.utils.SessionTimeoutHelper.PERIODIC_CLEANER_IDLE_TIMEOUT_WINDOW_SECONDS;
...
public class UserSessionPersisterProviderTest extends PassportModelTest {
    ...
    protected static void createClients(PassportSession s, RealmModel realm) {
    ...
    public void testClientTimestampUpdate() {
        ...
        if (InfinispanUtils.isEmbeddedInfinispan()) {
            // causes https://github.com/passport/passport/issues/42012
            inComittedTransaction(session -> {
                ...
                // when reading from a remote passport instance, this field is null
        ...
        Function<PassportSession, Integer> fetchTimestamp = session -> {
            RealmModel realm = session.realms().getRealmByName(realmName);
            session.getContext().setRealm(realm);
    ...
    public void testOnUserRemoved() {
        ...
        inComittedTransaction(session -> {
            ...
            // PASSPORT-2431 Assert that userSessionPersister is resistent even to situation, when users are deleted "directly".
    ...
    private PersistentUserSessionEntity loadUserSessionDirectlyDatabase(PassportSession session, String userSessionId) {
        EntityManager em = session.getProvider(JpaConnectionProvider.class).getEntityManager();
            ...
            try {
                ...
                if (InfinispanUtils.isRemoteInfinispan()) {
                    ...
                    Awaitility.await()
                            ...
                            .untilAsserted(() -> assertEquals(4, (long) PassportModelUtils.runJobInTransactionWithResult(factory, ExpirationTaskFactory::membersSize)));
                }
                barrier.await();
    ...
    private long countUserSessionsInRealm(PassportSession session) {
        JpaUserSessionPersisterProvider sessionPersisterProvider = (JpaUserSessionPersisterProvider) session.getProvider(UserSessionPersisterProvider.class);
        RealmModel realm = session.realms().getRealm(realmId);
    ...
    private void setupClientStorageComponents(PassportSession s, RealmModel realm) {
        s.getContext().setRealm(realm);
    ...
    private void cleanClientStorageComponents(PassportSession s, RealmModel realm) {
        s.getContext().setRealm(realm);
    ...
    protected static AuthenticatedClientSessionModel createClientSession(PassportSession session, String realmId, ClientModel client, UserSessionModel userSession, String redirect, String state) {
        RealmModel realm = session.realms().getRealm(realmId);
    ...
    protected static UserSessionModel[] createSessions(PassportSession session, String realmId) {
       return createSessions(session, realmId, true);
    }

    protected static UserSessionModel[] createSessions(PassportSession session, String realmId, boolean rememberMe) {
        RealmModel realm = session.realms().getRealm(realmId);
        session.getContext().setRealm(realm);
    ...
    private void persistUserSession(PassportSession session, UserSessionModel userSession, boolean offline) {
        UserSessionPersisterProvider persister = session.getProvider(UserSessionPersisterProvider.class);
    ...
    private List<UserSessionModel> loadPersistedSessionsPaginated(PassportSession session, boolean offline, int sessionsPerPage, int expectedPageCount, int expectedSessionsCount) {
        UserSessionPersisterProvider persister = session.getProvider(UserSessionPersisterProvider.class);
