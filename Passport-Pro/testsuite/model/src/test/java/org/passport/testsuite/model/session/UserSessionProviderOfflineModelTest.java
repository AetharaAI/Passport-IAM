...
public class UserSessionProviderOfflineModelTest extends PassportModelTest {
    ...
    public void testOfflineClientSessionLoading() {
        ...
        inComittedTransaction((Consumer<PassportSession>) session -> UserSessionPersisterProviderTest.createSessions(session, realmId));
    ...
    public void testExpirationTaskExists() {
        ...
        Assume.assumeFalse(MultiSiteUtils.isPersistentSessionsEnabled());
        inComittedTransaction(session -> {
            var providerFactory = session.getPassportSessionFactory().getProviderFactory(UserSessionProvider.class);
