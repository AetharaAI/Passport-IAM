...
public class UserSessionExpirationTest extends PassportModelTest {
    ...
    public void testExpirationEvents() {
        ...
        withRealmConsumer(realmId, (session, realm) -> {
            // Time offset is automatically cleaned up in PassportModelTest.cleanEnvironment()
    ...
    private static long eventsCount(PassportSession session) {
    ...
    private static Map<String, String> events(PassportSession session) {
        EventStoreProvider provider = session.getProvider(EventStoreProvider.class);
        return provider.createQuery()
                .type(EventType.USER_SESSION_DELETED)
                .getResultStream()
