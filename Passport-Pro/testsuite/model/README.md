...
Model testsuite runs tests on raw `PassportSessionFactory` which is
initialized only with those providers that are explicitly enabled
in a used profile via `passport.model.parameters` system property.

This allows writing tests and running those in different
configurations quickly.

The valid parameters are names of classes in `org.passport.testsuite.model.parameters`
