 ...
 * @version $Revision: 1 $
 */
public class RoleMappingParser extends AbstractPassportSamlAdapterV1Parser<Set<String>> {

    private static final RoleMappingParser INSTANCE = new RoleMappingParser();

    private RoleMappingParser() {
        super(PassportSamlAdapterV1QNames.ROLE_IDENTIFIERS);
    }
    ...
    @Override
    protected void processSubElement(XMLEventReader xmlEventReader, Set<String> target, PassportSamlAdapterV1QNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case ATTRIBUTE:
                target.add(StaxParserUtil.getRequiredAttributeValueRP(elementDetail, PassportSamlAdapterV1QNames.ATTR_NAME));
                break;
        }
    }
}
