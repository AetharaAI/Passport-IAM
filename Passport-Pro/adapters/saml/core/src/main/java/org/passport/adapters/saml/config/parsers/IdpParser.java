 ...
 * @version $Revision: 1 $
 */
public class IdpParser extends AbstractPassportSamlAdapterV1Parser<IDP> {

    private static final IdpParser INSTANCE = new IdpParser();

    private IdpParser() {
        super(PassportSamlAdapterV1QNames.IDP);
    }
    ...
    protected IDP instantiateElement(XMLEventReader xmlEventReader, StartElement element) throws ParsingException {
        ...
        idp.setEntityID(StaxParserUtil.getRequiredAttributeValueRP(element, PassportSamlAdapterV1QNames.ATTR_ENTITY_ID));

        Boolean signaturesRequired = StaxParserUtil.getBooleanAttributeValueRP(element, PassportSamlAdapterV1QNames.ATTR_SIGNATURES_REQUIRED);
        idp.setSignaturesRequired(signaturesRequired == null ? false : signaturesRequired);
        idp.setSignatureCanonicalizationMethod(StaxParserUtil.getAttributeValueRP(element, PassportSamlAdapterV1QNames.ATTR_SIGNATURE_CANONICALIZATION_METHOD));
        idp.setSignatureAlgorithm(StaxParserUtil.getAttributeValueRP(element, PassportSamlAdapterV1QNames.ATTR_SIGNATURE_ALGORITHM));
        idp.setMetadataUrl(StaxParserUtil.getAttributeValueRP(element, PassportSamlAdapterV1QNames.ATTR_METADATA_URL));
        return idp;
    }

    @Override
    protected void processSubElement(XMLEventReader xmlEventReader, IDP target, PassportSamlAdapterV1QNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case HTTP_CLIENT:
            ...
            case ALLOWED_CLOCK_SKEW:
                String timeUnitString = StaxParserUtil.getAttributeValueRP(elementDetail, PassportSamlAdapterV1QNames.ATTR_UNIT);
