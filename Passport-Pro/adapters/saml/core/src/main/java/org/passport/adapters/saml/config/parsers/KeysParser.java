 ...
 * @version $Revision: 1 $
 */
public class KeysParser extends AbstractPassportSamlAdapterV1Parser<List<Key>> {

    private static final KeysParser INSTANCE = new KeysParser();

    private KeysParser() {
        super(PassportSamlAdapterV1QNames.KEYS);
    }
    ...
    @Override
    protected void processSubElement(XMLEventReader xmlEventReader, List<Key> target, PassportSamlAdapterV1QNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case KEY:
