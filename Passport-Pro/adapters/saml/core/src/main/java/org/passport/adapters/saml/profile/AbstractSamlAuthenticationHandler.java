...
package org.passport.adapters.saml.profile;
...
import org.passport.adapters.saml.AbstractInitiateLogin;
import org.passport.adapters.saml.AdapterConstants;
import org.passport.adapters.saml.OnSessionCreated;
import org.passport.adapters.saml.SamlAuthenticationError;
import org.passport.adapters.saml.SamlDeployment;
import org.passport.adapters.saml.SamlPrincipal;
import org.passport.adapters.saml.SamlSession;
import org.passport.adapters.saml.SamlSessionStore;
import org.passport.adapters.saml.SamlUtil;
import org.passport.adapters.saml.profile.webbrowsersso.WebBrowserSsoAuthenticationHandler;
import org.passport.adapters.spi.AuthChallenge;
import org.passport.adapters.spi.AuthOutcome;
import org.passport.adapters.spi.HttpFacade;
...
import org.passport.dom.saml.v2.protocol.ExtensionsType;
...
import org.passport.dom.saml.v2.protocol.StatusType;
...
import org.passport.saml.processing.api.saml.v2.sig.SAML2Signature;
...
import org.passport.saml.processing.core.util.RedirectBindingSignatureUtil;
import org.passport.saml.processing.core.util.XMLEncryptionUtil;
import org.passport.saml.processing.web.util.PostBindingUtil;
import org.passport.saml.validators.ConditionsValidator;
...
import static org.passport.adapters.saml.SamlPrincipal.DEFAULT_ROLE_ATTRIBUTE_NAME;
...
public abstract class AbstractSamlAuthenticationHandler implements SamlAuthenticationHandler {
    ...
    protected AuthOutcome handleSamlResponse(String samlResponse, String relayState, OnSessionCreated onCreateSession) {
        ...
        if (statusResponse instanceof ResponseType) {
            ...
            if (sessionStore.isLoggingOut()) {
                ...
                try {
                    // PASSPORT-2107 - handle user not authenticated due passive mode. Return special outcome so different authentication mechanisms can behave accordingly.
            ...
            log.warn("Passport Adapter obtained Response, that is not understood. This may be because the containers " +
                    "cookies are not properly configured with SameSite settings. Refer to PASSPORT-14103 for more details.");
    ...
    private String getMessageSigningKeyId(SAML2Object doc) {
        ...
        for (Object ext : extensions.getAny()) {
            ...
            String res = PassportKeySamlExtensionGenerator.getMessageSigningKeyIdFromElement((Element) ext);
    ...
    protected AuthOutcome handleLoginResponse(SAMLDocumentHolder responseHolder, boolean postBinding, OnSessionCreated onCreateSession) {
        if (!sessionStore.isLoggingIn()) {
            log.warn("Adapter obtained LoginResponse, however containers session is not aware of sending any request. " +
                    ...
                    "with SameSite settings. Refer to PASSPORT-14103 for more details.");
        }
