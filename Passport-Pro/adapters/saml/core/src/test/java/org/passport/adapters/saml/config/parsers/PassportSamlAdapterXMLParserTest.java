...
import org.passport.adapters.saml.config.SP;
 ...
 * @version $Revision: 1 $
 */
public class PassportSamlAdapterXMLParserTest {

    private static final String CURRENT_XSD_LOCATION = "/schema/passport_saml_adapter_1_13.xsd";
    ...
    public void testValidationSimpleFile() throws Exception {
        testValidationValid("passport-saml.xml");
    }

    @Test
    public void testValidationMultipleKeys() throws Exception {
        testValidationValid("passport-saml-multiple-signing-keys.xml");
    }

    @Test
    public void testValidationWithHttpClient() throws Exception {
        testValidationValid("passport-saml-wth-http-client-settings.xml");
    }

    @Test
    public void testValidationWithMetadataUrl() throws Exception {
        testValidationValid("passport-saml-with-metadata-url.xml");
    }

    @Test
    public void testValidationWithAllowedClockSkew() throws Exception {
        testValidationValid("passport-saml-with-allowed-clock-skew-with-unit.xml");
    }

    @Test
    public void testValidationWithRoleMappingsProvider() throws Exception {
        testValidationValid("passport-saml-with-role-mappings-provider.xml");
    }

    @Test
    public void testValidationWithKeepDOMAssertion() throws Exception {
        testValidationValid("passport-saml-keepdomassertion.xml");
        // check keep dom assertion is TRUE
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-keepdomassertion.xml", PassportSamlAdapter.class);
    ...
    public void testValidationKeyInvalid() throws Exception {
        InputStream schemaIs = PassportSamlAdapterV1Parser.class.getResourceAsStream(CURRENT_XSD_LOCATION);
        InputStream is = getClass().getResourceAsStream("passport-saml-invalid.xml");
        assertThat(is, Matchers.notNullValue());
    ...
    public void testParseSimpleFileNoNamespace() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-no-namespace.xml", PassportSamlAdapter.class);
    }

    @Test
    public void testXmlParserBaseFile() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml.xml", PassportSamlAdapter.class);
    ...
    private <T> T parsePassportSamlAdapterConfig(String fileName, Class<T> targetClass) throws ParsingException, IOException {
        try (InputStream is = getClass().getResourceAsStream(fileName)) {
            PassportSamlAdapterParser parser = PassportSamlAdapterParser.getInstance();
    ...
    public void testXmlParserMultipleSigningKeys() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-multiple-signing-keys.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), hasSize(1));
        SP sp = config.getSps().get(0);
        IDP idp = sp.getIdp();
    ...
    public void testXmlParserHttpClientSettings() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-wth-http-client-settings.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), hasSize(1));
        SP sp = config.getSps().get(0);
        IDP idp = sp.getIdp();
    ...
    public void testXmlParserSystemPropertiesNoPropertiesSet() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-properties.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        ...
        assertThat(sp.getEntityID(), is("sp"));
        assertThat(sp.getSslPolicy(), is("${passport-saml-properties.sslPolicy}"));
    ...
    public void testXmlParserSystemPropertiesWithPropertiesSet() throws Exception {
        try {
            System.setProperty("passport-saml-properties.entityID", "meid");
            System.setProperty("passport-saml-properties.sslPolicy", "INTERNAL");
            System.setProperty("passport-saml-properties.signaturesRequired", "true");
            ...
            System.clearProperty("passport-saml-properties.entityID");
            System.clearProperty("passport-saml-properties.sslPolicy");
            System.clearProperty("passport-saml-properties.signaturesRequired");
        }
    }

    @Test
    public void testMetadataUrl() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-with-metadata-url.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), Matchers.contains(instanceOf(SP.class)));
        SP sp = config.getSps().get(0);
        IDP idp = sp.getIdp();
    ...
    public void testAllowedClockSkewDefaultUnit() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-with-allowed-clock-skew-default-unit.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), Matchers.contains(instanceOf(SP.class)));
        SP sp = config.getSps().get(0);
        IDP idp = sp.getIdp();
    ...
    public void testAllowedClockSkewWithUnit() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-with-allowed-clock-skew-with-unit.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), Matchers.contains(instanceOf(SP.class)));
        SP sp = config.getSps().get(0);
        IDP idp = sp.getIdp();
    ...
    public void testParseRoleMappingsProvider() throws Exception {
        PassportSamlAdapter config = parsePassportSamlAdapterConfig("passport-saml-with-role-mappings-provider.xml", PassportSamlAdapter.class);
        assertThat(config, notNullValue());
        assertThat(config.getSps(), Matchers.contains(instanceOf(SP.class)));
        SP sp = config.getSps().get(0);
