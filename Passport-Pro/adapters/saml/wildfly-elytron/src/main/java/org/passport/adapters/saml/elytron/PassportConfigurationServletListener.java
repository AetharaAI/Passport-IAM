...
package org.passport.adapters.saml.elytron;
...
import org.passport.adapters.saml.DefaultSamlDeployment;
import org.passport.adapters.saml.SamlConfigResolver;
import org.passport.adapters.saml.SamlDeployment;
import org.passport.adapters.saml.SamlDeploymentContext;
...
import org.passport.adapters.saml.elytron.infinispan.InfinispanSessionCacheIdMapperUpdater;
import org.passport.adapters.spi.InMemorySessionIdMapper;
import org.passport.adapters.spi.SessionIdMapper;
import org.passport.adapters.spi.SessionIdMapperUpdater;
import org.passport.saml.common.exceptions.ParsingException;

import org.jboss.logging.Logger;

/**
 * <p>A {@link ServletContextListener} that parses the passport adapter configuration and set the same configuration
 * as a {@link ServletContext} attribute in order to provide to {@link PassportHttpServerAuthenticationMechanism} a way
 ...
 */
public class PassportConfigurationServletListener implements ServletContextListener {

    protected static Logger log = Logger.getLogger(PassportConfigurationServletListener.class);
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        String configResolverClass = servletContext.getInitParameter("passport.config.resolver");
        ...
        if (deploymentContext == null) {
            if (configResolverClass != null) {
                try {
                    ...
                    log.infov("Using {0} to resolve Passport configuration on a per-request basis.", configResolverClass);
                } catch (Exception ex) {
                    log.errorv("The specified resolver {0} could NOT be loaded. Passport is unconfigured and will deny all requests. Reason: {1}", new Object[] { configResolverClass, ex.getMessage() });
                ...
                if (is == null) {
                    log.warn("No adapter configuration.  Passport is unconfigured and will deny all requests.");
                ...
                log.debug("Passport is using a per-deployment configuration.");
            }
        }
    ...
    private static InputStream getConfigInputStream(ServletContext context) {
        ...
        if (is == null) {
            String path = context.getInitParameter("passport.config.file");
            if (path == null) {
                log.debug("using /WEB-INF/passport-saml.xml");
                is = context.getResourceAsStream("/WEB-INF/passport-saml.xml");
            } else {
                try {
    ...
    public void addTokenStoreUpdaters(ServletContext servletContext) {
        ...
        try {
            String idMapperSessionUpdaterClasses = servletContext.getInitParameter("passport.sessionIdMapperUpdater.classes");
            ...
            for (String clazz : idMapperSessionUpdaterClasses.split("\\s*,\\s*")) {
                if (! clazz.isEmpty()) {
                    if (Objects.equals("org.passport.adapters.saml.wildfly.infinispan.InfinispanSessionCacheIdMapperUpdater", clazz)) {
