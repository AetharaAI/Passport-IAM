...
import org.passport.saml.processing.core.saml.v2.util.DocumentUtil;
...
import static org.passport.common.util.HtmlUtils.escapeAttribute;
import static org.passport.saml.common.util.StringUtil.isNotNull;
...
public class BaseSAML2BindingBuilder<T extends BaseSAML2BindingBuilder> {
    ...
    public void signAssertion(Document samlDocument) throws ProcessingException {
        Element originalAssertionElement = org.passport.saml.common.util.DocumentUtil.getChildElement(samlDocument.getDocumentElement(), new QName(JBossSAMLURIConstants.ASSERTION_NSURI.get(), JBossSAMLConstants.ASSERTION.get()));
        if (originalAssertionElement == null) return;
        Node clonedAssertionElement = originalAssertionElement.cloneNode(true);
        Document temporaryDocument;

        try {
            temporaryDocument = org.passport.saml.common.util.DocumentUtil.createDocument();
    ...
    public static String getSAMLResponse(Document responseDoc) throws ProcessingException, ConfigurationException, IOException {
        byte[] responseBytes = org.passport.saml.common.util.DocumentUtil.getDocumentAsString(responseDoc).getBytes(GeneralConstants.SAML_CHARSET);
        return PostBindingUtil.base64Encode(new String(responseBytes, GeneralConstants.SAML_CHARSET));
    }
