...
The module holds the codebase to build the Passport Operator on top of [Quarkus](https://quarkus.io/).
Using the [Quarkus Operator SDK](https://github.com/quarkiverse/quarkus-operator-sdk).

Also see [Operator guides](https://www.passport-pro.ai/guides#operator)
...
The Passport image can be configured, when starting the operator, using the Java property:

```
kc.operator.passport.image
```

And the imagePullPolicy with:

```
kc.operator.passport.image-pull-policy
```
...
Install the CRD definition and the operator in the cluster in the `passport` namespace:

```bash
kubectl create namespace passport
...
`local_apiserver` : the default, where resources will be deployed to a jenvtest controlled api server (not a full kube environment) and the operator will run locally - not all tests can run in this mode. This is the fastest mode of testing as no externally managed kube environment is needed. As long as your test does not need a running Passport instance, this level of testing should be applicable.
...
To avoid skipping tests that are depending on custom Passport images, you need to build those first:

```bash
./build-testing-docker-images.sh [SOURCE PASSPORT IMAGE TAG] [SOURCE PASSPORT IMAGE]
```

And run the tests passing an extra Java property:

```bash
-Dtest.operator.custom.image=custom-passport:latest
```
...
```bash
 mvn clean verify \
      ...
      -Dquarkus.container-image.image=quay.io/passport/passport-operator:nightly \
      -Dtest.operator.deployment=remote
```
