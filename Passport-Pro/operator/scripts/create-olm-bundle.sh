...
REPLACES_VERSION=$2
# Ex: passport/passport-operator:999.0.0-SNAPSHOT
...
unzip -q -d ../olm/$VERSION ../target/passport-operator-*-olm.zip
...
yq ea -i ".metadata.name = \"passport-operator.v$VERSION\"" "$CSV_PATH"
...
then
  ...
  yq ea -i ".spec.replaces = \"passport-operator.v$REPLACES_VERSION\"" "$CSV_PATH"
fi
...
yq ea -i '.spec.install.spec.deployments[0].spec.selector.matchLabels.name = "passport-operator"' "$CSV_PATH"
yq ea -i '.spec.install.spec.deployments[0].spec.template.metadata.labels.name = "passport-operator"' "$CSV_PATH"
...
yq ea -i '.spec.install.spec.deployments[0].spec.template.spec.containers[0].env += [{"name": "OPERATOR_NAME", "value": "passport-operator"}]' "$CSV_PATH"
