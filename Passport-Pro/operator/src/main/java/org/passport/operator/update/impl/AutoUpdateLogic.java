...
public class AutoUpdateLogic extends BaseUpdateLogic {
    ...
    private final PassportUpdateJobDependentResource updateJobResource;

    public AutoUpdateLogic(Context<Passport> context, Passport passport, PassportUpdateJobDependentResource updateJobResource) {
    ...
    Optional<UpdateControl<Passport>> onUpdate() {
        ...
        if (existingJob.isEmpty()) {
            updateJobResource.reconcile(passport, context);
            Log.debug("Creating Update Job");
            return Optional.of(UpdateControl.noUpdate());
        }

        // Passport CR may be updated while the job is running; we need to delete and start over.
        if (!PassportUpdateJobDependentResource.isJobFromCurrentPassportCr(existingJob.get(), passport)) {
