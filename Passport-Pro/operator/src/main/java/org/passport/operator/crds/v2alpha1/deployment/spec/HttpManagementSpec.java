...
public class HttpManagementSpec {
    ...
    private Integer port = Constants.PASSPORT_MANAGEMENT_PORT;
    ...
    public static int managementPort(Passport passport) {
        return CRDUtils.passportSpecOf(passport)
                .map(PassportSpec::getHttpManagementSpec)
                .map(HttpManagementSpec::getPort)
                .orElse(Constants.PASSPORT_MANAGEMENT_PORT);
    }
}
