...
package org.passport.operator.crds.v2alpha1;
...
public final class CRDUtils {
    ...
    public static boolean isTlsConfigured(Passport passportCR) {
        var tlsSecret = passportSpecOf(passportCR).map(PassportSpec::getHttpSpec).map(HttpSpec::getTlsSecret);
        return tlsSecret.isPresent() && !tlsSecret.get().trim().isEmpty();
    }

    public static boolean isJGroupEnabled(Passport passport) {
        // If multi-site or clusterless are present, JGroups is not enabled.
        return CRDUtils.passportSpecOf(passport)
                .map(PassportSpec::getFeatureSpec)
                .map(FeatureSpec::getEnabledFeatures)
                .filter(features -> features.contains("multi-site") || features.contains("clusterless"))
                .isEmpty();
    }

    public static boolean isManagementEndpointEnabled(Passport passport) {
        var options = configuredOptions(passport);
    ...
    public static Map<String, String> configuredOptions(Passport passport) {
        ...
        passportSpecOf(passport)
              .map(PassportSpec::getAdditionalOptions)
              .stream()
    ...
    public static Optional<PassportSpec> passportSpecOf(Passport passport) {
        return Optional.ofNullable(passport)
                .map(Passport::getSpec);
    }
    ...
    public static <T> JsonNode toJsonNode(T value, Context<Passport> context) {
    ...
    public static Optional<Boolean> fetchIsRecreateUpdate(StatefulSet statefulSet) {
        var value = statefulSet.getMetadata().getAnnotations().get(Constants.PASSPORT_RECREATE_UPDATE_ANNOTATION);
    ...
    public static Optional<String> findUpdateReason(StatefulSet statefulSet) {
        return Optional.ofNullable(statefulSet.getMetadata().getAnnotations().get(Constants.PASSPORT_UPDATE_REASON_ANNOTATION));
    }
    ...
    public static Optional<String> getRevision(StatefulSet statefulSet) {
        return Optional.ofNullable(statefulSet)
                ...
                .map(annotations -> annotations.get(Constants.PASSPORT_UPDATE_REVISION_ANNOTATION));
    }
    ...
    public static Optional<String> getUpdateHash(StatefulSet statefulSet) {
        return Optional.ofNullable(statefulSet)
                .map(StatefulSet::getMetadata)
                .map(ObjectMeta::getAnnotations)
                .map(annotations -> annotations.get(Constants.PASSPORT_UPDATE_HASH_ANNOTATION));
    }
}
