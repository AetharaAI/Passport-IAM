...
import org.passport.operator.update.UpdateLogic;
...
abstract class BaseUpdateLogic implements UpdateLogic {
    ...
    protected final Context<Passport> context;
    protected final Passport passport;
    private Consumer<PassportStatusAggregator> statusConsumer = PassportStatusAggregator::resetUpdateType;

    BaseUpdateLogic(Context<Passport> context, Passport passport) {
        this.context = context;
        this.passport = passport;
    }

    @Override
    public final Optional<UpdateControl<Passport>> decideUpdate() {
    ...
    @Override
    public final void updateStatus(PassportStatusAggregator statusAggregator) {
     ...
     * {@link StatefulSet}.
     */
    abstract Optional<UpdateControl<Passport>> onUpdate();
    ...
    private static Map<String, EnvVar> envVars(Container container) {
        ...
        return container.getEnv().stream()
                .filter(envVar -> !envVar.getName().equals(PassportDeploymentDependentResource.POD_IP))
                .filter(envVar -> !envVar.getName().equals(PassportDeploymentDependentResource.HOST_IP_SPI_OPTION))
