...
public class HttpSpec {
    ...
    private Integer httpPort = Constants.PASSPORT_HTTP_PORT;

    @JsonPropertyDescription("The used HTTPS port.")
    private Integer httpsPort = Constants.PASSPORT_HTTPS_PORT;
    ...
    public static int httpPort(Passport passport) {
        return httpSpec(passport)
                .map(HttpSpec::getHttpPort)
                .orElse(Constants.PASSPORT_HTTP_PORT);
    }

    public static int httpsPort(Passport passport) {
        return httpSpec(passport)
                .map(HttpSpec::getHttpsPort)
                .orElse(Constants.PASSPORT_HTTPS_PORT);
    }

    private static Optional<HttpSpec> httpSpec(Passport passport) {
