 ...
 */
public class PassportStatusAggregator {
    private final PassportStatusCondition readyCondition = new PassportStatusCondition();
    private final PassportStatusCondition hasErrorsCondition = new PassportStatusCondition();
    private final PassportStatusCondition rollingUpdate = new PassportStatusCondition();
    private final PassportStatusCondition updateType = new PassportStatusCondition();
    ...
    private final PassportStatusBuilder statusBuilder;
    private final Map<String, PassportStatusCondition> existingConditions;
    private final Long observedGeneration;

    /**
     * @param generation the observedGeneration for conditions
     */
    public PassportStatusAggregator(Long generation) {
     ...
     * @param generation the observedGeneration for conditions
     */
    public PassportStatusAggregator(PassportStatus current, Long generation) {
        if (current != null) { // 6.7 fabric8 no longer requires this null check
            statusBuilder = new PassportStatusBuilder(current);
            existingConditions = Optional.ofNullable(current.getConditions()).orElse(List.of()).stream().collect(Collectors.toMap(PassportStatusCondition::getType, Function.identity()));
        } else {
            statusBuilder = new PassportStatusBuilder();
            existingConditions = Map.of();
        }

        observedGeneration = generation;

        readyCondition.setType(PassportStatusCondition.READY);

        hasErrorsCondition.setType(PassportStatusCondition.HAS_ERRORS);

        rollingUpdate.setType(PassportStatusCondition.ROLLING_UPDATE);

        updateType.setType(PassportStatusCondition.UPDATE_TYPE);
    }

    public PassportStatusAggregator addNotReadyMessage(String message) {
    ...
    public PassportStatusAggregator addErrorMessage(String message) {
    ...
    public PassportStatusAggregator addWarningMessage(String message) {
        errorMessages.add("warning: " + message);
        hasErrorsCondition.setObservedGeneration(observedGeneration);
        return this;
    }

    public PassportStatusAggregator addRollingUpdateMessage(String message) {
     ...
     */
    public PassportStatusAggregator apply(Consumer<PassportStatusBuilder> toApply) {
    ...
    static void updateConditionFromExisting(PassportStatusCondition condition, Map<String, PassportStatusCondition> existingConditions, String now) {
