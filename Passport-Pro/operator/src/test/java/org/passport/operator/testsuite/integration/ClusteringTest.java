...
import org.passport.operator.crds.v2alpha1.realmimport.PassportRealmImportStatusCondition;
...
import static org.passport.operator.controllers.PassportDeploymentDependentResource.KC_TELEMETRY_SERVICE_NAME;
...
public class ClusteringTest extends BaseOperatorTest {
    ...
    public void testMultipleDeployments() throws InterruptedException {
        ...
        var kc1 = getTestPassportDeployment(true);
        ...
        try {
            ...
            K8sUtils.deployPassport(k8sclient, kc1, true);
        } catch (Exception e) {
            k8sclient.resources(Passport.class).list().getItems().stream().forEach(k -> {
                Log.infof("Passport %s status: %s", k.getMetadata().getName(), Serialization.asYaml(k.getStatus()));
            });
        ...
        assertThat(k8sclient.resources(Passport.class).list().getItems().size()).isEqualTo(2);
    ...
    @Test
    public void testPassportScaleAsExpected() {
        // given a starting point of a default passport with null/default instances
        var kc = getTestPassportDeployment(false);
        kc.getSpec().setInstances(null);
        var crSelector = k8sclient.resource(kc);
        K8sUtils.deployPassport(k8sclient, kc, true);

        var kcPodsSelector = k8sclient.pods().inNamespace(namespace).withLabel("app", "passport");
        ...
        assertThat(scale.getStatus().getSelector()).isEqualTo("app=passport,app.kubernetes.io/managed-by=passport-operator,app.kubernetes.io/instance=example-kc");

        // when scale it to 0
        Passport scaled = crSelector.scale(0);
        ...
        Awaitility.await()
                ...
                .pollDelay(1, SECONDS)
                .ignoreExceptions()
                .untilAsserted(() -> CRAssert.assertPassportStatusCondition(crSelector.get(), PassportStatusCondition.READY, true));
        ...
        Awaitility.await()
                ...
                .pollDelay(1, SECONDS)
                .ignoreExceptions()
                .untilAsserted(() -> CRAssert.assertPassportStatusCondition(crSelector.get(), PassportStatusCondition.READY, false));
    ...
    @Test
    public void testPassportCacheIsConnected() throws Exception {
        // given
        Log.info("Setup");
        var kc = getTestPassportDeployment(false);
        var crSelector = k8sclient.resource(kc);
        K8sUtils.deployPassport(k8sclient, kc, false);
        ...
        var realmImportSelector = k8sclient.resources(PassportRealmImport.class).inNamespace(namespace).withName("example-token-test-kc");

        Log.info("Waiting for a stable Passport Cluster");
        Awaitility.await()
                .atMost(10, MINUTES)
                .pollDelay(5, SECONDS)
                .ignoreExceptions()
                .untilAsserted(() -> {
                    ...
                    CRAssert.assertPassportRealmImportStatusCondition(realmImportSelector.get(), PassportRealmImportStatusCondition.DONE, true);
                    Log.info("Checking Passport is stable.");
                    CRAssert.assertPassportStatusCondition(crSelector.get(), PassportStatusCondition.READY, true);
                });
        // Remove the completed pod for the job
        realmImportSelector.delete();

        Log.info("Testing the Passport Cluster");
        Awaitility.await()
                .atMost(20, MINUTES)
                .pollDelay(5, SECONDS)
                .ignoreExceptions()
                .untilAsserted(() -> {
            // Get the list of Passport pods
            var pods = k8sclient
                    .pods()
                    .inNamespace(namespace)
                    .withLabels(Constants.DEFAULT_LABELS)
        ...
        String serviceName = PassportServiceDependentResource.getServiceName(kc);
        Awaitility.await()
                .atMost(20, MINUTES)
                .pollDelay(5, SECONDS)
                .ignoreExceptions()
            ...
            for (int i = 0; i < (targetInstances + 1); i++) {
                ...
                if (token2 == null) {
                    var tokenUrl = "https://" + serviceName + "." + namespace + ":" + Constants.PASSPORT_HTTPS_PORT + "/realms/token-test/protocol/openid-connect/token";
                ...
                String url = "https://" + serviceName + "." + namespace + ":" + Constants.PASSPORT_HTTPS_PORT + "/realms/token-test/protocol/openid-connect/userinfo";
