...
import org.passport.operator.crds.v2alpha1.deployment.spec.HttpManagementSpecBuilder;
import org.passport.operator.crds.v2alpha1.deployment.spec.NetworkPolicySpecBuilder;
...
public final class K8sUtils {
    ...
    public static Passport getDefaultPassportDeployment() {
        return getResourceFromFile("example-passport.yaml", Passport.class);
    }
    ...
    public static void deployPassport(KubernetesClient client, Passport kc, boolean waitUntilReady) {
        deployPassport(client, kc, waitUntilReady, true);
    }
    ...
    public static void deployPassport(KubernetesClient client, Passport kc, boolean waitUntilReady, boolean deployTlsSecret) {
        if (deployTlsSecret) {
        ...
        if (waitUntilReady) {
            waitForPassportToBeReady(client, kc);
        }
    }

    public static void waitForPassportToBeReady(KubernetesClient client, Passport kc) {
        Log.infof("Waiting for Passport \"%s\"", kc.getMetadata().getName());
        Awaitility.await()
                ...
                .untilAsserted(() -> {
                    ...
                    CRAssert.assertPassportStatusCondition(currentKc, PassportStatusCondition.READY, true);
                    CRAssert.assertPassportStatusCondition(currentKc, PassportStatusCondition.HAS_ERRORS, false);
                });
    }
    ...
    public static void enableNetworkPolicy(Passport passport) {
        var builder = new NetworkPolicySpecBuilder();
        passport.getSpec().setNetworkPolicySpec(builder.build());
    }

    public static void disableNetworkPolicy(Passport passport) {
        var builder = new NetworkPolicySpecBuilder();
        builder.withNetworkPolicyEnabled(false);
        passport.getSpec().setNetworkPolicySpec(builder.build());
    }

    public static int configureManagement(Passport passport, boolean randomPort) {
        if (!randomPort) {
            return Constants.PASSPORT_MANAGEMENT_PORT;
        }
        var port = ThreadLocalRandom.current().nextInt(10_000, 10_100);
        passport.getSpec().setHttpManagementSpec(new HttpManagementSpecBuilder().withPort(port).build());
        return port;
    }

    public static int enableHttp(Passport passport, boolean randomPort) {
        passport.getSpec().getHttpSpec().setHttpEnabled(true);
        if (randomPort) {
            var port = ThreadLocalRandom.current().nextInt(10_100, 10_200);
            passport.getSpec().getHttpSpec().setHttpPort(port);
            return port;
        }
        return Constants.PASSPORT_HTTP_PORT;
    }

    public static int configureHttps(Passport passport, boolean randomPort) {
        if (!randomPort) {
            return Constants.PASSPORT_HTTPS_PORT;
        }
        var port = ThreadLocalRandom.current().nextInt(10_200, 10_300);
        passport.getSpec().getHttpSpec().setHttpsPort(port);
        return port;
    }

    public static void disableHttps(Passport passport) {
        passport.getSpec().getHttpSpec().setTlsSecret(null);
    }
