...
public class WatchedSecretsTest extends BaseOperatorTest {
    ...
    private StatefulSet getStatefulSet(Passport kc) {
    ...
    public void testSecretsCanBeUnWatched() {
        ...
        Awaitility.await().untilAsserted(() -> {
            ...
            assertFalse(k8sclient.resources(StatefulSet.class).withName(kc.getMetadata().getName()).get().getMetadata()
                    .getAnnotations().get(WatchedResourcesTest.PASSPORT_WATCHING_ANNOTATION)
                    .contains(dbSecret.getMetadata().getName()));
        });
    }

    @Test
    public void testSingleSecretMultiplePassports() {
        ...
        var kc2 = getTestPassportDeployment(true);
        ...
        deployPassport(k8sclient, kc1, true);
        deployPassport(k8sclient, kc2, true);
        ...
        testDeploymentRestarted(Set.of(kc1), Set.of(kc2), () -> {
            deployPassport(k8sclient, kc1, false, false);
        });
    ...
    private void testDeploymentRestarted(Set<Passport> crsToBeRestarted, Set<Passport> crsNotToBeRestarted, Runnable action) {
        ...
        Set<Passport> allCrs = new HashSet<>(crsToBeRestarted);
    ...
    private Secret getDbSecret() {
		return new SecretBuilder(k8sclient.secrets().inNamespace(namespace).withName("passport-db-secret").get())
    ...
    private void hardcodeDBCredsInCR(Passport kc) {
