...
import org.passport.operator.testsuite.utils.TrustAllSSLContext;
...
import static org.passport.operator.testsuite.utils.K8sUtils.inClusterCurlCommand;
...
@QuarkusTest
public class PassportKubernetesJwtTest extends BaseOperatorTest {
    ...
    public void testSignedJWTs() throws IOException {
        ...
        try (LocalPortForward pf = k8sclient.pods().resource(kcPod)
                .portForward(8443)) {
            var passport = org.passport.admin.client.Passport.getInstance(
            ...
            passport.realm("test").clients().create(client).close();
            ...
            passport.realm("test").identityProviders().create(idp).close();
    ...
    private void assertSignedJWT(Passport kc) {
        Awaitility.await().atMost(10, MINUTES).ignoreExceptions().untilAsserted(() -> {
            ...
            String url =
                    "https://" + PassportServiceDependentResource.getServiceName(kc) + "." + namespace + ":" + PASSPORT_HTTPS_PORT + "/realms/test/protocol/openid-connect/token";
