...
import org.passport.operator.crds.v2alpha1.deployment.spec.CacheSpecBuilder;
...
public class CacheTest extends BaseOperatorTest {
    ...
    public void testCreateCacheConfigMapFileAfterDeployment() {
        ...
        // Check Operator has deployed Passport and the statefulset exists, this allows
        ...
        Awaitility.await().ignoreExceptions().atMost(2, TimeUnit.MINUTES).during(10, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    ...
                    CRAssert.assertPassportStatusCondition(passport, PassportStatusCondition.HAS_ERRORS, false);
        ...
        Awaitility.await().ignoreExceptions().atMost(3, TimeUnit.MINUTES).untilAsserted(() -> {
            StatefulSet deployment = stsResource.get();
            assertThat(deployment).isNotNull();
            // check the pod directly - it takes longer for us to update our Passport status
            // with an error
        ...
        K8sUtils.waitForPassportToBeReady(k8sclient, kc);
    }
