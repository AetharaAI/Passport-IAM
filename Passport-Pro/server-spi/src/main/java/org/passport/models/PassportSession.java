 ...
 * @version $Revision: 1 $
 */
public interface PassportSession extends AutoCloseable {

    PassportContext getContext();

    PassportTransactionManager getTransactionManager();

    /**
     * Get dedicated provider instance of provider type clazz that was created for this session.  If one hasn't been created yet,
     * find the factory and allocate by calling ProviderFactory.create(PassportSession).  The provider to use is determined
     * by the "provider" config entry in passport-server boot configuration. See the <a href="https://www.passport-pro.ai/docs/latest/server_development/index.html#_use_available_providers">Server developer guide</a> for the details.
     *
     *
     *
     * @param clazz
     ...
     * find the factory and allocate by calling ProviderFactory.create(PassportSession).
     ...
     * The relevant realm must be set prior to calling this method in the context, see {@link PassportContext#getRealm()}.
     * @param <T>
     * @param clazz
     ...
     * @return Provider configured according to the {@param componentId}, {@code null} if it cannot be instantiated.
     */
    <T extends Provider> T getComponentProvider(Class<T> clazz, String componentId, Function<PassportSessionFactory, ComponentModel> modelGetter);
    ...
    PassportSessionFactory getPassportSessionFactory();

    /**
     * Returns a managed provider instance.  Will start a provider transaction.  This transaction is managed by the PassportSession
     * transaction.
     *
     * @return
