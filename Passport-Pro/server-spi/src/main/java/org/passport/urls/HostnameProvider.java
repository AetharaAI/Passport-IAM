...
/**
 * The Hostname provider is used by Passport to decide URLs for frontend and backend requests. A provider can either
 ...
 *
 * Note: Do NOT use {@link PassportContext#getUri()} within a Hostname provider. It will result in an infinite loop.
 */
public interface HostnameProvider extends Provider {
    ...
    /**
     * Returns the context-path for Passport. This is useful when Passport is exposed on a different context-path on
    ...
    /**
     * Returns the base URI for Passport with the scheme, host, port, and context-path set for the given UrlType
     *
     * @param originalUriInfo the original URI
     * @param type type of the request
     * @return the base URI
