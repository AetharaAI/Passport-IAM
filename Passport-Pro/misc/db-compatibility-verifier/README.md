...
This Maven plugin is used to verify the database compatibility of Passport. It ensures that all database schema changes
...
```bash
mvn org.passport:db-compatibility-verifier-maven-plugin:999.0.0-SNAPSHOT:snapshot  \
...
```bash
mvn org.passport:db-compatibility-verifier-maven-plugin:999.0.0-SNAPSHOT:verify \
...
```bash
mvn org.passport:db-compatibility-verifier-maven-plugin:999.0.0-SNAPSHOT:supported \
  -Ddb.verify.supportedFile=<relative-path-to-json-file> \
  -Ddb.verify.unsupportedFile=<relative-path-to-json-file> \
...
```bash
mvn org.passport:db-compatibility-verifier-maven-plugin:999.0.0-SNAPSHOT:unsupported \
  -Ddb.verify.supportedFile=<relative-path-to-json-file> \
  -Ddb.verify.unsupportedFile=<relative-path-to-json-file> \
  -Ddb.verify.changset.id=<id> \
  -Ddb.verify.changset.author=<author> \
