package org.passport.db.compatibility.verifier;
...
public class VerifyCompatibilityMojo extends AbstractMojo {
    ...
    void checkMissingChangeSet(Set<ChangeSet> currentChanges, Set<ChangeSet> recordedChanges, File sFile, File uFile) throws MojoExecutionException {
        if (recordedChanges.equals(currentChanges)) {
            ...
            getLog().error("A ChangeSet that requires locking preventing other cluster members accessing the database or makes schema changes that breaks functionality in earlier Passport versions is NOT compatible with rolling upgrades");
            ...
            getLog().error("ChangeSets can be added to the supported or unsupported files using the org.passport:db-compatibility-verifier-maven-plugin. See the module README for usage instructions");
