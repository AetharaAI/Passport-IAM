...
for i in $(gh run list -L 100 -R "$USER/passport" -w "$WORKFLOW" -s failure --created "$DATE" --json databaseId | jq -r .[].databaseId); do
    echo -n "($i) "
    if [ ! -f "$LOGS/$i" ]; then
        gh run -R "$USER/passport" view --log-failed "$i" > "$LOGS/$i"
    fi
    if [ ! -f "$LOGS/$i.json" ]; then
        gh run -R "$USER/passport" view --json name,conclusion,databaseId,jobs "$i" > "$LOGS/$i.json"
    fi
done
