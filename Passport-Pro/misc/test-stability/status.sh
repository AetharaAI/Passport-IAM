...
echo "Status of $WORKFLOW in $USER/passport for branch $BRANCH"
echo ""

gh api -X GET "/repos/$USER/passport/actions/workflows/$WORKFLOW/runs" -F branch="$BRANCH" -F per_page=100 --paginate -F created="$DATE" | jq -r '.workflow_runs[] | [.status, .conclusion] | @csv' | sort | uniq -c
