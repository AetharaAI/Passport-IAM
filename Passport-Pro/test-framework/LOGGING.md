# Passport Test Framework - Logging
...
* Configure Passport logging - automatically configure Passport server logging
* Log forwarding - forward output from the managed Passport test server, and managed database containers

## Configuring logging

To configure default logging for a test module add `src/main/resources/passport-test.properties` with the default configuration. For example:
...
```
kc.test.log.category."org.passport".level=INFO
KC_TEST_LOG_CATEGORY__ORG_PASSPORT__LEVEL=INFO
```

Some useful categories include:

* `testinfo` - Test lifecycle information. This serves two purposes, firstly it tells you when a test class and test method starts, and if the test succeeded or not. Secondly, it breaks up the log output so you can see exactly what is causing logs for instance from the Passport server.
* `org.passport.testframework` - Logging from the test framework itself. Setting this to `DEBUG` can be helpful to debug any issues with the test framework itself, or custom suppliers.
* `org.passport` - Logging from the Passport server. If you set this to `DEBUG` for example, but don't want debug from the test framework, also explicitly set `org.passport.testframework` to for example `INFO`
* `managed.passport` - Log output from the managed Passport server if you are running the server in `distribution` mode (which is the default)
...
This provides a more complete example of a `src/test/resource/passport-test.properties` file:
...
kc.test.log.category."testinfo".level=INFO
kc.test.log.category."org.passport.testframework".level=INFO
kc.test.log.category."org.passport".level=WARN
kc.test.log.category."managed.passport".level=WARN
...
KC_TEST_LOG_CATEGORY__ORG_PASSPORT___LEVEL=DEBUG
KC_TEST_LOG_CATEGORY__ORG_PASSPORT_TEST__LEVEL=DEBUG
KC_TEST_LOG_CATEGORY__MANAGED_PASSPORT__LEVEL=DEBUG
...
```
KC_TEST_LOG_CATEGORY__ORG_PASSPORT__LEVEL=DEBUG mvn test -Dtest=ProblematicTest
```
