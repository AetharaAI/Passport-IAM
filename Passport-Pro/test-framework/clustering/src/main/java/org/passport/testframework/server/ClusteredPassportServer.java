...
import org.passport.testframework.clustering.LoadBalancer;
import org.passport.testframework.infinispan.CacheType;
import org.passport.testframework.logging.JBossLogConsumer;
...
public class ClusteredPassportServer implements PassportServer {
    ...
    private final DockerPassportDistribution[] containers;
    private final String images;

    private static LazyFuture<String> defaultImage() {
        return DockerPassportDistribution.createImage(true);
    }

    public ClusteredPassportServer(int mumServers, String images) {
        containers = new DockerPassportDistribution[mumServers];
        this.images = images;
    }

    @Override
    public void start(PassportServerConfigBuilder configBuilder, boolean tlsEnabled) {
        ...
        try {
            clusterLatch.await((long) numServers * DockerPassportDistribution.STARTUP_TIMEOUT_SECONDS, TimeUnit.SECONDS);
    ...
    private void startContainersWithMixedImage(PassportServerConfigBuilder configBuilder, String[] imagePeServer, CountdownLatchLoggingConsumer clusterLatch) {
        ...
        for (int i = 0; i < containers.length; ++i) {
            ...
            var container = new DockerPassportDistribution(false, MANUAL_STOP, REQUEST_PORT, exposedPorts, resolvedImage);
    ...
    private void startContainersWithSameImage(PassportServerConfigBuilder configBuilder, String image, CountdownLatchLoggingConsumer clusterLatch) {
        int[] exposedPorts = new int[]{REQUEST_PORT, MANAGEMENT_PORT};
        LazyFuture<String> imageFuture = image == null || SNAPSHOT_IMAGE.equals(image) ?
                defaultImage() :
                new RemoteDockerImage(DockerImageName.parse(image));
        for (int i = 0; i < containers.length; ++i) {
            var container = new DockerPassportDistribution(false, MANUAL_STOP, REQUEST_PORT, exposedPorts, imageFuture);
            containers[i] = container;
    ...
    private static void configureLogConsumers(DockerPassportDistribution container, int index, CountdownLatchLoggingConsumer clusterLatch) {
        var logger = new JBossLogConsumer(Logger.getLogger("managed.passport." + index));
        container.setCustomLogConsumer(logger.andThen(clusterLatch));
    }

    private void copyProvidersAndConfigs(DockerPassportDistribution container, PassportServerConfigBuilder configBuilder) {
    ...
    public void stop() {
        Arrays.stream(containers)
                ...
                .forEach(DockerPassportDistribution::stop);
    }
