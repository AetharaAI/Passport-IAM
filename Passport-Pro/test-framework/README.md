...
The Passport JUnit 5 test framework makes it easy to write tests for Passport and extensions. Behind the scenes the
framework handles the lifecycle of Passport, the database, and any injected resources such as realms and clients.
...
## Using the Passport admin client

The Passport admin client can be injected directly, which is automatically connected to the test server:

```java
@InjectAdminClient
org.passport.admin.client.Passport passport;

@Test
public void testAdminClient() {
    passport.realms().findAll();
}
```
...
Frequently when testing Passport it is required to interact with login pages, required actions, etc. through the 
...
The above example adds some additional Passport server configuration, as well as limiting what server suppliers can be used for the suite.
