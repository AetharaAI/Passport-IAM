...
| Value        | Description                                                                                            |
|--------------|--------------------------------------------------------------------------------------------------------|
| distribution | Runs the full distribution of Passport in a separate JVM process                                       |
| embedded     | Runs a Passport server embedded in the same JVM process                                                |
| remote       | Connects to a remote Passport server. Requires manually configuring the server as needed for the test. |
...
| `kc.test.server.config` / `KC_TEST_SERVER_CONFIG` | The name of a PassportServerConfig class to use when running the tests                                           |
...
Because some databases may require a special driver, you can specify it to the Passport server. To use it, Passport needs
...
KC_TEST_DATABASE=remote \
  ...
  KC_TEST_DATABASE_URL=jdbc:oracle:thin:@localhost:1521/passport \
