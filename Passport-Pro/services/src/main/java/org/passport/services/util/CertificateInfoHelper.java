...
public class CertificateInfoHelper {
    ...
    public static CertificateRepresentation getCertificateFromRequest(PassportSession session) throws IOException {
        CertificateRepresentation info = new CertificateRepresentation();
        ...
        if (keystoreFormat.equals(CERTIFICATE_PEM)) {
            ...
            // Validate format
            PassportModelUtils.getCertificate(pem);
            ...
            // Validate format
            PassportModelUtils.getPublicKey(pem);
        ...
        if (privateKey != null) {
            String privateKeyPem = PassportModelUtils.getPemFromKey(privateKey);
            info.setPrivateKey(privateKeyPem);
        }

        if (certificate != null) {
            String certPem = PassportModelUtils.getPemFromCertificate(certificate);
