...
public class DefaultBlockingBruteForceProtector extends DefaultBruteForceProtector {
    ...
    DefaultBlockingBruteForceProtector(PassportSessionFactory factory) {
        super(factory);
    }
    ...
    private boolean isLoginInProgress(PassportSession session, UserModel user) {
    ...
    private boolean tryEnlistBlockingTransactionOrSameThread(PassportSession session, UserModel user) {
    ...
    private void enlistRemoval(PassportSession session, String userId) {
        session.getTransactionManager().enlistAfterCompletion(new AbstractPassportTransaction() {
            @Override
            protected void commitImpl() {
