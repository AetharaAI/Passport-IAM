...
package org.passport.services.resources.admin.info;
...
import org.passport.crypto.ClientSignatureVerifierProvider;
...
import org.passport.policy.PasswordPolicyProvider;
import org.passport.policy.PasswordPolicyProviderFactory;
...
import org.passport.provider.ConfiguredPerClientProvider;
...
import org.passport.representations.idm.PasswordPolicyTypeRepresentation;
import org.passport.representations.idm.ProtocolMapperRepresentation;
import org.passport.representations.idm.ProtocolMapperTypeRepresentation;
import org.passport.representations.info.ClientInstallationRepresentation;
import org.passport.representations.info.CpuInfoRepresentation;
import org.passport.representations.info.CryptoInfoRepresentation;
import org.passport.representations.info.FeatureRepresentation;
import org.passport.representations.info.FeatureType;
import org.passport.representations.info.MemoryInfoRepresentation;
import org.passport.representations.info.ProfileInfoRepresentation;
...
import org.passport.representations.info.SpiInfoRepresentation;
import org.passport.representations.info.SystemInfoRepresentation;
import org.passport.representations.info.ThemeInfoRepresentation;
...
public class ServerInfoAdminResource {
    ...
    public ServerInfoAdminResource(PassportSession session, AdminAuth auth) {
    ...
    @Tag(name = PassportOpenAPI.Admin.Tags.ROOT)
    ...
    public ServerInfoRepresentation getInfo() {
        ...
        if (RealmManager.isAdministrationRealm(userRealm) || adminEvaluator.hasOneAdminRole(AdminRoles.VIEW_SYSTEM)) {
            ...
            info.setSystemInfo(SystemInfoRepresentation.create(session.getPassportSessionFactory().getServerStartupTimestamp(), Version.VERSION));
    ...
    private void setProviders(ServerInfoRepresentation info) {
        ...
        List<Spi> spis = new LinkedList<>(session.getPassportSessionFactory().getSpis());
        ...
        for (Spi spi : spis) {
            ...
            for (String name : providerIds) {
                ...
                ProviderFactory<?> pi = session.getPassportSessionFactory().getProviderFactory(spi.getProviderClass(), name);
    ...
    private LinkedList<String> filterThemes(Theme.Type type, LinkedList<String> themeNames) {
        ...
        if (filterAdminV2 || filterLoginV2) {
            filteredNames.remove("passport.v2");
        ...
        if (filterAccountV3) {
            filteredNames.remove("passport.v3");
        }
    ...
    private void setSocialProviders(ServerInfoRepresentation info) {
        ...
        Stream<ProviderFactory> providerFactories = session.getPassportSessionFactory().getProviderFactoriesStream(SocialIdentityProvider.class);
    ...
    private void setIdentityProviders(ServerInfoRepresentation info) {
        ...
        Stream<ProviderFactory> providerFactories = session.getPassportSessionFactory().getProviderFactoriesStream(IdentityProvider.class);
        setIdentityProviders(providerFactories, info.getIdentityProviders(), "User-defined");

        providerFactories = session.getPassportSessionFactory().getProviderFactoriesStream(SocialIdentityProvider.class);
    ...
    private void setClientInstallations(ServerInfoRepresentation info) {
        HashMap<String, List<ClientInstallationRepresentation>> clientInstallations = session.getPassportSessionFactory()
    ...
    private void setProtocolMapperTypes(ServerInfoRepresentation info) {
        HashMap<String, List<ProtocolMapperTypeRepresentation>> protocolMappers = session.getPassportSessionFactory()
    ...
    private void setBuiltinProtocolMappers(ServerInfoRepresentation info) {
        Map<String, List<ProtocolMapperRepresentation>> protocolMappers = session.getPassportSessionFactory()
    ...
    private void setPasswordPolicies(ServerInfoRepresentation info) {
        List<PasswordPolicyTypeRepresentation> passwordPolicyTypes= session.getPassportSessionFactory().getProviderFactoriesStream(PasswordPolicyProvider.class)
