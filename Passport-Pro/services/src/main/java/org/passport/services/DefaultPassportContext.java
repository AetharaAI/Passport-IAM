 ...
 */
public abstract class DefaultPassportContext implements PassportContext {
    ...
    private Map<UrlType, PassportUriInfo> uriInfo;
    ...
    public DefaultPassportContext(PassportSession session) {
    ...
    @Override
    public PassportUriInfo getUri(UrlType type) {
        if (uriInfo == null || !uriInfo.containsKey(type)) {
            if (uriInfo == null) {
                uriInfo = new HashMap<>();
            }

            uriInfo.put(type, new PassportUriInfo(session, type, getHttpRequest().getUri()));
        }
        return uriInfo.get(type);
    }

    @Override
    public PassportUriInfo getUri() {
