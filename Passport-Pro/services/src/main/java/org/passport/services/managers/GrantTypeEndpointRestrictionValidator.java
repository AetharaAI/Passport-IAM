...
import org.passport.protocol.oidc.encode.DefaultTokenContextEncoderProvider;
...
public class GrantTypeEndpointRestrictionValidator implements TokenVerifier.Predicate<AccessToken> {
    ...
    private GrantTypeEndpointRestrictionValidator(PassportSession session) {
     ...
     */
    public static TokenVerifier.Predicate<AccessToken> check(PassportSession session) {
    ...
    private String recoverGrantType(AccessToken token) throws VerificationException {
        ...
        if (grantType == null || grantType.isEmpty() || DefaultTokenContextEncoderProvider.UNKNOWN.equals(grantType)) {
            // Standard Passport token without specific grant-type context.
