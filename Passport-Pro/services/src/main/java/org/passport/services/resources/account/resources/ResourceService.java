...
import static org.passport.models.utils.ModelToRepresentation.toRepresentation;
...
public class ResourceService extends AbstractResourceService {
    ...
    private final org.passport.authorization.model.Resource resource;
    private final ResourceServer resourceServer;

    ResourceService(org.passport.authorization.model.Resource resource, PassportSession session, UserModel user,
    ...
    public Response user(@QueryParam("value") String value) {
        try {
            ...
            return Response.ok(toRepresentation(provider.getPassportSession(), provider.getRealm(), user)).build();
        ...
        for (Permission permission : permissions) {
            ...
            if (tickets.isEmpty()) {
                ...
                while (scopesIterator.hasNext()) {
                    org.passport.authorization.model.Scope scope = getScope(scopesIterator.next(), resourceServer);
    ...
    private void grantPermission(UserModel user, String scopeId) {
        org.passport.authorization.model.Scope scope = getScope(scopeId, resourceServer);
    ...
    private org.passport.authorization.model.Scope getScope(String scopeId, ResourceServer resourceServer) {
        org.passport.authorization.model.Scope scope = scopeStore.findByName(resourceServer, scopeId);
    ...
    private UserModel getUser(String requester) {
        UserProvider users = provider.getPassportSession().users();
