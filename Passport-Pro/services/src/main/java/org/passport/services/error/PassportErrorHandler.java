...
import static org.passport.services.resources.PassportApplication.getSessionFactory;

@Provider
public class PassportErrorHandler implements ExceptionMapper<Throwable> {

    private static final Logger logger = Logger.getLogger(PassportErrorHandler.class);
    ...
    public Response toResponse(Throwable throwable) {
        ...
        if (session == null) {
            ...
            return PassportModelUtils.runJobInTransactionWithResult(getSessionFactory(),
                    new PassportSessionTaskWithResult<Response>() {
                        @Override
                        public Response run(PassportSession session) {
    ...
    public static Response getResponse(PassportSession session, Throwable throwable) {
        PassportTransaction tx = session.getTransactionManager();
        tx.setRollbackOnly();
    ...
    private static RealmModel resolveRealm(PassportSession session) {
    ...
    private static Map<String, Object> initAttributes(PassportSession session, RealmModel realm, Theme theme, Locale locale, Response.Status responseStatus) throws IOException {
