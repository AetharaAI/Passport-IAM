...
import org.passport.authorization.AuthorizationService;
...
import org.passport.services.resources.account.AccountLoader;
...
import org.passport.wellknown.WellKnownProviderFactory;

import org.jboss.logging.Logger;

import static org.passport.utils.MediaType.APPLICATION_JWT;
...
public class RealmsResource {
    ...
    public Object getProtocol(final @PathParam("realm") String name,
        ...
        LoginProtocolFactory factory = (LoginProtocolFactory)session.getPassportSessionFactory().getProviderFactory(LoginProtocol.class, protocol);
        if(factory == null){
    ...
    public Response getRedirect(final @PathParam("realm") String realmName, final @PathParam("client_id") String clientId) {
        ...
        if (client.getRootUrl() != null && (client.getBaseUrl() == null || client.getBaseUrl().isEmpty())) {
            targetUri = PassportUriBuilder.fromUri(client.getRootUrl()).build();
        } else {
            targetUri = PassportUriBuilder.fromUri(ResolveRelative.resolveRelativeUri(session, client.getRootUrl(), client.getBaseUrl())).build();
        }
    ...
    private static void resolveRealmAndUpdateSession(PassportSession session, String realmName) {
    ...
    public static Response getWellKnownResponse(PassportSession session, String name, String alias, Logger logger) throws NotFoundException {
        resolveRealmAndUpdateSession(session, name);
        checkSsl(session, session.getContext().getRealm());

        WellKnownProviderFactory wellKnownProviderFactoryFound = session.getPassportSessionFactory().getProviderFactoriesStream(WellKnownProvider.class)
    ...
    /**
     * A JAX-RS sub-resource locator that uses the {@link org.passport.services.resource.RealmResourceSPI} to resolve sub-resources instances given an <code>unknownPath</code>.
     *
    ...
    private static void checkSsl(PassportSession session, RealmModel realm) {
