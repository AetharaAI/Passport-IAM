...
public class ResourcesService extends AbstractResourceService {
    ...
    public ResourcesService(PassportSession session, UserModel user, Auth auth, HttpRequest request) {
    ...
    public Response getResources(@QueryParam("name") String name,
        ...
        Map<org.passport.authorization.model.Resource.FilterOption, String[]> filters =
                new EnumMap<>(org.passport.authorization.model.Resource.FilterOption.class);

        filters.put(org.passport.authorization.model.Resource.FilterOption.OWNER, new String[] { user.getId() });

        if (name != null) {
            filters.put(org.passport.authorization.model.Resource.FilterOption.NAME, new String[] { name });
        }
    ...
    public Object getResource(@PathParam("id") String id) {
        org.passport.authorization.model.Resource resource = resourceStore.findById(null, id);
        ...
        return new ResourceService(resource, provider.getPassportSession(), user, auth, request);
    }

    private Collection<ResourcePermission> toPermissions(List<org.passport.authorization.model.Resource> resources, boolean withRequesters) {
        ...
        for (org.passport.authorization.model.Resource resource : resources) {
    ...
    private Link[] createPageLinks(Integer first, Integer max, int resultSize) {
        ...
        if (nextPage) {
            links.add(Link.fromUri(
                    PassportUriBuilder.fromUri(uriInfo.getRequestUri()).replaceQuery("first={first}&max={max}")
                            .build(first + max, max))
                    .rel("next").build());
        }
