...
import org.passport.http.FormPartValue;
...
import static org.passport.models.UserModel.DISABLED_REASON;
...
public class DefaultBruteForceProtector implements BruteForceProtector {
    ...
    protected PassportSessionFactory factory;

    public DefaultBruteForceProtector(PassportSessionFactory factory) {
        this.factory = factory;
    }

    protected void failure(PassportSession session, RealmModel realm, String userId, String remoteAddr, long failureTime) {
    ...
    protected UserLoginFailureModel getUserFailureModel(PassportSession session, RealmModel realm, String userId) {
        if (realm == null) return null;
        return session.loginFailures().getUserLoginFailure(realm, userId);
    }

    protected void sendEvent(PassportSession session, RealmModel realm, UserLoginFailureModel userLoginFailure, EventType type) {
    ...
    protected void success(PassportSession session, RealmModel realm, String userId) {
        UserLoginFailureModel userLoginFailure = getUserFailureModel(session, realm, userId);
    ...
    protected void processLogin(RealmModel realm, UserModel user, ClientConnection clientConnection, UriInfo uriInfo, boolean success) {
        ExecutorService executor = PassportModelUtils.runJobInTransactionWithResult(factory, session -> {
        ...
        executor.execute(() -> PassportModelUtils.runJobInTransaction(factory, s -> {
    ...
    @Override
    public boolean isTemporarilyDisabled(PassportSession session, RealmModel realm, UserModel user) {
    ...
    @Override
    public boolean isPermanentlyLockedOut(PassportSession session, RealmModel realm, UserModel user) {
    ...
    @Override
    public void cleanUpPermanentLockout(PassportSession session, RealmModel realm, UserModel user) {
