...
public class ClientAttributeCertificateResource {
    ...
    public ClientAttributeCertificateResource(AdminPermissionEvaluator auth, ClientModel client, PassportSession session, String attributePrefix, AdminEventBuilder adminEvent) {
    ...
    @Tag(name = PassportOpenAPI.Admin.Tags.CLIENT_ATTRIBUTE_CERTIFICATE)
    ...
    public CertificateRepresentation generate() {
        ...
        CertificateRepresentation info = PassportModelUtils.generateKeyPairCertificate(client.getClientId());
     ...
     * Only generated public certificate is saved in Passport DB - the private key is not.
     *
     * @param config Keystore configuration as JSON
     * @return
     */
    ...
    public byte[] generateAndGetKeystore(@Parameter(description = "Keystore configuration as JSON") final KeyStoreConfig config) {
        ...
        int keySize = config.getKeySize() != null && config.getKeySize() > 0
                ? config.getKeySize()
                : PassportModelUtils.DEFAULT_RSA_KEY_SIZE;
        int validity = config.getValidity() != null && config.getValidity() > 0
                ? config.getValidity()
                : PassportModelUtils.DEFAULT_CERTIFICATE_VALIDITY_YEARS;
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.YEAR, validity);
        CertificateRepresentation info = PassportModelUtils.generateKeyPairCertificate(client.getClientId(), keySize, calendar);
