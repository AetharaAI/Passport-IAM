...
import org.passport.exceptions.TokenVerificationException;
...
import static org.passport.utils.StringUtil.isNotBlank;
...
public class DPoPUtil {
     ...
     */
    public static void handleDPoPHeader(PassportSession passportSession,
                                        EventBuilder event,
                                        Cors cors,
        ...
        HttpRequest request = passportSession.getContext().getHttpRequest();
        ...
        try {
            DPoP dPoP = new DPoPUtil.Validator(passportSession).request(request).uriInfo(passportSession.getContext().getUri()).validate();
            passportSession.setAttribute(DPoPUtil.DPOP_SESSION_ATTRIBUTE, dPoP);
    ...
    private static DPoP validateDPoP(PassportSession session, URI uri, String method, String token, String accessToken, int lifetime, int clockSkew) throws VerificationException {
    ...
    public static void validateDPoPJkt(String dpopJkt, PassportSession session, EventBuilder event, Cors cors) {
        if (dpopJkt == null) {
            // if Passport did not receive dpop_jkt in an authorization request, Passport needs not to verify whether DPoP Proof public key thumbprint matches dpop_jkt.
            return;
        }
        // if Passport received dpop_jkt in an authorization request, Passport needs to verify whether DPoP Proof public key thumbprint matches dpop_jkt.
    ...
    private static List<String> getSupportedAlgorithms(PassportSession session, Class<? extends Provider> clazz, boolean includeNone) {
        Stream<String> supportedAlgorithms = session.getPassportSessionFactory().getProviderFactoriesStream(clazz)
    ...
    public static List<String> getDPoPSupportedAlgorithms(PassportSession session) {
    ...
    private static class DPoPReplayCheck implements TokenVerifier.Predicate<DPoP> {
        ...
        public DPoPReplayCheck(PassportSession session, int lifetime) {
    ...
    private static class DPoPIsActiveCheck implements TokenVerifier.Predicate<DPoP> {
        ...
        public DPoPIsActiveCheck(PassportSession session, int lifetime, int clockSkew) {
            this.lifetime = lifetime;
        ...
        public boolean test(DPoP t) throws DPoPVerificationException {
            ...
            //   case 1: a client's clock is ahead Passport's clock
            //   case 2: a client's clock is behind Passport's clock
            //
            // To remedy case 1, the valid time slot is as follows:
            //   current passport clock => "iat" - clock skew
            //   current passport clock => "nbf" - clock skew
            // To remedy case 2, the valid time slot is as follows:
            //   current passport clock <= "exp" + clock skew
            //     or
            //   current passport clock <= "iat" + life time + clock skew
            //
            // Therefore, the valid time slot is as follows:
            //    "iat" - clock skew <= passport's clock <= "exp" + clock skew
            //      or
            //    "iat" - clock skew <= passport's clock <= iat" + life time + clock skew
            //
    ...
    public static class Validator {
        ...
        public Validator(PassportSession session) {
            this.session = session;
        }
