...
public class SecureRequestObjectExecutorFactory implements ClientPolicyExecutorProviderFactory {
    ...
    private static final ProviderConfigProperty ALLOWED_CLOCK_SKEW_PROPERTY = new ProviderConfigProperty(
             ALLOWED_CLOCK_SKEW, "Allowed Clock Skew", "The allowed clock skew (sec) between a client and Passport. " +
            "For example, if nbf claim values of 'request' object is ahead from Passport's clock within the clock skew, Passport accept the object even if its nbf claim value indicates the future.",
