package org.passport.services.x509;
...
import org.passport.common.util.PemException;
...
public class NginxProxySslClientCertificateLookup extends AbstractClientCertificateFromHttpHeadersLookup {
    ...
    public NginxProxySslClientCertificateLookup(String sslClientCertHttpHeader,
      ...
      this.certIsUrlEncoded = certIsUrlEncoded;
        ...
        if (!this.isTruststoreLoaded) {
            log.warn("Passport Truststore is null or empty, but it's required for NGINX x509cert-lookup provider");
            log.warn("   see Passport documentation here : https://www.passport-pro.ai/docs/latest/server_installation/index.html#_truststore");
        }
    }
    ...
    protected void buildChain(HttpRequest httpRequest, List<X509Certificate> chain, X509Certificate clientCert) {
        ...
        // Rebuilding the end user certificate chain using Passport Truststore
     ...
     *  we are rebuilding here the end user certificate chain with Passport truststore.
     *  <br>
     *  Please note that Passport truststore must contain root and intermediate CA's certificates.
    ...
    private X509Certificate[] buildChain(X509Certificate endUserAuthCert) {
        ...
        try {
            ...
            if (!isTruststoreLoaded) {
                log.warn("Passport Truststore is null, but it is required !");
                log.warn("  see https://www.passport-pro.ai/docs/latest/server_installation/index.html#_truststore");
            ...
            // Disable CRL checks, as it's possibly done after depending on Passport settings
