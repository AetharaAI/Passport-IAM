...
import org.passport.AbstractOAuthClient;
...
public abstract class AbstractSecuredLocalService {
    ...
    public AbstractSecuredLocalService(PassportSession session, ClientModel client) {
    ...
    public Response loginRedirect(@QueryParam("code") String code,
        ...
        try {
            ...
            PassportUriBuilder redirect = PassportUriBuilder.fromUri(getBaseRedirectUri());
            if (path != null) {
                redirect.path(path);
            }
            if (referrer != null) {
