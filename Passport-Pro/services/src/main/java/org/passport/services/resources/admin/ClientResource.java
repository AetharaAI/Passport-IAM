...
import org.passport.protocol.oidc.OIDCClientSecretConfigWrapper;
...
import org.passport.services.clientpolicy.context.AdminClientUnregisterContext;
import org.passport.services.clientpolicy.context.AdminClientUpdateContext;
import org.passport.services.clientpolicy.context.AdminClientUpdatedContext;
import org.passport.services.clientpolicy.context.AdminClientViewContext;
import org.passport.services.clientpolicy.context.ClientSecretRotationContext;
import org.passport.services.clientregistration.ClientRegistrationTokenUtils;
...
public class ClientResource {
    ...
    public ClientResource(RealmModel realm, AdminPermissionEvaluator auth, ClientModel clientModel, PassportSession session, AdminEventBuilder adminEvent) {
        this.realm = realm;
        this.auth = auth;
    ...
    @Tag(name = PassportOpenAPI.Admin.Tags.CLIENTS)
    ...
    public CredentialRepresentation regenerateSecret() {
        try{
            ...
            String secret = PassportModelUtils.generateSecret(client);
     ...
     * by sending registration request to Passport
     *
     * @param formParams
     */
    @Path("nodes")
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Tag(name = PassportOpenAPI.Admin.Tags.CLIENTS)
    @Operation( summary = "Register a cluster node with the client Manually register cluster node to this client - usually it’s not needed to call this directly as adapter should handle by sending registration request to Passport")
    @APIResponse(responseCode = "204", description = "No Content")
    ...
    private void updateClientFromRep(ClientRepresentation rep, ClientModel client, PassportSession session) throws ModelDuplicateException {
     ...
     */
    public static void validateClientScopeAssignment(PassportSession session, ClientScopeModel clientScope,
                                                     boolean defaultScope, RealmModel realm) {
        LoginProtocolFactory loginProtocolFactory = (LoginProtocolFactory) session.getPassportSessionFactory()
    ...
    public static void updateClientServiceAccount(PassportSession session, ClientModel client, Boolean isServiceAccountEnabled) {
