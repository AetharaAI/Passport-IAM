...
import static org.passport.protocol.ProtocolMapperUtils.isEnabled;
...
public class ProtocolMappersResource {
    ...
    public ProtocolMappersResource(PassportSession session, ProtocolMapperContainerModel client, AdminPermissionEvaluator auth,
    ...
    @Tag(name = PassportOpenAPI.Admin.Tags.PROTOCOL_MAPPERS)
    ...
    private ProtocolMapperRepresentation toEffectiveProtocolMapperRep(ProtocolMapperModel model) {
        ProtocolMapper mapper = (ProtocolMapper) session.getPassportSessionFactory().getProviderFactory(ProtocolMapper.class, model.getProtocolMapper());
        if (mapper == null) {
