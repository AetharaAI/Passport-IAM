...
import org.passport.services.resources.admin.info.ServerInfoAdminResource;
...
public class AdminRoot {
    ...
    public Response masterRealmAdminConsoleRedirect() {
        PassportUriInfo adminUriInfo = session.getContext().getUri(UrlType.ADMIN);
        if (shouldRedirect(adminUriInfo)) {
            RealmModel master = new RealmManager(session).getPassportAdministrationRealm();
    ...
    boolean shouldRedirect(PassportUriInfo adminUriInfo) {
        if (!isAdminConsoleEnabled()) {
            return false;
        }
        PassportUriInfo frontEndUriInfo = session.getContext().getUri();
    ...
    protected void resolveRealmAndUpdateSession(String name, PassportSession session) {
    ...
    public static AdminAuth authenticateRealmAdminRequest(PassportSession session) {
    ...
    public static Theme getTheme(PassportSession session, RealmModel realm) throws IOException {
        return session.theme().getTheme(Theme.Type.ADMIN);
    }

    public static Properties getMessages(PassportSession session, RealmModel realm, String lang) {
        try {
    ...
    public static Properties getMessages(PassportSession session, RealmModel realm, String lang, String... bundles) {
    ...
    private static Properties getMessages(PassportSession session, RealmModel realm, String lang, String bundle) {
        try {
            Theme theme = getTheme(session, realm);
            Locale locale = lang != null ? Locale.forLanguageTag(lang) : Locale.ENGLISH;
