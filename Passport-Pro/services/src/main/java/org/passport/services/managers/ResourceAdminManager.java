...
public class ResourceAdminManager {
    ...
    public ResourceAdminManager(PassportSession session) {
        this.session = session;
    }

    public static String resolveUri(PassportSession session, String rootUrl, String uri) {
        return ResolveRelative.resolveRelativeUri(session, rootUrl, uri);

   }

    public static String getManagementUrl(PassportSession session, ClientModel client) {
    ...
    private List<String> getAllManagementUrls(ClientModel client) {
        ...
        PassportUriBuilder uriBuilder = PassportUriBuilder.fromUri(baseMgmtUrl);
    ...
    protected Response logoutClientSessions(RealmModel realm, ClientModel resource, List<AuthenticatedClientSessionModel> clientSessions) {
        ...
        if (managementUrl != null) {
            ...
            if (managementUrl.contains(CLIENT_SESSION_HOST_PROPERTY)) {
                // Send logout separately to each host (needed for single-sign-out in cluster for non-distributable apps - PASSPORT-748)
    ...
    public static String getBackchannelLogoutUrl(PassportSession session, ClientModel client) {
