...
class MgmtPermissions implements AdminPermissionEvaluator, AdminPermissionManagement, RealmsPermissionEvaluator {
    ...
    MgmtPermissions(PassportSession session, RealmModel realm) {
        this.session = session;
        this.realm = realm;
        PassportSessionFactory passportSessionFactory = session.getPassportSessionFactory();
        if (Profile.isFeatureEnabled(Profile.Feature.ADMIN_FINE_GRAINED_AUTHZ) || Profile.isFeatureEnabled(Profile.Feature.ADMIN_FINE_GRAINED_AUTHZ_V2)) {
            AuthorizationProviderFactory factory = (AuthorizationProviderFactory) passportSessionFactory.getProviderFactory(AuthorizationProvider.class);
            this.authz = factory.create(session, realm);
        }
    }

    MgmtPermissions(PassportSession session, RealmModel realm, AdminAuth auth) {
        this(session, realm);
        this.auth = auth;
    ...
    MgmtPermissions(PassportSession session, AdminAuth auth) {
        this.session = session;
        this.auth = auth;
        this.admin = auth.getUser();
        this.adminsRealm = auth.getRealm();
    ...
    private void initIdentity(PassportSession session, AdminAuth auth) {
        ...
        this.identity = new PassportIdentity(accessToken, session, adminsRealm);
    }

    MgmtPermissions(PassportSession session, RealmModel adminsRealm, UserModel admin) {
        this.session = session;
        this.admin = admin;
    ...
    MgmtPermissions(PassportSession session, RealmModel realm, RealmModel adminsRealm, UserModel admin) {
        this(session, realm);
        this.admin = admin;
        this.adminsRealm = adminsRealm;
