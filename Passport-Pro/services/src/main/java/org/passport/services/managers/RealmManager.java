...
import org.passport.models.utils.DefaultClientScopes;
...
import org.passport.protocol.oidc.mappers.AudienceResolveProtocolMapper;
...
import org.passport.services.clientregistration.policy.DefaultClientRegistrationPolicies;
import org.passport.sessions.AuthenticationSessionProvider;
import org.passport.storage.StoreMigrateRepresentationEvent;
import org.passport.storage.StoreSyncEvent;
import org.passport.utils.ReservedCharValidator;
import org.passport.utils.SMTPUtil;
...
public class RealmManager {
    ...
    public RealmManager(PassportSession session) {
    ...
    public RealmModel getPassportAdministrationRealm() {
    ...
    public RealmModel createRealm(String id, String name) {
        ...
        PassportModelUtils.setupDefaultRole(realm, Constants.DEFAULT_ROLES_ROLE_PREFIX + "-" + name.toLowerCase());
    ...
    private void setupOfflineTokens(RealmModel realm, RealmRepresentation realmRep) {
        RoleModel offlineRole = PassportModelUtils.setupOfflineRole(realm);
    ...
    protected void setupAdminConsole(RealmModel realm) {
        ...
        if (adminConsole == null) adminConsole = PassportModelUtils.createPublicClient(realm, Constants.ADMIN_CONSOLE_CLIENT_ID);
    ...
    public void setupAdminCli(RealmModel realm) {
        ...
        if (adminCli == null) {
            adminCli = PassportModelUtils.createPublicClient(realm, Constants.ADMIN_CLI_CLIENT_ID);
    ...
    public boolean removeRealm(RealmModel realm) {
        ...
        if (removed) {
            if (masterAdminClient != null) {
                session.clients().removeClient(getPassportAdministrationRealm(), masterAdminClient.getId());
            }
    ...
    public void updateRealmEventsConfig(RealmEventsConfigRepresentation rep, RealmModel realm) {
        ...
        if (rep.getEventsListeners() != null) {
            for (String el : rep.getEventsListeners()) {
                EventListenerProviderFactory elpf = (EventListenerProviderFactory) session.getPassportSessionFactory().getProviderFactory(EventListenerProvider.class, el);
                if (elpf == null) {
    ...
    public void setupMasterAdminManagement(RealmModel realm) {
        ...
        ClientModel masterApp = adminRealm.getClientByClientId(PassportModelUtils.getMasterRealmAdminManagementClientId(realm.getName()));
        if (masterApp == null) {
    ...
    private void createMasterAdminManagement(RealmModel realm) {
        ...
        ClientModel realmAdminApp = PassportModelUtils.createManagementClient(adminRealm, PassportModelUtils.getMasterRealmAdminManagementClientId(realm.getName()));
    ...
    private void setupRealmAdminManagement(RealmModel realm) {
        ...
        if (realmAdminClient == null) {
            realmAdminClient = PassportModelUtils.createManagementClient(realm, realmAdminClientId);
    ...
    private void setupAccountManagement(RealmModel realm) {
        ...
        if (accountClient == null) {
            accountClient = PassportModelUtils.createPublicClient(realm, Constants.ACCOUNT_MANAGEMENT_CLIENT_ID);
            ...
            PassportModelUtils.setupDeleteAccount(accountClient);
            ...
            if (accountConsoleClient == null) {
                accountConsoleClient = PassportModelUtils.createPublicClient(realm, Constants.ACCOUNT_CONSOLE_CLIENT_ID);
    ...
    public void setupBrokerService(RealmModel realm) {
        ...
        if (client == null) {
            client = PassportModelUtils.createManagementClient(realm, Constants.BROKER_SERVICE_CLIENT_ID);
    ...
    public RealmModel importRealm(RealmRepresentation rep, Runnable userImport) {
        ...
        try {
            ...
            if (rep.getDefaultRole() == null) {
                PassportModelUtils.setupDefaultRole(realm, determineDefaultRoleName(rep));
            } else {
            ...
            if (!hasRealmRole(rep, AccountRoles.DELETE_ACCOUNT)) {
                PassportModelUtils.setupDeleteAccount(realm.getClientByClientId(Constants.ACCOUNT_MANAGEMENT_CLIENT_ID));
            }
    ...
    private boolean hasRealmAdminManagementClient(RealmRepresentation rep) {
        String realmAdminClientId =  Config.getAdminRealm().equals(rep.getRealm()) ?  PassportModelUtils.getMasterRealmAdminManagementClientId(rep.getRealm()) : getRealmAdminClientId(rep);
    ...
    private void setupAuthorizationServices(RealmModel realm) {
        PassportModelUtils.setupAuthorizationServices(realm);
    }
    ...
    private void fireRealmPostCreate(RealmModel realm) {
        session.getPassportSessionFactory().publish(new RealmModel.RealmPostCreateEvent() {
            @Override
    ...
    public void setDefaultsForNewRealm(RealmModel realm) {
        // setup defaults
        setupRealmDefaults(realm);
        PassportModelUtils.setupDefaultRole(realm, Constants.DEFAULT_ROLES_ROLE_PREFIX + "-" + realm.getName().toLowerCase());
        setupRealmAdminManagement(realm);
        setupAccountManagement(realm);
        setupBrokerService(realm);
        setupAdminConsole(realm);
