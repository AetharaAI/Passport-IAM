...
import org.passport.protocol.oid4vc.model.JwtCNonce;
...
public class JwtCNonceHandler implements CNonceHandler {
    ...
    public JwtCNonceHandler(PassportSession passportSession) {
        this.passportSession = passportSession;
        this.signingKey = selectSigningKey(passportSession.getContext().getRealm());
    }
    ...
    public String buildCNonce(List<String> audiences, Map<String, Object> additionalDetails) {
        ...
        final String issuer = OID4VCIssuerWellKnownProvider.getIssuer(passportSession.getContext());
        ...
        SignatureProvider signatureProvider = passportSession.getProvider(SignatureProvider.class,
    ...
    public void verifyCNonce(String cNonce, List<String> audiences, @Nullable Map<String, Object> additionalDetails)
        ...
        List<TokenVerifier.Predicate<JsonWebToken>> verifiers = //
                new ArrayList<>(List.of(jwt -> {
                                            String expectedIssuer = OID4VCIssuerWellKnownProvider.getIssuer(passportContext);
        ...
        SignatureVerifierContext signatureVerifier = passportSession.getProvider(SignatureProvider.class,
                                                                                 signingKey.getAlgorithm())
                                                                    .verifier(signingKey);
    ...
    protected KeyWrapper selectSigningKey(RealmModel realm) {
        ...
        try {
            signingKey = passportSession.keys().getActiveKey(realm, KeyUse.SIG, Algorithm.ES256);
            ...
            signingKey = passportSession.keys().getActiveKey(realm, KeyUse.SIG, Algorithm.RS256);
        }
        return signingKey;
    }
}
