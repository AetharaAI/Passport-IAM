...
/**
 * This enum provides mapping between Passport provided encryption algorithms and algorithms from xmlsec.
...
public enum SAMLEncryptionAlgorithms {
    ...
    private final String passportIdentifier;
    private static final Map<String, SAMLEncryptionAlgorithms> forPassportIdentifier;
    private static final Map<String, SAMLEncryptionAlgorithms> forXMLEncIdentifier;

    static {
        Map<String, SAMLEncryptionAlgorithms> forPassportIdentifierTmp = new HashMap<>();
        Map<String, SAMLEncryptionAlgorithms> forXMLEncIdentifierTmp = new HashMap<>();
        for (SAMLEncryptionAlgorithms alg: values()) {
            forPassportIdentifierTmp.put(alg.getPassportIdentifier(), alg);
            for (String xmlAlg : alg.getXmlEncIdentifiers()) {
                forXMLEncIdentifierTmp.put(xmlAlg, alg);
            }
        }
        forPassportIdentifier = Collections.unmodifiableMap(forPassportIdentifierTmp);
        forXMLEncIdentifier = Collections.unmodifiableMap(forXMLEncIdentifierTmp);
    }

    SAMLEncryptionAlgorithms(String passportIdentifier, String... xmlEncIdentifier) {
        ...
        this.passportIdentifier = passportIdentifier;
    }
    ...
    /**
     * Getter for the passport identifier.
     * @return The passport identifier.
     */
    ...
    /**
     * Returns the SAMLEncryptionAlgorithms for the passport identifier.
     * @param passportIdentifier The passport identifier
     * @return The associated SAMLEncryptionAlgorithms or null
     */
    public static SAMLEncryptionAlgorithms forPassportIdentifier(String passportIdentifier) {
        return forPassportIdentifier.get(passportIdentifier);
    }
}
