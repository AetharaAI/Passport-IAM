...
import org.passport.protocol.oid4vc.model.SupportedProofTypeData;
...
import static org.passport.services.clientpolicy.executor.FapiConstant.ALLOWED_ALGORITHMS;
...
public class AttestationValidatorUtil {
    ...
    public static KeyAttestationJwtBody validateAttestationJwt(
        ...
        if (header.getX5c() != null && !header.getX5c().isEmpty()) {
            verifier = verifierFromX5CChain(header.getX5c(), header.getAlgorithm().name(), passportSession);
            ...
            verifier = verifierFromResolvedJWK(resolvedJwk, header.getAlgorithm().name(), passportSession);
        } else {
        ...
        validateAttestationPayload(passportSession, vcIssuanceContext, attestationBody);
    ...
    private static void validateAttestationPayload(
        ...
        PassportContext passportContext = passportSession.getContext();
        CNonceHandler cNonceHandler = passportSession.getProvider(CNonceHandler.class);
        ...
        // Validate nonce if present. If provided, it must correspond to a nonce value provided by Passport.
        if (attestationBody.getNonce() != null) {
            ...
            try {
                cNonceHandler.verifyCNonce(
                        ...
                        List.of(OID4VCIssuerWellKnownProvider.getCredentialsEndpoint(passportContext)),
                        Map.of(JwtCNonceHandler.SOURCE_ENDPOINT,
                                OID4VCIssuerWellKnownProvider.getNonceEndpoint(passportContext))
                );
    ...
    private static SignatureVerifierContext verifierFromX5CChain(
            List<String> x5cList,
            String alg,
            PassportSession passportSession) throws VCIssuerException {
        ...
        try {
            ...
            for (String certBase64 : x5cList) {
                // Use Passport's Base64 implementation for decoding x5c certificates
            ...
            return verifierFromResolvedJWK(certJwk, alg, passportSession);
