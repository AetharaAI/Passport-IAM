 ...
 * @version $Revision: 1 $
 */
public class PassportSamlClientInstallation implements ClientInstallationProvider {
    ...
    public Response generateInstallation(PassportSession session, RealmModel realm, ClientModel client, URI baseUri) {
        ...
        buffer.append("<passport-saml-adapter>\n");
        baseXml(session, realm, client, baseUri, samlClient, buffer);
        buffer.append("</passport-saml-adapter>\n");
        return Response.ok(buffer.toString(), MediaType.TEXT_PLAIN_TYPE).build();
    }

    public static void baseXml(PassportSession session, RealmModel realm, ClientModel client, URI baseUri, SamlClient samlClient, StringBuilder buffer) {
    ...
    public String getDisplayType() {
        return "Passport SAML Adapter passport-saml.xml";
    }

    @Override
    public String getHelpText() {
        return "Passport SAML adapter configuration file you must edit. Put this in WEB-INF directory of your WAR.";
    }

    @Override
    public String getFilename() {
        return "passport-saml.xml";
    }
