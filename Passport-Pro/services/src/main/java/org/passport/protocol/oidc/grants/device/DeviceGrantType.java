...
package org.passport.protocol.oidc.grants.device;
...
import org.passport.models.OAuth2DeviceCodeModel;
import org.passport.models.OAuth2DeviceUserCodeModel;
...
import org.passport.protocol.oidc.grants.OAuth2GrantTypeBase;
import org.passport.protocol.oidc.grants.device.clientpolicy.context.DeviceTokenRequestContext;
import org.passport.protocol.oidc.grants.device.clientpolicy.context.DeviceTokenResponseContext;
import org.passport.protocol.oidc.grants.device.endpoints.DeviceEndpoint;
import org.passport.protocol.oidc.utils.PkceUtils;
...
import static org.passport.protocol.oidc.OIDCLoginProtocolService.tokenServiceBaseUrl;
...
public class DeviceGrantType extends OAuth2GrantTypeBase {
    ...
    public static Response denyOAuth2DeviceAuthorization(AuthenticationSessionModel authSession, LoginProtocol.Error error, PassportSession session) {
        PassportContext context = session.getContext();
        RealmModel realm = context.getRealm();
        PassportUriInfo uri = context.getUri();
    ...
    public static Response approveOAuth2DeviceAuthorization(AuthenticationSessionModel authSession, AuthenticatedClientSessionModel clientSession, PassportSession session) {
        PassportContext context = session.getContext();
        RealmModel realm = context.getRealm();
        PassportUriInfo uriInfo = context.getUri();
    ...
    public static OAuth2DeviceCodeModel getDeviceByDeviceCode(PassportSession session, RealmModel realm, ClientModel client, EventBuilder event, String deviceCode) {
    ...
    public static boolean isDeviceCodeDeniedForDeviceVerificationFlow(PassportSession session, RealmModel realm, AuthenticationSessionModel authSession) {
    ...
    public static void removeDeviceByDeviceCode(PassportSession session, String deviceCode) {
        SingleUseObjectProvider singleUseStore = session.singleUseObjects();
        singleUseStore.remove(OAuth2DeviceCodeModel.createKey(deviceCode));
    }

    public static void removeDeviceByUserCode(PassportSession session, RealmModel realm, String userCode) {
        SingleUseObjectProvider singleUseStore = session.singleUseObjects();
        singleUseStore.remove(OAuth2DeviceUserCodeModel.createKey(realm, userCode));
    }

    public static boolean isPollingAllowed(PassportSession session, OAuth2DeviceCodeModel deviceCodeModel) {
        SingleUseObjectProvider singleUseStore = session.singleUseObjects();
        return singleUseStore.putIfAbsent(deviceCodeModel.serializePollingKey(), deviceCodeModel.getPollingInterval());
    }

    public static boolean approveUserCode(PassportSession session, RealmModel realm, String userCode, String userSessionId, Map<String, String> additionalParams) {
        SingleUseObjectProvider singleUseStore = session.singleUseObjects();
    ...
    public static boolean denyUserCode(PassportSession session, RealmModel realm, String userCode) {
        SingleUseObjectProvider singleUseStore = session.singleUseObjects();
        OAuth2DeviceCodeModel deviceCodeModel = DeviceEndpoint.getDeviceByUserCode(session, realm, userCode);
    ...
    public Response process(Context context) {
        ...
        if (codeChallengeMethod != null && !codeChallengeMethod.isEmpty()) {
            ...
            // PKCE Activation is OFF, execute the codes implemented in PASSPORT-2604
