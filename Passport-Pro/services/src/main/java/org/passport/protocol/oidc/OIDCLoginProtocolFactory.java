...
import org.passport.protocol.oidc.mappers.AcrProtocolMapper;
import org.passport.protocol.oidc.mappers.AddressMapper;
import org.passport.protocol.oidc.mappers.AllowedWebOriginsProtocolMapper;
import org.passport.protocol.oidc.mappers.AudienceResolveProtocolMapper;
import org.passport.protocol.oidc.mappers.FullNameMapper;
import org.passport.protocol.oidc.mappers.SubMapper;
import org.passport.protocol.oidc.mappers.UserAttributeMapper;
import org.passport.protocol.oidc.mappers.UserClientRoleMappingMapper;
import org.passport.protocol.oidc.mappers.UserPropertyMapper;
import org.passport.protocol.oidc.mappers.UserRealmRoleMappingMapper;
...
import static org.passport.models.ImpersonationSessionNote.IMPERSONATOR_ID;
import static org.passport.models.ImpersonationSessionNote.IMPERSONATOR_USERNAME;
import static org.passport.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_FAIL_FAST;
import static org.passport.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_NUMBER;
import static org.passport.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_OVERALL_SIZE;
import static org.passport.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_SIZE;
import static org.passport.protocol.oidc.OIDCProviderConfig.DEFAULT_REQ_PARAMS_DEFAULT_MAX_SIZE;
...
public class OIDCLoginProtocolFactory extends AbstractLoginProtocolFactory {
    ...
    /**
     * @deprecated To be removed in Passport 27
     */
    ...
    protected void createDefaultClientScopesImpl(RealmModel newRealm) {
        ...
        if (offlineRole != null) {
            ClientScopeModel offlineAccessScope = PassportModelUtils.getClientScopeByName(newRealm, OAuth2Constants.OFFLINE_ACCESS);
    ...
    public ClientScopeModel addRolesClientScope(RealmModel newRealm) {
        ClientScopeModel rolesScope = PassportModelUtils.getClientScopeByName(newRealm, ROLES_SCOPE);
        if (rolesScope == null) {
    ...
    public ClientScopeModel addWebOriginsClientScope(RealmModel newRealm) {
        ClientScopeModel originsScope = PassportModelUtils.getClientScopeByName(newRealm, WEB_ORIGINS_SCOPE);
    ...
    public ClientScopeModel addMicroprofileJWTClientScope(RealmModel newRealm) {
        ClientScopeModel microprofileScope = PassportModelUtils.getClientScopeByName(newRealm, MICROPROFILE_JWT_SCOPE);
    ...
    public ClientScopeModel addAcrClientScope(RealmModel newRealm) {
        if (Profile.isFeatureEnabled(Profile.Feature.STEP_UP_AUTHENTICATION)) {
            ClientScopeModel acrScope = PassportModelUtils.getClientScopeByName(newRealm, ACR_SCOPE);
            if (acrScope == null) {
    ...
    public ClientScopeModel addBasicClientScope(RealmModel newRealm) {
        ClientScopeModel basicScope = PassportModelUtils.getClientScopeByName(newRealm, BASIC_SCOPE);
        if (basicScope == null) {
    ...
    public ClientScopeModel addServiceAccountClientScope(RealmModel newRealm) {
        ClientScopeModel serviceAccountScope = PassportModelUtils.getClientScopeByName(newRealm, ServiceAccountConstants.SERVICE_ACCOUNT_SCOPE);
    ...
    @Override
    public Object createProtocolEndpoint(PassportSession session, EventBuilder event) {
    ...
    public void setupClientDefaults(ClientRepresentation rep, ClientModel newClient) {
        ...
        if (rep.isBearerOnly() == null
            ...
            if (newClient.getSecret() == null) {
                PassportModelUtils.generateSecret(newClient);
            }
        }
