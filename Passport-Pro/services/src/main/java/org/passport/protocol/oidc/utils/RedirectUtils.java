...
public class RedirectUtils {
    ...
    public static String verifyRedirectUri(PassportSession session, String redirectUri, ClientModel client) {
        return verifyRedirectUri(session, redirectUri, client, true);
    }

    public static String verifyRedirectUri(PassportSession session, String redirectUri, ClientModel client, boolean requireRedirectUri) {
        if (client != null)
            return verifyRedirectUri(session, client.getRootUrl(), redirectUri, client.getRedirectUris(), requireRedirectUri);
        return null;
    }

    public static Set<String> resolveValidRedirects(PassportSession session, String rootUrl, Set<String> validRedirects) {
    ...
    public static String verifyRedirectUri(PassportSession session, String rootUrl, String redirectUri, Set<String> validRedirects, boolean requireRedirectUri) {
        PassportUriInfo uriInfo = session.getContext().getUri();
        ...
        if (redirectUri == null) {
            ...
            if (valid == null && "http".equals(originalRedirect.getScheme()) && LOOPBACK_INTERFACES.contains(originalRedirect.getHost())) {
                String redirectWithDefaultPort = PassportUriBuilder.fromUri(originalRedirect).port(80).buildAsString();
    ...
    private static String relativeToAbsoluteURI(PassportSession session, String rootUrl, String relative) {
        if (rootUrl != null) {
