...
package org.passport.protocol.oid4vc.issuance;
...
import org.passport.crypto.CryptoUtils;
...
import org.passport.jose.jwe.JWEConstants;
...
import org.passport.protocol.oid4vc.OID4VCLoginProtocolFactory;
import org.passport.protocol.oid4vc.issuance.credentialbuilder.CredentialBuilder;
import org.passport.protocol.oid4vc.issuance.credentialbuilder.CredentialBuilderFactory;
import org.passport.protocol.oid4vc.model.CredentialIssuer;
import org.passport.protocol.oid4vc.model.CredentialRequestEncryptionMetadata;
import org.passport.protocol.oid4vc.model.CredentialResponseEncryptionMetadata;
import org.passport.protocol.oid4vc.model.SupportedCredentialConfiguration;
import org.passport.protocol.oidc.utils.JWKSServerUtils;
...
import org.passport.urls.UrlType;
import org.passport.util.JsonSerialization;
import org.passport.utils.MediaType;
import org.passport.wellknown.WellKnownProvider;

import org.apache.http.HttpHeaders;
import org.jboss.logging.Logger;

import static org.passport.OID4VCConstants.SIGNED_METADATA_JWT_TYPE;
import static org.passport.OID4VCConstants.WELL_KNOWN_OPENID_CREDENTIAL_ISSUER;
import static org.passport.constants.OID4VCIConstants.BATCH_CREDENTIAL_ISSUANCE_BATCH_SIZE;
import static org.passport.crypto.KeyType.RSA;
import static org.passport.jose.jwk.RSAPublicJWK.RS256;
...
public class OID4VCIssuerWellKnownProvider implements WellKnownProvider {
    ...
    public OID4VCIssuerWellKnownProvider(PassportSession passportSession) {
    ...
    public Object getConfig() {
        ...
        return getMetadataResponse(issuer, passportSession);
    }

    public CredentialIssuer getIssuerMetadata() {
        PassportContext context = passportSession.getContext();
        ...
        CredentialResponseEncryptionMetadata responseEnc = getCredentialResponseEncryption(passportSession);
        CredentialRequestEncryptionMetadata requestEnc = getCredentialRequestEncryption(passportSession);
        ...
        addDeprecationHeadersIfOldRoute(passportSession);
        ...
        return new CredentialIssuer()
                ...
                .setCredentialsSupported(getSupportedCredentials(passportSession))
                ...
                .setBatchCredentialIssuance(getBatchCredentialIssuance(passportSession));
    }

    public Object getMetadataResponse(CredentialIssuer issuer, PassportSession session) {
    ...
    private CredentialIssuer.BatchCredentialIssuance getBatchCredentialIssuance(PassportSession session) {
     ...
     */
    public Optional<String> generateSignedMetadata(CredentialIssuer metadata, PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private String getSigningAlgorithm(RealmModel realm, PassportSession session) {
     ...
     */
    public static CredentialResponseEncryptionMetadata getCredentialResponseEncryption(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
     ...
     */
    public static CredentialRequestEncryptionMetadata getCredentialRequestEncryption(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
     ...
     */
    public static List<String> getSupportedEncryptionAlgorithms(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
        KeyManager keyManager = session.keys();
     ...
     */
    private static JSONWebKeySet buildJwks(PassportSession session) {
        RealmModel realm = session.getContext().getRealm();
     ...
     */
    public static Map<String, SupportedCredentialConfiguration> getSupportedCredentials(PassportSession passportSession) {
        List<String> globalSupportedSigningAlgorithms = getSupportedAsymmetricSignatureAlgorithms(passportSession);

        RealmModel realm = passportSession.getContext().getRealm();
        Map<String, SupportedCredentialConfiguration> supportedCredentialConfigurations =
                passportSession.clientScopes()
                        .getClientScopesByProtocol(realm, OID4VCIConstants.OID4VC_PROTOCOL)
                        .map(CredentialScopeModel::new)
                        .map(credentialScope -> {
                            SupportedCredentialConfiguration config = SupportedCredentialConfiguration.parse(passportSession,
                                    credentialScope,
                                    globalSupportedSigningAlgorithms
                            );
                            applyFormatSpecificMetadata(passportSession, config, credentialScope);
                            return config;
                        })
    ...
    private static void applyFormatSpecificMetadata(PassportSession passportSession,
        ...
        CredentialBuilder credentialBuilder = passportSession.getPassportSessionFactory()
                ...
                .findFirst()
                .map(factory -> factory.create(passportSession, null))
                .orElse(null);
    ...
    public static SupportedCredentialConfiguration toSupportedCredentialConfiguration(PassportSession passportSession,
        ...
        SupportedCredentialConfiguration config = SupportedCredentialConfiguration.parse(passportSession,
                credentialModel,
                globalSupportedSigningAlgorithms);
        applyFormatSpecificMetadata(passportSession, config, credentialModel);
        return config;
    }
     ...
     */
    public static String getIssuer(PassportContext context) {
     ...
     */
    public static String getNonceEndpoint(PassportContext context) {
     ...
     */
    public static String getCredentialsEndpoint(PassportContext context) {
     ...
     */
    public static List<String> getAuthorizationServers(PassportSession session) {
     ...
     * This includes all asymmetric algorithms supported by Passport (RSA, EC, EdDSA).
     */
    public static List<String> getSupportedAsymmetricSignatureAlgorithms(PassportSession session) {
     ...
     */
    private void addDeprecationHeadersIfOldRoute(PassportSession session) {
