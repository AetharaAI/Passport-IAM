...
public class SAMLDecryptionKeysLocator implements XMLEncryptionUtil.DecryptionKeyLocator {
    ...
    public SAMLDecryptionKeysLocator(PassportSession session, RealmModel realm, String requestedAlgorithm) {
    ...
    private Predicate<KeyWrapper> hasMatchingAlgorithm(String algorithm) {
        ...
        if (usedAlgorithm == null) {
            throw new IllegalStateException("Passport does not support encryption keys for given algorithm: " + algorithm);
        }

        return keyWrapper -> Objects.equals(keyWrapper.getAlgorithmOrDefault(), usedAlgorithm.getPassportIdentifier());
    }
