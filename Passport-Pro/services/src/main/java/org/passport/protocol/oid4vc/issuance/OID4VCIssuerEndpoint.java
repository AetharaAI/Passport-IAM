...
import org.passport.OID4VCConstants;
...
import org.passport.jose.JOSEHeader;
import org.passport.jose.jwe.JWE;
import org.passport.jose.jwe.JWEException;
import org.passport.jose.jwe.JWEHeader;
...
import org.passport.protocol.oid4vc.issuance.credentialbuilder.CredentialBody;
...
import org.passport.protocol.oid4vc.issuance.credentialoffer.CredentialOfferStorage;
import org.passport.protocol.oid4vc.issuance.credentialoffer.CredentialOfferStorage.CredentialOfferState;
import org.passport.protocol.oid4vc.issuance.keybinding.CNonceHandler;
import org.passport.protocol.oid4vc.issuance.keybinding.JwtCNonceHandler;
import org.passport.protocol.oid4vc.issuance.keybinding.ProofValidator;
import org.passport.protocol.oid4vc.issuance.mappers.OID4VCMapper;
import org.passport.protocol.oid4vc.issuance.signing.CredentialSigner;
import org.passport.protocol.oid4vc.issuance.signing.CredentialSignerException;
import org.passport.protocol.oid4vc.model.AttestationProof;
import org.passport.protocol.oid4vc.model.ClaimsDescription;
import org.passport.protocol.oid4vc.model.CredentialIssuer;
import org.passport.protocol.oid4vc.model.CredentialOfferURI;
import org.passport.protocol.oid4vc.model.CredentialRequest;
...
import org.passport.protocol.oid4vc.model.CredentialResponseEncryption;
...
import org.passport.protocol.oid4vc.model.ErrorResponse;
import org.passport.protocol.oid4vc.model.ErrorType;
import org.passport.protocol.oid4vc.model.Format;
import org.passport.protocol.oid4vc.model.JwtProof;
import org.passport.protocol.oid4vc.model.NonceResponse;
import org.passport.protocol.oid4vc.model.OfferUriType;
import org.passport.protocol.oid4vc.model.PreAuthorizedCode;
import org.passport.protocol.oid4vc.model.PreAuthorizedGrant;
import org.passport.protocol.oid4vc.model.Proofs;
...
import org.passport.protocol.oid4vc.utils.ClaimsPathPointer;
import org.passport.protocol.oidc.grants.PreAuthorizedCodeGrantType;
import org.passport.protocol.oidc.grants.PreAuthorizedCodeGrantTypeFactory;
import org.passport.protocol.oidc.rar.AuthorizationDetailsProcessor;
...
import org.passport.saml.processing.api.util.DeflateUtil;
...
import static org.passport.OAuth2Constants.OPENID_CREDENTIAL;
import static org.passport.constants.OID4VCIConstants.CREDENTIAL_OFFER_CREATE;
import static org.passport.constants.OID4VCIConstants.OID4VC_PROTOCOL;
import static org.passport.protocol.oid4vc.model.ErrorType.INVALID_CREDENTIAL_OFFER_REQUEST;
import static org.passport.protocol.oid4vc.model.ErrorType.INVALID_CREDENTIAL_REQUEST;
import static org.passport.protocol.oid4vc.model.ErrorType.UNKNOWN_CREDENTIAL_CONFIGURATION;
import static org.passport.protocol.oid4vc.model.ErrorType.UNKNOWN_CREDENTIAL_IDENTIFIER;
...
public class OID4VCIssuerEndpoint {
     ...
     * signer is directly loaded from the Passport container.
     */
    private final Map<String, CredentialBuilder> credentialBuilders;

    public OID4VCIssuerEndpoint(PassportSession session,
    ...
    public OID4VCIssuerEndpoint(PassportSession passportSession) {
        this.session = passportSession;
        this.bearerTokenAuthenticator = new AppAuthManager.BearerTokenAuthenticator(passportSession);
    ...
    /**
     * Create credential builders from configured component models in Passport.
     *
     * @return a map of the created credential builders with their supported formats as keys.
     */
    private Map<String, CredentialBuilder> loadCredentialBuilders(PassportSession passportSession) {
        PassportSessionFactory passportSessionFactory = passportSession.getPassportSessionFactory();
        return passportSessionFactory.getProviderFactoriesStream(CredentialBuilder.class)
