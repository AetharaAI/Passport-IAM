 ...
 */
package org.passport.protocol.oidc.grants.ciba.resolvers;
...
public class DefaultCIBALoginUserResolver implements CIBALoginUserResolver {
    ...
    public DefaultCIBALoginUserResolver(PassportSession session) {
        this.session = session;
    }

    @Override
    public UserModel getUserFromLoginHint(String loginHint) {
        return PassportModelUtils.findUserByNameOrEmail(session, session.getContext().getRealm(), loginHint);
    }
    ...
    public UserModel getUserFromInfoUsedByAuthentication(String info) {
        return PassportModelUtils.findUserByNameOrEmail(session, session.getContext().getRealm(), info);
    }
