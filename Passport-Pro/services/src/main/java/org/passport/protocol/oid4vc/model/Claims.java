...
public class Claims extends ArrayList<Claim> {
    ...
    public static Claims parse(PassportSession passportSession, CredentialScopeModel credentialScope) {
        Claims claims = new Claims();
        credentialScope.getOid4vcProtocolMappersStream().forEach(protocolMapper -> {
            Optional<Claim> claim = Claim.parse(passportSession, credentialScope.getFormat(), protocolMapper);
