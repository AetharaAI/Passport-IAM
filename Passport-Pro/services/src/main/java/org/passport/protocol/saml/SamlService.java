...
import org.passport.broker.saml.SAMLDataMarshaller;
...
import org.passport.dom.saml.v2.protocol.ArtifactResponseType;
import org.passport.dom.saml.v2.protocol.AuthnRequestType;
import org.passport.dom.saml.v2.protocol.LogoutRequestType;
import org.passport.dom.saml.v2.protocol.NameIDPolicyType;
import org.passport.dom.saml.v2.protocol.RequestAbstractType;
...
import org.passport.protocol.oidc.utils.RedirectUtils;
import org.passport.protocol.saml.preprocessor.SamlAuthenticationPreprocessor;
import org.passport.protocol.saml.profile.ecp.SamlEcpProfileService;
import org.passport.protocol.saml.profile.util.Soap;
import org.passport.protocol.saml.util.ArtifactBindingUtils;
import org.passport.rotation.KeyLocator;
import org.passport.saml.BaseSAML2BindingBuilder;
import org.passport.saml.SAML2LogoutResponseBuilder;
import org.passport.saml.SAML2NameIDBuilder;
import org.passport.saml.SAMLRequestParser;
...
import org.passport.saml.common.util.StaxUtil;
import org.passport.saml.processing.api.saml.v2.request.SAML2Request;
import org.passport.saml.processing.core.saml.v2.common.IDGenerator;
...
import org.passport.saml.processing.core.saml.v2.writers.SAMLRequestWriter;
import org.passport.saml.processing.core.util.PassportKeySamlExtensionGenerator;
import org.passport.saml.processing.web.util.PostBindingUtil;
import org.passport.saml.processing.web.util.RedirectBindingUtil;
...
import org.passport.services.scheduled.ScheduledTaskRunner;
...
import org.passport.transaction.AsyncResponseTransaction;
...
import static org.passport.common.util.StackUtil.getShortStackTrace;
...
public class SamlService extends AuthorizationEndpointBase {
    ...
    public SamlService(PassportSession session, EventBuilder event, DestinationValidator destinationValidator) {
        super(session, event);
    ...
    public abstract class BindingProtocol {
        ...
        protected abstract Response error(PassportSession session, AuthenticationSessionModel authenticationSession, Response.Status status, String message, Object... parameters);
        ...
        protected void handleArtifact(AsyncResponse asyncResponse, String artifact, String relayState) {
            logger.tracef("Passport obtained artifact %s. %s", artifact, getShortStackTrace());
            //Find client
            ClientModel client;
            try {
            ...
            try {
                ...
                ScheduledTaskRunner task = new ScheduledTaskRunner(session.getPassportSessionFactory(), artifactResolutionRunnable);
        ...
        protected Response logoutRequest(LogoutRequestType logoutRequest, ClientModel client, String relayState) {
            ...
            if (authResult != null) {
                ...
                userSession.setNote(AuthenticationManager.PASSPORT_LOGOUT_PROTOCOL, SamlProtocol.LOGIN_PROTOCOL);
            ...
            if (samlClient.requiresRealmSignature()) {
                ...
                if (! postBinding && samlClient.addExtensionsElementWithKeyInfo()) {    // Only include extension if REDIRECT binding and signing whole SAML protocol message
                    builder.addExtension(new PassportKeySamlExtensionGenerator(keys.getKid()));
                }
            }
            try {
                if (postBinding) {
        ...
        /**
         * PASSPORT-12616, PASSPORT-12944: construct the expected destination URI using the configured base URI.
         *
         * @param session a reference to the {@link PassportSession}.
         * @return the constructed {@link URI}.
         */
        protected URI getExpectedDestinationUri(final PassportSession session) {
    ...
    protected class PostBindingProtocol extends BindingProtocol {
        ...
        @Override
        protected Response error(PassportSession session, AuthenticationSessionModel authenticationSession, Response.Status status, String message, Object... parameters) {
    ...
    protected class RedirectBindingProtocol extends BindingProtocol {
        ...
        @Override
        protected boolean containsUnencryptedSignature(SAMLDocumentHolder documentHolder) {
            PassportUriInfo uriInformation = session.getContext().getUri();
    ...
    public static String getIDPMetadataDescriptor(UriInfo uriInfo, PassportSession session, RealmModel realm) {
        try {
     ...
     */
    public AuthenticationSessionModel getOrCreateLoginSessionForIdpInitiatedSso(PassportSession session, RealmModel realm, ClientModel client, String relayState) {
    ...
    public Response artifactResolve(ArtifactResolveType artifactResolveMessage, SAMLDocumentHolder artifactResolveHolder) throws ParsingException, ConfigurationException, ProcessingException {
        ...
        if (CommonClientSessionModel.Action.LOGGING_OUT.name().equals(clientSessionModel.getAction())) {
            ...
            // If Passport sent LogoutResponse we need to also remove UserSession
            if (artifactResponseType.getAny() instanceof StatusResponseType
                ...
                if (!UserSessionModel.State.LOGGED_OUT_UNCONFIRMED.equals(userSessionModel.getState())) {
                    logger.warnf("Passport issued LogoutResponse for clientSession %s, however user session %s was not in LOGGED_OUT_UNCONFIRMED state.",
        ...
        public void run(PassportSession session){
            ...
            try (CloseableHttpResponse result = httpClient.execute(httpPost)) {
                try {
                    ...
                    LoginProtocolFactory factory = (LoginProtocolFactory) session.getPassportSessionFactory().getProviderFactory(LoginProtocol.class, "saml");
                    if (factory == null) {
