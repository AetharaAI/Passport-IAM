...
package org.passport.authentication.authenticators.util;
...
import static org.passport.models.Constants.NO_LOA;
...
public class LoAUtil {
    ...
    public static int getMaxAgeFromLoaConditionConfiguration(AuthenticatorConfigModel loaConditionConfig) {
        try {
            ...
            // Backwards compatibility with Passport 17
     ...
     */
    public static Map<String, Integer> getCredentialTypesToLoAMap(PassportSession session, RealmModel realm, AuthenticationFlowModel topFlow) {
    ...
    private static void fillCredentialsToLoAMap(PassportSession session, RealmModel realm, AuthenticationFlowModel authFlow, Set<String> availableCredentialTypes, AtomicReference<Integer> currentLevel, Map<String, Integer> result) {
        realm.getAuthenticationExecutionsStream(authFlow.getId()).forEachOrdered(execution -> {
            if (execution.isAuthenticatorFlow()) {
                ...
                if (ConditionalLoaAuthenticatorFactory.PROVIDER_ID.equals(execution.getAuthenticator())) {
                    ...
                    AuthenticatorFactory factory = (AuthenticatorFactory) session.getPassportSessionFactory().getProviderFactory(Authenticator.class, execution.getAuthenticator());
