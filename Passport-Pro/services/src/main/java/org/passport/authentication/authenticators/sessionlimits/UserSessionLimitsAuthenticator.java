package org.passport.authentication.authenticators.sessionlimits;
...
public class UserSessionLimitsAuthenticator implements Authenticator {
    ...
    public UserSessionLimitsAuthenticator(PassportSession session) {
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        logger.debugf("session-limiter's current passport clientId: %s", currentClient.getClientId());
        ...
        if (context.getRealm() != null && context.getUser() != null) {
            ...
            logger.debugf("session-limiter's count of total user sessions for this passport client: %s", userSessionCountForClient);
    ...
    private List<UserSessionModel> getUserSessionsForClientIfEnabled(List<UserSessionModel> userSessionsForRealm, ClientModel currentClient, int userClientLimit) {
        ...
        logger.debugf("total user sessions for this passport client will not be counted. Will be logged as 0 (zero)");
