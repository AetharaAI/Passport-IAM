...
public class RegistrationUserCreation implements FormAction, FormActionFactory {
    ...
    /**
     * Get user profile instance for current HTTP request (PassportSession) and for given context. This assumes that there is
     * single user registered within HTTP request, which is always the case in Passport
     */
    public UserProfile getOrCreateUserProfile(FormContext formContext, MultivaluedMap<String, String> formData) {
        PassportSession session = formContext.getSession();
