...
public class OTPFormAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator, CredentialValidator<OTPCredentialProvider> {
    ...
    public List<RequiredActionFactory> getRequiredActions(PassportSession session) {
        return Collections.singletonList((UpdateTotp)session.getPassportSessionFactory().getProviderFactory(RequiredActionProvider.class, UserModel.RequiredAction.CONFIGURE_TOTP.name()));
    }
    ...
    @Override
    public OTPCredentialProvider getCredentialProvider(PassportSession session) {
