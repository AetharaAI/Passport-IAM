...
import org.passport.authentication.authenticators.conditional.ConditionalAuthenticator;
...
public class DefaultAuthenticationFlow implements AuthenticationFlow {
    ...
    private AuthenticatorFactory getAuthenticatorFactory(AuthenticationExecutionModel model) {
        AuthenticatorFactory factory = (AuthenticatorFactory) processor.getSession().getPassportSessionFactory().getProviderFactory(Authenticator.class, model.getAuthenticator());
        if (factory == null) {
    ...
    private Response processSingleFlowExecutionModel(AuthenticationExecutionModel model, boolean calledFromFlow) {
        ...
        if (!selectionOptions.isEmpty() && calledFromFlow) {
            ...
            factory = (AuthenticatorFactory) processor.getSession().getPassportSessionFactory().getProviderFactory(Authenticator.class, model.getAuthenticator());
            if (factory == null) {
                throw new RuntimeException("Unable to find factory for AuthenticatorFactory: " + model.getAuthenticator() + " did you forget to declare it in a META-INF/services file?");
            }
