...
public class TokenAuthEmailAuthenticator implements EmailAuthenticator {
    ...
    public void connect(PassportSession session, Map<String, String> config, Transport transport) throws EmailException {
        try {
            ...
            logger.debugf("AuthenticationFailed-Exception for SMTP in realm %s failed response was %s, will try again", PassportSessionUtil.getRealmNameFromContext(session), e.getMessage());
            ...
            try {
                ...
                logger.warnf("Retry after AuthenticationFailed-Exception for SMTP in realm %s failed response was %s", PassportSessionUtil.getRealmNameFromContext(session), ex);
                throw new EmailException("Retry after AuthenticationFailed-Exception for SMTP failed.", ex);
            }

        } catch (MessagingException e) {
            throw new EmailException("Connect failed for SMTP " + PassportSessionUtil.getRealmNameFromContext(session), e);
        }
    }

    private String gatherValidToken(PassportSession session, Map<String, String> config) throws EmailException {
    ...
    private Optional<String> getAccessToken(PassportSession session, JsonNode response) {
        if (response.has("access_token")) {
            return Optional.of(response.get("access_token").asText());
        } else {
            logger.warnf("Got no access_token from response for SMTP auth in realm %s, response was %s", PassportSessionUtil.getRealmNameFromContext(session), response);
            return Optional.empty();
        }
    }

    private Optional<LocalDateTime> getExpiresIn(PassportSession session, JsonNode response) {
        ...
        if (response.has("expires_in")) {
            ...
            logger.warnf("Got no expires_in from response for SMTP auth in realm %s, response was %s", PassportSessionUtil.getRealmNameFromContext(session), response.asText());
            return Optional.of((LocalDateTime.now().plusSeconds(FALLBACK_EXPIRES_AT_IN_SECONDS)));
        }
    }

    private JsonNode fetchTokenViaHTTP(PassportSession session, String authTokenUrl, String authTokenScope, String authTokenClientId, String authTokenClientSecret) throws IOException {
