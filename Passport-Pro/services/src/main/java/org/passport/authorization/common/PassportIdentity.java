 ...
 */
public class PassportIdentity implements Identity {
    ...
    public PassportIdentity(PassportSession passportSession) {
        this(Tokens.getAccessToken(passportSession), passportSession);
    }

    public PassportIdentity(PassportSession passportSession, IDToken token) {
        this(token, passportSession, passportSession.getContext().getRealm());
    }

    public PassportIdentity(IDToken token, PassportSession passportSession, RealmModel realm) {
        if (token == null) {
        ...
        if (passportSession == null) {
            throw new ErrorResponseException("no_passport_session", "No passport session", Status.FORBIDDEN);
        }
        if (realm == null) {
            throw new ErrorResponseException("no_passport_session", "No realm set", Status.FORBIDDEN);
        }
        ...
        if (token instanceof AccessToken) {
            ...
            ClientSessionContext clientSessionCtx = DefaultClientSessionContext.fromClientSessionScopeParameter(clientSessionModel, passportSession);
            this.accessToken = new TokenManager().createClientAccessToken(passportSession, realm, client, userSession.getUser(), userSession, clientSessionCtx);
        }
        ...
        if (clientModel != null && clientModel.isServiceAccountsEnabled()) {
            clientUser = this.passportSession.users().getServiceAccount(clientModel);
        }
    ...
    public PassportIdentity(AccessToken accessToken, PassportSession passportSession) {
        this(accessToken, passportSession, passportSession.getContext().getRealm());
    }

    public PassportIdentity(AccessToken accessToken, PassportSession passportSession, RealmModel realm) {
        if (accessToken == null) {
            throw new ErrorResponseException("invalid_bearer_token", "Could not obtain bearer access_token from request.", Status.FORBIDDEN);
        }
        if (passportSession == null) {
    ...
    private UserModel getUserFromToken() {
        if (accessToken.getSessionState() == null) {
            return TokenManager.lookupUserFromStatelessToken(passportSession, realm, accessToken);
        }

        // Avoid further loookup if verified userSession already set in the context
        UserSessionModel userSession = passportSession.getContext().getUserSession();
