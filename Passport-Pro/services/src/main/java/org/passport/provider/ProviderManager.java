...
public class ProviderManager {
    ...
    private final PassportDeploymentInfo info;
    ...
    public ProviderManager(PassportDeploymentInfo info, ClassLoader baseClassLoader, String... resources) {
        this.info = info;
        ...
        if (resources != null) {
            for (String r : resources) {
                ...
                for (ProviderLoaderFactory f : factories) {
                    if (f.supports(type)) {
                        PassportDeploymentInfo resourceInfo = PassportDeploymentInfo.create().services();
    ...
    public ProviderManager(PassportDeploymentInfo info, ClassLoader baseClassLoader, Collection<ProviderLoader> additionalProviderLoaders) {
        this.info = info;
        addDefaultLoaders(baseClassLoader);
    ...
    // Compare provider factories of same providerId. Just one of them needs to be chosen to be used in Passport
    public ProviderFactory compareFactories(ProviderFactory p1, ProviderFactory p2) {
        ...
        if (DefaultPassportSessionFactory.isInternal(p1) ^ DefaultPassportSessionFactory.isInternal(p2)) {
            return DefaultPassportSessionFactory.isInternal(p1) ? p2 : p1;
        }
    ...
    public synchronized PassportDeploymentInfo getInfo() {
        return  info;
    }
