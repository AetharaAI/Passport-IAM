 ...
 * @version $Revision: 1 $
 */
public class ExternalPassportRoleToRoleMapper extends AbstractClaimToRoleMapper {

    public static final String[] COMPATIBLE_PROVIDERS = {PassportOIDCIdentityProviderFactory.PROVIDER_ID};
    ...
    public static final String PROVIDER_ID = "passport-oidc-role-to-role-idp-mapper";
    ...
    protected boolean applies(IdentityProviderMapperModel mapperModel, BrokeredIdentityContext context) {
        JsonWebToken token = (JsonWebToken)context.getContextData().get(PassportOIDCIdentityProvider.VALIDATED_ACCESS_TOKEN);
        String[] parseRole = PassportModelUtils.parseRole(mapperModel.getConfig().get(EXTERNAL_ROLE));
    ...
    public String getHelpText() {
        return "Looks for an external role in a passport access token.  If external role exists, grant the user the specified realm or client role.";
    }
