...
import org.passport.protocol.saml.SAMLDecryptionKeysLocator;
import org.passport.protocol.saml.SamlMetadataKeyLocator;
import org.passport.protocol.saml.SamlMetadataPublicKeyLoader;
import org.passport.protocol.saml.SamlPrincipalType;
...
import org.passport.saml.processing.core.saml.v2.constants.X500SAMLProfileConstants;
import org.passport.saml.processing.core.saml.v2.util.ArtifactResponseUtil;
...
public class SAMLEndpoint {
    ...
    public SAMLEndpoint(PassportSession session, SAMLIdentityProvider provider, SAMLIdentityProviderConfig config, UserAuthenticationIdentityProvider.AuthenticationCallback callback, DestinationValidator destinationValidator) {
    ...
    protected abstract class Binding {
        ...
        protected Response logoutRequest(LogoutRequestType request, String relayState) {
            ...
            if (config.isWantAuthnRequestsSigned()) {
                ...
                if (! postBinding && config.isAddExtensionsElementWithKeyInfo()) {    // Only include extension if REDIRECT binding and signing whole SAML protocol message
                    builder.addExtension(new PassportKeySamlExtensionGenerator(keyName));
                }
            }
            try {
                if (postBinding) {
        ...
        private AuthenticationSessionModel samlIdpInitiatedSSO(final String clientUrlName) {
            ...
            LoginProtocolFactory factory = (LoginProtocolFactory) session.getPassportSessionFactory().getProviderFactory(LoginProtocol.class, SamlProtocol.LOGIN_PROTOCOL);
