...
import org.passport.crypto.MacSignatureSignerContext;
...
import org.passport.protocol.oidc.AccessTokenIntrospectionProviderFactory;
...
import org.passport.protocol.oidc.endpoints.TokenIntrospectionEndpoint;
...
public abstract class AbstractOAuth2IdentityProvider<C extends OAuth2IdentityProviderConfig> extends AbstractIdentityProvider<C> implements ExchangeTokenToIdentityProviderToken, ExchangeExternalToken {
    ...
    public AbstractOAuth2IdentityProvider(PassportSession session, C config) {
    ...
    protected SimpleHttpRequest getRefreshTokenRequest(PassportSession session, String refreshToken, String clientId, String clientSecret) {
    ...
    private OAuthResponse refreshToken(OAuthResponse previousResponse, PassportSession session) throws IOException {
        ...
        if (getConfig().isJWTAuthentication()) {
            ...
            try (VaultStringSecret vaultStringSecret = session.vault().getStringSecret(getConfig().getClientSecret())) {
                if (getConfig().isBasicAuthentication()) {
                    ...
                    String header = org.passport.util.BasicAuthHelper.RFC6749.createHeader(getConfig().getClientId(), clientSecret);
    ...
    protected void validateExternalTokenWithIntrospectionEndpoint(TokenExchangeContext tokenExchangeContext) {
        ...
        if (tokenClientId != null && tokenClientId.equals(getConfig().getClientId())) {
            ...
            if (tokenAudiences.contains(getConfig().getClientId())) {
                ...
                // Consider valid introspection also if token contains audience where URL is Passport server (either as issuer or as token-endpoint URL).
