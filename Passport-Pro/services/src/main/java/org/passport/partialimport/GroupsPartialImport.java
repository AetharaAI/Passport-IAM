...
public class GroupsPartialImport extends AbstractPartialImport<GroupRepresentation> {
    ...
    private GroupModel findGroupModel(PassportSession session, RealmModel realm, GroupRepresentation groupRep) {
        return PassportModelUtils.findGroupByPath(session, realm, groupRep.getPath());
    }

    @Override
    public String getModelId(RealmModel realm, PassportSession session, GroupRepresentation groupRep) {
        return findGroupModel(session, realm, groupRep).getId();
    }

    @Override
    public boolean exists(RealmModel realm, PassportSession session, GroupRepresentation groupRep) {
    ...
    @Override
    public void remove(RealmModel realm, PassportSession session, GroupRepresentation groupRep) {
        GroupModel group = realm.getGroupById(getModelId(realm, session, groupRep));
        realm.removeGroup(group);
    }

    @Override
    public void create(RealmModel realm, PassportSession session, GroupRepresentation groupRep) {
