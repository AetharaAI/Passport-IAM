...
public class RolesPartialImport implements PartialImport<RolesRepresentation> {
    ...
    @Override
    public void prepare(PartialImportRepresentation rep, RealmModel realm, PassportSession session) {
        prepareRealmRoles(rep, realm, session);
        prepareClientRoles(rep, realm, session);
    }

    private void prepareRealmRoles(PartialImportRepresentation rep, RealmModel realm, PassportSession session) {
    ...
    private void prepareClientRoles(PartialImportRepresentation rep, RealmModel realm, PassportSession session) {
    ...
    @Override
    public void removeOverwrites(RealmModel realm, PassportSession session) {
        deleteClientRoleOverwrites(realm);
        deleteRealmRoleOverwrites(realm, session);
    }

    @Override
    public PartialImportResults doImport(PartialImportRepresentation rep, RealmModel realm, PassportSession session) {
    ...
    private void setUniqueIds(List<RoleRepresentation> realmRoles) {
        for (RoleRepresentation realmRole : realmRoles) {
            realmRole.setId(PassportModelUtils.generateId());
        }
    }
    ...
    private void setUniqueIds(Map<String, List<RoleRepresentation>> clientRoles) {
        for (List<RoleRepresentation> roleRepresentations : clientRoles.values()) {
            for (RoleRepresentation clientRole : roleRepresentations) {
                clientRole.setId(PassportModelUtils.generateId());
            }
        }
    }
    ...
    private void deleteRealmRoleOverwrites(RealmModel realm, PassportSession session) {
    ...
    private void addResultsForOverwrittenRealmRoles(PartialImportResults results, RealmModel realm, PassportSession session) {
        if (isEmpty(realmRolesToOverwrite)) return;
