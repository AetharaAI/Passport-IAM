...
public class UsersPartialImport extends AbstractPartialImport<UserRepresentation> {
    ...
    @Override
    public String getModelId(RealmModel realm, PassportSession session, UserRepresentation user) {
    ...
    @Override
    public boolean exists(RealmModel realm, PassportSession session, UserRepresentation user) {
        return userNameExists(realm, session, user) || userEmailExists(realm, session, user);
    }

    private boolean userNameExists(RealmModel realm, PassportSession session, UserRepresentation user) {
        return session.users().getUserByUsername(realm, user.getUsername()) != null;
    }

    private boolean userEmailExists(RealmModel realm, PassportSession session, UserRepresentation user) {
    ...
    @Override
    public void remove(RealmModel realm, PassportSession session, UserRepresentation user) {
    ...
    @Override
    public void create(RealmModel realm, PassportSession session, UserRepresentation user) {
        if (user.getId() == null) {
            user.setId(PassportModelUtils.generateId());
        }
