...
public class IdentityProvidersPartialImport extends AbstractPartialImport<IdentityProviderRepresentation> {
    ...
    @Override
    public String getModelId(RealmModel realm, PassportSession session, IdentityProviderRepresentation idpRep) {
        return session.identityProviders().getByAlias(getName(idpRep)).getInternalId();
    }

    @Override
    public boolean exists(RealmModel realm, PassportSession session, IdentityProviderRepresentation idpRep) {
    ...
    @Override
    public void remove(RealmModel realm, PassportSession session, IdentityProviderRepresentation idpRep) {
        session.identityProviders().remove(getName(idpRep));
    }

    @Override
    public void create(RealmModel realm, PassportSession session, IdentityProviderRepresentation idpRep) {
        idpRep.setInternalId(PassportModelUtils.generateId());
