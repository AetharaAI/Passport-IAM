...
public class DefaultPassportSessionFactoryTest {
    ...
    public void testProviderInitialization() {
        DefaultPassportSessionFactory factory = new DefaultPassportSessionFactory() {

            @Override
            public PassportSession create() {
                return null;
            }
        };
    ...
    public void defaultProviderFromConfigTest() {
        ...
        Assert.assertEquals("one", DefaultPassportSessionFactory.resolveDefaultProvider(map, spi));

        // Highest priority selected
        config.defaultProvider = null;
        Assert.assertEquals("three", DefaultPassportSessionFactory.resolveDefaultProvider(map, spi));

        // No default, with order=0
        map.values().stream().forEach(p -> ((DummyProviderFactory) p).order = 0);
        Assert.assertNull(DefaultPassportSessionFactory.resolveDefaultProvider(map, spi));

        // Provider with id=default selected
        map.put("default", new DummyProviderFactory("default", 0));
        Assert.assertEquals("default", DefaultPassportSessionFactory.resolveDefaultProvider(map, spi));
        ...
        try {
            DefaultPassportSessionFactory.resolveDefaultProvider(map, spi);
    ...
    private class SimpleProviderFactory<T extends Provider> implements ProviderFactory<T> {
        ...
        @Override
        public T create(PassportSession session) {
            return null;
        }
