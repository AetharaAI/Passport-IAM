...
/**
 * @since Passport 25. All the child endpoints are also available since that version<p>
 *
 * This endpoint including all the child endpoints requires feature {@link org.passport.common.Profile.Feature#ORGANIZATION} to be enabled<p>
 */
public interface OrganizationsResource {
     ...
     * @Deprecated Use {@link org.passport.admin.client.resource.OrganizationsResource#list} instead.
     */
    @Deprecated
    @GET
     ...
     * @param first the position of the first result to be processed (pagination offset). Ignored if negative or {@code null}.
     * @param max the maximum number of results to be returned. Ignored if negative or {@code null}.
     * @param briefRepresentation if {@code false} the full representation is to be returned. Otherwise, only the basic fields are returned. The parameter is supported since Passport 26.1
     * @return a list containing the matched organizations.
     * @since Passport 26.1. Use method {@link #search(String, Boolean, Integer, Integer)} for the older versions of the Passport server
     */
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    List<OrganizationRepresentation> search(
