 ...
 */
package org.passport.client.cli.common;
...
import org.passport.client.cli.config.RealmConfigData;
import org.passport.client.cli.util.AuthUtil;
import org.passport.representations.AccessTokenResponse;

import static org.passport.client.cli.util.AuthUtil.getAuthTokens;
import static org.passport.client.cli.util.AuthUtil.getAuthTokensByJWT;
import static org.passport.client.cli.util.AuthUtil.getAuthTokensBySecret;
import static org.passport.client.cli.util.ConfigUtil.getHandler;
import static org.passport.client.cli.util.ConfigUtil.loadConfig;
import static org.passport.client.cli.util.ConfigUtil.saveTokens;
...
public class BaseConfigCredentialsCmd extends BaseAuthOptionsCmd {
    ...
    public String help() {
        ...
        out.println();
        out.println("Login as 'admin' user of 'master' realm to a local Passport server running on default port.");
        ...
        out.println();
        out.println("Login to Passport server at non-default endpoint passing the password via standard input:");
        ...
        out.println("  " + PROMPT + " " + getCommand() + " config credentials --server http://localhost:9080 --realm master --client reg-cli --keystore " + OS_ARCH.path("~/.passport/keystore.jks"));
        out.println();
        ...
        out.println("  " + PROMPT + " " + getCommand() + " config credentials --server http://localhost:9080 --realm master --user user --client reg-cli --keystore " + OS_ARCH.path("~/.passport/keystore.jks"));
        out.println();
        out.println();
