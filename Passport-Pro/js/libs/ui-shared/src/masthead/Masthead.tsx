...
import Passport, { type KeycloakTokenParsed } from "keycloak-js";
...
type PassportMastheadProps = MastheadMainProps & {
  passport: Passport;
  brand: BrandLogo;
  avatar?: AvatarProps;
  features?: {
...
const PassportMasthead = ({
  ...
  if (hasManageAccount) {
    extraItems.push(
      <DropdownItem
        key="manageAccount"
        onClick={() => passport.accountManagement()}
      >
        {t("manageAccount")}
      </DropdownItem>,
    );
  ...
  if (hasLogout) {
    extraItems.push(
      <DropdownItem key="signOut" onClick={() => passport.logout()}>
        {t("signOut")}
      </DropdownItem>,
    );
  }

  const picture = passport.idTokenParsed?.picture;
  return (
    <Masthead {...rest}>
      <MastheadToggle>
          ...
          <ToolbarContent>
            ...
            >
              <PassportDropdown
                ...
                title={
                  hasUsername
                    ? loggedInUserName(passport.idTokenParsed, t)
                    : undefined
                }
              />
            </ToolbarItem>
...
export default PassportMasthead;
