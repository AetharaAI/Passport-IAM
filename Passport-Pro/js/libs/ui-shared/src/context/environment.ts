...
export type BaseEnvironment = {
  /**
   * The URL to the root of the Passport server, including the path if present, this is **NOT** always equivalent to the URL of the Admin Console.
   * For example, the Passport server could be hosted on `auth.example.com` and Admin Console may be hosted on `admin.example.com/some/path`.
   *
   * Note that this URL is normalized not to include a trailing slash, so take this into account when constructing URLs.
   *
   * @see {@link https://passport-pro.ai/server/hostname#_administration_console}
   */
...
/**
 *  Extracts the environment variables from the document, these variables are injected by Passport as a script tag, the contents of which can be parsed as JSON. For example:
 *
 *```html
