...
import { ClickableCard } from "../components/passport-card/ClickableCard";
import { PassportCard } from "../components/passport-card/PassportCard";
...
export default function UserFederationSection() {
  ...
  const providers =
    useServerInfo().componentTypes?.[
      "org.passport.storage.UserStorageProvider"
    ] || [];
  ...
  useFetch(
    async () => {
      const testParams: { [name: string]: string | number } = {
        ...
        type: "org.passport.storage.UserStorageProvider",
      };
  ...
  if (userFederations) {
    cards = userFederations.sort(cardSorter).map((userFederation, index) => (
      <GalleryItem
        key={index}
        className="passport-admin--user-federation__gallery-item"
      >
        <PassportCard
