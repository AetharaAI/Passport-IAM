import type ComponentRepresentation from "@passport/passport-admin-client/lib/defs/componentRepresentation";
import type ComponentTypeRepresentation from "@passport/passport-admin-client/lib/defs/componentTypeRepresentation";
import { DirectionType } from "@passport/passport-admin-client/lib/resources/userStorageProvider";
import {
  HelpItem,
  PassportSelect,
  PassportSpinner,
  ...
  useFetch(
    async () => {
      const components = await adminClient.components.listSubComponents({
        id,
        type: "org.passport.storage.ldap.mappers.LDAPStorageMapper",
      });
        ...
        dropdownItems={
          isNew
            ...
            : [
                ...
                ...(mapper?.metadata.fedToPassportSyncSupported
                  ? [
                      <DropdownItem
                        key="fedSync"
                        onClick={() => sync("fedToPassport")}
                      >
                        {t(mapper.metadata.fedToPassportSyncMessage)}
                      </DropdownItem>,
                    ]
                  : []),
                ...(mapper?.metadata.passportToFedSyncSupported
                  ? [
                      <DropdownItem
                        key="ldapSync"
                        onClick={async () => {
                          await sync("passportToFed");
                        }}
                      >
                        {t(mapper.metadata.passportToFedSyncMessage)}
                      </DropdownItem>,
                    ]
                  : []),
              ]
      ...
      <PageSection variant="light" isFilled>
        <FormProvider {...form}>
          ...
          >
            ...
            <input
              type="hidden"
              defaultValue="org.passport.storage.ldap.mappers.LDAPStorageMapper"
