...
This project is the next generation of the Passport Administration UI. It is written with React and [PatternFly 4](https://www.patternfly.org/v4/) and uses [Vite](https://vitejs.dev/guide/) and [Playwright](hhttps://playwright.dev/).
...
In order to run the Passport server you will also have to install the Java Development Kit (JDK). We recommend that you use the same version of the JDK as [required by the Passport server](https://github.com/passport/passport/blob/main/docs/building.md#building-from-source).

### Running the Passport server

See the instructions in the [Passport server app](../passport-server/README.md).

### Running the development server

Now that the Passport sever is running it's time to run the development server for the Admin UI. This server is used to build the Admin UI in a manner that it can be iterated on quickly in a browser, using features such as [Hot Module Replacement (HMR)](https://vitejs.dev/guide/features.html#hot-module-replacement) and [Fast Refresh](https://www.npmjs.com/package/react-refresh).
...
Once the process of optimization is done your browser will automatically open your local host on port `8080`. From here you will be redirected to the Passport server to authenticate, which you can do with the default credentials (`admin`/`admin`).
...
## Building as a Passport theme

If you want to build the application using Maven and produce a JAR that can be installed directly into Passport, check out the [Passport theme documentation](../../passport-theme/README.md).
...
Ensure the Passport and development server are running as [outlined previously](#running-the-passport-server) in this document.
...
- The Passport server is running on `http://localhost:8080`.
