...
This project is the next generation of the Passport Account UI. It is written with React and [PatternFly](https://www.patternfly.org/) and uses [Vite](https://vitejs.dev/guide/).
...
```bash
npm i @passport/passport-account-ui
```

## Usage

To use these pages you'll need to add `PassportProvider` in your component hierarchy to setup what client, realm and url to use.
...
Example:
```ts
import { savePersonalInfo, useEnvironment } from "@passport/passport-account-ui";

//...
function App() {
  // the save function also needs to have the context so that it knows where to POST
  // this hook gives us access to the `PassportProvider` context
