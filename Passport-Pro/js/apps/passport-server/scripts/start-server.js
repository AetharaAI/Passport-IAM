...
const LOCAL_DIST_NAME = "passport-999.0.0-SNAPSHOT.tar.gz";
...
async function startServer() {
  let { scriptArgs, passportArgs } = handleArgs(process.argv.slice(2));
...
function handleArgs(args) {
  ...
  // we need to remove the args that belong to the script so that we can pass the rest through to passport
  tokens
  ...
  return { scriptArgs: values, passportArgs: args };
}
...
async function getNightlyAsset() {
  ...
  return release.data.assets.find(
    ({ name }) => name === "passport-999.0.0-SNAPSHOT.tar.gz",
  );
}
