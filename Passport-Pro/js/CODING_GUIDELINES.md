...
We run the UI in dev mode and configure passport to proxy the request this can be done by adding `KC_ADMIN_VITE_URL=http://localhost:5174` as environment variable to the startup of passport or by using the start script:

```bash
pnpm --filter passport-server start --admin-dev
```

This script will download the nightly version of passport and start it with the added environment variable.
Afterwards one needs to start the dev server with:

```bash
pnpm --filter passport-admin-ui run dev
```
...
See [README.md](apps/passport-server/README.md) for more information.
...
The only place where it is valid to use the non-null assertion operator is on the types that are provided by the [Admin API client](https://github.com/passport/passport-nodejs-admin-client). The reason for this is that the types are generated from Java code, which does not explicitly provide information about the nullability of fields (more on that [here](https://github.com/passport/passport-nodejs-admin-client/issues/187)).
...
For the Admin UI, we modify the PatternFly convention to namespace the classes and variables to the Passport packages.

**Class name**

```css
.passport-admin--block[__element][--modifier][--state][--breakpoint][--pseudo-element]
```

**Examples of custom CSS classes**

```css
// Modification to all data tables throughout Passport admin
.passport-admin--data-table {
...
}

// Data tables throughout passport that are marked as compact
.passport-admin--data-table--compact {
...
}

// Child elements of a compact data-table
// Don’t increase specificity with a selector like this:
// .passport-admin--data-table--compact .data-table-item
// Instead, follow the pattern for a single class on the child
.passport-admin--data-table__data-table-item--compact {
...
}

// Compact data table just in the management UI at the lg or higher breakpoint
.passport-admin--data-table--compact--lg {
...
}
```
...
```css
--passport-admin--block[__element][--modifier][--state][--breakpoint][--pseudo-element]--PropertyCamelCase
```
...
--passport-admin--brand--Height: var(--pf-v5-global--spacer--xl);
```
...
.passport-admin--manage-columns__modal .pf-v5-c-dropdown {
  margin-bottom: 24px;
}

// Do use a new class
// Do use a PatternFly global spacer variable
.passport-admin--manage-columns__dropdown {
...
switch (titleStatus) {
  case "success":
    return (
      <>
        <InfoCircleIcon
          className="passport-admin--icon--info" // use a new passport class
        />{" "}
        {titleText}{" "}
      </>
    );
  case "failure":
    return (
      <>
        <InfoCircleIcon className="passport-admin--icon--info" /> {titleText}{" "}
      </>
    );
}
```
