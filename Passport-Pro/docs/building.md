...
To build Passport without running the tests:

    ./mvnw clean install -DskipTests

To build Passport and run the tests (note that this might take several hours):

    ./mvnw clean install

To build Passport with adapters run:
...
Classes from `org.passport.testsuite.*` packages aren't suitable to be used in production.
...
Follow the [`quarkus` module documentation](../quarkus/README.md#running-in-passport-development-mode) for instructions how to run Passport in dev environment.

## Working with the codebase

We don't currently enforce a code style in Passport, but a good reference is the code style used by WildFly. This can be 
