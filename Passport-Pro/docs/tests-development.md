...
IDE an embedded Passport server is executed automatically.

A good test to start looking at is org.passport.testsuite.forms.LoginTest. It is reasonable straightforward to understand
this test.
...
All tests don't have to be fully black box testing as the testsuite deploys a special feature to the Passport server
that allows running code within the server. This allows writing tests that can execute functions not exposed through
APIs as well as access data that is not exposed. For an example on how to do this look at org.passport.testsuite.runonserver.RunOnServerTest.
