...
summary="How to configure Passport server to not store users from identity providers in the database."
includedOptions="">

When using Passport as an identity broker, you can configure the Passport server
...
In order to use transient users, you need to enable them in the Passport
...
- It is possible to leverage offline sessions with transient users.
  ...
  be stored in the Passport database until the respective session expires or the token is revoked.
