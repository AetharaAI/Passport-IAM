...
if (email.endsWith('@passport-pro.ai')) {
    $evaluation.grant();
}
```
...
You can also use Role-Based Access Control (RBAC) in your policies. In the example below, we check if a user is granted with a `passport_user` *realm* role:

```javascript
const context = $evaluation.getContext();
const identity = context.getIdentity();

if (identity.hasRealmRole('passport_user')) {
    $evaluation.grant();
}
```
...
or has an e-mail from `passport-pro.ai` domain:
...
NOTE: When writing your own rules, keep in mind that the *$evaluation* object is an object implementing *org.passport.authorization.policy.evaluation.Evaluation*. For more information about what you can access from this interface, see the <<_policy_evaluation_api, Evaluation API>>.
