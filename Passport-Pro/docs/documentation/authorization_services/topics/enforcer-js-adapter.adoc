...
You can obtain this library by installing it https://www.npmjs.com/package/passport-js[from NPM]:
...
Next, you can create a `PassportAuthorization` instance as follows:

[source,javascript]
----
import Passport from "passport-js";
import PassportAuthorization from "passport-js/authz";
...
const authorization = new PassportAuthorization(passport);
...
The `passport-js/authz` library provides two main features:
...
The `passport-js/authz` library provides an `entitlement` function that you can use to obtain an RPT from the server by providing
