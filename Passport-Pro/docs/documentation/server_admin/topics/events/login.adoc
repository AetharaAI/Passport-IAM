...
----
11:36:09,965 WARN  [org.passport.events] (default task-51) type=LOGIN_ERROR, realmId=master,
                    clientId=myapp,
...
The Logging Event Listener logs events to the `org.passport.events` log category. {project_name} does not include debug log events in server logs, by default.

To include debug log events in server logs:

. Change the log level for the `org.passport.events` category
