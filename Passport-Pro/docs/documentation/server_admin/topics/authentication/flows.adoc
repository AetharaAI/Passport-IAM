...
See *Passport JavaScript adapter* in the link:{securing_apps_link}[securing apps] section for more details.
...
snippet from the OIDC login URL path (`/auth`) with `/registrations` . So the full URL might be similar to the following: `https://passport.example.com/realms/your_realm/protocol/openid-connect/registrations`.
