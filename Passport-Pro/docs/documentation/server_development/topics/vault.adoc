...
You can use a vault SPI from `org.passport.vault` package to write custom extension for {project_name} to connect to arbitrary vault implementation.
...
The example uses `PassportSession.vault()` as the entrypoint for accessing
