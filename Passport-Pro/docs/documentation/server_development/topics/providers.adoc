...
`META-INF/services/org.passport.theme.ThemeSelectorProviderFactory`.
...
public class MyThemeSelectorProvider implements ThemeSelectorProvider {
    ...
    public MyThemeSelectorProvider(PassportSession session) {
    }
...
Example service configuration file (`META-INF/services/org.passport.theme.ThemeSelectorProviderFactory`):
...
To configure your provider, see the link:https://www.passport-pro.ai/server/configuration-provider[Configuring Providers] {section}.
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
  ...
  <dependencies>
    <dependency>
      <groupId>org.passport</groupId>
      <artifactId>passport-model-jpa</artifactId>
...
specified in the link:https://www.passport-pro.ai/server/configuration-provider[Configuring Providers] {section}. On the other hand, this may not be always possible.
...
To show info from your provider it is enough to implement `org.passport.provider.ServerInfoAwareProviderFactory` interface in your `ProviderFactory`.
...
usage of the `PassportSession`, which is available to your provider as described in the section <<_implementing_spi,Implementing an SPI>>.
...
In the code, you can obtain the provider such as `passportSession.getProvider(HostnameProvider.class)`
...
need to be retrieved with the usage of `PassportSessionFactory`. It is not recommended to implement your own providers this way as it may be deprecated in the future.
...
If your provider needs additional dependencies not already provided by Passport copy these to the `providers` directory.

After registering new providers or dependencies Passport needs to be re-built with a non-optimized start or the `kc.[sh|bat] build` command.
...
var inputRequest = passportSession.getContext().getHttpRequest();
...
[source]
----
META-INF/passport-scripts.json
...
The `META-INF/passport-scripts.json` is a file descriptor that provides metadata information about the scripts you want to deploy. It is a JSON file with the following structure:
...
For each script file in your `JAR` file, you need a corresponding entry in `META-INF/passport-scripts.json` that maps your scripts files to a specific provider type. For that you should provide the following properties for each entry:
