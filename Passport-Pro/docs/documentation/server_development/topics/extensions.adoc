...
. implement the interface `org.passport.provider.Spi` and define the ID of your SPI and the `ProviderFactory` and `Provider` classes. That looks like this:
+
[source,java]
----
...
. Create the file `META-INF/services/org.passport.provider.Spi` and add the class of your SPI to it. For example:
+
[source]
----
ExampleSpi
...
is always scoped per application, however `ExampleService` is scoped per-request (or more accurately per `PassportSession` lifecycle).
