...
`_PASSPORT_ROOT_{kc_realms_path}/master/login-actions/action-token` via `key` parameter, it is validated and a proper action
...
above), it can be serialized and signed as such using Passport's `JWSBuilder` class. This way has been already
implemented in `serialize(session, realm, uriInfo)` method of `org.passport.authentication.actiontoken.DefaultActionToken`
...
should consider implementing a descendant of `org.passport.representations.JsonWebToken` class that would implement
`org.passport.models.ActionTokenKeyModel` interface. In that case, you can take advantage of the existing
`org.passport.authentication.actiontoken.DefaultActionToken` class as it already satisfies both these conditions,
 ...
 *  `org.passport.authentication.actiontoken.ActionTokenHandler` - actual handler of action token for a particular
...
`org.passport.models.SingleUseObjectProvider` in action token handler to invalidate the used tokens manually.
+
Default implementation of most of the `ActionTokenHandler` methods is the
`org.passport.authentication.actiontoken.AbstractActionTokenHandler` abstract class in `passport-services` module. The
only method that needs to be implemented is `handleToken(token, context)` that performs the actual action.

 *  `org.passport.authentication.actiontoken.ActionTokenHandlerFactory` - factory that instantiates action token
