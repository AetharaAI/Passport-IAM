...
Let's walk through the steps from when a client first redirects to passport to authenticate the user.
...
To create an authenticator, you must at minimum implement the org.passport.authentication.AuthenticatorFactory and Authenticator interfaces.
...
* A class that extends org.passport.credential.CredentialModel, and that can generate the correct format of the credential in the database
* A class implementing the org.passport.credential.CredentialProvider and interface, and a class implementing its CredentialProviderFactory factory interface.
...
This jar must contain a file named  `org.passport.authentication.AuthenticatorFactory` and must be contained in the `META-INF/services/` directory of your jar.
This file must list the fully qualified class name of each AuthenticatorFactory implementation you have in the jar.
For example:

[source,java]
----
org.passport.examples.authenticator.SecretQuestionAuthenticatorFactory
org.passport.examples.authenticator.AnotherProviderFactory
----
...
`org.passport.models.credential.PasswordCredentialModel` class.
...
public class SecretQuestionCredentialProvider implements CredentialProvider<SecretQuestionCredentialModel>, CredentialInputValidator {
    ...
    public SecretQuestionCredentialProvider(PassportSession session) {
...
[source,java]
----
public SecretQuestionCredentialProvider getCredentialProvider(PassportSession session) {
...
This page builder is called `org.passport.login.LoginFormsProvider`. The LoginFormsProvider.createForm() method loads a Freemarker template file from your login theme.
...
{project_name} comes with a Freemarker https://www.passport-pro.ai/ui-customization/themes[theme and template engine].
...
This file should be added to the `theme-resources/templates` in your JAR, see https://www.passport-pro.ai/ui-customization/themes[Working with themes: Theme resource provider] for more details.
...
You will package your classes within a single jar.
This jar does not have to be separate from other provider classes but it must contain a file named  `org.passport.authentication.RequiredActionFactory`                and must be contained in the `META-INF/services/` directory of your jar.
This file must list the fully qualified classname of each RequiredActionFactory implementation you have in the jar.
For example:

[source,java]
----
org.passport.examples.authenticator.SecretQuestionRequiredActionFactory
----
...
You will package your classes within a single jar.
This jar must contain a file named  `org.passport.authentication.FormActionFactory`                and must be contained in the `META-INF/services/` directory of your jar.
...
org.passport.authentication.forms.RegistrationUserCreation
org.passport.authentication.forms.RegistrationRecaptcha
----
...
Client side::
Here you need to implement `org.passport.adapters.authentication.ClientCredentialsProvider` and put the implementation either to:
...
* Some JAR file, which will be added into WEB-INF/lib of your WAR
  * Some JAR file, which will be used as jboss module and configured in jboss-deployment-structure.xml of your WAR.                                In all cases, you also need to create the file `META-INF/services/org.passport.adapters.authentication.ClientCredentialsProvider`                                either in the WAR or in your JAR.

Server side::
  Here you need to implement `org.passport.authentication.ClientAuthenticatorFactory` and `org.passport.authentication.ClientAuthenticator` . You also need to add the file `META-INF/services/org.passport.authentication.ClientAuthenticatorFactory` with the name of the implementation classes.
