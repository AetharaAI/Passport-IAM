...
Also notice that we are proxying the local user using the `org.passport.models.utils.UserModelDelegate` class.
...
an additional interface you can implement to deal with this, `org.passport.storage.user.ImportSynchronization`:
