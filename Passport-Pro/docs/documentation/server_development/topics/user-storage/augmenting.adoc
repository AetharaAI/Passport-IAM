...
`org.passport.storage.federated.UserFederatedStorageProvider` class.
...
The `UserFederatedStorageProvider` instance is available on the `UserStorageUtil.userFederatedStorage(PassportSession)` method.
...
{project_name} comes with a helper class `org.passport.storage.adapter.AbstractUserAdapterFederatedStorage`
