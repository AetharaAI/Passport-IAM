...
Since version 20, Passport relies only on Quarkus. Unlike WildFly, Quarkus is not an Application Server.
ifeval::[{project_community}==true]
For more detail, see https://www.passport-pro.ai/migration/migrating-to-quarkus#_quarkus_is_not_an_application_server.
endif::[]

Therefore, the User Storage Providers cannot be packaged within any Jakarta EE component or make it an EJB as was the case when Passport ran over WildFly in previous versions.
...
ifeval::[{project_community}==true]
See https://www.passport-pro.ai/migration/migrating-to-quarkus#_migrating_custom_providers[Migrating custom providers].
endif::[]
