...
The passport examples directory contains an example of a simple User Federation Provider backed by a simple properties file. See `providers/federation-provider`
...
the user into Passport storage. How much metadata you import is fully up to you. This import is done by invoking methods on the object returned by
`PassportSession.userFederatedStorage()` to add and import user information. The proxy() method will be called whenever Passport has found an imported UserModel.
