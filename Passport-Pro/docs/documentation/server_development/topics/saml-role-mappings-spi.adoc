...
The `org.passport.adapters.saml.RoleMappingsProvider` SPI doesn't impose any restrictions on the mappings that can be performed.
...
To implement a custom role mappings provider one first needs to implement the `org.passport.adapters.saml.RoleMappingsProvider`
interface. Then, a `META-INF/services/org.passport.adapters.saml.RoleMappingsProvider` file containing the fully qualified name
...
`passport-saml.xml` or in the `passport-saml` subsystem. So to enable your custom provider simply make sure that its id is
