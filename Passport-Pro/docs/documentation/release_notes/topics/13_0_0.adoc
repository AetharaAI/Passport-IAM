...
Passport now supports communication with clients using SAML _Artifact_ binding. A new `Force Artifact Binding` option
...
this version, Passport SAML client adapter does NOT support Artifact binding.
...
Passport can now leverage PKCE when brokering to an external OpenID Connect IdP.
