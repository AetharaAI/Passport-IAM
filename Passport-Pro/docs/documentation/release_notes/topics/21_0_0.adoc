...
In Passport 19 the new admin console was graduated to the new default admin console, and the old admin console was
deprecated. In this release the old admin console has been removed completely.

= Passport uses Micrometer for metrics

Passport provides an optional a metrics endpoint which exports metrics in the Prometheus format.
...
= Java 11 support for Passport server deprecated

Running the Passport server with Java 11 is now deprecated, and planned to be removed in Passport 22.
...
See this https://github.com/passport/passport/discussions/16446[discussion] for more details.

= SAML SP metadata changes

Prior to this release, SAML SP metadata contained the same key for both
signing and encryption use. Starting with this version of Passport,
...
the supported XML-Enc algorithms with the mapping to Passport realm keys.
See the link:{upgradingguide_link}[{upgradingguide_name}] for more details.

[cols="1,1"]
|===
|*XML-Enc algorithm*
|*Passport realm key algorithm*
...
of Passport 20 release. See link:{upgradingguide_link}[{upgradingguide_name}] for more details.
...
`EXISTS` or `NOT_EXISTS`. See JavaDoc of Passport 21 for more details.

= FIPS 140-2 preview support

FIPS 140-2 support in Passport, which was experimental in the previous release, is now promoted to preview. There were many fixes and improvements to create this preview version.
For the details, see the https://www.passport-pro.ai/server/fips[FIPS documentation]. Feedback is welcome!
...
added by proxies that would otherwise be altered or lost when proxy servers are involved in the path of the request, Passport
can now leverage the standard `Forwarded` header for the same purpose.

For more details, see the https://www.passport-pro.ai/server/reverseproxy[Using a reverse proxy] {section}.

Please, make sure your proxy is also overriding the `Forwarded` header when making requests to Passport nodes.

= The container image is now based on ubi9-micro

To enhance security, the https://quay.io/repository/passport/passport?tab=info[Passport Container Image] has been modified in two ways: First, it is now based on UBI9, rather than UBI8. Second, we have switched to `+-micro+`, whereas `+-minimal+` was used before.

The change to UBI9 will not have any impact on most users. In rare cases the glibc error https://github.com/passport/passport/issues/17290[CPU does not support x86-64-v2] may appear. `+x86-64-v2+` has been available from processors since 2009. You're most likely to encounter this issue when your virtualization environment is misconfigured.

The change from `+-minimal+` to `+-micro+` has more potential impact. Users making simple customizations to the image won't notice any difference, however any user that installs RPMs will need to change how they do that. The https://www.passport-pro.ai/server/containers[Running Passport in a container] {section} has been updated to show you how.

As a result of these changes, there has been an 82% reduction in known CVEs affecting the Passport Container Image!
