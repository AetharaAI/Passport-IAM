...
For applications that use Passport as an OAuth 2.0 Authorization Server there is now support to revoke refresh tokens
...
a cleaner implementation within Passport, but also allows full customisation if needed. Security headers are now set
...
Passport server was upgraded to use WildFly 19 under the covers.


== Other improvements

* Support for invoking Application Initiated Actions added to Passport JavaScript adapter
