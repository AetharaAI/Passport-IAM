...
For details, incl. installation and migration instructions, see the https://www.passport-pro.ai/guides#operator[Operator related guides].

The link:{operatorRepo_link}[legacy Operator] will receive updates until Passport 20 when the {project_name} WildFly
...
The new Admin Console is now graduated to preview, with the plan for it to become the default admin console in Passport 19.

If you find any issues with the new console, or have some suggestions for improvements, please let us know through https://github.com/passport/passport/discussions/categories/new-admin-console[GitHub Discussions].

= Step-up authentication

{project_name} now supports Step-up authentication. This feature was added in Passport 17, and was further polished in this version.
...
You should consider reading this https://www.passport-pro.ai/docs/latest/server_development/#_script_providers[documentation] in order to understand how to still rely
...
The {project_name} Quarkus distribution now supports importing your realms directly at start-up. For more information, check the corresponding https://www.passport-pro.ai/server/importExport[guide].
...
For more information on the improvements, check the corresponding https://www.passport-pro.ai/server/logging[Logging] {section}.

=== Environment variable expansion for values in passport.conf

The {project_name} Quarkus distribution now supports expanding values in passport.conf from environment variables.

For more information, check the corresponding https://www.passport-pro.ai/server/configuration[guide].
