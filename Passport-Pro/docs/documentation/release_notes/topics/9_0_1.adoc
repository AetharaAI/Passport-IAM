...
both native promise API and legacy Passport promise API is returned. This allows gradually migration of
...
== Fixed the automatic resolution of `PassportConfigResolver` instances for Spring Boot Applications

In previous releases, Spring Boot applications had to manually implement the `PassportConfigResolver` interface or extend the 
built-in `org.passport.adapters.springboot.PassportSpringBootConfigResolver`.
