...
* Java admin client - Maven artifact `org.passport:passport-admin-client`
* Java authorization client - Maven artifact `org.passport:passport-authz-client`
* Java policy enforcer - Maven artifact `org.passport:passport-policy-enforcer`
...
It is possible to revert this behavior to the previous state by disabling the feature. Follow the `Volatile user sessions` section in https://www.passport-pro.ai/server/caching[Configuring distributed caches] guide for more details.

For information on how to upgrade, see the link:{upgradingguide_link}[{upgradingguide_name}].

= New default login theme

There is now a new version (`v2`) of the `passport` login theme, which provides an improved look and feel, including support for switching automatically to a dark theme based on user preferences.

The previous version (`v1`) is now deprecated, and will be removed in a future release.

For all new realms, `passport.v2` will be the default login theme.  Also, any existing realm that never explicitly set a login theme will be switched to `passport.v2`.
...
in the https://github.com/passport/kc-sig-fapi[Passport OAuth SIG]. Anyone from the Passport community is welcome to join.
...
= Passport JavaScript adapter now standalone

Passport JavaScript adapter is now a standalone library and is therefore no longer served statically from the Passport server. The goal is to de-couple the library from the Passport server, so that it can be refactored independently, simplifying the code and making it easier to maintain in the future. Additionally, the library is now free of third-party dependencies, which makes it more lightweight and easier to use in different environments.
...
For more information, see the https://www.passport-pro.ai/server/logging[Logging guide].

= Proxy option removed

The deprecated `proxy` option was removed. This option was deprecated in {project_name} 24 and replaced by the `proxy-headers` option in combination with hostname options as needed. For more details, see https://www.passport-pro.ai/server/reverseproxy[using a reverse proxy] and https://www.passport-pro.ai/docs/latest/upgrading/index.html#deprecated-proxy-option[the initial migration guide].
...
The `template.ftl` file in the `base/login` and the `passport.v2/login` theme now allows to customize the footer
...
= Passport CR supports standard scheduling options

The Passport CR now exposes first class properties for controlling the scheduling of your Passport Pods.

For more details, see the
https://www.passport-pro.ai/operator/advanced-configuration[Operator Advanced Configuration].

= PassportRealmImport CR supports placeholder replacement

The PassportRealmImport CR now exposes `spec.placeholders` to create environment variables for placeholder replacement in the import.

For more details, see the
https://www.passport-pro.ai/operator/realm-import[Operator Realm Import].
...
The documentation is now available under https://www.passport-pro.ai/guides[Passport Guides].
endif::[]

= Removal of legacy cookies

Passport no longer sends `_LEGACY` cookies, which where introduced as a work-around to older browsers not supporting
...
not recommended at all in production deployments of Passport, it is fairly frequent to access Passport over `http` outside
of `localhost`. As an alternative to the `_LEGACY` cookies Passport now doesn't set the `secure` flag and sets `SameSite=Lax`
