...
* Passport troubleshooting dashboard - showing metrics related to service level indicators and troubleshooting.
* Passport capacity planning dashboard - showing metrics related to estimating the load handled by Passport.
...
For more information, check the link:https://www.passport-pro.ai/server/caching#_securing_transport_stacks[Securing Transport Stacks] in the distributed caches guide.
...
Read the section https://www.passport-pro.ai/operator/rolling-updates[Managing Rolling Updates] in the {project_name} Operator Advanced Configuration guide for more details on this functionality.

The checks to determine if a rolling update is possible are also available on the {project_name} command line so you can use them in your deployment pipeline. Continue reading in the https://www.passport-pro.ai/server/update-compatibility[Update Compatibility Tool] guide for more information about the functionality available on the command line.

= Metrics on user activities

Event metrics provide admins an aggregated view of the different user activities in a Passport instance.
...
For more information, check the link:https://www.passport-pro.ai/observability/event-metrics[Monitoring user activities with event metrics] {section}.
...
For more details, see the https://www.passport-pro.ai/server/logging[Logging guide].
...
Check the `crl-storage` section in the link:https://www.passport-pro.ai/server/all-provider-config[All provider configuration] {section} to know the options for the new cache provider.
...
Read more about this in the https://www.passport-pro.ai/operator/advanced-configuration[Operator Advanced configuration].
...
For more information, check the link:https://www.passport-pro.ai/server/management-interface#_tls_support[Configuring the Management Interface] guide.
...
In this release, we are adding a new method `getCredentials(RealmModel, UserModel)` to the `org.passport.credential.CredentialInputUpdater` interface so that
...
The Passport outgoing link:{adminguide_email_link}[SMTP mail configuration] now supports token authentication (XOAUTH2).
...
You will find significant development and discussions in the https://github.com/passport/kc-sig-fapi[Passport OAuth SIG]. Anyone from the Passport community is welcome to join and provide the feedback.
