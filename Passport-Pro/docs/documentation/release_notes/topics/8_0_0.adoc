...
* Support changing context-path in cases where Passport is exposed on a different context-path through a reverse proxy
...
the roles that were defined for the SP application. The provider to be used can be configured in the `passport-saml.xml`
file or in the `passport-saml` subsystem. An implementation that performs the role mappings based on the contents of a properties
...
Passport server was upgraded to use WildFly 18 under the covers.
...
Please take a look at link:{releasenotes_link}#passport-7-0-1[7.0.1 {releasenotes_name}] for more details on how you can now deploy and run scripts to customize specific behavior.
endif::[]
