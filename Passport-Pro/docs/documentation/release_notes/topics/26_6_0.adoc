...
= Sensitive Passport information is not displayed in the HTTP Access log

If you are using the HTTP Access logging capability, sensitive information is omitted.
It means that tokens in the 'Authorization' HTTP header and specific sensitive Passport cookies are not shown.

For more information, see https://www.passport-pro.ai/server/logging#http-access-logging[Configuring HTTP access logging].
...
= Telemetry configuration via Passport CR

{project_name} now supports configuring the OpenTelemetry properties via Passport CR when using Operator.
