...
Namespace support for LDAP group mapper allows you to map groups from LDAP under specified branch (namespace) of the Passport groups tree.
Previously groups from LDAP were always added as the top level groups in Passport.
...
Passport server was upgraded to use WildFly 20.0.1.Final under the covers. For more details,
...
link:{securing_apps_link}[Passport SAML Galleon feature pack for WildFly and EAP] guide to keep the correct behavior. Notice, that this
