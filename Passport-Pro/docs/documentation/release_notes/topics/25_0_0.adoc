...
For OAuth 2.0/OIDC, this includes removal of the Tomcat adapter, WildFly/EAP adapter, Servlet Filter adapter, `PassportInstalled` desktop adapter, the `jaxrs-oauth-client` adapter, JAAS login modules, Spring adapter and SpringBoot adapters.
You can check https://www.passport-pro.ai/2023/03/adapter-deprecation-update.html[our older post] for the list of some alternatives.
...
check the https://github.com/passport/passport-quickstarts[quickstarts] for some examples where this authorization client library is used together with the 3rd party Java adapters like
...
* `PASSPORT_LOCALE` now sets `SameSite=None`
* `PASSPORT_REMEMBER_ME` now sets `SameSite=None`
...
* `LocaleSelectorProvider.PASSPORT_LOCALE` - replaced by `CookieType.LOCALE`
...
The Passport 23 release provided improvements for when a user is authenticated in parallel in multiple browser tabs. However, this improvement did not address the case when an authentication session
...
Passport supports a new password policy that allows you to deny user passwords which contains the user username.
...
For users of the `passport-authz-client` library, calling `AuthorizationResource.getPermissions(...)` now correctly returns a `List<Permission>`.
...
It allows to not expose it to the users as standard Passport endpoints in Kubernetes environments.
...
For more information, see the https://www.passport-pro.ai/server/logging[Configuring logging] guide.
...
In this release, we are delivering Passport Organizations as a technology preview feature.
