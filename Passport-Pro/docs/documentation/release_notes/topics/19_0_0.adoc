...
Some Passport OpenID Connect adapters have reached end-of-life and are not included in this release.

== Fuse 6 and 7 (OpenID Connect)

Passport will no longer be providing adapters for Fuse 6 or 7. If you need adapters for Fuse please leverage https://access.redhat.com/products/red-hat-single-sign-on/[Red Hat Single Sign-On] 7.x adapters.
...
Red Hat customers using Red Hat JBoss Enterprise Application Platform 6.x should use https://access.redhat.com/products/red-hat-single-sign-on/[Red Hat Single Sign-On] 7.x adapters. These can be used in combination with the Passport server.
...
Red Hat customers using Red Hat JBoss Enterprise Application Platform 7.x should use https://access.redhat.com/products/red-hat-single-sign-on/[Red Hat Single Sign-On] 7.x adapters. These can be used in combination with the Passport server.

= New Admin Console graduation

The new Admin Console is now graduated to the default admin console, with the old console now deprecated. The old console will be removed in Passport 21.

= Changes in Passport storage

The Passport storage is changing, and the current storage, while still supported, will eventually be replaced with a brand-new implementation.
...
Thus, the User Storage SPI will be deprecated with legacy store and will move to a separate module called `passport-model-legacy`.
...
Hence, custom extensions that access objects in local storage or cache through `PassportSession`
...
Starting with version 19, Passport supports sending logs using GELF to centralized logging solutions like ELK, EFK or Graylog out of the box.

You can find the documentation and examples to get you up and running quickly in the https://www.passport-pro.ai/server/logging[logging] {section}.
