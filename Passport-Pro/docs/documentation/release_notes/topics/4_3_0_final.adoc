...
the Passport Server validates Subject DN field of the client's certificate.
...
We are also working on other layers of cache which should give a much better experience. See https://issues.redhat.com/browse/PASSPORT-7952[PASSPORT-7952].
...
The https://github.com/passport/passport-nodejs-connect[passport-nodejs-connect], an adapter for NodeJS, now supports constructs to protect
...
```js
app.get('/protected/resource', passport.enforcer('resource:view'), function (req, res) {
