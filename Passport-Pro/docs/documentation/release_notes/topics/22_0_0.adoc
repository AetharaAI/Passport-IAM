...
Running the Passport server with Java 11 is no longer supported. Java 11 was deprecated in Passport 21 with the announced plan to be removed in Passport 22.

== Upgrade to Quarkus 3.x

Passport upgraded to version 3.2.0.Final of the Quarkus Java framework.
...
As part of upgrading to Quarkus 3.x Passport migrated its codebase from Java EE (Enterprise Edition) to its successor Jakarta EE, which brings various changes into Passport.
...
The previous and now removed WildFly distribution provided a built-in vault provider that reads secrets from a keystore-backed Elytron credential store. As this is no longer available, we have added a new implementation of the Passport Vault SPI called Passport KeyStore Vault. As the name suggests, this implementation reads secrets from a Java keystore file. Such secrets can be then used within multiple places of the Administration Console. For further details, see https://www.passport-pro.ai/server/vault[our guide] and the latest https://www.passport-pro.ai/docs/latest/server_admin/index.html#_vault-administration[documentation].

== KeyStore Config Source added

In relation to the KeyStore Vault news, we also integrated Quarkus's recently released feature called KeyStore Config Source. This means that among the already existing configuration sources (CLI parameters, environment variables and files), you can now configure your Passport server via configuration properties stored in a Java keystore file. You can learn more about this feature in the https://www.passport-pro.ai/server/configuration[Configuration guide].
...
Installations which use Passport's `--proxy` configuration setting with mode *passthrough* should review the documentation as the behavior of this mode has changed.
...
In version 21.1.0 of Passport the new Account Console (version 3) was introduced as an experimental feature. Starting this version it has been promoted to a preview feature.
...
With this release, we have removed the legacy Promise API methods from the Passport JS adapter. This means that calling `.success()` and `.error()` on promises returned from the adapter is no longer possible.

== Required to be instantiated with the `new` operator

In a previous release we started to actively log deprecation warnings when the Passport JS adapter is constructed without the `new` operator. Starting this release doing so will throw an exception instead. This is to align with the expected behavior of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes[JavaScript classes], which will allow further refactoring of the adapter in the future.
...
The are additional fields available in the passport.status to facilitate passport being a scalable resource. There are also additional fields that make the status easier to interpret such as observedGeneration and condition observedGeneration and lastTransitionTime fields.
...
In scenarios where advanced management is needed you may now directly update most fields on operator managed resources that have not been set by the operator directly. This can be used as an alternative to the unsupported stanza of the Passport spec. Like the unsupported stanza these direct modifications are not considered supported. If your modifications prevent the operator from being able to manage the resource, there Passport CR will show this error condition and the operator will log it.
...
select truststore for TLS-secured LDAP connection: either internal Passport truststore is
...
The `openshift-integration` preview feature that allowed replacing the internal IdP in OpenShift 3.x with Passport was removed from Passport codebase into separate extension project.
