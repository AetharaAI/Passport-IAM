...
In the past months, the Passport team spent a huge amount of effort in polishing the user
...
ifeval::[{project_community}==true]
We would like to give huge thanks to the awesome Passport community as lots of ideas, requirements and contributions came from the community! Special thanks to:
...
Back in 2022 we announced the https://www.passport-pro.ai/2022/02/adapter-deprecation.html[deprecation of Passport adapters in Passport 19].
To give the community more time to adopt this https://www.passport-pro.ai/2023/03/adapter-deprecation-update.html[was delayed].

With that in mind, this will be the last major release of Passport to include OpenID Connect and SAML adapters.
...
for continuing to support this is down to the fact that the majority of the SAML codebase in Passport was a contribution
...
For details refer to the https://www.passport-pro.ai/server/features[features guide].

== {project_name} CR Truststores

You may also take advantage of the new server-side handling of truststores by using the Passport CR, for example:
...
See the https://www.passport-pro.ai/docs/latest/server_admin/index.html#saml-v2-0-identity-providers[documentation] for more details about the new options.
...
For more details, see https://www.passport-pro.ai/server/features[Enabling and disabling features].

= Passport CR Optimized Field

The Passport CR now includes an `startOptimized` field, which may be used to override the default assumption about whether to use the `--optimized` flag for the start command.
...
For details, see the https://www.passport-pro.ai/server/reverseproxy[Reverse Proxy Guide].
...
The Passport CR now allows for specifying the `cache-config-file` option by using the `cache` spec `configMapFile` field, for example:
...
The Passport CR now allows for specifying the `resources` options for managing compute resources for the Passport container.
It provides the ability to request and limit resources independently for the main {project_name} deployment via the Passport CR, and for the realm import Job via the Realm Import CR.
...
with GELF, Passport will no longer support the GELF log handler out-of-the-box. This feature will be removed in a future
