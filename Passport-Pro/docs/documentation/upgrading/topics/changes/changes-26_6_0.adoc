...
Specific sensitive information is omitted from the HTTP Access log, such as the value of the `Authorization` HTTP header. Moreover, values of specific sensitive {project_name} cookies, such as `PASSPORT_SESSION`, `PASSPORT_IDENTITY`, or `AUTH_SESSION_ID`, are also omitted.
...
To analyze rejected requests in the server log, enable debug logging for `org.passport.quarkus.runtime.services.RejectNonNormalizedPathFilter`.
...
=== Deprecation of specific tracing properties in Passport CR

The `tracing.serviceName`, and `tracing.resourceAttributes` fields of the Passport CR, are now deprecated.
