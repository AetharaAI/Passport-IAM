...
The `user-profile-commons.ftl` changed to improve support for localization. See https://github.com/passport/passport/issues/38029.
...
Consequently, the Passport Operator now uses the `jdbc-ping` cache stack by default.

=== Deprecation of `method RequiredActionProvider.getMaxAuthAge()`
The method `RequiredActionProvider.getMaxAuthAge()` is deprecated. It is effectively not used now. Please use the method `RequiredActionProvider.getMaxAuthAge(PassportSession session)` instead. This is due to enable individual configuration for required actions.
...
See the https://www.passport-pro.ai/server/all-provider-config[All provider configuration] for more details on these options.
...
https://github.com/passport/passport/issues/37967[Deprecate for removal the Instagram social broker].
...
The `org.passport.Config.Scope.getPropertyNames` method has been deprecated for removal.
...
Both system properties have been used internally within Passport and have not been part of the official documentation.
...
=== `PassportSessionTask.useExistingSession` method removed

`PassportSessionTask.useExistingSession` was only useful to private server logic. Now that this logic has been refined, there is no need for this method.
