...
to this configuration in `conf/passport.conf`:
...
For more information about the datasource options, check the link:https://www.passport-pro.ai/server/db#multiple-datasources[Configure multiple datasources] guide.
...
[source]
----
https://passport.example.com/realms/{realm}/.well-known/oauth-authorization-server
----
...
[source]
----
https://passport.example.com/.well-known/oauth-authorization-server/realms/{realm}
----
...
The `SimpleHttp` util in the `server-spi-private` module was refactored and moved to the `org.passport.http.simple` package.
