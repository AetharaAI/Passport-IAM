...
- For the Node.js adapter, the same guideline applies as for the JavaScript adapter. You are encouraged to update to the latest version as the older version of the adapter uses the deprecated `redirect_uri` parameter.
  ...
  when your application directly uses the method `passport.logoutUrl`, you can consider adding `idTokenHint` as the second argument to this method. The possibility to add `idTokenHint` as second argument was newly
...
WARNING: The backwards compatibility switch will be removed in some future version - probably Passport 23. You are encouraged to update your clients as soon as possible
...
More details about how to deploy scripts to the server are available in the https://www.passport-pro.ai/docs/latest/server_development/#_script_providers[documentation]. Note that to use scripts, you are still
...
* After the upgrade, deploy your scripts following the instructions in the https://www.passport-pro.ai/docs/latest/server_development/#_script_providers[documentation].
