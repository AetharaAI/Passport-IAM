...
by Passport or `AbstractIdentityProvider` you need to update your implementation to implement
...
=== Not recommended to use org.passport.credential.UserCredentialManager directly in your extensions

If you have user storage extension and you reference the class `org.passport.credential.UserCredentialManager` from your providers, it is recommended to avoid using this class directly as it might be
...
public SubjectCredentialManager credentialManager() {
    return new UserCredentialManager(passportSession, realmModel, this);
}
----
...
----
@Override
public org.passport.models.UserCredentialManager credentialManager() {
    return passportSession.users().getUserCredentialManager(this);
}
----
...
The inner class `AuthenticationManager.AuthResult` in the `passport-services` module is now a record.
