...
Updated the RESTEasy version of Passport Admin REST Client to the next major version.
...
This change might require changes to the H2 JDBC URL or migration of the H2 database files in an existing Passport setup.

== Changes to the H2 JDBC URL

For Passport to run with its JPA legacy store with H2 version 2.x, the JDBC URL requires the attribute `NON_KEYWORDS=VALUE`.

In a setup where H2 is initialized by Passport without extra parameters, Passport will append the attribute automatically.
...
If the connection factory for the H2 database is initialized outside Passport, that initialization needs to take care of adding the `NON_KEYWORDS` attribute.
...
Purge existing H2 database files to start with an empty database, export and import the realms using Passport's export and import functionality, or refer to the http://www.h2database.com/html/migration-to-v2.html[migration notes on the H2 database project's website] for details on how to migrate H2 database contents.

= Breaking changes in the new version of Passport Operator

NOTE: In order to use the newest version of the Passport Operator, the manual reinstallation and upgrade of your CRs are required. There’s no automated migration.

This release contains the following breaking changes in Passport CRs:

== serverConfiguration free-form field was renamed

From now on it’s called `additionalOptions`. The idea behind this decision is to align it more with the Passport Quarkus distribution and achieve/preserve a naming consistency.
`serverConfiguration` can be still used for configuring options that haven’t got a declared alternative in Passport custom resources (CRs). A good example of such use can be service providers.
...
spec:
    ...
  hostname:
    hostname: [passport-server-hostname]
...
The default channel of Passport Operator Lifecycle Manager was changed to `fast`.


= Deprecated methods from data providers and models were removed

Prior to Passport 15, there was a clean-up of provider and model interfaces
where we deprecated some methods. Javadoc of these methods contained
a corresponding replacement method (see Javadoc of Passport 19 release).
...
+
For example in https://github.com/passport/passport/blob/19.0.3/server-spi/src/main/java/org/passport/models/GroupProvider.java[`GroupProvider`] interface
+
[source,java]
----
@Deprecated
...
https://issues.redhat.com/browse/PASSPORT-14011[PASSPORT-14011].

* Consistent parameter ordering - methods now have strict parameter
ordering where `RealmModel` is always the first parameter.
+
For example in https://github.com/passport/passport/blob/19.0.3/server-spi/src/main/java/org/passport/storage/user/UserLookupProvider.java[`UserLookupProvider`] interface
+
[source,java]
----
@Deprecated
...
(`o.k.` stands for `org.passport.` package)
...
** `o.k.models.PassportSessionFactory`
...
All changes are linked to the following https://github.com/passport/passport/issues/14720[issue].
