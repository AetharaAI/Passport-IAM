...
The Client policies feature was a preview feature since Passport 12 and did not have proper support. If you tried this feature and configured some client policies or client profiles
in Passport 12 or Passport 13, then you will need to configure your client policies and client profiles again in the new version. The format of the configuration changed significantly
as it was only a preview and hence we do not provide automatic migration of client policies and client profiles created in Passport 12 or Passport 13.
...
An example with the necessary changes can be found in the `passport/examples/themes/theme/sunrise` directory.
...
option. Such users have to log in to Passport using Instagram IdP before September 30th, 2020. After that day they'll have
...
a new account in Passport. This is because Instagram user IDs are not compatible between the old and new API, however the
new API temporarily returns both new and old user IDs to allow migration. Passport automatically migrates the ID once user
logs in.

==== Non-standard token introspection endpoint removed

In previous versions, Passport advertised two introspection endpoints: `token_introspection_endpoint` and `introspection_endpoint`.
...
----
SELECT REALM_ID, NAME, COUNT(*) FROM PASSPORT_GROUP WHERE PARENT_GROUP is NULL GROUP BY REALM_ID, NAME HAVING COUNT(*) > 1;
----
...
PASSPORT-12579-add-not-null-constraint::passport failed.`
...
In the past if native promise was available a wrapper was returned that provided both the legacy Passport promise and
...
Changes to {project_name} Authorization JavaScript adapter::
  The {project_name} Authorization JavaScript adapter (passport-authz.js) changed in order to comply with the changes introduced by UMA 2.0 while keeping
...
Changes to {project_name} Authorization Client Java API::
  ...
  were moved to a different package in `org.passport:passport-core`.
...
The passport-server-spi module has been split into passport-server-spi and passport-server-spi-private. APIs within
passport-server-spi will not change between minor releases, while we reserve the right and may quite likely change
APIs in passport-server-spi-private between minor releases.
...
`passport.saml.key_trans.rsa_v1.5` set to `true`.
...
In 2.3.0 release we added support for Public Key Rotation. When admin rotates the realm keys in Passport admin console, the Client
Adapter will be able to recognize it and automatically download new public key from Passport. However this automatic download of new
...
and never download the public key from Passport. In theory, one reason for this can be to avoid man-in-the-middle attack if you have untrusted network between adapter and Passport,
however in that case, it is much better option to use HTTPS, which will secure all the requests between adapter and Passport.
...
The script to add admin users to Passport has been renamed to `add-user-passport`.
...
it was possible to access the Passport server from 2 different contexts (internal and external), which was causing issues in token validations etc.
...
through load balancer but send them to local Passport server directly) you may need to handle it at hosts configuration (DNS) level.
...
You also need to update `passport-server.json` as it's changed due to this.

==== Adapter subsystems only bring in dependencies if Passport is on

Previously, if you had installed our SAML or OIDC Passport subsystem adapters into WildFly or JBoss EAP, we would automatically include Passport client jars into EVERY application regardless if you were using Passport or not.
These libraries are now only added to your deployment if you have Passport authentication turned on for that adapter (via the subsystem, or auth-method in web.xml)
...
Most of our modules and source code have been consolidated into two maven modules:  passport-server-spi and passport-services.
SPI interfaces are in server-spi, implementations are in passport-services.
All JPA dependent modules have been consolidated under passport-model-jpa.
Same goes with mongo and Infinispan under modules passport-model-mongo and passport-model-infinispan.

==== For adapters, session id changed after login

To plug a security attack vector, for platforms that support it (Tomcat 8, Undertow/WildFly), the Passport OIDC and SAML adapters will change the session id after login.
You can turn off this behavior check adapter config switches.

==== SAML SP Client adapter changes

Passport SAML SP Client adapter now requires a specific endpoint, `/saml` to be registered with your IDP.
...
So if you're using Admin REST API or Passport admin-client, you should update your configuration to use `admin-cli` instead of `security-admin-console` as the latter one doesn't have direct access grants enabled anymore by default.

==== Option 'Update Profile On First Login' moved from Identity provider to Review Profile authenticator

In this version, we added `First Broker Login`, which allows you to specify what exactly should be done when new user is logged through Identity provider (or Social provider), but no Passport user linked to the social account exists yet.
...
Old versions of Passport allowed reusing refresh tokens multiple times.
Passport still permits this, but also have an option `Revoke refresh token` to disallow it.
...
The most important classes used in your application ( for example AccessToken or PassportSecurityContext ) as well as the SPI are still unchanged.
However, a slight chance exists that you will be affected and will need to update imports of your classes.
For example if you are using multitenancy and PassportConfigResolver, you will be affected as for example class HttpFacade was moved to different package - it is `org.passport.adapters.spi.HttpFacade` now.
...
However in all cases, you will need to update `standalone/configuration/passport-server.json` and add `userSessionPersister` like this:
...
If you have realmCache or userCache set in passport-server.json to mem or none please remove these.
...
If you have userSession set in passport-server.json to mem, jpa or mongo please remove it.
...
Following on from the distribution changes that was done in the last release the standalone download of Passport is now based on WildFly 9.0.0.Final.
...
Passport is now available in 3 downloads: standalone, overlay and demo bundle.
The standalone is intended for production and non-JEE developers.
Overlay is aimed at adding Passport to an existing WildFly 8.2 or EAP 6.4 installation and is mainly for development.
Finally we have a demo (or dev) bundle that is aimed at developers getting started with Passport.
This bundle contains a WildFly server, with Passport server and adapter included.
...
This has been simplified to only requiring a plain text file (`META-INF/passport-themes.json`) to describe the themes included in a JAR.
...
You can first go to the Passport admin console and copy Redirect URI from the page where you configure the identity provider.
...
* adapters are now a separate download.  They are not included in appliance and war distribution.  We have too many now and the distro
  is getting bloated.
* `org.passport.adapters.tomcat7.PassportAuthenticatorValve`
+`org.passport.adapters.tomcat.PassportAuthenticatorValve`
...
* The as7-eap-subsystem and passport-wildfly-subsystem have been merged into one passport-subsystem.  If you have an existing standalone.xml
  or domain.xml, you will need edit near the top of the file and change the extension module name to org.passport.passport-subsystem.
  For AS7 only, the extension module name is org.passport.passport-as7-subsystem.
...
* Upgrade your adapters, old adapters are not compatible with Passport 1.1.  We interpreted JSON Web Token and OIDC ID Token specification incorrectly.  'aud'
...
* For all clients except bearer-only applications, you must specify at least one redirect URI.  Passport
  will not allow you to log in unless you have specified a valid redirect URI for that application.
* Direct Grant API
+`ON`

* standalone/configuration/passport-server.json
* JavaScript adapter
* Session Timeout
...
* SkeletonKeyToken, SkeletonKeyScope, SkeletonKeyPrincipal, and SkeletonKeySession have been renamed to:
  AccessToken, AccessScope, PassportPrincipal, and PassportAuthenticatedSession respectively.
...
* Because of the "secret" credential change to Application and OAuth Client, you'll have to update
  your passport.json configuration files and regenerate a secret within the Application or OAuth Client
