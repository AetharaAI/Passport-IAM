...
To prevent losing user sessions upgrade to Passport 25 first and enable the persistent sessions feature as outlined in the migration guide for {project_name} 25.
...
The following method was added to `org.passport.cluster.ClusterProvider`:
...
Passport Pods will now have default affinities to prevent multiple instances from the same CR from being deployed on the same node, and all Pods from the same CR will prefer to be in the same zone to prevent stretch cache clusters.
...
= Deprecations in `passport-common` module

The following items have been deprecated for removal in upcoming {project_name} versions with no replacement:

- `org.passport.common.util.reflections.Reflections.newInstance(java.lang.Class<T>)`
- `org.passport.common.util.reflections.Reflections.newInstance(java.lang.Class<?>, java.lang.String)`
- `org.passport.common.util.reflections.SetAccessiblePrivilegedAction`
- `org.passport.common.util.reflections.UnSetAccessiblePrivilegedAction`

= Consistent usage of UTF-8 charset for URL encoding

`org.passport.common.util.Encode` now always uses the `UTF-8` charset for URL encoding instead relying implicitly on the `file.encoding` system property.
...
This release introduced the capability to easily add a custom footer to the login pages for the `base/login` and `passport.v2/login` theme.
In order to use a custom footer, create a `footer.ftl` file in your custom login theme with the desired content.

For more details, see link:https://www.passport-pro.ai/ui-customization/themes#_theme_custom_footer[Adding a custom footer to a login theme].
...
To disable this behavior, use the SPI option `spi-single-use-object-infinispan-persist-revoked-tokens` as outlined in the https://www.passport-pro.ai/server/all-provider-config[All provider configuration] {section}.
...
or Passport CR as outlined in the blueprints. All `remote-store` configurations must be removed from the cache configuration file.
...
The Passport High Availability Guide contains a blueprint on how to set this up.
...
To protect users from using it accidentally via a manual configuration in Passport's cache XML or via the CLI options, this is now guarded with a feature flag `cache-embedded-remote-store`. It is marked as experimental and is therefore not supported. {project_name} 26 will not start with such a configuration and show an error instead unless this experimental feature is enabled.
...
Consequently, the environment variables `PASSPORT_ADMIN` and `PASSPORT_ADMIN_PASSWORD` have been deprecated. You should use `KC_BOOTSTRAP_ADMIN_USERNAME` and `KC_BOOTSTRAP_ADMIN_PASSWORD` instead. These are also general options, so they may be specified via the cli or other config sources, for example `--bootstrap-admin-username=admin`. For more information, see the new https://www.passport-pro.ai/server/bootstrap-admin-recovery[Bootstrap admin and recovery] guide.
...
Note: This feature required changes to the Passport JS adapter, therefore it is recommended to upgrade to the latest version of the adapter if you want to make use of this feature.

= Deprecations in `passport-services` module
...
If you wish to disable placeholder replacement for the `import` command, add the system property `-Dpassport.migration.replace-placeholders=false`
...
NOTE: The `+spi-truststore-file-*+` options and the truststore related options `+https-trust-store-*+` are deprecated, we strongly recommend to use System Truststore. For more details refer to the relevant https://www.passport-pro.ai/server/passport-truststore[guide].
...
see the https://www.passport-pro.ai/server/logging[Logging guide].

= Paths for `common` theme resources have changed

Some of the paths for the `common` resources of the `passport` theme have changed, specifically the resources for third-party libraries. Make sure to update your custom themes accordingly:
...
For more details proceed to the https://www.passport-pro.ai/server/caching[Configuring distributed caches] guide.
...
The `org.passport.bom:passport-adapter-bom` and `org.passport.bom:passport-misc-bom` BOM files are removed. The adapter BOM was no longer useful because most of the Java adapters
are removed. The misc BOM had contained only one artifact, `passport-test-helper`, and that artifact is also removed in this release.

= passport-test-helper is removed

The maven artifact `org.passport:passport-test-helper` is removed in this release. The artifact provided a few helper methods for dealing with a Java admin client. If you use the
...
This release includes several changes to Passport JS library that should be taken into account. The main motivation for these changes is to de-couple the library from the Passport server, so that it can be refactored independently, simplifying the code and making it easier to maintain in the future. The changes are as follows:

== The library is no longer served statically from the server

The Passport JS library is no longer served statically from the Passport server. This means that the following URLs are no longer available:

- `/js/passport-authz.js`
- `/js/passport-authz.min.js`
- `/js/passport.js`
- `/js/passport.min.js`
- `/js/{version}/passport-authz.js`
- `/js/{version}/passport-authz.min.js`
- `/js/{version}/passport.js`
- `/js/{version}/passport.min.js`

Additionally, the `passportJsUrl` property that linked to the library on these URLs has been removed from the admin console theme. If your custom theme was using this property to include the library, you should update your theme to include the library using a different method.

If you are not already you should include the library in your project using a package manager like link:https://nodejs.org/en/learn/getting-started/an-introduction-to-the-npm-package-manager[NPM]. The library is available on the NPM registry as link:https://www.npmjs.com/package/passport-js[`passport-js`]. You can install it using the following command:
...
----
<!-- Before -->
<script src="/path/to/passport.js"></script>
<script>
...
<!-- After -->
<script type="module">
    import Passport from '/path/to/passport.js';
    const passport = new Passport();
</script>
----
...
<script type="importmap">
    {
        "imports": {
            "passport-js": "/path/to/passport.js"
        }
    }
</script>
<script type="module">
...
== The configuration for the `Passport` instance is now required

Previously it was possible to construct a `Passport` instance without passing any configuration. The configuration would then automatically be loaded from the server from a `passport.json` file based on the path of the included `passport.js` script. Since the library is no longer statically served from the server this feature has been removed. You now need to pass the configuration explicitly when constructing a `Passport` instance:
...
// After
const passport = new Passport({
    url: "http://passport-server",
    realm: "my-realm",
    clientId: "my-app"
});

// Alternatively, you can pass a URL to a `passport.json` file.
// Note this is not recommended as it creates additional network requests, and is prone to change in the future.
const passport = new Passport('http://passport-server/path/to/passport.json');
----

== Methods for login are now `async`

Passport JS now utilizes the Web Crypto API for various cryptographic functions. Due to the asynchronous nature of this API the following public methods will now always return a `Promise`:
...
// Before
passport.login();
const loginUrl = passport.createLoginUrl();
const registerUrl = passport.createRegisterUrl();

// After
await passport.login();
const loginUrl = await passport.createLoginUrl();
const registerUrl = await passport.createRegisterUrl();
----
...
Passport JS now requires a link:https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Secure_Contexts[secure context] to run. The reason for this is that the library now uses the Web Crypto API for various cryptographic functions. This API is only available in secure contexts, which are contexts that are served over HTTPS, `localhost` or a `.localhost` domain. If you are using the library in a non-secure context you'll need to update your development environment to use a secure context.
