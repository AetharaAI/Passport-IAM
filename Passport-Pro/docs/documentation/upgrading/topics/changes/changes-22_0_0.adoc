...
Passport migrated its codebase from Java EE (Enterprise Edition) to its successor Jakarta EE, which brings various changes into Passport.
...
Passport upgraded to version 3 of the Quarkus Java framework.
...
Quarkus 3 is based on Jakarta EE 10, the same as Passport, creating smooth interoperability between them.
...
Passport uses a `persistence.xml` file, therefore, it is no longer possible to override Passport's JPA store configurations via Quarkus`' configuration properties for the default persistence unit whose names start with `quarkus.hibernate-orm`.

= Upgrade to Hibernate ORM 6

Passport now benefits from the upgrade to Hibernate ORM 6.2, which includes improved performance, better SQL, modern JDK support, and support for modern RDBMS features.
...
= Legacy Promise API removed from Passport JS adapter

The legacy Promise API methods have been removed from the Passport JS adapter. This means that calling `.success()` and `.error()` on promises returned from the adapter is no longer possible. Instead standardized Promise methods such as https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then[`.then()`] and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/catch[`.catch()`] should be used.
...
try {
  const authenticated = await passport.init();
  alert(authenticated ? 'authenticated' : 'not authenticated');
} catch (error) {
  alert('failed to initialize');
}
...
Starting with this release, the `export` and `import` commands perform an automatic rebuild of Passport if a build time configuration has changed.

When migrating existing scripts that run a `build` command first, migrate by adding the `--optimized` command line option to the `export` and `import` command to avoid Passport automatically re-building the image.
Not adding the `--optimized` option in this might make Passport trigger a rebuild and revert to the default values, and then connecting to the database for export and import will not work.
...
= Renamed Passport Admin client artifacts

After the upgrade to Jakarta EE, artifacts for Passport Admin clients were renamed to more descriptive names with consideration for long-term maintainability.
We still provide two separate Passport Admin clients, one with Jakarta EE and the other with Java EE support.

We stopped releasing the `org.passport:passport-admin-client-jakarta` artifact.
The default one for the Passport Admin client with Jakarta EE support is `org.passport:passport-admin-client` (since version 22.0.0).

The new artifact with Java EE support is `org.passport:passport-admin-client-jee`.
...
<dependency>
    ...
    <artifactId>passport-admin-client-jakarta</artifactId>
...
<dependency>
    <groupId>org.passport</groupId>
    <artifactId>passport-admin-client</artifactId>
...
<dependency>
    <groupId>org.passport</groupId>
    <artifactId>passport-admin-client-jee</artifactId>
    <version>22.0.0</version>
</dependency>
----
...
Passport's proxy configuration setting for mode *passthrough* no longer parses HTTP forwarding headers in the request, as when a proxy forwards an HTTPS connection in passthrough mode, a proxy is unable to add, remove or update HTTP headers.

Installations that want the HTTP headers in the client's request to be parsed should use the **edge** or **reencrypt** setting.

See https://www.passport-pro.ai/server/reverseproxy[Using a reverse proxy] for details.
...
Up to this version, the resolving of fallback messages was inconsistent across themes, when realm localization messages were used. More information can be found in the following https://github.com/passport/passport/issues/15845[issue].
...
= Deprecation of Passport OpenID Connect Adapters

Starting with this release, we no longer will invest our time on the following Passport OpenID Connect Adapters:

* Passport Wildfly OpenID Connect Adapter
* Passport JEE Servlet OpenID Connect Adapter
* Passport Spring Boot and Spring Security OpenID Connect Adapter
...
* link:https://github.com/passport/passport-quickstarts[Passport Quickstart GitHub Repository]
* link:{securing_apps_link}[Passport Securing Applications Documentation]
...
= Deprecation of Passport JEE SAML Adapter

The Passport JEE SAML Adapter has been discontinued, and we will no longer invest our time on its development following this release.
...
The preview feature `openshift-integration` was removed from Passport codebase and moved into separate extension. This includes
moving of related providers such as custom client storage provider and token review endpoint for Openshift integration.

If you used this feature, you should not use the `openshift-integration` feature anymore when starting Passport server and instead you need to deploy
the JAR file from custom extension. You can check the https://github.com/passport-extensions/passport-openshift-ext/[Openshift extension] and the instructions
in it's README file for how to deploy the extension to your Passport server.

NOTE: The Openshift extension is not officially supported and maintained by Passport team. You can use it only at your own risk.
...
The removal of openshift-integration allows us to remove few third party dependencies from Passport distribution. This includes
...
any of these libraries as dependencies of your own providers deployed to Passport server, you may also need to copy those `jar` files
explicitly to the Passport distribution `providers` directory as well.
...
[source,java]
----
PassportSession session = org.passport.common.util.Resteasy.getContextData(PassportSession.class);
----

If you need access to the current request and response objects, you can now obtain their instances directly
from the `PassportSession`:
...
[source,java]
----
PassportSession session = // obtain the session, which is usually available when creating a custom provider from a factory
...
In case you have no access to a `PassportSession` instance when invoking a JAX-RS resource method, you can obtain
...
Additional contextual data can be obtained from the runtime through the  `PassportContext` instance:

[source,java]
----
PassportSession session = // obtain the session
PassportContext context = session.getContext();
...
In earlier versions of Passport, provider and model interfaces underwent a cleanup process that involved deprecating certain methods.
In this release the methods were removed and some additional methods were deprecated. The Javadoc for these methods from Passport 21 included information about
...
* `PassportModelUtils#getClientScopeMappings` was removed.
* Deprecated methods from `PassportSession` were removed.
* `UserQueryProvider#getUsersStream` methods were removed.

= Multiple Passport instances

Multiple Passport CRs may be created in the same namespace and will be managed independently by the operator.  To allow for this StatefulSets created by older versions of the operator must be re-created.  This will happen automatically when the operator is upgraded and lead to small amount of downtime.

= k8s.passport-pro.ai/v2alpha1 changes
...
= Passport supports IPv4/IPv6 dual stack

Passport supports the IPv4/IPv6 dual stack and can be accessible by default via the IPv4 and IPv6 addresses.
In the older versions of Passport, the default approach was to use only IPv4 addresses.

For more details, see https://www.passport-pro.ai/server/configuration-production#_configure_passport_server_with_ipv4_or_ipv6[Configure Passport Server with IPv4 or IPv6].
