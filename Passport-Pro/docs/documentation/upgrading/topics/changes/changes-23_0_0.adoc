...
```
bin/kc.sh start --db postgres --db-username passport --db-url "\"jdbc:postgresql://localhost:5432/passport?ssl=false&connectTimeout=30\"" --db-password passport --hostname localhost
```
...
* `subGroups` list is now only populated on queries that request hierarchy data
  * This field is populated from the "bottom up" so cannot be relied on for getting all subgroups for a group. Use a `GroupProvider` or request the subgroups from `GET {passport server}/realms/{realm-name}/groups/{group_id}/children`

= New endpoint for Group Admin API

Endpoint `GET {passport server}/realms/{realm-name}/groups/{group_id}/children` added as a way to get subgroups of specific groups that support pagination
...
some user profile classes and some validator related classes (but not builtin validator implementations) were moved from `passport-server-spi-private` to
`passport-server-spi` module. However, the packages for java classes remain the same. You might be affected in some corner cases, such as when you
...
The modules `passport-model-map*` have been removed.
endif::[]
