...
----
# Hostname v1
bin/kc.[sh|bat] start --hostname=mypassport-pro.ai --https-port=8543 --hostname-path=/auth --hostname-strict-https=true

# Hostname v2
bin/kc.[sh|bat] start --hostname=https://mypassport-pro.ai:8543/auth
----
...
----
# Hostname v1
bin/kc.[sh|bat] start --hostname=mypassport-pro.ai --hostname-strict-backchannel=true

# Hostname v2
bin/kc.[sh|bat] start --hostname=mypassport-pro.ai --hostname-backchannel-dynamic=false
----
...
For more details and more comprehensive scenarios, see https://www.passport-pro.ai/server/hostname[Configuring the hostname (v2)].
...
For more details see the https://www.passport-pro.ai/server/features[Enabling and disabling features] {section}.
The https://www.passport-pro.ai/high-availability/single-cluster/concepts-memory-and-cpu-sizing[sizing guide] contains a new paragraph describing the updated resource requirements when this feature is enabled.
...
With persistent sessions enabled, the in-memory caches for online user sessions, offline user sessions, online client sessions and offline client sessions are limited to 10000 entries per node by default which will reduce the overall memory usage of Passport for larger installations.
...
To configure the cache size in an external {jdgserver_name} in a {project_name} multi-site setup, consult the updated https://www.passport-pro.ai/high-availability/multi-cluster/deploy-infinispan-kubernetes-crossdc[Deploy Infinispan for HA with the Infinispan Operator] {section}.
...
--
+
TIP: If the remote {jdgserver_name} is used in a multi-site setup, you can reduce the resource consumption by the external Infinispan by configuring the number of entries in memory. Use the settings outlined in https://www.passport-pro.ai/high-availability/multi-cluster/deploy-infinispan-kubernetes-crossdc[Deploy Infinispan for HA with the Infinispan Operator] {section}.
...
In some scenarios like brokering Passport uses HTTP to talk to external servers.
...
The module `org.passport:passport-model-legacy` module was deprecated in a previous release and is removed in this release. Use the `org.passport:passport-model-storage` module instead.
...
* XA Transaction recovery support is enabled by default. Transaction logs will be stored at PASSPORT_HOME/data/transaction-logs.
...
When searching for users by user attribute, Passport no longer searches for user attribute names forcing lower case comparisons. This means Passport's native index on the user attribute table will now be used when searching. If you have created your own index based on `lower(name)`to speed up searches, you can now remove it.
...
A new `Nonce backwards compatible` mapper is also included in the software that can be assigned to client scopes to revert to the old behavior. For example, the JS adapter checked the returned `nonce` claim in all the tokens before fixing issue https://github.com/passport/passport/issues/26651[#26651] in version 24.0.0. Therefore, if an old version of the JS adapter is used, the mapper should be added to the required clients by using client scopes.
...
* Adding the `Session State (session_state)` mapper in case of using the Passport Javascript adapter 24.0.3 or older
* Adding the `Nonce backwards compatible` mapper in case of using a Passport Javascript adapter that is older than Passport 24

You can add the protocol mappers directly to the corresponding client or to some client scope, which can be used by your client applications relying on older versions of the Passport Javascript adapter.
...
That means these endpoints are no longer exposed to the standard Passport ports `8080` and `8443`.
...
For more details, see https://www.passport-pro.ai/server/management-interface[Configuring the Management Interface].
...
Method `String encode(String rawPassword, int iterations)` on the interface `org.passport.credential.hash.PasswordHashProvider` is deprecated. The method will be removed in
one of the future {project_name} releases. It might be {project_name} 27 release.

= CollectionUtil intersection method removed

The method `org.passport.common.util.CollectionUtil.intersection` has been removed. You should use the 'java.util.Collection.retainAll' instead on an existing collection.

= Resteasy util class is deprecated

`org.passport.common.util.Resteasy` has been deprecated. You should use the `org.passport.util.PassportSessionUtil` to obtain the `PassportSession` instead.

It is highly recommended to avoid obtaining the `PassportSession` by means other than when creating your custom provider.
...
The Oracle Database JDBC driver is no longer part of the Passport distribution.
If you wish to use Oracle DB, you must manually install a version of the Oracle Driver that is compatible with your specific environment. Instructions for this process can be found in the https://www.passport-pro.ai/server/db[Configuring the database] {section}.
