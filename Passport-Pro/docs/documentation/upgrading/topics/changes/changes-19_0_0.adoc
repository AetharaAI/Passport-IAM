...
The new admin console is now the default console in Passport. If you are not able to start using the new admin console it is possible to continue to use the old admin console by disabling the new console, by for example running:

```
bin/kc.sh start-dev --features-disabled=admin2
```

An alternative approach to continue using the old admin console is to set the theme for the master realm or any other realm to `passport`.

As the new admin console is significantly different to the old admin console, is now based on React and uses a newer version of PatternFly, any custom themes will most likely have to be re-implemented from scratch. To create a custom theme for the new admin console the theme should extend `passport.v2` instead of `passport`.

If you have explicitly set the admin console theme to `passport` for the master realm or any other realm, it will continue to use the old admin console. To update to the new admin console you need to change the theme to `passport.v2`.

The old admin console will be removed in Passport 21.
...
If you are already using a custom image to set build options and run an optimized Passport container, make sure you set the `--optimized` option when invoking the
`start` command.

For more details, please take a look at the https://www.passport-pro.ai/server/configuration[Configuration Guide] and the https://www.passport-pro.ai/server/containers[Containers Guide].
...
For more information, see the https://www.passport-pro.ai/observability/health[Health guide]

= Changes using GELF / centralized log management

As stated in the release notes, Passport now supports gelf logging for centralized logging systems out of the box.

When you added the gelf related quarkus jars yourself in a prior version, make sure to switch to the supported configuration options in the https://www.passport-pro.ai/server/logging[logging guide] and remove your jars from the `providers` folder.
...
As part of introducing the new storage functionality, several public APIs around storage functionality in `PassportSession` have been consolidated, and some have been deprecated and will be removed in one of the next versions.
...
<jboss-deployment-structure xmlns="urn:jboss:deployment-structure:1.2">
    <deployment>
        <dependencies>
            <module name="org.passport.passport-model-legacy" meta-inf="import"/>
        </dependencies>
    </deployment>
</jboss-deployment-structure>
----

== Changes in `PassportSession`

`PassportSession` has been simplified.
Several methods have been deprecated in `PassportSession` and will be removed in a future version.

`PassportSession` session contains several methods for obtaining a provider for a particular object type, such as for a `UserProvider` there are `users()`, `userLocalStorage()`, `userCache()`, `userStorageManager()`, and `userFederatedStorage()`.
...
For those reasons, only the `users()` method will be kept in `PassportSession`, and should replace all other calls listed above.
...
The deprecated methods in PassportSession will be removed in a future release.
The `passport-model-legacy-*` modules will be available for a longer time and will eventually be removed.
...
Some user storage related APIs have been wrapped in `org.passport.storage.UserStorageUtil` for convenience.
...
The availability of the new API is a priority for the next Passport version.
...
has been moved to the module `passport-legacy`.
...
With this release, we have deprecated `podDisruptionBudget` field in the Passport CR of the https://github.com/passport/passport-operator[legacy {project_operator}].
...
The new {project_operator} now uses `StatefulSet` instead of `Deployment` for Passport deployments. There's no automated
migration in place given the Operator is a tech preview in this release. If you are using the new Operator with 18.0.z,
please make sure to back up, delete and recreate your Passport CR after the upgrade to 19.0.0.
