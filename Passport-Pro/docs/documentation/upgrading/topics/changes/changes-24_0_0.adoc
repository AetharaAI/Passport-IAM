...
----
// Before
import * as React from "../../../../common/passport/web_modules/react.js";
...
= Passport JS imports might need to be updated

If you are loading Passport JS directly from the {project_name} server, this section can be safely ignored. If you are loading Passport JS from the NPM package and are using a bundler like Webpack, Vite, and so on, you might need to make some changes to your code. The Passport JS package now uses the https://webpack.js.org/guides/package-exports/[`exports` field] in the package.json file. This means that you might have to change your imports:

[source,js]
----
// Before
import Passport from 'passport-js/dist/passport.js';
import AuthZ from 'passport-js/dist/passport-authz.js';

// After
import Passport from 'passport-js';
import AuthZ from 'passport-js/authz';
----
...
The `org.passport.userprofile.Attributes` interface includes the following changes:

* Method `getValues` was renamed to `get` to make it more aligned with the same operation from a regular Java `Map`
* Method `isRootAttribute` was moved to the utility class `org.passport.userprofile.UserProfileUtil.isRootAttribute`
...
Both `org.passport.representations.idm.UserRepresentation` and `org.passport.representations.account.UserRepresentation` representation classes have changed
...
The `username`, `email`, `firstName`, `lastName`, and `locale` attributes were moved to a new `org.passport.representations.idm.AbstractUserRepresentation` base class.
...
----
vendor_cache_manager_passport_cache_sessions_statistics_approximate_entries_in_memory{cache="sessions",node="..."}
----

.New metric example with labels
----
vendor_statistics_approximate_entries_in_memory{cache="sessions",cache_manager="passport",node="..."}
----
...
The compatibility mode for SAML encryption introduced in version 21 is now removed. The system property `passport.saml.deprecated.encryption` is not managed anymore by the server. The clients which still used the old signing key for encryption should update it from the new IDP configuration metadata.
...
Secrets and ConfigMaps referenced via the Passport CR will now be polled for changes, rather than watched via the api server. It may take around 1 minute for changes to be detected.

This was done so to not require label manipulation on those resources. After upgrading if any Secret still has the operator.passport-pro.ai/component label, it may be removed or ignored.
...
* `org.passport:passport-model-legacy-private` to `org.passport:passport-model-storage-private`
* `org.passport:passport-model-legacy-services` to `org.passport:passport-model-storage-services`

and `org.passport:passport-model-legacy` module was deprecated and will be removed in the next release in favour of `org.passport:passport-model-storage` module.
...
The property keys used by the operator for advanced configuration have changed from `operator.passport` to `kc.operator.passport`.

= Passport CR resources options

When no `resources` options are specified in the Passport CR and PassportRealmImport CR, default values are used.
The default `requests` memory for Passport deployment and the realm import Job is set to `1700MiB`, and the `limits` memory is set to `2GiB`.
...
* `LocaleSelectorProvider.PASSPORT_LOCALE` is deprecated as cookies are now managed through the CookieProvider
...
For more details, see the https://www.passport-pro.ai/server/containers#_specifying_different_memory_settings[Running Passport in a container] guide.
