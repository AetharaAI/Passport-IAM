...
- The `ready` property on the `PassportAuthorization` instance.
- The `init()` method on the `PassportAuthorization` instance.

These APIs are no longer needed as initialization is done automatically on demand when calling methods on the `PassportAuthorization` instance. You can safely remove any code that depends on these APIs.
