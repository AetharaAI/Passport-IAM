...
Start MySQL:
    ...
    docker run --name mysql -e MYSQL_DATABASE=passport -e MYSQL_USER=passport -e MYSQL_PASSWORD=passport -e MYSQL_ROOT_PASSWORD=passport -d mysql
   
Run tests:

    mvn install -Dpassport.connectionsJpa.url=jdbc:mysql://`docker inspect --format '{{ .NetworkSettings.IPAddress }}' mysql`/passport -Dpassport.connectionsJpa.driver=com.mysql.jdbc.Driver -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport    
...
Start PostgreSQL:
    ...
    docker run --name postgres -e POSTGRES_DATABASE=passport -e POSTGRES_USER=passport -e POSTGRES_PASSWORD=passport -e POSTGRES_ROOT_PASSWORD=passport -d postgres
   
Run tests:

    mvn install -Dpassport.connectionsJpa.url=jdbc:postgresql://`docker inspect --format '{{ .NetworkSettings.IPAddress }}' postgres`:5432/passport -Dpassport.connectionsJpa.driver=org.postgresql.Driver -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport    
...
Start MariaDB:
    ...
    docker run --name mariadb -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=passport -e MYSQL_USER=passport -e MYSQL_PASSWORD=passport -d mariadb:10.1
   
Run tests:

    mvn install -Dpassport.connectionsJpa.url=jdbc:mariadb://`docker inspect --format '{{ .NetworkSettings.IPAddress }}' mariadb`/passport -Dpassport.connectionsJpa.driver=org.mariadb.jdbc.Driver -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport    
...
Run tests:
    ...
    mvn install -Dpassport.connectionsJpa.url=jdbc:mysql://`docker inspect --format '{{ .NetworkSettings.IPAddress }}' tidb`:4000/test -Dpassport.connectionsJpa.driver=com.mysql.jdbc.Driver -Dpassport.connectionsJpa.user=root -Dpassport.connectionsJpa.password=    
...
If you want to run tests using a pre-configured Passport distribution (instead of Undertow):
