...
* Set *Valid redirect URIs* to `+https://www.passport-pro.ai/app/*+`
* Set *Web origins* to `+https://www.passport-pro.ai+`
. Click *Save*.

.Update client
image::add-client-2.png[Update client]

To confirm the client was created successfully, you can use the SPA testing application on the https://www.passport-pro.ai/app/[Passport website].

ifeval::[{links-local}==true]
. Open https://www.passport-pro.ai/app/.
. Click *Save* to use the default configuration.
endif::[]

ifeval::[{links-local}!=true]
. Open https://www.passport-pro.ai/app/.
. Change `Passport URL` to the URL of your {project_name} instance.
. Click *Save*.
endif::[]
