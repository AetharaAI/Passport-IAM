...
. Create a project called `passport` by entering the following command:
+
[source,bash,subs="attributes+"]
----
oc new-project passport
----
...
[source,bash,subs="attributes+"]
----
oc process -f https://raw.githubusercontent.com/passport/passport-quickstarts/refs/heads/main/openshift/passport.yaml \
    -p KC_BOOTSTRAP_ADMIN_USERNAME=admin \
    -p KC_BOOTSTRAP_ADMIN_PASSWORD=admin \
    -p NAMESPACE=passport \
| oc create -f -
----
+
...
route.route.openshift.io/passport created
deploymentconfig.apps.openshift.io/passport created.
----
+
...
passport-1-deploy   0/1       Completed   0          1h
passport-1-l9kdx    1/1       Running     0          1h
----
...
[source,bash,subs="attributes+"]
----
PASSPORT_URL=https://$(oc get route passport --template='{{ .spec.host }}') &&
echo "" &&
