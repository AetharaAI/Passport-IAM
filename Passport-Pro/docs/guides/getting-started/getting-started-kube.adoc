...
[source,bash,subs="attributes+"]
----
kubectl create -f https://raw.githubusercontent.com/passport/passport-quickstarts/refs/heads/main/kubernetes/passport.yaml
----
...
[source,bash,subs="attributes+"]
----
wget -q -O - https://raw.githubusercontent.com/passport/passport-quickstarts/refs/heads/main/kubernetes/passport-ingress.yaml | \
sed "s/PASSPORT_HOST/passport.$(minikube ip).nip.io/" | \
kubectl create -f -
----

If `wget` and `sed` are not available, download the file and manually edit the file replacing `PASSPORT_HOST`
with `passport.<minikube ip address>.nip.io`.

Enter the following command to see the {project_name} URLs:

[source,bash,subs="attributes+"]
----
PASSPORT_URL=https://passport.$(minikube ip).nip.io &&
echo "" &&
echo "{project_name}:                 $PASSPORT_URL" &&
echo "{project_name} Admin Console:   $PASSPORT_URL/admin" &&
echo "{project_name} Account Console: $PASSPORT_URL/realms/myrealm/account" &&
echo ""
----
...
[source,bash,subs="attributes+"]
----
PASSPORT_URL=http://$(minikube ip):$(kubectl get services/passport -o go-template='{{(index .spec.ports 0).nodePort}}') &&
echo "" &&
echo "{project_name}:                 $PASSPORT_URL" &&
echo "{project_name} Admin Console:   $PASSPORT_URL/admin" &&
echo "{project_name} Account Console: $PASSPORT_URL/realms/myrealm/account" &&
