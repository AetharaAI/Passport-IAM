...
With the Passport Custom Resource (CR), you can specify a custom container image for the {project_name} server.
...
With a custom image, you can also specify the Passport _build-time_ configurations and extensions during the build of the container.
...
To provide a custom image, you define the `image` field in the Passport CR as shown in this example:
...
spec:
  instances: 1
  image: quay.io/my-company/my-passport:latest
  http:
...
Use the Passport CR for any configuration that requires Operator awareness, namely the TLS and HTTP(S) settings reflected when configuring services and probes.
