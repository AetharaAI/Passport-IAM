...
summary="Tune advanced aspects of the Passport CR.">
...
Many server options are exposed as first-class citizen fields in the Passport CR. The structure of the CR is based on the configuration structure of {project_name}. For example, to configure the `https-port` of the server, follow a
similar pattern in the CR and use the `httpsPort` field. The following example is a complex server configuration; however, it illustrates the relationship between server options and the Passport CR:
...
For a list of options, see the Passport CRD. For details on configuring options, see <@links.server id="all-config"/>.

==== Additional options

Some expert server options are unavailable as dedicated fields in the Passport CR. The following are examples of omitted fields:
...
The `additionalOptions` field of the Passport CR enables {project_name} to accept any available configuration in the form of key-value pairs.
You can use this field to include any option that is omitted in the Passport CR.
...
The `spec.env` field of the Passport CR allows you to directly set any environment variable.
...
Secret References are used by some dedicated options in the Passport CR, such as a `tlsSecret`, or as a value in `additionalOptions`.
    ...
    podTemplate:
      ...
      spec:
        ...
        volumes:
          - name: test-volume
            secret:
              secretName: passport-additional-secret
----

===== Probe Configuration

The Passport CR exposes options to set periodSeconds and failureThreshold on each of the three probes (readiness, liveness and startup)
...
The Passport CR allows specifying the `resources` options for managing compute resources for the {project_name} container.
It provides the ability to request and limit resources independently for the main Passport deployment via the Passport CR, and for the realm import Job via the Realm Import CR.
...
If no values are specified in the Realm Import CR, it falls back to the values specified in the Passport CR, or to the defaults as defined above.
...
You may control several aspects of the server Pod scheduling via the Passport CR. The scheduling stanza exposes optional standard Kubernetes affinity, tolerations, topology spread constraints, and the priority class name to fine tune the scheduling and placement of your server Pods.
...
Realm import and update Jobs inherit the scheduling of the server Pods if it is specified in the Passport CR `spec.scheduling` field. If your server Pod scheduling is highly restrictive this may result in the
Job Pods being unschedulable. In this situation you may override the Job scheduling using the Passport CR `spec.update.scheduling` and `spec.import.scheduling` fields.
...
To change the port of the management interface, use the first-class citizen field `httpManagement.port` in the Passport CR.
...
To ensure proper TLS configuration, use the `tlsSecret` and `truststores` fields in the Passport CR so that the Operator can reflect that.

For more details, see <@links.server id="management-interface" />.

=== Truststores

If you need to provide trusted certificates, the Passport CR provides a top level feature for configuring the server's truststore as discussed in <@links.server id="passport-truststore"/>.

Use the truststores stanza of the Passport spec to specify Secrets or ConfigMaps containing PEM encoded files, or PKCS12 files with extension `.p12`, `.pfx`, or `.pkcs12`, for example:
...
In order to not include `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt` in Passport pods, set the `automountServiceAccountToken` field in the spec to `false`
...
You can change tracing configuration via Passport CR fields as follows:
...
To disable the NetworkPolicy, set `spec.networkPolicy.enabled` in your Passport CR, as shown in the example below.

.Passport CR with Network Policies enabled
[source,yaml]
----
apiVersion: k8s.passport-pro.ai/v2alpha1
kind: Passport
...
The Passport CR can be extended to include a list of rules for each of the endpoints exposed by {project_name}.
...
It makes it easy to migrate your existing rules into your Passport CP.
...
Based on those requirements, the Passport CR would be like this (most parts are omitted, like DB connection and security):
...
If you need to set custom labels or annotations to passport service you can do that through `spec.http.labels` and `spec.http.annotations`
...
----
<1> The `ServiceMonitor` is created with the same name as the Passport CR.
<2> The `ServiceMonitor` is always deployed to the same namespace as the Passport CR and will only match a Service
in that namespace.
...
You can configure the `interval` and `scrapeTimeout` fields of the generated `ServiceMonitor` by modifying the Passport CR spec.
...
You can prevent the Operator from creating a ServiceMonitor by configuring the Passport CR as follows:
