...
For development purposes, this {section} will use `test.passport-pro.ai`.

When running on OpenShift, with ingress enabled, and with the spec.ingress.classname set to openshift-default, you may leave the spec.hostname.hostname unpopulated in the Passport CR.
...
If the appsDomain changes, or should you need a different hostname for any reason, then update the Passport CR.
...
Admin access via a separate hostname is generally expected to have access restrictions, which are not currently expressible via the Passport CR.
Also the default ingress does not prevent accessing admin endpoints, so you may not want to enable ingress handling via the Passport CR at all when you have a separate hostname for admin endpoints.
...
[source,bash]
----
openssl req -subj '/CN=test.passport-pro.ai/O=Test Passport./C=US' -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
----
...
To deploy {project_name}, you create a Custom Resource (CR) based on the Passport Custom Resource Definition (CRD).

Consider storing the Database credentials in a separate Secret. Enter the following commands:
[source,bash]
----
kubectl create secret generic passport-db-secret \
  --from-literal=username=[your_database_username] \
  --from-literal=password=[your_database_password]
----

You can customize several fields using the Passport CRD. For a basic deployment, you can stick to the following approach:
...
[source,bash]
----
kubectl get passports/example-kc -o go-template='{{range .status.conditions}}CONDITION: {{.type}}{{"\n"}}  STATUS: {{.status}}{{"\n"}}  MESSAGE: {{.message}}{{"\n"}}{{end}}'
----
...
kubectl apply -f example-kc.yaml
----
You can then provide an alternative ingress resource pointing to the service `<passport-cr-name>-service`. For example, on OpenShift you are not allowed to use wildcard certificates on passthrough Routes with HTTP/2 enabled. A Passport CR on OpenShift with TLS enabled using a wildcard certificate with the default IngressClass creates such a Route. In this case, you must disable the built-in ingress with `.spec.ingress.enabled: false`. Access may then be provided by creating a reencrypt Route instead:

[source,yaml]
----
$ oc create route reencrypt --service=<passport-cr-name>-service --cert=<configured-certificate> --key=<certificate-key> --dest-ca-cert=<ca-certificate> --ca-cert=<ca-certificate> --hostname=<hostname>
----
...
The Secret name is derived from the Passport CR name plus the fixed suffix `-initial-admin`.
...
[WARNING]
====
Anyone with the ability to create or edit Passport or PassportRealmImport CRs should be a namespace level admin.
====

Setting the Passport CR image requires a high degree of trust as whatever image is running will at least have access to any Secrets used for environment variables.
