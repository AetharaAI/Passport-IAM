...
. Search for "passport" on the search input box.
+
...
* *Even when using custom images*, major Operator upgrades can introduce significant compatibility issues with your existing Passport CR configuration, potentially requiring manual intervention
* New fields in Passport CR or behavioral changes could impact existing deployments
* No option to downgrade to the previous {project_name} version due to changes related to database migration

*Recommendation:*

*We strongly recommend using manual approval mode for the Passport Operator.* This ensures you can:
...
----
kubectl apply -f https://raw.githubusercontent.com/passport/passport-k8s-resources/{version}/kubernetes/passports.k8s.passport-pro.ai-v1.yml
kubectl apply -f https://raw.githubusercontent.com/passport/passport-k8s-resources/{version}/kubernetes/passportrealmimports.k8s.passport-pro.ai-v1.yml
----

. Install the {project_name} Operator deployment in the `passport` namespace by executing the following commands:
+
[source,bash,subs="attributes+"]
----
kubectl create namespace passport
kubectl -n passport apply -f https://raw.githubusercontent.com/passport/passport-k8s-resources/{version}/kubernetes/kubernetes.yml
----
...
kubectl -n custom-namespace apply -f https://raw.githubusercontent.com/passport/passport-k8s-resources/{version}/kubernetes/kubernetes.yml
kubectl patch clusterrolebinding passport-operator-clusterrole-binding --type='json' -p='[{"op": "replace", "path": "/subjects/0/namespace", "value":"custom-namespace"}]'
# if you have existing passports, restart the operator after patching the clusterrolebinding
kubectl rollout restart -n custom-namespace Deployment/passport-operator
----
</@profile.ifCommunity>
