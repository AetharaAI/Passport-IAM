...
Specify the update strategy within the `spec` section of the Passport CR YAML definition:
...
[WARNING]
====
If the Passport CR configured a `podTemplate` as part of the `unsupported` configuration parameters, the Passport Operator will do its best to use those settings for the started Job. Still it might miss some settings due to the flexibility of the `podTemplate` feature and its unsupported nature.
...
The Passport CR status of `RecreateUpdateUsed` indicates the update strategy employed during the last update operation.
