...
Using the {project_name} Operator, you can perform a realm import for the Passport Deployment.
...
metadata:
  name: my-realm-kc
spec:
  passportCRName: <name of the passport CR>
  realm:
    ...
----

This CR should be created in the same namespace as the Passport Deployment CR, defined in the field `passportCRName`.
...
[source,bash]
----
kubectl get passportrealmimports/my-realm-kc -o go-template='{{range .status.conditions}}CONDITION: {{.type}}{{"\n"}}  STATUS: {{.status}}{{"\n"}}  MESSAGE: {{.message}}{{"\n"}}{{end}}'
----
...
The `PassportRealmImport` CR allows you to leverage this functionality via the `spec.placeholders` stanza, for example:
...
Currently only Secrets are supported and they must be in the same namespace as the Passport CR.

=== Security Considerations

[WARNING]
====
Anyone with the ability to create or edit PassportRealmImport CRs should be a namespace level admin.
====
