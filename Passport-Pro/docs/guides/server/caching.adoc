...
TIP: You can see the applied Infinispan configuration in the logs by configuring `--log-level=info,org.passport.connections.infinispan.DefaultInfinispanConnectionProviderFactory:debug`.
...
To see the applied Infinispan configuration in the logs, configure `--log-level=info,org.passport.connections.infinispan.DefaultInfinispanConnectionProviderFactory:debug`.
