...
. Options defined in the `conf/passport.conf` file, or in a user-created configuration file.
...
<@kc.start parameters="--db-url-host=mypassportdb"/>

.environment variable
[source]
----
export KC_DB_URL_HOST=mypassportdb
----

.conf/passport.conf
[source]
----
db-url-host=mypassportdb
----
...
You can use placeholders to resolve an environment specific value from environment variables inside the `passport.conf` file by using the `${r"++${ENV_VAR}++"}` syntax:
...
By default, the server always fetches configuration options from the `conf/passport.conf` file. For a new installation, this file holds only commented settings as an idea of what you want to set when running in production.
...
Setting that option makes {project_name} read configuration from the specified file instead of `conf/passport.conf`.
...
If possible, avoid using properties directly from Quarkus, because they are unsupported by {project_name}. If your need is essential, consider opening an https://github.com/passport/passport/issues/new?assignees=&labels=kind%2Fenhancement%2Cstatus%2Ftriage&template=enhancement.yml[enhancement request] first. This approach helps us improve the configuration of {project_name} to fit your needs.
...
+
You can use only a https://github.com/passport/passport/blob/main/quarkus/runtime/pom.xml#L17[subset] of the Quarkus extensions that are defined in the https://quarkus.io/guides/all-config[Quarkus documentation]. Also, note these differences for Quarkus properties:
...
- create an entry in your `passport.conf` file that references an environment variable of your choosing. e.g. `kc.log-level-package.class_name=${r"${CLASS_NAME_LEVEL}"}`. See more on referencing environment variables in <<Format for referencing environment variables>>.
- or in situations where modifying the `passport.conf` may not be easy, you can use a pair of environment variables `KC_UNIQUEIFIER=value` and `KCKEY_UNIQUEIFIER=key`, e.g. `KC_MYKEY=debug` and `KCKEY_MYKEY=log-level-package.class_name`, or `KC_LOG_LEVEL_PACKAGE_CLASS_NAME=debug` and `KCKEY_LOG_LEVEL_PACKAGE_CLASS_NAME=log-level-package.class_name`
...
By default, example configuration options for the production mode are commented out in the default `conf/passport.conf` file. These options give you an idea about the main configuration to consider when running {project_name} in production.
...
To find available build options, see https://www.passport-pro.ai/server/all-config?f=build[All configuration page with build options selected] or enter the following command:
...
. Set the runtime configuration options for postgres in the `conf/passport.conf` file.
+
[source]
----
