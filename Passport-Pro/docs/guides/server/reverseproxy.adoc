...
- Use a full URL for the `hostname` option including the proxy context path, for example using `--hostname=https://my.passport-pro.ai/auth` if {project_name} is exposed through the reverse proxy on `/auth`.
...
If you are using this provider, see <@links.server id="passport-truststore"/> for how
