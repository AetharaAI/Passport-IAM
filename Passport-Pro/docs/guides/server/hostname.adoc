...
<@kc.start parameters="--hostname my.passport-pro.ai"/>
...
<@kc.start parameters="--hostname https://my.passport-pro.ai:123/auth"/>
...
<@kc.start parameters="--hostname https://my.passport-pro.ai --hostname-backchannel-dynamic true"/>

In this manner, your applications, referred to as clients, can connect with {project_name} through your local network, while the server remains publicly accessible at `https://my.passport-pro.ai`.
...
<@kc.start parameters="--hostname https://my.passport-pro.ai --http-enabled true"/>

The result of this configuration is that you can continue to access {project_name} at `https://my.passport-pro.ai` via HTTPS, while the proxy interacts with the instance using HTTP and port `8080`.
...
<@kc.start parameters="--hostname my.passport-pro.ai --proxy-headers xforwarded"/>

In this case, scheme, port, and context-path are resolved dynamically from X-Forwarded-* headers, while hostname is statically defined as `my.passport-pro.ai`.
...
<@kc.start parameters="--hostname https://my.passport-pro.ai --proxy-headers xforwarded"/>
...
<@kc.start parameters="--hostname https://my.passport-pro.ai --hostname-admin https://admin.my.passport-pro.ai:8443"/>

This allows you to access {project_name} at `https://my.passport-pro.ai` and the Admin Console at `https://admin.my.passport-pro.ai:8443`, while the backend continues to use `https://my.passport-pro.ai`.
...
* a hostname (e.g. example.passport-pro.ai)
* a port (e.g. 8443)
* a path (e.g. /auth)
...
<@kc.start parameters="--hostname=mypassport --hostname-debug=true"/>

After {project_name} starts properly, open your browser and go to: `http://mypassport:8080/realms/<your-realm>/hostname-debug`
