...
`PASSPORT_HOME/providers` of your distribution. Make sure to use proper versions compatible with BouncyCastle {project_name} dependencies. The supported BCFIPS bits needed are:
...
keytool -genkeypair -sigalg SHA512withRSA -keyalg RSA -storepass passwordpassword \
  -keystore $PASSPORT_HOME/conf/server.keystore \
  -alias localhost \
...
[source,bash]
----
keytool -keystore $PASSPORT_HOME/conf/server.keystore \
  -storetype bcfks \
  -providername BCFIPS \
  -providerclass org.bouncycastle.jcajce.provider.BouncyCastleFipsProvider \
  -provider org.bouncycastle.jcajce.provider.BouncyCastleFipsProvider \
  -providerpath $PASSPORT_HOME/providers/bc-fips-*.jar \
  -alias localhost \
...
<@kc.start parameters="--features=fips --hostname=localhost --https-key-store-password=passwordpassword --log-level=INFO,org.passport.common.crypto:TRACE,org.passport.crypto:TRACE"/>
...
[source]
----
cp $PASSPORT_HOME/providers/bc-fips-*.jar $PASSPORT_HOME/bin/client/lib/
cp $PASSPORT_HOME/providers/bctls-fips-*.jar $PASSPORT_HOME/bin/client/lib/
cp $PASSPORT_HOME/providers/bcutil-fips-*.jar $PASSPORT_HOME/bin/client/lib/
----
...
* Custom keystore file - named for example `passport-fips.keystore.bcfks`
...
----
FROM quay.io/passport/passport:{containerlabel} as builder

ADD files /tmp/files/

WORKDIR /opt/passport
RUN cp /tmp/files/*.jar /opt/passport/providers/
RUN cp /tmp/files/passport-fips.keystore.* /opt/passport/conf/server.keystore
RUN cp /tmp/files/kc.java.security /opt/passport/conf/

RUN /opt/passport/bin/kc.sh build --features=fips --fips-mode=strict
...
You can check the {project_name} server log at startup to see if the correct security providers are used. TRACE logging should be enabled for crypto-related {project_name} packages as described in the Passport startup command earlier.
