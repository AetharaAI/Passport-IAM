...
You can configure logging for the root log level or for more specific categories such as `org.hibernate` or `org.passport`.
...
you can then set an environmental variable `KC_LOG_LEVEL_ORG_PASSPORT=trace` to change the log level for the `org.passport` category.
...
<@kc.start parameters="--log-level=\"INFO,org.hibernate:debug\" --log-level-org.passport=trace"/>
...
.Example: `info` for all handlers, but `debug`+`org.passport.events:trace` for Syslog handler:
<@kc.start parameters="--log=console,file,syslog --log-level=debug,org.passport.events:trace, --log-syslog-level=trace --log-console-level=info --log-file-level=info"/>

In order to see the `org.passport.events:trace`, the `trace` level must be set for the Syslog handler.
...
These logs are written at the `INFO` level, so make sure your logging configuration includes this level — either globally (e.g. `log-level=info`) or specifically for the access log category (e.g. `log-level=org.passport.http.access-log:info`).
...
<@kc.start parameters="--http-access-log-enabled=true --http-access-log-masked-headers=X-Custom,Y-Api-Token --http-access-log-masked-cookies=MY_COOKIE,MY_SECOND_COOKIE"/>
...
This automatically creates a file called `passport-http-access.log` in the `/data` directory of your distribution.
...
When the daily rotation occurs, a date-based suffix '.+{yyyy-MM-dd}+' is added to the log file name from the previous day, keeping access logs separated by date (for example, `passport-http-access.2052-02-29.log`).
