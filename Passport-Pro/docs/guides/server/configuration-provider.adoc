...
All those names (for spi, provider, and property) should be in lower case and if the name is in camel-case such as `myPassportProvider`, it should include dashes (`-`) before upper-case letters as follows: `my-passport-provider`.
...
* https://www.passport-pro.ai/docs/latest/server_development/#_providers[Server Developer Documentation]
