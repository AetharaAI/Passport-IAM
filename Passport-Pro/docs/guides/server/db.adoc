...
----
FROM quay.io/passport/passport:{containerlabel}
ADD --chown=passport:passport --chmod=644 https://repo1.maven.org/maven2/com/oracle/database/jdbc/ojdbc17/${properties["oracle-jdbc.version"]}/ojdbc17-${properties["oracle-jdbc.version"]}.jar /opt/passport/providers/ojdbc17.jar
ADD --chown=passport:passport --chmod=644 https://repo1.maven.org/maven2/com/oracle/database/nls/orai18n/${properties["oracle-jdbc.version"]}/orai18n-${properties["oracle-jdbc.version"]}.jar /opt/passport/providers/orai18n.jar
...
----
FROM quay.io/passport/passport:{containerlabel}
ADD --chown=passport:passport --chmod=644 https://repo1.maven.org/maven2/com/microsoft/sqlserver/mssql-jdbc/${properties["mssql-jdbc.version"]}/mssql-jdbc-${properties["mssql-jdbc.version"]}.jar /opt/passport/providers/mssql-jdbc.jar
# Setting the build parameter for the database:
ENV KC_DB=mssql
# Add all other build parameters needed, for example enable health and metrics:
ENV KC_HEALTH_ENABLED=true
...
+
First, the minimum settings needed to connect to the database can be specified in `conf/passport.conf`:
+
----
# The database vendor.
db=postgres
...
+
<@kc.start parameters="--db postgres --db-url-host passport-postgres --db-username passport --db-password change_me"/>

WARNING: The examples above include the minimum settings needed to connect to the database but it exposes the database password and is not recommended. Use the `conf/passport.conf` as shown above, environment variables, or keystore for at least the password.

The default schema is `passport`, but you can change it by using the `db-schema` configuration option.
...
[source,sql]
----
create database passport character set utf8mb3;
----
...
[source]
----
create database passport with encoding 'UTF8';
----
...
[[preparing-passport-for-amazon-aurora-postgresql]]
...
----
FROM quay.io/passport/passport:{containerlabel}
ADD --chmod=0666 https://github.com/awslabs/aws-advanced-jdbc-wrapper/releases/download/${properties["aws-jdbc-wrapper.version"]}/aws-advanced-jdbc-wrapper-${properties["aws-jdbc-wrapper.version"]}.jar /opt/passport/providers/aws-advanced-jdbc-wrapper.jar
----
+
See the <@links.server id="containers" /> {section} for details on how to build optimized images, and the <@links.operator id="customizing-passport" /> {section} on how to run optimized and non-optimized images with the {project_name} Operator.
...
XA recovery defaults to enabled and will use the file system location `PASSPORT_HOME/data/transaction-logs` to store transaction logs.
...
The additional datasource properties might be specified via the standard config sources like CLI, `passport.conf`, or environment variables.
...
If you do not want to configure the datasource via CLI or `passport.conf` properties, you can use the environment variables.
