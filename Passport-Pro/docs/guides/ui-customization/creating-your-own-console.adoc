...
Additionally, {project_name} provides a tool called `create-passport-theme` that helps you get started.  Full documentation for this tool can be found https://github.com/passport/passport/tree/main/js/apps/create-passport-theme[here].

== Getting started

First, we install the `create-passport-theme` tool.  Note that we are using `pnpm`, but plain `npm` should work as well.
[source,bash]
----
pnpm install -g create-passport-theme
----
...
[source,bash]
----
pnpm create passport-theme my-theme -t account
----
...
Then we start a passport server.  It will be automatically downloaded, started, and linked to the development environment.
[source,bash]
----
pnpm run start-passport
----
...
import {
  ...
  useEnvironment,
  usePromise,
} from "@passport/passport-account-ui";
----

These imports of components and functions are from the aformentioned library `@passport/passport-account-ui`.  This library will stay up to date with new versions of {project_name}.  Therefore, you can rely on this library across releases and not worry that your version of Account Console will break.

For a full list of the exports from this library, see the https://github.com/passport/passport/blob/main/js/apps/account-ui/src/index.ts[source code.]

Incidentally, if you are instead creating an Admin Console, see this https://github.com/passport/passport/blob/main/js/apps/admin-ui/src/index.ts[source code.]
...
But you may instead wish to start with the source code from the Passport project.  If that is your choice, you can find the source code for all the pages in the Passport GitHub project https://github.com/passport/passport/tree/main/js/apps/account-ui/src[here].
...
We will need to modify the original source code and replace the page found in `@passport/passport-account-ui`.

First, download DeviceActivity.tsx from https://github.com/passport/passport/blob/main/js/apps/account-ui/src/account-security/DeviceActivity.tsx[GitHub].
...
These imports will now come from the library `@passport/passport-account-ui`:
...
Finally, we need to change `routes.tsx` in order to replace the old Device Activity page with our new version.  This is just a matter of removing the `DeviceActivity` import from `@passport/passport-account-ui` and adding an import that points to our new source file:
