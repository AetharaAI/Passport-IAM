...
{project_name} comes bundled with default themes in the JAR file `passport-themes-{project_versionMvn}.jar` inside the server distribution.
...
. Run Passport with the following options:
+
[source,bash]
----
...
. To see the effect of changing the parent theme, set `parent=passport` in `theme.properties` and refresh the login page.
...
* common - Override the common resource path. The default value is `common/passport` when not specified. This value would be used as value of suffix of ${r"`${url.resourcesCommonPath}`"}, which is used typically in freemarker templates (prefix of ${r"`${url.resoucesCommonPath}`"} value is theme root uri).
...
file in the corresponding type of the passport theme (`themes/passport/<THEME TYPE>/theme.properties`).
...
Icons for social providers are already defined in `base` login theme properties (`themes/passport/login/theme.properties`), where you can inspire yourself.
...
For a list of templates in other theme types look at the `theme/base/<THEME_TYPE>` directory in the JAR file at `$PASSPORT_HOME/lib/lib/main/org.passport.passport-themes-<VERSION>.jar`.
...
. Add a file `META-INF/passport-themes.json` to the
...
+
The contents of `META-INF/passport-themes.json` in this case would be:
+
[source,json]
----
{
...
== Dark Mode
Dark mode is a feature that changes the color scheme to a dark background with lighter text.  For example, themes based on PatternFly, such as Passport's admin console and account console, support dark mode.  But for some themes, dark mode doesn't work well for various reasons.  So Passport allows you to disable dark mode from the admin console.
...
An example use-case would be a https://www.passport-pro.ai/docs/latest/server_development/index.html#_auth_spi[custom authenticator] that requires additional templates and resources.
endif::[]
...
With internationalization enabled, the locale is resolved according to the logic described in the https://www.passport-pro.ai/docs/latest/server_admin/#_user_locale_selection[Server Administration documentation].
...
The `LocaleSelectorProvider` interface has a single method, `resolveLocale`, which must return a locale given a `RealmModel` and a nullable `UserModel`. The actual request is available from the `PassportSession#getContext` method.
...
* For more details on creating and deploying a custom provider, see https://www.passport-pro.ai/docs/latest/server_development/index.html#_providers[Service Provider Interfaces].
