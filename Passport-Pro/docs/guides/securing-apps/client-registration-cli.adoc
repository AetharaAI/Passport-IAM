...
[filename]`PASSPORT_HOME` refers to a directory where the {project_name} Server distribution was unpacked.
...
[options="npwrap"]
----
$ kcreg.sh config truststore --trustpass $PASSWORD ~/.passport/truststore.jks
----
* Windows:
+
[options="npwrap"]
----
c:\> kcreg config truststore --trustpass %PASSWORD% %HOMEPATH%\.passport\truststore.jks
----
...
[options="nowrap"]
----
$ kcreg.sh get myclient -e install > passport.json
----
* Windows:
[options="nowrap"]
----
C:\> kcreg get myclient -e install > passport.json
----
