...
NOTE: Docker authentication is disabled by default. To enable see the https://www.passport-pro.ai/server/features[Enabling and disabling features] {section}.
...
Once the above configuration has taken place, and the passport server and Docker registry are running, docker authentication should be successful:
