...
`org.passport.adapters.saml.RoleMappingsProvider` SPI implementation that is to be used by the SAML adapter.
...
provider is identified by the id `properties-based-role-mapper` and is implemented by the `org.passport.adapters.saml.PropertiesBasedRoleMapper`
class.
...
To add a custom role mappings provider one simply needs to implement the `org.passport.adapters.saml.RoleMappingsProvider` SPI.
