...
----
<context-param>
    <param-name>passport.sessionIdMapperUpdater.classes</param-name>
    <param-value>org.passport.adapters.saml.wildfly.infinispan.InfinispanSessionCacheIdMapperUpdater</param-value>
</context-param>
----
...
`passport.sessionIdMapperUpdater.infinispan.cacheName`. The cache container containing the cache will be the same as
the one containing the deployment session cache, but can be overridden by a context parameter
`passport.sessionIdMapperUpdater.infinispan.containerName`.
