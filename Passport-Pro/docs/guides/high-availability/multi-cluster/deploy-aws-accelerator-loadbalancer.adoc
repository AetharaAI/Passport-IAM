...
An AWS Network Load Balancer (NLB) is created on both ROSA clusters in order to make the Passport
...
cat <<EOF | kubectl apply -n $NAMESPACE -f - #<1>
  ...
  spec:
    ...
    selector:
      app: passport
      app.kubernetes.io/instance: passport
...
.. Login to the ROSA cluster
+
.. Ensure the Passport CR has the following configuration
+
[source,yaml]
----
<#noparse>
...
----
<1> The hostname clients use to connect to Passport
<2> Disable the default ingress as all {project_name} access should be via the provisioned NLB
+
To ensure that request forwarding works as expected, it is necessary for the Passport CR to specify the hostname through
