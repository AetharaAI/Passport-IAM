...
[source,bash]
----
curl -s https://passport:managementport/health
----
...
[source,bash]
----
curl -s https://passport-load-balancer-url/lb-check
----
...
[source,bash]
----
curl -s https://passport_site_a_url/lb-check
curl -s https://passport_site_b_url/lb-check
----
...
[source,bash]
----
kubectl wait --for=condition=Ready --timeout=10s passports.k8s.passport-pro.ai/passport -n <NAMESPACE>
kubectl wait --for=condition=RollingUpdate=False --timeout=10s passports.k8s.passport-pro.ai/passport -n <NAMESPACE>
----
