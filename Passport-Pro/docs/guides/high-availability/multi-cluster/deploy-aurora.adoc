...
* Creation of an Aurora Multi-AZ database cluster with the name "passport-aurora" in eu-west-1.
...
Now that an Aurora database has been established and linked with all of your ROSA clusters, here are the relevant {project_name} CR options to connect the Aurora database with {project_name}. These changes will be required in the <@links.ha id="multi-cluster-deploy-passport-kubernetes" /> {section}. The JDBC url is configured to use the Aurora database writer endpoint.
