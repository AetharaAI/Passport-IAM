...
[#connecting-infinispan-to-passport]
== Connecting {jdgserver_name} with {project_name}

Now that the {jdgserver_name} server is running, here are the relevant {project_name} CR changes necessary to connect it to {project_name}. These changes will be required in the <@links.ha id="multi-cluster-deploy-passport-kubernetes" /> {section}.

. Create a Secret with the username and password to connect to the external {jdgserver_name} deployment:
+
[source,yaml]
----
include::../examples/generated/passport-ispn.yaml[tag=passport-ispn-secret]
----
...
[NOTE]
====
All the memory, resource and database configurations are skipped from the CR below as they have been described in the <@links.ha id="multi-cluster-deploy-passport-kubernetes" /> {section} already.
Administrators should leave those configurations untouched.
====
+
[source,yaml]
----
include::../examples/generated/passport-ispn.yaml[tag=passport-ispn]
----
...
You must deploy the `PassportCR` in the other cluster too, to create the caches there, otherwise {project_name} fails to start until they exist.
====
...
After the AWS Aurora database and {jdgserver_name} are deployed and running, use the procedure in the <@links.ha id="multi-cluster-deploy-passport-kubernetes" /> {section} to deploy {project_name} and connect it to all previously created building blocks.
