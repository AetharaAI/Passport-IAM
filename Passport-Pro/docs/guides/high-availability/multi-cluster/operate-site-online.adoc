...
This procedure describes how to re-add a Passport site to the Global Accelerator, after it has previously been taken offline,
so that it can once again service client requests.

== Procedure

Follow these steps to re-add a Passport site to the AWS Global Accelerator so that it can handle client requests.
