...
* The user event metric `passport_user_events_total` for event type `login` includes both password-based logins and cookie-based logins, still it can serve as a first approximate input for this sizing guide.
* To find out number of password validations performed by {project_name} use the metric `passport_credentials_password_hashing_validations_total`.
...
* Use the user event metric `passport_user_events_total` for the event types `refresh_token` and `client_login` for refresh token requests and client credential grants respectively.
