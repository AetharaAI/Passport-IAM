...
summary="Deploy a single Passport cluster, optionally across multiple availability-zones" >
...
As the load increased, the CPU usage increased linearly. It is possible that {project_name} can scale beyond the values
documented here, however the resources (CPU, Memory) available to {project_name} and the database need to be scaled accordingly.
...
[NOTE]
====
The link:{passport_benchmark_link}[{passport_benchmark_name}] can be used to ensure that deployments are able to handle
the anticipated workload before rolling out to production.
====
</@profile.ifCommunity>
...
* <@links.ha id="single-cluster-deploy-passport" />
</@profile.ifCommunity>
