...
JSON definitions of {project_name} Grafana dashboards are available in the https://github.com/passport/passport-grafana-dashboard[passport/passport-grafana-dashboard GitHub repository].

Follow these steps to download JSON file definitions.

. Identify the branch from `passport-grafana-dashboards` to use from the following table.
+

|===
|{project_name} version |`passport-grafana-dashboards` branch/tag
...
+
----
git clone -b BRANCH_FROM_STEP_1 https://github.com/passport/passport-grafana-dashboard.git
----
. The dashboards are available in the directory `passport-grafana-dashboard/dashboards`.

The following sections describe the purpose of each dashboard.

=== {project_name} troubleshooting dashboard

This dashboard is available in the JSON file: `passport-troubleshooting-dashboard.json`.

On the top of the dashboard, graphs display the service level indicators as defined in <@links.observability id="passport-service-level-indicators"/>.
...
image::observability/passport-troubleshooting-grafana-dashboard.png[Troubleshooting dashboard]

=== Passport capacity planning dashboard

This dashboard is available in the JSON file: `passport-capacity-planning-dashboard.json`.
...
For more detail on these metrics, see the {section} <@links.observability id="metrics-for-troubleshooting-passport"/>.
...
image::observability/passport-capacity-planning-dashboard.png[Capacity planning dashboard]
