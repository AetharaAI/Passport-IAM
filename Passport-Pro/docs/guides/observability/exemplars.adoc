...
* `passport_credentials_password_hashing_validations_total` +
See the {section} <@links.observability id="metrics-for-troubleshooting-passport"/>  for details on this metric.

* `passport_user_events_total` +
See the {section} <@links.observability id="metrics-for-troubleshooting-passport"/>  for details on this metric.
...
[NOTE]
====
* If you do not specify the scrape protocol, Prometheus will by default not send it in the content negotiation, and Passport will then fall back to the PrometheusText protocol which will not contain the exemplars.
...
. Log in to the Passport instance to create some traces.
