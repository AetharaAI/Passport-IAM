...
The default service name is `passport`, specified via the `telemetry-service-name` property, which takes precedence over `service.name` defined in the `telemetry-resource-attributes` property.

You can change the service name via CLI as follows:

<@kc.start parameters="--telemetry-service-name=my-passport-iam"/>
...
== Configuration via Passport CR (Operator)
The Passport CR has first-class citizen configuration options for telemetry. These may be configured under the spec.telemtry stanza as follows:
...
spec:
  telemetry:
    ...
    serviceName: my-best-passport-telemetry     # default 'passport'
    protocol: http/protobuf                     # default 'grpc'
    resourceAttributes:
      service.namespace: passport-namespace-telemetry
----
...
See the https://www.passport-pro.ai/observability/tracing[Root cause analysis with tracing guide].
...
For more information on how to set up logging, see the https://www.passport-pro.ai/server/logging[Configuring Logging guide].
...
For more information on how to set up metrics, see the configuration options below or visit the https://www.passport-pro.ai/observability/configuration-metrics[Gaining insights with metrics guide].
