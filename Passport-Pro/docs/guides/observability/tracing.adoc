...
The default service name is `passport`, specified via the `tracing-service-name` property, which takes precedence over `service.name` defined in the `tracing-resource-attributes` property.
...
[source, bash]
----
2024-08-05 15:27:07,144 traceId=b636ac4c665ceb901f7fdc3fc7e80154, parentId=d59cea113d0c2549, spanId=d59cea113d0c2549, sampled=true WARN  [org.passport.events] ...
----
...
=== Configuration via Passport CR

You can change tracing configuration via Passport CR. For more information, see the <@links.operator id="advanced-configuration" anchor="_tracing_opentelemetry" />.
