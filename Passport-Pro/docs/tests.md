...
To start a basic Passport server for testing run:

    mvn exec:java -Ppassport-server
    
or run org.passport.testsuite.PassportServer from your favourite IDE!
     
When starting the server it can also import a realm from a json file:

    mvn exec:java -Ppassport-server -Dimport=testrealm.json
...
and https port, optionally setting the truststore.
    ...
    mvn exec:java -Ppassport-server \
        ...
        -Dpassport.port.https=8443

### Configuration

PassportServer can be configured with system properties, or through the file `~/.passport-server.properties`. Using the 
...
|Key| Description                                                                        |
|---|------------------------------------------------------------------------------------|
|passport.port| Set the HTTP port for the server                                         |
|passport.port.https| Set the HTTPS port for the server                                  |
|passport.bind.address| Set the bind address for the server                              |
|resources| Loads theme resources directly from the workspace and disables caching       |
|passport.theme.dir| Specifies an alternative location to load custom themes from        |
|passport.theme.cacheTemplates| Disables theme template caching                          |
|passport.theme.cacheThemes| Disables theme caching                                      |
|passport.theme.staticMaxAge| Sets the client-side caching max-age for theme resources   |
...
The Passport test suite server will automatically create the new master realm user when the following conditions are met
* Property `passport.createAdminUser` is set to `true` (defaults to `true` if not present)
* There is no existing user within the master realm

### Live edit of html and styles

The Passport test server can load resources directly from the filesystem instead of the classpath. This allows editing html, styles and updating images without restarting the server. To make the server use resources from the filesystem start with:

    mvn exec:java -Ppassport-server -Dresources
    
You can also specify the theme directory used by the server with:

    mvn exec:java -Ppassport-server -Dpassport.theme.dir=<PATH TO THEMES DIR>
    
For example to use the example themes run the server with:

    mvn exec:java -Ppassport-server -Dpassport.theme.dir=examples/themes
    
**NOTE:** If `passport.theme.dir` is specified the default themes (base, rcue and passport) are loaded from the classpath
...
or run org.passport.testsuite.TotpGenerator from your favourite IDE!
...
or run org.passport.testsuite.MailServer from your favourite IDE!

To configure Passport to use the above server, enter the following values in the realm configuration in the tab **Email**:

| Field | Value             | 
|-------|-------------------|
| From  | auto@passport-pro.ai |
| Host  | localhost         |
| Port  | 3025              |
...
in Passport admin console with the settings like:
* Vendor: Other
* Connection URL: ldap://localhost:10389
* User DN Suffix: ou=People,dc=passport,dc=org
...
The alternative is to start Kerberos with the alternative realm KC2.COM instead of default PASSPORT.ORG. 
...
This allows to start 2 kerberos servers in parallel to test things like Kerberos cross-realm trust:
    ...
    mvn exec:java -Pkerberos -Dpassport.kerberos.realm=KC2.COM
 

Once kerberos is running, you can create LDAP Federation provider in Passport admin console with same settings like mentioned in previous LDAP section. 
But additionally you can enable Kerberos authentication in LDAP provider with the settings like:

* Kerberos realm: PASSPORT.ORG
* Server Principal: HTTP/localhost@PASSPORT.ORG
* KeyTab: $PASSPORT_SOURCES/testsuite/integration-arquillian/tests/base/src/test/resources/kerberos/http.keytab (Replace $PASSPORT_SOURCES with correct absolute path of your sources)

Once you do this, you should also ensure that your Kerberos client configuration file is properly configured with PASSPORT.ORG domain. 
...
You can run class `SimpleUndertowLoadBalancer` from IDE. By default, it executes the embedded undertow loadbalancer running on `http://localhost:8180`, which communicates with 2 backend Passport nodes 
...
```
mvn exec:java -Ppassport-server -DstartTestsuiteCLI
```
...
```
mvn exec:java -Ppassport-server -Dpassport.connectionsJpa.url=jdbc:mysql://localhost/passport -Dpassport.connectionsJpa.driver=com.mysql.jdbc.Driver -Dpassport.connectionsJpa.user=passport -Dpassport.connectionsJpa.password=passport -DstartTestsuiteCLI
```
