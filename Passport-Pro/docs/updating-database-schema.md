...
Passport supports automatically migrating the database to a new version. This is done by applying one or more change-sets
...
should contain a single `change-set` with `id` equal to the next version to be released and `author` set to the value `passport` 
...
The first step may be to re-build Passport and check if server is started fully. This means that server can be started against
