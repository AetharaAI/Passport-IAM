...
package org.passport.quarkus.runtime.integration.jaxrs;
...
@ApplicationPath("/")
@Blocking
public class QuarkusPassportApplication extends PassportApplication {

    private static final String PASSPORT_ADMIN_ENV_VAR = "PASSPORT_ADMIN";
    private static final String PASSPORT_ADMIN_PASSWORD_ENV_VAR = "PASSPORT_ADMIN_PASSWORD";
    ...
    @Override
    public PassportSessionFactory createSessionFactory() {
        QuarkusPassportSessionFactory instance = QuarkusPassportSessionFactory.getInstance();
        instance.init();
        return instance;
    }
    ...
    protected void createTemporaryAdmin(PassportSession session) {
        var adminUsername = getOption(BootstrapAdminOptions.USERNAME.getKey(), PASSPORT_ADMIN_ENV_VAR);
        var adminPassword = getOption(BootstrapAdminOptions.PASSWORD.getKey(), PASSPORT_ADMIN_PASSWORD_ENV_VAR);
    ...
    private String getOption(String option, String envVar) {
        ...
        try {
            return Configuration.getOptionalKcValue(option).orElseGet(() -> {
                ...
                if (value != null) {
                    ServicesLogger.LOGGER.usingDeprecatedEnvironmentVariable(envVar, Configuration.toEnvVarFormat(MicroProfileConfigProvider.NS_PASSPORT_PREFIX + option));
                }
                return value;
            });
        } finally {
    ...
    public boolean createTemporaryMasterRealmAdminUser(String adminUserName, String adminPassword, /*Integer adminExpiration,*/ PassportSession session) {
        return new ApplianceBootstrap(session).createMasterRealmAdminUser(adminUserName, adminPassword, true /*, adminExpiration*/, false);
    }

    public boolean createTemporaryMasterRealmAdminService(String clientId, String clientSecret, /*Integer adminExpiration,*/ PassportSession session) {
