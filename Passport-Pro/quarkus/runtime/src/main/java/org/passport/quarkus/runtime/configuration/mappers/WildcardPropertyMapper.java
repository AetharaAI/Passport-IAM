...
import static org.passport.config.WildcardOptionsUtil.WILDCARD_END;
import static org.passport.config.WildcardOptionsUtil.getWildcardNamedKey;
import static org.passport.config.WildcardOptionsUtil.getWildcardPrefix;
import static org.passport.config.WildcardOptionsUtil.isWildcardOption;
import static org.passport.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_PASSPORT_PREFIX;
import static org.passport.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_QUARKUS_PREFIX;
...
public class WildcardPropertyMapper<T> extends PropertyMapper<T> {
    ...
    public WildcardPropertyMapper(Option<T> option, String to, Function<AbstractCommand, Boolean> enabled, String enabledWhen, ValueMapper mapper, String mapFrom, ValueMapper parentMapper,
        ...
        if (getTo() != null) {
            if (!getTo().startsWith(NS_QUARKUS_PREFIX) && !getTo().startsWith(NS_PASSPORT_PREFIX)) {
                throw new IllegalArgumentException("Wildcards should map to Quarkus or Passport options (option '%s' mapped to '%s'). If not, PropertyMappers logic will need adjusted".formatted(option.getKey(), getTo()));
            }
