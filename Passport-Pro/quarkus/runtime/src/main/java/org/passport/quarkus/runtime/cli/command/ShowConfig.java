...
import static org.passport.quarkus.runtime.configuration.Configuration.getConfigValue;
import static org.passport.quarkus.runtime.configuration.Configuration.getPropertyNames;
import static org.passport.quarkus.runtime.configuration.mappers.PropertyMappers.maskValue;
...
public final class ShowConfig extends AbstractCommand {
    ...
    protected void runCommand() {
        ...
        spec.commandLine().getOut().printf("Current Mode: %s%n", Environment.getPassportModeFromProfile(profile));
        ...
        StreamSupport.stream(getPropertyNames().spliterator(), false).forEachOrdered(property -> {
            ...
            } else if (!property.startsWith(MicroProfileConfigProvider.NS_PASSPORT_PREFIX)) {
                return;
            }
    ...
    private void printProperty(String property, PropertyMapper<?> mapper, ConfigValue configValue) {
        ...
        spec.commandLine().getOut().printf("\t%s =  %s (%s)%n", property, value, PassportConfigSourceProvider.getConfigSourceDisplayName(sourceName));
    }
