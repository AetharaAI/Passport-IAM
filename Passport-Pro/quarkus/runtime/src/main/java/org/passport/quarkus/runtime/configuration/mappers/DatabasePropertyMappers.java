...
import org.passport.config.OptionBuilder;
...
import static org.passport.config.DatabaseOptions.Datasources.OPTIONS_DATASOURCES;
import static org.passport.config.DatabaseOptions.Datasources.getDatasourceOption;
import static org.passport.config.DatabaseOptions.Datasources.getKeyForDatasource;
import static org.passport.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_PASSPORT_PREFIX;
import static org.passport.quarkus.runtime.configuration.mappers.DatabasePropertyMappers.Datasources.appendDatasourceMappers;
...
final class DatabasePropertyMappers implements PropertyMapperGrouping {
     ...
     * Passport shutdown. We cannot rely on the default H2 ShutdownHook as this can result in the DB being
     ...
     * Agroal connection pool is closed on Passport shutdown.
     */
    private static String transformMinPoolSize(String database, ConfigSourceInterceptorContext context) {
        Supplier<String> getParentPoolMinSize = () -> Optional.ofNullable(context.proceed(NS_PASSPORT_PREFIX + DatabaseOptions.DB_POOL_MIN_SIZE.getKey()))
