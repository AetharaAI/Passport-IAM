...
/**
 * A configuration source for {@code passport.conf}.
 */
public class PassportPropertiesConfigSource extends AbstractLocationConfigSourceLoader {

    public static final int PROPERTIES_FILE_ORDINAL = 475;

    private static final String PASSPORT_CONFIG_FILE_ENV = "KC_CONFIG_FILE";
    private static final String PASSPORT_CONF_FILE = "passport.conf";
    public static final String PASSPORT_CONFIG_FILE_PROP = NS_PASSPORT_PREFIX + "config.file";
    ...
    public static class InFileSystem extends PassportPropertiesConfigSource implements ConfigSourceProvider {
        ...
        public Path getConfigurationFile() {
            String filePath = System.getProperty(PASSPORT_CONFIG_FILE_PROP);

            if (filePath == null) {
                filePath = System.getenv(PASSPORT_CONFIG_FILE_ENV);
            }

            if (filePath == null) {
                filePath = Environment.getHomeDir()
                        .map(f -> Paths.get(f, "conf", PassportPropertiesConfigSource.PASSPORT_CONF_FILE).toFile())
    ...
    private static Map<String, String> transform(Map<String, String> properties) {
        ...
        properties.entrySet().forEach(entry -> {
            result.put(MicroProfileConfigProvider.NS_PASSPORT_PREFIX + entry.getKey(), entry.getValue());
        });
