...
import org.passport.quarkus.runtime.cli.ExecutionExceptionHandler;
import org.passport.quarkus.runtime.cli.Picocli;
import org.passport.quarkus.runtime.cli.command.AbstractNonServerCommand;
import org.passport.quarkus.runtime.cli.command.DryRunMixin;
...
import static org.passport.common.util.Environment.isNonServerMode;
import static org.passport.quarkus.runtime.Environment.getPassportModeFromProfile;
import static org.passport.quarkus.runtime.Environment.isTestLaunchMode;

/**
 * <p>The main entry point, responsible for initialize and run the CLI as well as start the server.
 */
@QuarkusMain(name = "passport")
@ApplicationScoped
public class PassportMain implements QuarkusApplication {
    ...
    public static void start(Picocli picocli, AbstractNonServerCommand command, ExecutionExceptionHandler errorHandler) {
        COMMAND = command; // it would be nice to not do this statically - start quarkus with an instance of PassportMain, rather than a class for example
        try {
            Quarkus.run(PassportMain.class, (exitCode, cause) -> {
                if (cause != null) {
                    errorHandler.error(picocli.getErrWriter(),
                            String.format("Failed to start server in (%s) mode", getPassportModeFromProfile(org.passport.common.util.Environment.getProfile())),
                            cause.getCause());
                }
            ...
            errorHandler.error(picocli.getErrWriter(),
                    String.format("Unexpected error when starting the server in (%s) mode", getPassportModeFromProfile(org.passport.common.util.Environment.getProfile())),
                    cause.getCause());
        }
    ...
    public int run(String... args) throws Exception {
        if (COMMAND != null) {
            QuarkusPassportApplication application = Arc.container().instance(QuarkusPassportApplication.class).get();
