...
package org.passport.quarkus.runtime.storage.database.liquibase;
...
import org.passport.connections.jpa.updater.liquibase.LiquibaseConstants;
import org.passport.connections.jpa.updater.liquibase.ThreadLocalSessionContext;
import org.passport.connections.jpa.updater.liquibase.conn.PassportLiquibase;
import org.passport.connections.jpa.updater.liquibase.conn.LiquibaseConnectionProvider;
import org.passport.connections.jpa.updater.liquibase.conn.MySQLCustomChangeLogHistoryService;
...
public class QuarkusJpaUpdaterProvider implements JpaUpdaterProvider {
    ...
    public QuarkusJpaUpdaterProvider(PassportSession session) {
    ...
    private void update(Connection connection, File file, String defaultSchema) {
        ...
        try {
            if (needVerifyMasterChangelog()) {
                // Run update with passport master changelog first
                PassportLiquibase liquibase = getLiquibaseForPassportUpdate(connection, defaultSchema);
                if (file != null) {
            ...
            for (JpaEntityProvider jpaProvider : jpaProviders) {
                ...
                if (customChangelog != null) {
                    ...
                    PassportLiquibase liquibase = getLiquibaseForCustomProviderUpdate(connection, defaultSchema, customChangelog, jpaProvider.getClass().getClassLoader(), changelogTableName);
                    updateChangeSet(liquibase, exportWriter);
                }
            }
    ...
    protected void updateChangeSet(PassportLiquibase liquibase, Writer exportWriter) throws LiquibaseException  {
        ...
        // See https://issues.jboss.org/browse/PASSPORT-3769 for discussion relevant to why reset needs to be here
    ...
    private void outputChangeLogTableCreationScript(Liquibase liquibase, final Writer exportWriter) throws DatabaseException {
        ...
        loggingExecutor.comment("* Passport database creation script - apply this script to empty DB *");
        ...
        // in org.passport.connections.jpa.updater.liquibase.lock.CustomLockService.init() called indirectly from
        // PassportApplication constructor (search for waitForLock() call). Hence it is not included in the creation script.
    ...
    public Status validate(Connection connection, String defaultSchema) {
        ...
        try {
            if (needVerifyMasterChangelog()) {
                // Validate with passport master changelog first
                PassportLiquibase liquibase = getLiquibaseForPassportUpdate(connection, defaultSchema);
    ...
    protected Status validateChangeSet(PassportLiquibase liquibase, String changelog) throws LiquibaseException {
        final Status result;
        List<ChangeSet> changeSets = getLiquibaseUnrunChangeSets(liquibase);
    ...
    private void resetLiquibaseServices(PassportLiquibase liquibase) {
    ...
    private PassportLiquibase getLiquibaseForPassportUpdate(Connection connection, String defaultSchema) throws LiquibaseException {
        LiquibaseConnectionProvider liquibaseProvider = session.getProvider(LiquibaseConnectionProvider.class);
        return liquibaseProvider.getLiquibase(connection, defaultSchema);
    }

    private PassportLiquibase getLiquibaseForCustomProviderUpdate(Connection connection, String defaultSchema, String changelogLocation, ClassLoader classloader, String changelogTableName) throws LiquibaseException {
        LiquibaseConnectionProvider liquibaseProvider = session.getProvider(LiquibaseConnectionProvider.class);
