...
import static org.passport.config.LoggingOptions.DEFAULT_LOG_FORMAT;
import static org.passport.config.LoggingOptions.DEFAULT_SYSLOG_OUTPUT;
...
public class LoggingConfigurationTest extends AbstractConfigurationTest {
    ...
    public void syslogDefaults() {
        ...
        assertExternalConfig(Map.of(
                ...
                "quarkus.log.syslog.app-name", "passport",
    ...
    public void syslogDifferentValues() {
        putEnvVars(Map.of(
                ...
                "KC_LOG_SYSLOG_APP_NAME", "passport2",
        ...
        assertExternalConfig(Map.of(
                ...
                "quarkus.log.syslog.app-name", "passport2",
    ...
    public void logLevelTakesPrecedenceOverCategoryLevel() {
        ConfigArgsConfigSource.setCliArgs("--log-level=org.passport:error");
        SmallRyeConfig config = createConfig();
        assertEquals("INFO", config.getConfigValue("quarkus.log.level").getValue());
        assertEquals("ERROR", config.getConfigValue("quarkus.log.category.\"org.passport\".level").getValue());

        onAfter();
        ConfigArgsConfigSource.setCliArgs("--log-level=org.passport:error", "--log-level-org.passport=trace");
        config = createConfig();
        assertEquals("INFO", config.getConfigValue("quarkus.log.level").getValue());
        assertEquals("TRACE", config.getConfigValue("quarkus.log.category.\"org.passport\".level").getValue());
    }

    @Test
    public void unknownCategoryLevelIsResolvedFromRootLevel() {
        ConfigArgsConfigSource.setCliArgs("--log-level=warn,org.passport:error", "--log-level-org.passport=trace");
        SmallRyeConfig config = createConfig();
    ...
    public void testWildcardCliOptionCanBeMappedToQuarkusOption() {
        ConfigArgsConfigSource.setCliArgs("--log-level-org.passport=trace");
        SmallRyeConfig config = createConfig();
        assertEquals("TRACE", config.getConfigValue("quarkus.log.category.\"org.passport\".level").getValue());
    ...
    public void testWildcardEnvVarOptionCanBeMappedToQuarkusOption() {
        ...
        assertNull(config.getConfigValue("quarkus.log.category.\"org.passport\".level").getValue());
