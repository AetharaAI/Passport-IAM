...
package org.passport.quarkus.runtime.cli;
...
import org.passport.quarkus.runtime.PassportMain;
import org.passport.quarkus.runtime.cli.command.AbstractAutoBuildCommand;
import org.passport.quarkus.runtime.cli.command.AbstractCommand;
import org.passport.quarkus.runtime.configuration.AbstractConfigurationTest;
...
public class PicocliTest extends AbstractConfigurationTest {
    ...
    private class NonRunningPicocli extends Picocli {
        ...
        public void initConfig(AbstractCommand command) {
            PassportConfigSourceProvider.reload();
    ...
    NonRunningPicocli pseudoLaunch(String... args) {
        ...
        PassportMain.main(args, nonRunningPicocli);
    ...
    public void testReaugFromProdToDev() {
        ...
        assertEquals("dev", nonRunningPicocli.buildProps.getProperty(org.passport.common.util.Environment.PROFILE));
    }
    ...
    public void testReaugFromDevToProdExport() {
        ...
        assertEquals(AbstractAutoBuildCommand.REBUILT_EXIT_CODE, nonRunningPicocli.exitCode);
        assertTrue(nonRunningPicocli.reaug);
        assertEquals("prod", nonRunningPicocli.buildProps.getProperty(org.passport.common.util.Environment.PROFILE));
    }
    ...
    public void wrongLevelForCategory() {
        NonRunningPicocli nonRunningPicocli = pseudoLaunch("start-dev", "--log-level-org.passport", "wrong");
        assertEquals(CommandLine.ExitCode.USAGE, nonRunningPicocli.exitCode);
    ...
    public void wildcardLevelForCategory() {
        NonRunningPicocli nonRunningPicocli = pseudoLaunch("start-dev", "--log-level-org.passport=warn");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        var value = nonRunningPicocli.config.getConfigValue("quarkus.log.category.\"org.passport\".level");
        assertEquals("quarkus.log.category.\"org.passport\".level", value.getName());
        assertEquals("WARN", value.getValue());
        value = nonRunningPicocli.config.getConfigValue("quarkus.log.category.\"org.passport1\".level");
        assertEquals("quarkus.log.category.\"org.passport1\".level", value.getName());
    ...
    public void httpAccessLog() {
        ...
        assertExternalConfig("quarkus.http.access-log.masked-cookies", "AUTH_SESSION_ID,KC_AUTH_SESSION_HASH,PASSPORT_IDENTITY,PASSPORT_SESSION,AUTH_SESSION_ID_LEGACY,PASSPORT_IDENTITY_LEGACY,PASSPORT_SESSION_LEGACY");
        onAfter();
        ...
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig("quarkus.http.access-log.masked-cookies", "AUTH_SESSION_ID,KC_AUTH_SESSION_HASH,PASSPORT_IDENTITY,PASSPORT_SESSION,AUTH_SESSION_ID_LEGACY,PASSPORT_IDENTITY_LEGACY,PASSPORT_SESSION_LEGACY,MY_CUSTOM_COOKIE");
        onAfter();
        ...
        assertExternalConfig(Map.of(
                ...
                "quarkus.http.access-log.base-file-name", "passport-http-access",
    ...
    public void telemetryParentHeaders() {
        ...
        nonRunningPicocli = pseudoLaunch(enableAll.apply("--telemetry-header-Something=passport"));
        assertNoError(nonRunningPicocli);
        assertExternalConfig("quarkus.otel.exporter.otlp.traces.headers", "Authorization=Bearer asdlkfjadsflkj,Something=passport");
        assertExternalConfig("quarkus.otel.exporter.otlp.logs.headers", "Authorization=Bearer asdlkfjadsflkj,Something=passport");
        assertExternalConfig("quarkus.otel.exporter.otlp.metrics.headers", "Authorization=Bearer asdlkfjadsflkj,Something=passport");
        onAfter();
    ...
    public void otelTracing() {
        ...
        // combined endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--tracing-enabled=true", "--telemetry-endpoint=http://passport-pro.ai", "--tracing-endpoint=http://passport2.org");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.endpoint").getValue(), is("http://passport-pro.ai")); // Tracing option takes precedence
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.traces.endpoint").getValue(), is("http://passport2.org")); // Tracing option takes precedence
        onAfter();

        // telemetry endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--tracing-enabled=true", "--telemetry-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.traces.endpoint").getValue(), is("http://passport-pro.ai"));
        onAfter();

        // parent + child endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--tracing-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455", "--tracing-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.endpoint").getValue(), is("http://passport-passport-passport-pro.ai:3455"));
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.traces.endpoint").getValue(), is("http://passport-pro.ai"));
        onAfter();

        // parent endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--tracing-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.endpoint").getValue(), is("http://passport-passport-passport-pro.ai:3455")); // value inherited
        assertThat(nonRunningPicocli.config.getConfigValue("quarkus.otel.exporter.otlp.traces.endpoint").getValue(), is("http://passport-passport-passport-pro.ai:3455")); // value inherited
    }
    ...
    public void otelLogs() {
        ...
        // otel endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-logs", "--telemetry-logs-enabled=true", "--telemetry-logs-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig("quarkus.otel.exporter.otlp.logs.endpoint", "http://passport-pro.ai");
        onAfter();

        // parent + child endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-logs", "--telemetry-logs-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455", "--telemetry-logs-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig(Map.of(
                "quarkus.otel.exporter.otlp.endpoint", "http://passport-passport-passport-pro.ai:3455",
                "quarkus.otel.exporter.otlp.logs.endpoint", "http://passport-pro.ai"
        ));
        onAfter();

        // parent endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-logs", "--telemetry-logs-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig(Map.of(
                "quarkus.otel.exporter.otlp.endpoint", "http://passport-passport-passport-pro.ai:3455",
                "quarkus.otel.exporter.otlp.logs.endpoint", "http://passport-passport-passport-pro.ai:3455"
        ));
        onAfter();
    ...
    public void otelMetrics() {
        ...
        // otel endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-metrics", "--metrics-enabled=true", "--telemetry-metrics-enabled=true", "--telemetry-metrics-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig(Map.of(
                "quarkus.otel.enabled", "true",
                "quarkus.otel.exporter.otlp.endpoint", "http://localhost:4317",
                "quarkus.otel.exporter.otlp.metrics.endpoint", "http://passport-pro.ai"
        ));
        onAfter();

        // parent + child endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-metrics", "--metrics-enabled=true", "--telemetry-metrics-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455", "--telemetry-metrics-endpoint=http://passport-pro.ai");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig(Map.of(
                "quarkus.otel.enabled", "true",
                "quarkus.otel.exporter.otlp.endpoint", "http://passport-passport-passport-pro.ai:3455",
        ...
        // parent endpoint
        nonRunningPicocli = pseudoLaunch("start-dev", "--features=opentelemetry-metrics", "--metrics-enabled=true", "--telemetry-metrics-enabled=true", "--telemetry-endpoint=http://passport-passport-passport-pro.ai:3455");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertExternalConfig(Map.of(
                "quarkus.otel.enabled", "true",
                "quarkus.otel.exporter.otlp.endpoint", "http://passport-passport-passport-pro.ai:3455",
                "quarkus.otel.exporter.otlp.metrics.endpoint", "http://passport-passport-passport-pro.ai:3455"
        ));
        onAfter();
    ...
    public void testWindowsServiceOnWin() {
        ...
        PassportMain.main(new String[] {"tools", "windows-service", "--help"}, nonRunningPicocli);
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        assertTrue(nonRunningPicocli.getOutString().contains("install"));
        onAfter();
        PassportMain.main(new String[] {"tools", "windows-service"}, nonRunningPicocli);
        assertEquals(CommandLine.ExitCode.USAGE, nonRunningPicocli.exitCode);
        assertTrue(nonRunningPicocli.getErrString().contains("Missing required subcommand"));
        onAfter();
        PassportMain.main(new String[] {"tools", "windows-service", "uninstall", "--db=bar"}, nonRunningPicocli);
        assertEquals(CommandLine.ExitCode.USAGE, nonRunningPicocli.exitCode);
    }
