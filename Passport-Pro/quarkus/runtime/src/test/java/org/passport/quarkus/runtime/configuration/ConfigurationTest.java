...
package org.passport.quarkus.runtime.configuration;
...
import org.passport.quarkus.runtime.configuration.mappers.HttpPropertyMappers;
import org.passport.quarkus.runtime.vault.FilesKeystoreVaultProviderFactory;
import org.passport.quarkus.runtime.vault.FilesPlainTextVaultProviderFactory;
...
public class ConfigurationTest extends AbstractConfigurationTest {
    ...
    @Test
    public void testPassportConfPlaceholder() {
    ...
    public void testDefaultValueFromProperty() {
        System.setProperty("passport.frontendUrl", "http://defaultvalueprop.unittest");
    ...
    public void testPropertyMapping() {
        ConfigArgsConfigSource.setCliArgs("--db=mariadb", "--db-url=jdbc:mariadb://localhost/passport");
        SmallRyeConfig config = createConfig();
        assertEquals(MariaDBDialect.class.getName(), config.getConfigValue("kc.db-dialect").getValue());
        assertEquals("jdbc:mariadb://localhost/passport", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
    }
    ...
    public void testDatabaseDefaults() {
        ...
        assertEquals("jdbc:h2:file:" + Environment.getHomeDir().orElseThrow() + "/data/h2/passportdb;NON_KEYWORDS=VALUE;DB_CLOSE_ON_EXIT=FALSE;DB_CLOSE_DELAY=0", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());

        ConfigArgsConfigSource.setCliArgs("--db=dev-mem");
        config = createConfig();
        assertEquals(H2Dialect.class.getName(), config.getConfigValue("kc.db-dialect").getValue());
        assertEquals("jdbc:h2:mem:passportdb;NON_KEYWORDS=VALUE;DB_CLOSE_ON_EXIT=FALSE;DB_CLOSE_DELAY=0", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
    ...
    public void testDatabaseKindProperties() {
        ConfigArgsConfigSource.setCliArgs("--db=postgres", "--db-url=jdbc:postgresql://localhost/passport", "--db-username=postgres");
        SmallRyeConfig config = createConfig();
        assertEquals("org.hibernate.dialect.PostgreSQLDialect",
            config.getConfigValue("kc.db-dialect").getValue());
        assertEquals("jdbc:postgresql://localhost/passport", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
    ...
    public void testSetDbUrlOverridesDefaultDataSource() {
        ConfigArgsConfigSource.setCliArgs("--db=mariadb", "--db-url-host=myhost", "--db-url=jdbc:mariadb://localhost/passport");
        SmallRyeConfig config = createConfig();
    ...
    public void testDatabaseProperties() {
        ...
        SmallRyeConfig config = createConfig();
        assertEquals(H2Dialect.class.getName(), config.getConfigValue("kc.db-dialect").getValue());
        assertEquals("jdbc:h2:file:test-dir/data/h2/passportdb;;test=test;test1=test1;NON_KEYWORDS=VALUE;DB_CLOSE_ON_EXIT=FALSE;DB_CLOSE_DELAY=0", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
        ...
        config = createConfig();
        assertEquals("jdbc:mariadb://localhost:3306/passport?test=test&test1=test1", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
        assertEquals(MariaDBDialect.class.getName(), config.getConfigValue("kc.db-dialect").getValue());
        assertEquals(MariaDbDataSource.class.getName(), config.getConfigValue("quarkus.datasource.jdbc.driver").getValue());

        ConfigArgsConfigSource.setCliArgs("--db=postgres");
        config = createConfig();
        assertEquals("jdbc:postgresql://localhost:5432/passport?test=test&test1=test1", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
        ...
        assertEquals("jdbc:postgresql://localhost:5432/passport?targetServerType=any", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());

        ConfigArgsConfigSource.setCliArgs("--db=postgres", "--db-driver=software.amazon.jdbc.Driver");
        config = createConfig();
        assertNull(config.getConfigValue("quarkus.datasource.jdbc.additional-jdbc-properties.targetServerType").getValue());

        ConfigArgsConfigSource.setCliArgs("--db=postgres", "--db-url=jdbc:postgresql://localhost:5432/passport?targetServerType=any");
        config = createConfig();
        assertNull(config.getConfigValue("quarkus.datasource.jdbc.additional-jdbc-properties.targetServerType").getValue());
    }
    ...
    public void testNestedDatabaseProperties() {
        SmallRyeConfig config = createConfig();
        assertEquals("jdbc:h2:file:"+Environment.getHomeDir().orElseThrow()+"/data/passportdb", config.getConfigValue("quarkus.datasource.foo").getValue());
    ...
    public void testDevThemeProperties() {
        ...
        System.setProperty(org.passport.common.util.Environment.PROFILE, "dev");
    ...
    public void testClusterConfig() {
        ...
        System.clearProperty(org.passport.common.util.Environment.PROFILE);
    ...
    public void testDatabaseDriverSetExplicitly() {
        ConfigArgsConfigSource.setCliArgs("--db=mssql", "--db-url=jdbc:sqlserver://localhost/passport");
        ...
        SmallRyeConfig config = createConfig();
        assertEquals("jdbc:sqlserver://localhost/passport", config.getConfigValue("quarkus.datasource.jdbc.url").getValue());
        assertEquals("mssql", config.getConfigValue("quarkus.datasource.db-kind").getValue());
    ...
    public void testResolvePropertyFromDefaultProfile() {
        Environment.setProfile(org.passport.common.util.Environment.NON_SERVER_MODE);
        var config = createConfig();
    ...
    @Test
    public void testPassportConfQuarkusPropertyNotUsed() {
        ConfigArgsConfigSource.setCliArgs("");
        SmallRyeConfig config = createConfig();
    ...
    @Test
    public void testQuarkusLogPropDependentUponPassport() {
