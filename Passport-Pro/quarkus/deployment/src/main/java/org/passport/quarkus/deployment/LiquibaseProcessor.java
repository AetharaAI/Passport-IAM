...
import org.passport.connections.jpa.updater.liquibase.lock.DummyLockService;
...
import static org.passport.quarkus.deployment.PassportProcessor.getDefaultDataSource;

class LiquibaseProcessor {

    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    void configure(PassportRecorder recorder, List<JdbcDataSourceBuildItem> jdbcDataSources, CombinedIndexBuildItem indexBuildItem) {
    ...
    private void filterImplementations(Class<?> types, String dbKind, Set<ClassInfo> classes) {
        if (Database.class.equals(types)) {
            ...
            classes.removeIf(classInfo -> !org.passport.config.database.Database.isLiquibaseDatabaseSupported(classInfo.name().toString(), dbKind));
        }
    }
}
