...
import org.passport.config.LoggingOptions;
import org.passport.config.ManagementOptions;
...
import org.passport.config.TransactionOptions;
import org.passport.config.database.Database;
...
import org.passport.connections.jpa.updater.liquibase.LiquibaseJpaUpdaterProviderFactory;
import org.passport.connections.jpa.updater.liquibase.conn.DefaultLiquibaseConnectionProvider;
import org.passport.infinispan.util.InfinispanUtils;
import org.passport.policy.BlacklistPasswordPolicyProviderFactory;
...
import org.passport.quarkus.runtime.PassportRecorder;
import org.passport.quarkus.runtime.cli.Picocli;
import org.passport.quarkus.runtime.configuration.Configuration;
import org.passport.quarkus.runtime.configuration.PassportConfigSourceProvider;
import org.passport.quarkus.runtime.configuration.MicroProfileConfigProvider;
import org.passport.quarkus.runtime.configuration.PersistedConfigSource;
import org.passport.quarkus.runtime.configuration.PropertyMappingInterceptor;
import org.passport.quarkus.runtime.configuration.mappers.PropertyMapper;
import org.passport.quarkus.runtime.configuration.mappers.PropertyMappers;
import org.passport.quarkus.runtime.configuration.mappers.WildcardPropertyMapper;
import org.passport.quarkus.runtime.integration.resteasy.PassportHandlerChainCustomizer;
import org.passport.quarkus.runtime.integration.resteasy.PassportTracingCustomizer;
import org.passport.quarkus.runtime.logging.ClearMappedDiagnosticContextFilter;
import org.passport.quarkus.runtime.services.health.PassportClusterReadyHealthCheck;
import org.passport.quarkus.runtime.services.health.PassportReadyHealthCheck;
import org.passport.quarkus.runtime.storage.database.jpa.NamedJpaConnectionProviderFactory;
import org.passport.quarkus.runtime.themes.FlatClasspathThemeResourceProviderFactory;
import org.passport.quarkus.runtime.validation.HibernateValidatorFactoryCustomizer;
...
import org.passport.theme.FolderThemeProviderFactory;
import org.passport.theme.JarThemeProviderFactory;
import org.passport.theme.ThemeResourceSpi;
import org.passport.transaction.JBossJtaTransactionManagerLookup;
...
import org.passport.vault.FilesKeystoreVaultProviderFactory;
import org.passport.vault.FilesPlainTextVaultProviderFactory;
...
import static org.passport.config.DatabaseOptions.DB;
import static org.passport.connections.jpa.util.JpaUtils.loadSpecificNamedQueries;
import static org.passport.quarkus.runtime.Environment.getCurrentOrCreateFeatureProfile;
import static org.passport.quarkus.runtime.Providers.getProviderManager;
import static org.passport.quarkus.runtime.configuration.Configuration.getOptionalBooleanKcValue;
import static org.passport.quarkus.runtime.configuration.Configuration.getOptionalKcValue;
import static org.passport.quarkus.runtime.configuration.Configuration.getOptionalValue;
import static org.passport.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_PASSPORT_PREFIX;
import static org.passport.quarkus.runtime.storage.database.jpa.QuarkusJpaConnectionProviderFactory.DEFAULT_PERSISTENCE_UNIT;
import static org.passport.quarkus.runtime.storage.database.jpa.QuarkusJpaConnectionProviderFactory.QUERY_PROPERTY_PREFIX;
import static org.passport.representations.provider.ScriptProviderDescriptor.AUTHENTICATORS;
import static org.passport.representations.provider.ScriptProviderDescriptor.MAPPERS;
import static org.passport.representations.provider.ScriptProviderDescriptor.POLICIES;
import static org.passport.representations.provider.ScriptProviderDescriptor.SAML_MAPPERS;
import static org.passport.theme.ClasspathThemeProviderFactory.PASSPORT_THEMES_JSON;

class PassportProcessor {

    private static final Logger logger = Logger.getLogger(PassportProcessor.class);
    ...
    private static final String PASSPORT_SCRIPTS_JSON_PATH = "META-INF/passport-scripts.json";
    ...
    static {
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(AUTHENTICATORS, PassportProcessor::registerScriptAuthenticator);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(POLICIES, PassportProcessor::registerScriptPolicy);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(MAPPERS, PassportProcessor::registerScriptMapper);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(SAML_MAPPERS, PassportProcessor::registerSAMLScriptMapper);
    }
    ...
    FeatureBuildItem getFeature() {
        return new FeatureBuildItem("passport");
    }
     ...
     * <p>
     * It prevents issues like https://github.com/passport/passport/issues/45501
     */
    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    @Consume(RuntimeConfigSetupCompleteBuildItem.class)
    @Produce(ConfigBuildItem.class)
    void initConfig(PassportRecorder recorder) {
    ...
    void configureProfile(PassportRecorder recorder) {
    ...
    @BuildStep
    @Consume(ConfigBuildItem.class)
    void filterAllRequests(BuildProducer<FilterBuildItem> filters, PassportRecorder recorder) {
    ...
    @Consume(ConfigBuildItem.class)
    void filterAllManagementRequests(BuildProducer<ManagementInterfaceFilterBuildItem> filters, PassportRecorder recorder) {
        var filter = recorder.getRejectNonNormalizedPathFilter();
        if (filter != null) {
    ...
    @BuildStep
    @Consume(ConfigBuildItem.class)
    void configureTruststore(PassportRecorder recorder) {
    ...
    @Record(ExecutionTime.STATIC_INIT)
    void setDefaultUserProfileConfig(PassportRecorder recorder, UserProfileBuildItem configuration) {
    ...
    void checkPersistenceUnits(List<PersistenceXmlDescriptorBuildItem> descriptors) {
        ...
        List<String> notSetPersistenceUnitsDBKinds = descriptors.stream()
                ...
                .map(PassportProcessor::getDatasourceNameFromPersistenceXml)
    ...
    private boolean missingDbKind(String datasourceName) {
        String key = NS_PASSPORT_PREFIX.concat(DatabaseOptions.Datasources.getNamedKey(DatabaseOptions.DB, datasourceName).orElseThrow());
        ...
        try {
            ...
            if (!missing) {
                logger.warnf(
                        "You have set DB kind for '%s' datasource via a Quarkus property. This approach is deprecated and you should use the Passport 'db-kind-%s' property.",
    ...
    void configurePersistenceUnits(HibernateOrmConfig config,
        ...
        for (PersistenceXmlDescriptorBuildItem item : descriptors) {
            ...
            if (DEFAULT_PERSISTENCE_UNIT.equals(descriptor.getName())) {
                ...
                runtimeConfigured.produce(new HibernateOrmIntegrationRuntimeConfiguredBuildItem("passport", defaultUnitDescriptor.getName())
                ...
                runtimeConfigured.produce(new HibernateOrmIntegrationRuntimeConfiguredBuildItem("passport", descriptor.getName())
    ...
    private void configureDefaultPersistenceUnitEntities(ParsedPersistenceXmlDescriptor descriptor, CombinedIndexBuildItem indexBuildItem,
        ...
        for (AnnotationInstance annotation : annotations) {
            ...
            if (!userManagedEntities.contains(targetName)
                    && (!targetName.startsWith("org.passport") || targetName.startsWith("org.passport.testsuite"))) {
    ...
    @Produce(PassportSessionFactoryPreInitBuildItem.class)
    void configurePassportSessionFactory(PassportRecorder recorder, List<PersistenceXmlDescriptorBuildItem> descriptors) {
    ...
    private List<ClasspathThemeProviderFactory.ThemesRepresentation> loadThemesFromClassPath() {
        try {
            ...
            Enumeration<URL> resources = Thread.currentThread().getContextClassLoader().getResources(PASSPORT_THEMES_JSON);
    ...
    void configureConfigSources(BuildProducer<StaticInitConfigBuilderBuildItem> configSources) {
        configSources.produce(new StaticInitConfigBuilderBuildItem(PassportConfigSourceProvider.class.getName()));
    }
    ...
    void prepareTestEnvironment(BuildProducer< StaticInitConfigBuilderBuildItem> configSources, DevServicesDatasourceResultBuildItem dbConfig) {
        configSources.produce(new StaticInitConfigBuilderBuildItem("org.passport.quarkus.runtime.configuration.test.TestPassportConfigSourceProvider"));
    ...
    /**
     * This will cause quarkus to include specified modules in the jandex index. For example passport-services is needed as it includes
    ...
    void index(BuildProducer<IndexDependencyBuildItem> indexDependencyBuildItemBuildProducer) {
        ...
        indexDependencyBuildItemBuildProducer.produce(new IndexDependencyBuildItem("org.passport", "passport-services"));
    ...
    void indexJpaStore(BuildProducer<IndexDependencyBuildItem> indexDependencyBuildItemBuildProducer) {
        indexDependencyBuildItemBuildProducer.produce(new IndexDependencyBuildItem("org.passport", "passport-model-jpa"));
    }
    ...
    private static void disableClusterHealthCheck(BuildProducer<BuildTimeConditionBuildItem> removeBeans, CombinedIndexBuildItem index) {
        ClassInfo clusterHealth = index.getIndex().getClassByName(DotName.createSimple(PassportClusterReadyHealthCheck.class));
    ...
    private static void disableReadyHealthCheck(BuildProducer<BuildTimeConditionBuildItem> removeBeans, CombinedIndexBuildItem index) {
        ClassInfo disabledBean = index.getIndex().getClassByName(DotName.createSimple(PassportReadyHealthCheck.class.getName()));
        removeBeans.produce(new BuildTimeConditionBuildItem(disabledBean.asClass(), false));
    }

    @BuildStep(onlyIfNot = IsTest.class) // needed for embedded Passport
    @Consume(ProfileBuildItem.class)
    ...
    void configureResteasy(CombinedIndexBuildItem index,
        ...
        chainCustomizers.add(new PassportHandlerChainCustomizer());

        if (Configuration.isTrue(TracingOptions.TRACING_ENABLED)) {
            chainCustomizers.add(new PassportTracingCustomizer());
            ...
            preExceptionMapperHandlerBuildItemBuildProducer
                    .produce(new PreExceptionMapperHandlerBuildItem(new PassportTracingCustomizer.EndHandler()));
        }
    ...
    @BuildStep
    @Record(ExecutionTime.STATIC_INIT)
    void setCryptoProvider(PassportRecorder recorder) {
        FipsMode fipsMode = getOptionalValue(NS_PASSPORT_PREFIX + SecurityOptions.FIPS_MODE.getKey())
    ...
    void configureDevMode(BuildProducer<HotDeploymentWatchedFileBuildItem> hotFiles) {
        hotFiles.produce(new HotDeploymentWatchedFileBuildItem("META-INF/passport.conf"));
    }

    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    void configureProtoStreamSchemas(PassportRecorder recorder) {
    ...
    private Map<String, ProviderFactory<?>> loadDeployedScriptProviders(ClassLoader classLoader, Spi spi) {
        ...
        if (supportsDeployeableScripts(spi)) {
            try {
                Enumeration<URL> descriptorsUrls = classLoader.getResources(PASSPORT_SCRIPTS_JSON_PATH);
    ...
    private List<ScriptProviderDescriptor> getScriptProviderDescriptorsFromJarFile(URL url) throws IOException {
        ...
        try (JarFile jarFile = new JarFile(file.substring("file:".length(), file.indexOf(JAR_FILE_SEPARATOR)))) {
            JarEntry descriptorEntry = jarFile.getJarEntry(PASSPORT_SCRIPTS_JSON_PATH);
    ...
    private boolean isInternal(ProviderFactory<?> factory) {
        ...
        return packageName.startsWith("org.passport") && !packageName.startsWith("org.passport.examples");
    }
    ...
    private void checkProviders(Spi spi,
        ...
        if (provider != null) {
            ...
            String defaultProvider = DefaultPassportSessionFactory.resolveDefaultProvider(factories, spi);
