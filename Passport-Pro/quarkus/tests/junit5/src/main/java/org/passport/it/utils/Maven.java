...
public final class Maven {
    ...
    public static Path getPassportQuarkusModulePath() {
        // Find passport-parent module first
        ...
        for (LocalProject m = ctx.getCurrentProject(); m != null; m = m.getLocalParent()) {
            if ("passport-parent".equals(m.getArtifactId())) {
                // When found, advance to quarkus module
                return m.getDir().resolve("quarkus");
            }
        }

        throw new RuntimeException("Failed to find passport-parent module.");
    }
