...
import org.passport.quarkus.runtime.cli.command.Start;
import org.passport.quarkus.runtime.cli.command.StartDev;
import org.passport.quarkus.runtime.configuration.Configuration;
import org.passport.quarkus.runtime.integration.QuarkusPlatform;
...
import static org.passport.it.junit5.extension.DistributionTest.ReInstall.BEFORE_ALL;
import static org.passport.it.junit5.extension.DistributionType.RAW;
import static org.passport.quarkus.runtime.Environment.forceTestLaunchMode;

public class CLITestExtension extends QuarkusMainTestExtension {

    private static final String SYS_PROPS = "sys-props";
    private PassportDistribution dist;
    ...
    public void beforeEach(ExtensionContext context) throws Exception {
        ...
        if (distConfig != null) {
            ...
            Passport.initSys(launch == null ? new String[] {} : launch.value());
    ...
    private void copyTestProvider(TestProvider provider) {
        ...
        if (isRaw()) {
            try {
                dist.unwrap(RawPassportDistribution.class).copyProvider(provider.value().getDeclaredConstructor().newInstance());
    ...
    private PassportDistribution createDistribution(DistributionTest config, Storage storeConfig, WithDatabase databaseConfig) {
        return new PassportDistributionDecorator(storeConfig, databaseConfig, config, DistributionType.getCurrent().orElse(RAW).newInstance(config));
    }
    ...
    public Object resolveParameter(ParameterContext parameterContext, ExtensionContext context)
        ...
        if (type.equals(PassportDistribution.class)) {
    ...
    public boolean supportsParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
            throws ParameterResolutionException {
        Class<?> type = parameterContext.getParameter().getType();
        return type == LaunchResult.class || type == CLIResult.class || type == RawDistRootPath.class || type == PassportDistribution.class;
    }
    ...
    private RawDistRootPath getDistPath(){
        return new RawDistRootPath(dist.unwrap(RawPassportDistribution.class).getDistPath());
    }
}
