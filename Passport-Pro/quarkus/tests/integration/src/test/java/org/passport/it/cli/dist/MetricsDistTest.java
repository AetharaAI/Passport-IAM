...
public class MetricsDistTest {
    ...
    @Test
    @Launch({ "start-dev" })
    void testMetricsEndpointNotEnabled(PassportDistribution distribution) {
    ...
    void testMetricsEndpoint(CLIResult cliResult) {
        // See https://github.com/passport/passport/issues/36927
    ...
    void testMetricsEndpointWithCacheMetricsHistogramsAndExemplars(PassportDistribution distribution) {
    ...
    void testMetricsEndpointWithUserEventMetrics(PassportDistribution distribution) {
        runClientCredentialGrantWithUnknownClientId(distribution);

        distribution.setRequestPort(9000);
        when().get("/metrics").then()
                .statusCode(200)
                .body(containsString("passport_user_events_total{error=\"client_not_found\",event=\"client_login\",realm=\"master\"}"));

    }

    @Test
    @Launch({ "start-dev", "--metrics-enabled=true", "--features=user-event-metrics", "--event-metrics-user-enabled=false" })
    void testMetricsEndpointWithoutUserEventMetrics(PassportDistribution distribution) {
        runClientCredentialGrantWithUnknownClientId(distribution);

        distribution.setRequestPort(9000);
        when().get("/metrics").then()
                .statusCode(200)
                .body(not(containsString("passport_user_events_total{error=\"client_not_found\",event=\"client_login\",realm=\"master\"}")));

    }

    private static void runClientCredentialGrantWithUnknownClientId(PassportDistribution distribution) {
        distribution.setRequestPort(8080);
    ...
    @Test
    void testUsingRelativePath(PassportDistribution distribution) {
    ...
    @Test
    void testMultipleRequests(PassportDistribution distribution) throws Exception {
