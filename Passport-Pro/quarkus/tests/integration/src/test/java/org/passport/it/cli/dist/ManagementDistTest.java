...
import org.passport.it.junit5.extension.DistributionType;
...
public class ManagementDistTest {
    ...
    void testManagementDisabled(LaunchResult result, PassportDistribution distribution) {
        CLIResult cliResult = (CLIResult) result;
        cliResult.assertNoMessage("Management interface listening on");
    ...
    void testManagementEnabled(LaunchResult result) {
        ...
        when().get("/").then()
                .statusCode(200)
                .and()
                .body(containsString("Passport Management Interface"),
    ...
    void testManagementDifferentPort(LaunchResult result, PassportDistribution distribution) {
        CLIResult cliResult = (CLIResult) result;
    ...
    void testManagementRootRedirects(LaunchResult result, PassportDistribution distribution) {
        assertRelativePath(result, "/management");

        distribution.setRequestPort(8080);

        given().redirects().follow(false).when().get("/").then().statusCode(302).header("Location", is("/auth"));
        when().get("/").then().statusCode(200).body(containsString("Welcome to Passport"));
        when().get("/auth").then().statusCode(200).body(containsString("Welcome to Passport"));
    }
