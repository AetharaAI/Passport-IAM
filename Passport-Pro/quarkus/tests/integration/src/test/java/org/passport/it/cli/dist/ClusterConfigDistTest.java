...
import org.passport.it.junit5.extension.Storage;
...
public class ClusterConfigDistTest {
    ...
    @Test
    @Launch({ "start", "--cache=ispn", "--log-level=info,org.passport.connections.infinispan:debug", "--http-enabled=true", "--hostname-strict=false"})
    ...
    @Test
    void testAbsoluteCacheFile(PassportDistribution dist, @TempDir Path tempDir) throws Exception {
    ...
    public static class ConfigureCacheUsingAsyncEncryption implements Consumer<PassportDistribution> {
    ...
    public static class ConfigureCustomCache implements Consumer<PassportDistribution> {
