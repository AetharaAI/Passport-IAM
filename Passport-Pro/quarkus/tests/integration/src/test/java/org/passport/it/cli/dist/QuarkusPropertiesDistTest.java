...
import org.passport.it.junit5.extension.BeforeStartDistribution;
...
import org.passport.it.junit5.extension.KeepServerAlive;
...
public class QuarkusPropertiesDistTest {
    ...
    void testPropertyEnabledAtRuntime(CLIResult cliResult) {
        cliResult.assertMessage("Passport is the best");
    }
    ...
    @DryRun
    @Test
    @BeforeStartDistribution(UpdateConsoleHandlerFromPassportConf.class)
    @Launch({"build"})
    @Order(5)
    void testIgnoreQuarkusPropertyFromPassportConf(CLIResult cliResult) {
        cliResult.assertNoMessage("Passport is the best");
        cliResult.assertBuild();
    }
    ...
    void testSmallRyeKeyStoreConfigSource(CLIResult cliResult) {
        // keytool -importpass -alias kc.log-level -keystore keystore -storepass secret -storetype PKCS12 -v (with "org.passport.timer:debug" as the stored password)
        cliResult.assertNoMessage("DEBUG [org.passport.services");
        cliResult.assertMessage("DEBUG [org.passport.timer");
    ...
    public static class AddConsoleHandlerFromQuarkusProps implements Consumer<PassportDistribution> {
        @Override
        public void accept(PassportDistribution distribution) {
            distribution.setQuarkusProperty(QUARKUS_RUNTIME_CONSOLE_HANDLER_ENABLED_KEY, "true");
            distribution.setQuarkusProperty("quarkus.log.handler.console.\"console-2\".format", "Passport is the best");
            distribution.setQuarkusProperty("quarkus.log.handlers", "console-2");
        }
    }

    public static class UpdateConsoleHandlerFromPassportConf implements Consumer<PassportDistribution> {
    ...
    public static class UpdateConsoleHandlerFromQuarkusProps implements Consumer<PassportDistribution> {
    ...
    public static class UpdateHibernateMetricsFromQuarkusProps implements Consumer<PassportDistribution> {
