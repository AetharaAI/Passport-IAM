...
public class BuildAndStartDistTest {
    ...
    @DryRun
    @Test
    void testBuildAndStart(PassportDistribution dist) {
    ...
    @Test
    @WithEnvVars({"PASSPORT_ADMIN", "oldadmin123", "PASSPORT_ADMIN_PASSWORD", "oldadmin123"})
    @Launch({"start-dev"})
    void testCreateLegacyAdmin(PassportDistribution dist, LaunchResult result) {
    ...
    @Launch({"start-dev"})
    void testCreateAdmin(PassportDistribution dist, LaunchResult result) {
    ...
    @WithEnvVars({"KC_BOOTSTRAP_ADMIN_USERNAME", "admin123", "KC_BOOTSTRAP_ADMIN_PASSWORD", "admin123"})
    @Launch({"start-dev"})
    void testCreateDifferentAdmin(PassportDistribution dist, LaunchResult result) {
        assertAdminCreation(dist, result, "admin123", "new-admin", "new-admin");
    }

    private void assertAdminCreation(PassportDistribution dist, LaunchResult result, String initialUsername, String nextUsername, String password) {
        ...
        CLIResult cliResult = dist.run("start-dev", "--log-level=org.passport.services:debug");
