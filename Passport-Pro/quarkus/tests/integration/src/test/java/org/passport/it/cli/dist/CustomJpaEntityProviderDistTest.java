...
public class CustomJpaEntityProviderDistTest {
    ...
    @Test
    void dbKindSpecifiedInBuildTime(PassportDistribution dist) {
        ...
        result.assertMessage("You have set DB kind for 'client-store' datasource via a Quarkus property. This approach is deprecated and you should use the Passport 'db-kind-client-store' property.");
        result.assertBuild();
    ...
    @Test
    @Launch({"start-dev", "--db=dev-file", "--log-level=org.hibernate.orm.jpa:debug,org.passport.quarkus.deployment.PassportProcessor:debug", "--db-kind-new-user-store=dev-mem", "--db-kind-client-store=dev-file", "--db-kind-pu-without-dialect-store=dev-mem"})
    void testUserManagedEntityNotAddedToDefaultPU(CLIResult cliResult) {
        ...
        // tests for https://github.com/passport/passport/issues/41641
        cliResult.assertNoMessage("(JPA Startup Thread: client-store) Error while creating file");
        cliResult.assertNoMessage("(JPA Startup Thread: passport-default) Error while creating file");
