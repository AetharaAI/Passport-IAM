...
public class TruststoreDistTest {
    ...
    @Test
    void testMutualAuthWithTruststorePaths(PassportDistribution dist) {
        String[] truststoreNames = new String[] { "passport-truststore.p12", "self-signed.pem" };
        ...
        rawDist.run("--verbose", "start", "--db=dev-file", "--http-enabled=true", "--hostname=mypassport-pro.ai",
        ...
        given().trustStore(TruststoreDistTest.class.getResource("/self-signed-truststore.p12").getPath(), TruststoreBuilder.DUMMY_PASSWORD)
                ...
                .get("https://mypassport-pro.ai:8443").then().body(Matchers.containsString("https://mypassport-pro.ai"));
    }

    @Test
    void testMutualAuthWithDefaultTruststoresDir(PassportDistribution dist) {
        String[] truststoreNames = new String[] { "passport-truststore.p12", "self-signed.pem" };
        Stream.of(truststoreNames).forEach(truststoreName -> {
