...
import org.passport.it.junit5.extension.TestProvider;
import org.passport.it.junit5.extension.WithEnvVars;
import org.passport.it.resource.realm.TestRealmResourceTestProvider;
...
public class ProxyHostnameV2DistTest {
    ...
    public void testSchemeAndPortFromRequestWhenNoProxySet() {
        assertFrontEndUrl("http://mypassport-pro.ai:8080", "http://mypassport-pro.ai:8080/");
    ...
    @Test
    void testTrustedProxiesWithoutProxyHeaders(PassportDistribution distribution) {
    ...
    @Test
    void testTrustedProxiesWithInvalidAddress(PassportDistribution distribution) {
    ...
    public void testProxyNotTrusted() {
        assertForwardedHeaderIsIgnored();
        assertForwardedHeaderIsIgnored();
        given().header("X-Forwarded-Host", "test:123").when().get("http://mypassport-pro.ai:8080/realms/master/test-resources/trusted").then().statusCode(204);
    }
    ...
    public void testProxyTrusted() {
        given().header("X-Forwarded-Host", "test:123").when().get("http://mypassport-pro.ai:8080/realms/master/test-resources/trusted").then().statusCode(200);
    }
    ...
    @Test
    @Launch({ "start-dev", "--hostname=https://mypassport-pro.ai:8443/path", "--proxy-headers=forwarded", "--hostname-backchannel-dynamic=true" })
    public void testForwardedProxyHeadersWithPathAndDynamicBackchannel(LaunchResult result) {
        assertFrontEndUrl("https://mypassport-pro.ai:8443", "https://mypassport-pro.ai:8443/path/");
        // a backend url generated via the frontend protocol/host/port should be a front-end url
        assertBackEndUrl("https://mypassport-pro.ai:8443", "https://mypassport-pro.ai:8443/path/");
    ...
    private void assertForwardedHeader() {
        ...
        assertForwardedHeader("http://mypassport-pro.ai:8080/realms/master/protocol/openid-connect/auth?client_id=security-admin-console", "https://test:1234/admin", ADDRESS);
        assertForwardedHeader("http://mypassport-pro.ai:8080/realms/master/protocol/openid-connect/auth?client_id=security-admin-console", "https://test:1234/admin", NOT_ADDRESS);
    }
    ...
    private void assertXForwardedHeaders() {
        ...
        given().header("X-Forwarded-Host", "test:123").when().get("http://mypassport-pro.ai:8080").then().header(HttpHeaders.LOCATION, containsString("http://test:123/admin"));
    ...
    private void assertXForwardedHeadersAreIgnored() {
        given().header("X-Forwarded-Host", "test").when().get("http://mypassport-pro.ai:8080").then().header(HttpHeaders.LOCATION, containsString("http://mypassport-pro.ai:8080/admin"));
