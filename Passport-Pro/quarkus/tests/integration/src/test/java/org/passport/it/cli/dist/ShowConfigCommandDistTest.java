...
public class ShowConfigCommandDistTest {
    ...
    void testShowConfigPicksUpRightConfigDependingOnCurrentMode(PassportDistribution distribution) {
    ...
    @Test
    @RawDistOnly(reason = "Containers are immutable")
    void testShowConfigCommandHidesCredentialsInProfiles(PassportDistribution distribution) {
        CLIResult result = distribution.run(String.format("%s=%s", CONFIG_FILE_LONG_NAME, Paths.get("src/test/resources/ShowConfigCommandTest/passport.conf").toAbsolutePath().normalize()), ShowConfig.NAME, "all");
    ...
    @Test
    @RawDistOnly(reason = "Containers are immutable")
    void testSmallRyeKeyStoreConfigSource(PassportDistribution distribution) {
        // keystore is shared with QuarkusPropertiesDistTest#testSmallRyeKeyStoreConfigSource
        CLIResult result = distribution.run(String.format("%s=%s", CONFIG_FILE_LONG_NAME, Paths.get("src/test/resources/ShowConfigCommandTest/passport-keystore.conf").toAbsolutePath().normalize()), ShowConfig.NAME, "all");
        String output = result.getOutput();
    ...
    @Test
    @RawDistOnly(reason = "Containers are immutable")
    void testConfigSourceNames(PassportDistribution distribution) {
        ...
        result = distribution.run(String.format("%s=%s", CONFIG_FILE_LONG_NAME, Paths.get("src/test/resources/ShowConfigCommandTest/passport-keystore.conf").toAbsolutePath().normalize()), ShowConfig.NAME, "all", "--db=dev-file");
        ...
        result.assertMessage("(passport-keystore.conf)");
    }
}
