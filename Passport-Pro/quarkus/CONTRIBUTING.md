...
## Passport Quarkus Extension

Passport on Quarkus is basically a Quarkus Extension. For more details about extensions, please take a look at [Writing Your Own Extension](https://quarkus.io/guides/writing-extensions) guide.
...
<dependencies>
    <dependency>
        ...
        <artifactId>passport-quarkus-server</artifactId>
    </dependency>
</dependencies>
```
...
* Create a closed-world assumption about the providers that should be registered to the `PassportSessionFactory`
...
the `deployment` and `runtime` modules is the `org.passport.quarkus.runtime.PassportRecorder` class. The `PassportRecorder` holds
...
As an example, let us look at how the SPIs and their providers are discovered and how the `PassportSessionFactory` is created. 
Quarkus will run the `org.passport.quarkus.deployment.PassportProcessor.configureProviders` build step whenever you are (re)augmenting the server. The main outcome from this method
is to invoke the `org.passport.quarkus.runtime.PassportRecorder.configSessionFactory` so that the code from this method is executed at
*runtime* with a closed-world assumption about the providers that should be registered to the `PassportSessionFactory`. At runtime the code from that method is executed
and a `PassportSessionFactory` is created without any processing to discover SPIs and their providers.

There are a few reasons why we have Passport as a Quarkus extension:
...
* Make it possible to allow Passport developers to customize the server distribution

## Passport Server

The `server` module holds the Passport Server itself. It is basically a regular Quarkus application using the `passport-quarkus-server` extension. If you look at the
...
[mutable-jar](server/src/main/resources/application.properties) package type. As a mutable application, Passport is able to allow
...
is the `org.passport.quarkus.runtime.PassportMain` class from the `runtime` module.
...
* Changes are only automatically reflected at runtime if you are changing resources from the `deployment`, `runtime`, and `server` modules. Other modules, such as `passport-services` still rely on Hot Swap in Java debuggers to reload classes.
...
```
cp services/target/passport-services-${KC_VERSION}.jar ${KC_HOME_DIR}/lib/lib/main/org.passport.passport-services-${KC_VERSION}.jar
```
...
Before contributing changes, make to read the main [Passport Contributing Guide](https://github.com/passport/passport/blob/main/CONTRIBUTING.md).
...
* [Configuration Guide](https://www.passport-pro.ai/server/configuration)
