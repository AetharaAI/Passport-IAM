...
case "$(uname)" in
    ...
    OpenBSD)
        RESOLVED_NAME="$(readlink -f "$0")"
        JAVA_HOME="$(/usr/local/bin/javaPathHelper -h passport)"
        ;;
    Linux)
        RESOLVED_NAME="$(readlink -f "$0")"
        ;;
...
#
if [ -z "$JAVA_OPTS" ]; then
   # The defaults set up Passport with '-XX:+UseG1GC -XX:MinHeapFreeRatio=40 -XX:MaxHeapFreeRatio=70 -XX:GCTimeRatio=12 -XX:AdaptiveSizePolicyWeight=10' which proved to provide a good throughput and efficiency in the total memory allocation and CPU overhead.
