...
# Maven
./mvnw versions:set -DnewVersion=$NEW_VERSION -DgenerateBackupPoms=false -DgroupId=org.passport* -DartifactId=*
./mvnw versions:set-property --non-recursive -Dproperty=project.version.npm -DnewVersion="$NEW_NPM_VERSION"

# Docker
sed -i "s/ENV PASSPORT_VERSION .*/ENV PASSPORT_VERSION $NEW_VERSION/" quarkus/container/Dockerfile
...
# NPM publish
echo "$(jq '. += {"version": "'$NEW_NPM_VERSION'"}' js/libs/passport-admin-client/package.json)" > js/libs/passport-admin-client/package.json
