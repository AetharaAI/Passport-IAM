...
public class LDAPDnTest {
    ...
    public void testDn() throws Exception {
        LDAPDn dn = LDAPDn.fromString("dc=passport, dc=org");
        dn.addFirst("ou", "People");
        Assert.assertEquals("ou=People,dc=passport,dc=org", dn.toString());

        dn.addFirst("uid", "Johny,Depp+Pepp\\Foo");
        Assert.assertEquals("uid=Johny\\,Depp\\+Pepp\\\\Foo,ou=People,dc=passport,dc=org", dn.toString());
        Assert.assertEquals(LDAPDn.fromString("uid=Johny\\,Depp\\+Pepp\\\\Foo,ou=People,dc=passport,dc=org"), dn);

        Assert.assertEquals("ou=People,dc=passport,dc=org", dn.getParentDn().toString());

        Assert.assertTrue(dn.isDescendantOf(LDAPDn.fromString("dc=passport, dc=org")));
        Assert.assertTrue(dn.isDescendantOf(LDAPDn.fromString("dc=org")));
        Assert.assertTrue(dn.isDescendantOf(LDAPDn.fromString("DC=passport, DC=org")));
        Assert.assertFalse(dn.isDescendantOf(LDAPDn.fromString("dc=passportk, dc=org")));
    ...
    public void testEmptyRDN() throws Exception {
        ...
        Assert.assertEquals("ou=,dc=passport,dc=org", dn.toString());

        dn.addFirst("uid", "Johny,Depp+Pepp\\Foo");
        Assert.assertEquals("uid=Johny\\,Depp\\+Pepp\\\\Foo,ou=,dc=passport,dc=org", dn.toString());

        dn = LDAPDn.fromString("uid=Johny\\,Depp\\+Pepp\\\\Foo,ou=,O=passport,C=org");
        Assert.assertTrue(dn.isDescendantOf(LDAPDn.fromString("ou=, O=passport,C=org")));
        Assert.assertTrue(dn.isDescendantOf(LDAPDn.fromString("OU=, o=passport,c=org")));
        Assert.assertFalse(dn.isDescendantOf(LDAPDn.fromString("ou=People, O=passport,C=org")));
    }
    ...
    public void testCorrectEscape() throws Exception {
        ...
        Assert.assertEquals("cn=Johny\\,Džýa Foo,dc=passport,dc=org", dn.toString());
        Assert.assertEquals("Johny,Džýa Foo", dn.getFirstRdn().getAttrValue("cn"));

        dn = LDAPDn.fromString("dc=passport, dc=org");
        dn.addFirst("cn", "Johny,Džýa Foo ");
        Assert.assertEquals("cn=Johny\\,Džýa Foo\\ ,dc=passport,dc=org", dn.toString());
        Assert.assertEquals("Johny,Džýa Foo ", dn.getFirstRdn().getAttrValue("cn"));
        ...
        Assert.assertEquals("cn=Johny\\,Džýa\\ ,dc=passport,dc=org", dn.toString());
    ...
    public void testDNWithMultivaluedRDN() throws Exception {
        LDAPDn dn = LDAPDn.fromString("cn=John Do\\+eř+uid=john,dc=passport+ou=foo, dc=org");
        ...
        Assert.assertEquals("dc=passport+ou=foo,dc=org", dn.getParentDn().toString());
