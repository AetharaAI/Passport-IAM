...
package org.passport.storage.ldap.mappers.membership.group;
...
import org.passport.storage.ldap.mappers.AbstractLDAPStorageMapper;
import org.passport.storage.ldap.mappers.membership.CommonLDAPGroupMapper;
import org.passport.storage.ldap.mappers.membership.CommonLDAPGroupMapperConfig;
...
import org.passport.storage.ldap.mappers.membership.UserRolesRetrieveStrategy;
...
public class GroupLDAPStorageMapper extends AbstractLDAPStorageMapper implements CommonLDAPGroupMapper {
    ...
    public SynchronizationResult syncDataFromFederationProviderToPassport(RealmModel realm) {
        ...
        logger.debugf("Syncing groups from LDAP into Passport DB. Mapper is [%s], LDAP provider is [%s]", mapperModel.getName(), ldapProvider.getModel().getName());
        ...
        if (config.isPreserveGroupsInheritance()) {
            try {
                ...
                updatePassportGroupTree(realm, groupTrees, ldapGroupsMap, syncResult);
    ...
    private void syncExistingGroup(RealmModel realm, GroupModel kcExistingGroup, Map.Entry<String, LDAPObject> groupEntry,
        ...
        try {
            ...
            PassportModelUtils.runJobInTransaction(ldapProvider.getSession().getPassportSessionFactory(), session -> {
    ...
    private void syncFlatGroupStructure(RealmModel realm, SynchronizationResult syncResult, Map<String, LDAPObject> ldapGroupsMap) {
        ...
        for (Iterator<Map.Entry<String, LDAPObject>> it = entries.iterator(); it.hasNext(); ) {
            ...
            PassportModelUtils.runJobInTransaction(ldapProvider.getSession().getPassportSessionFactory(), session -> {
                ...
                // PASSPORT-8253 The retrieval of the current realm to operate at, was intentionally left
        ...
        // Possibly remove passport groups, which don't exist in LDAP
        if (config.isDropNonExistingGroupsDuringSync()) {
            dropNonExistingKcGroups(realm, syncResult, visitedGroupIds);
        }
    }

    private void updatePassportGroupTree(RealmModel realm, List<GroupTreeResolver.GroupTreeEntry> groupTrees, Map<String, LDAPObject> ldapGroups, SynchronizationResult syncResult) {
        Set<String> visitedGroupIds = new HashSet<>();

        for (GroupTreeResolver.GroupTreeEntry groupEntry : groupTrees) {
            updatePassportGroupTreeEntry(realm, groupEntry, ldapGroups, null, syncResult, visitedGroupIds);
        }
    ...
    private void updatePassportGroupTreeEntry(RealmModel realm, GroupTreeResolver.GroupTreeEntry groupTreeEntry, Map<String, LDAPObject> ldapGroups, GroupModel kcParent, SynchronizationResult syncResult, Set<String> visitedGroupIds) {
        ...
        if (kcGroup != null) {
            logger.debugf("Updated Passport group '%s' from LDAP", kcGroup.getName());
        ...
        for (GroupTreeResolver.GroupTreeEntry childEntry : groupTreeEntry.getChildren()) {
            updatePassportGroupTreeEntry(realm, childEntry, ldapGroups, kcGroup, syncResult, visitedGroupIds);
        }
    }

    private void dropNonExistingKcGroups(RealmModel realm, SynchronizationResult syncResult, Set<String> visitedGroupIds) {
        // Remove passport groups, which don't exist in LDAP
        ...
        getAllKcGroups(realm, parent)
                ...
                .forEach(kcGroup -> {
                    logger.debugf("Removing Passport group '%s', which doesn't exist in LDAP", kcGroup.getName());
    ...
    public SynchronizationResult syncDataFromPassportToFederationProvider(RealmModel realm) {
        ...
        logger.debugf("Syncing groups from Passport into LDAP. Mapper is [%s], LDAP provider is [%s]", mapperModel.getName(), ldapProvider.getModel().getName());
        ...
        // Map to track all LDAP groups also exist in Passport
        ...
        getKcSubGroups(realm, null)
                .forEach(kcGroup -> processPassportGroupSyncToLDAP(kcGroup, ldapGroupsMap, ldapGroupNames, syncResult));
        ...
        if (config.isPreserveGroupsInheritance()) {
            getKcSubGroups(realm, null)
                    .forEach(kcGroup -> processPassportGroupMembershipsSyncToLDAP(kcGroup, ldapGroupsMap));
        }
    ...
    private void processPassportGroupSyncToLDAP(GroupModel kcGroup, Map<String, LDAPObject> ldapGroupsMap, Set<String> ldapGroupNames, SynchronizationResult syncResult) {
        ...
        kcGroup.getSubGroupsStream()
                .forEach(kcSubgroup -> processPassportGroupSyncToLDAP(kcSubgroup, ldapGroupsMap, ldapGroupNames, syncResult));
    }

    // Update memberships of group in LDAP based on subgroups from KC. Do it recursively
    private void processPassportGroupMembershipsSyncToLDAP(GroupModel kcGroup, Map<String, LDAPObject> ldapGroupsMap) {
        ...
        for (GroupModel kcSubgroup : kcSubgroups) {
            processPassportGroupMembershipsSyncToLDAP(kcSubgroup, ldapGroupsMap);
        }
    }
    ...
    public List<UserModel> getGroupMembers(RealmModel realm, GroupModel kcGroup, int firstResult, int maxResults) {
        if (config.getMode() == LDAPGroupMapperMode.IMPORT) {
            // only results from Passport should be returned, or imported LDAP and KC items will duplicate
    ...
    public void addGroupMappingInLDAP(RealmModel realm, GroupModel kcGroup, LDAPObject ldapUser) {
        ...
        if (ldapGroup == null) {
            // Needs to partially sync Passport groups to LDAP
            if (config.isPreserveGroupsInheritance()) {
                ...
                processPassportGroupSyncToLDAP(highestGroupToSync, syncedLDAPGroups, new HashSet<>(), new SynchronizationResult());
                processPassportGroupMembershipsSyncToLDAP(highestGroupToSync, syncedLDAPGroups);
                ...
                processPassportGroupSyncToLDAP(kcGroup, new HashMap<>(), new HashSet<>(), new SynchronizationResult());
                ldapGroup = loadLDAPGroupByName(groupName);
            }
        }
    ...
    public void onImportUserFromLDAP(LDAPObject ldapUser, UserModel user, RealmModel realm, boolean isCreate) {
        ...
        if (mode == LDAPGroupMapperMode.IMPORT && isCreate) {
            ...
            if (!ldapGroups.isEmpty()) {
                GroupModel parent = getKcGroupsPathGroup(realm);
                // Import role mappings from LDAP into Passport DB
                for (LDAPObject ldapGroup : ldapGroups) {
    ...
    protected GroupModel getKcGroupsPathGroup(RealmModel realm) {
        return config.isTopLevelGroupsPath() ? null : PassportModelUtils.findGroupByPath(getSession(), realm, config.getGroupsPath());
    }
    ...
    protected boolean isGroupInGroupPath(RealmModel realm, GroupModel group) {
        ...
        GroupModel groupPathGroup = PassportModelUtils.findGroupByPath(getSession(), realm, config.getGroupsPath());
