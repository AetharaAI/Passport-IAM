...
package org.passport.storage.ldap;
...
import org.passport.storage.ldap.idm.query.internal.LDAPQueryConditionsBuilder;
import org.passport.storage.ldap.idm.store.ldap.LDAPIdentityStore;
...
import org.passport.storage.ldap.mappers.KerberosPrincipalAttributeMapperFactory;
import org.passport.storage.ldap.mappers.LDAPConfigDecorator;
...
import org.passport.storage.ldap.mappers.msad.MSADUserAccountControlStorageMapper;
import org.passport.storage.ldap.mappers.msad.MSADUserAccountControlStorageMapperFactory;
...
public class LDAPStorageProviderFactory implements UserStorageProviderFactory<LDAPStorageProvider>, ImportSynchronization {
    ...
    @Override
    public LDAPStorageProvider create(PassportSession session, ComponentModel model) {
    ...
    protected Map<ComponentModel, LDAPConfigDecorator> getLDAPConfigDecorators(PassportSession session, ComponentModel ldapModel) {
        ...
        return realm.getComponentsStream(ldapModel.getId(), LDAPStorageMapper.class.getName())
                .filter(mapperModel -> session.getPassportSessionFactory()
                ...
                .collect(Collectors.toMap(Function.identity(), mapperModel ->
                        (LDAPConfigDecorator) session.getPassportSessionFactory()
    ...
    public void validateConfiguration(PassportSession session, RealmModel realm, ComponentModel config) throws ComponentValidationException {
        ...
        if (config.getId() == null) {
            ...
            config.setId(PassportModelUtils.generateShortId());
        }
    }
    ...
    public void onCreate(PassportSession session, RealmModel realm, ComponentModel model) {
        ...
        mapperModel = PassportModelUtils.createComponentModel("username", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID, LDAPStorageMapper.class.getName(),
        ...
        if (ldapConfig.getRdnLdapAttribute().equalsIgnoreCase(LDAPConstants.CN)) {
            ...
            if (usernameLdapAttribute.equalsIgnoreCase(LDAPConstants.CN)) {
                ...
                mapperModel = PassportModelUtils.createComponentModel("first name", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
                ...
                if (editMode == UserStorageProvider.EditMode.WRITABLE) {
                    ...
                    mapperModel = PassportModelUtils.createComponentModel("username-cn", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
                            UserAttributeLDAPStorageMapper.USER_MODEL_ATTRIBUTE, UserModel.USERNAME,
                    ...
                    mapperModel = PassportModelUtils.createComponentModel("full name", model.getId(), FullNameLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        mapperModel = PassportModelUtils.createComponentModel("last name", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        mapperModel = PassportModelUtils.createComponentModel("email", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        mapperModel = PassportModelUtils.createComponentModel("creation date", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        mapperModel = PassportModelUtils.createComponentModel("modify date", model.getId(), UserAttributeLDAPStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        if (activeDirectory) {
            mapperModel = PassportModelUtils.createComponentModel("MSAD account controls", model.getId(), MSADUserAccountControlStorageMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
        ...
        if (kerberosConfig.getKerberosPrincipalAttribute() != null) {
            mapperModel = PassportModelUtils.createComponentModel("Kerberos principal attribute mapper", model.getId(), KerberosPrincipalAttributeMapperFactory.PROVIDER_ID, LDAPStorageMapper.class.getName());
            realm.addComponentModel(mapperModel);
        }
        ...
        if (!activeDirectory && syncRegistrations && ldapConfig.useExtendedPasswordModifyOp()) {
            mapperModel = PassportModelUtils.createComponentModel("random initial password", model.getId(), HardcodedLDAPAttributeMapperFactory.PROVIDER_ID,LDAPStorageMapper.class.getName(),
    ...
    public SynchronizationResult sync(PassportSessionFactory sessionFactory, String realmId, UserStorageProviderModel model) {
        ...
        try (LDAPQuery userQuery = createQuery(sessionFactory, realmId, model)) {
            ...
            // TODO: Remove all existing passport users, which have federation links, but are not in LDAP. Perhaps don't check users, which were just added or updated during this sync?
    ...
    protected void syncMappers(PassportSessionFactory sessionFactory, final String realmId, final ComponentModel model) {
                ...
                realm.getComponentsStream(model.getId(), LDAPStorageMapper.class.getName())
                        .forEach(mapperModel -> {
                            SynchronizationResult syncResult = session.getProvider(LDAPStorageMapper.class, mapperModel)
                                    .syncDataFromFederationProviderToPassport(realm);
    ...
    protected SynchronizationResult syncImpl(PassportSessionFactory sessionFactory, LDAPQuery userQuery, final String realmId, final ComponentModel fedModel) {
     ...
     * @return
     */
    private LDAPQuery createQuery(PassportSessionFactory sessionFactory, final String realmId, final ComponentModel model) {
        class QueryHolder {
            LDAPQuery query;
        }
    ...
    protected SynchronizationResult importLdapUsers(PassportSessionFactory sessionFactory, final String realmId, final ComponentModel fedModel, List<LDAPObject> ldapUsers) {
        final SynchronizationResult syncResult = new SynchronizationResult();
                    ...
                    public void run(PassportSession session) {
                        ...
                        if (!userModelOptional.isPresent() && currentUserLocal == null) {
                            ...
                            if ((fedModel.getId().equals(currentUser.getFederationLink())) && (ldapUser.getUuid().equals(currentUser.getFirstAttribute(LDAPConstants.LDAP_ID)))) {
                                ...
                                logger.warnf("User with ID '%s' is not updated during sync as he already exists in Passport database but is not linked to federation provider '%s'", ldapUser.getUuid(), fedModel.getName());
