...
public interface LDAPStorageMapper extends Provider {
    ...
    /**
     * Sync data from federated storage to Passport. It's useful just if mapper needs some data preloaded from federated storage (For example
     * load roles from federated provider and sync them to Passport database)
     *
     * Applicable just if sync is supported
     *
     */
    SynchronizationResult syncDataFromFederationProviderToPassport(RealmModel realm);

    /**
     * Sync data from Passport back to federated storage
     *
     **/
    SynchronizationResult syncDataFromPassportToFederationProvider(RealmModel realm);
    ...
    /**
     * Called when importing user from LDAP to local passport DB.
     *
     * @param ldapUser
     * @param user
     * @param realm
     * @param isCreate true if we importing new user from LDAP. False if user already exists in Passport, but we are upgrading (syncing) it from LDAP
     */
    void onImportUserFromLDAP(LDAPObject ldapUser, UserModel user, RealmModel realm, boolean isCreate);


    /**
     * Called when register new user to LDAP - just after user was created in Passport DB
     *
     * @param ldapUser
     * @param localUser
     * @param realm
    ...
    /**
     * Method that returns user model attributes, which this mapper maps to Passport users
     *
