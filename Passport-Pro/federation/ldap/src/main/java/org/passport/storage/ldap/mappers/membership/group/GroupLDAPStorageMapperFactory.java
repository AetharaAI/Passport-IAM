...
import org.passport.storage.ldap.mappers.AbstractLDAPStorageMapperFactory;
...
public class GroupLDAPStorageMapperFactory extends AbstractLDAPStorageMapperFactory {
    ...
    private static List<ProviderConfigProperty> getProps(ComponentModel parent) {
        ...
        ProviderConfigurationBuilder config = ProviderConfigurationBuilder.create()
                ...
                .helpText("Flag whether group inheritance from LDAP should be propagated to Passport. If false, then all LDAP groups will be mapped as flat top-level groups in Passport. Otherwise group inheritance is " +
                        "preserved into Passport, but the group sync might fail if LDAP structure contains recursions or multiple parent groups per child groups")
        ...
        if (importEnabled) {
            config.property().name(GroupMapperConfig.MODE)
                    ...
                    .helpText("LDAP_ONLY means that all group mappings of users are retrieved from LDAP and saved into LDAP. READ_ONLY is Read-only LDAP mode where group mappings are " +
                            ...
                            "they are saved to local passport DB.")
                    .type(ProviderConfigProperty.LIST_TYPE)
                    .options(MODES)
                    .defaultValue(mode)
                    .add();
        ...
        config.property().name(GroupMapperConfig.USER_ROLES_RETRIEVE_STRATEGY)
                ...
                .helpText("List of names of attributes divided by comma. This points to the list of attributes on LDAP group, which will be mapped as attributes of Group in Passport. " +
                        "Leave this empty if no additional group attributes are required to be mapped in Passport. ")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
                .property().name(GroupMapperConfig.DROP_NON_EXISTING_GROUPS_DURING_SYNC)
                .label("Drop non-existing groups during sync")
                .helpText("If this flag is true, then during sync of groups from LDAP to Passport, we will keep just those Passport groups, which still exists in LDAP. Rest will be deleted")
                .type(ProviderConfigProperty.BOOLEAN_TYPE)
                .defaultValue("false")
                .add()
                .property().name(GroupMapperConfig.LDAP_GROUPS_PATH)
                .label("Groups Path")
                .helpText("Passport group path the LDAP groups are added to. For example if value '/Applications/App1' is used, "
                		+ "then LDAP groups will be available in Passport under group 'App1', which is child of top level group 'Applications'. "
                		+ "The default value is '/' so LDAP groups will be mapped to the Passport groups at the top level. "
                		+ "The configured group path must already exists in the Passport when creating this mapper.")
                .type(ProviderConfigProperty.STRING_TYPE)
                .defaultValue("/")
                .add();
        return config.build();
    ...
    public String getHelpText() {
        return "Used to map group mappings of groups from some LDAP DN to Passport group mappings";
    }
    ...
    public Map<String, Object> getTypeMetadata() {
        ...
        metadata.put("fedToPassportSyncSupported", true);
        metadata.put("fedToPassportSyncMessage", "sync-ldap-groups-to-passport");
        metadata.put("passportToFedSyncSupported", true);
        metadata.put("passportToFedSyncMessage", "sync-passport-groups-to-ldap");
    ...
    public void validateConfiguration(PassportSession session, RealmModel realm, ComponentModel config) throws ComponentValidationException {
        ...
        if (!GroupMapperConfig.DEFAULT_LDAP_GROUPS_PATH.equals(group) && PassportModelUtils.findGroupByPath(session, realm, group) == null) {
