...
package org.passport.storage.ldap.mappers.msadlds;
...
public class MSADLDSUserAccountControlStorageMapperFactory extends AbstractLDAPStorageMapperFactory {
    ...
    public String getHelpText() {
        return "Mapper specific to MSAD LDS. It's able to integrate the MSAD LDS user account state into Passport account state (account enabled, password is expired etc). It's using msDS-UserAccountDisabled and pwdLastSet MSAD attributes for that. " +
                "For example if pwdLastSet is 0, the Passport user is required to update password, if msDS-UserAccountDisabled is 'TRUE' the Passport user is disabled as well etc. Mapper is also able to handle exception code from LDAP user authentication.";
    }
