...
public class GroupTreeResolver {
    ...
    /**
     * Fully resolves list of group trees to be used in Passport. The input is group info (usually from LDAP) where each "Group" object contains
     * just it's name and direct children.
     *
     * The operation also performs validation as rules for LDAP are less strict than for Passport (In LDAP, the recursion is possible and multiple parents of single group is also allowed)
     *
     * @param groups
    ...
    public List<GroupTreeEntry> resolveGroupTree(List<Group> groups, boolean ignoreMissingGroups) throws GroupTreeResolveException {
        ...
        for (Map.Entry<String, List<String>> group : parentsTree.entrySet()) {
            ...
            if (parentCount == 0) {
                ...
                throw new GroupTreeResolveException("Group '" + group.getKey() + "' detected to have multiple parents. This is not allowed in Passport. Parents are: " + group.getValue());
            }
        }
