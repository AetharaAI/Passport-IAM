...
public final class LDAPContextManager implements AutoCloseable {
    ...
    public LDAPContextManager(PassportSession session, LDAPConfig connectionProperties) {
        this.session = session;
        this.ldapConfig = connectionProperties;
    }

    public static LDAPContextManager create(PassportSession session, LDAPConfig connectionProperties) {
    ...
    public static Hashtable<Object, Object> getNonAuthConnectionProperties(LDAPConfig ldapConfig) {
        ...
        if (!ldapConfig.isStartTls() && LDAPUtil.shouldUseTruststoreSpi(ldapConfig)) {
            env.put("java.naming.ldap.factory.socket", "org.passport.truststore.SSLSocketFactory");
        }
