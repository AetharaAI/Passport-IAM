...
public class FullNameLDAPStorageMapperFactory extends AbstractLDAPStorageMapperFactory {
    ...
    private static List<ProviderConfigProperty> getConfigProps(ComponentModel parent) {
        ...
        return ProviderConfigurationBuilder.create()
                ...
                .property().name(FullNameLDAPStorageMapper.READ_ONLY)
                           .label("Read Only")
                           .helpText("For Read-only is data imported from LDAP to Passport DB, but it's not saved back to LDAP when user is updated in Passport.")
                           .type(ProviderConfigProperty.BOOLEAN_TYPE)
                           .defaultValue(String.valueOf(readOnly))
                .add()
                .property().name(FullNameLDAPStorageMapper.WRITE_ONLY)
                           .label("Write Only")
                           .helpText("For Write-only is data propagated to LDAP when user is created or updated in Passport. But this mapper is not used to propagate data from LDAP back into Passport. " +
                        "This setting is useful if you configured separate firstName and lastName attribute mappers and you want to use those to read attribute from LDAP into Passport")
                           .type(ProviderConfigProperty.BOOLEAN_TYPE)
                           .defaultValue(String.valueOf(!readOnly))
                            .add()
                           .build();
    ...
    public String getHelpText() {
        return "Used to map full-name of user from single attribute in LDAP (usually 'cn' attribute) to firstName and lastName attributes of UserModel in Passport DB";
    }
