...
public class KerberosFederationProvider implements UserStorageProvider,
    ...
    public KerberosFederationProvider(PassportSession session, UserStorageProviderModel model, KerberosFederationProviderFactory factory) {
    ...
    public boolean updateCredential(RealmModel realm, UserModel user, CredentialInput input) {
        ...
        if (kerberosConfig.getEditMode() == EditMode.READ_ONLY) {
            throw new ReadOnlyException("Can't change password in Passport database. Change password with your Kerberos server");
        }
        return false;
    }
    ...
    protected UserModel findOrCreateAuthenticatedUser(RealmModel realm, KerberosPrincipal kerberosPrincipal) {
        ...
        if (user != null) {
            ...
            logger.debug("Kerberos authenticated user " + kerberosPrincipal + " found in Passport storage");
        ...
        logger.debug("Kerberos authenticated user " + kerberosPrincipal + " not in Passport storage. Creating him");
        return importUserToPassport(realm, kerberosPrincipal);
    }

    protected UserModel importUserToPassport(RealmModel realm, KerberosPrincipal kerberosPrincipal) {
        ...
        logger.debugf("Creating kerberos user %s with username: %s, email: %s to local Passport storage", kerberosPrincipal, username, email);
