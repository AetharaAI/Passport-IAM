...
package org.passport.federation.sssd;
...
import org.passport.federation.sssd.api.Sssd;
import org.passport.federation.sssd.api.Sssd.User;
import org.passport.federation.sssd.impl.PAMAuthenticator;
...
public class SSSDFederationProvider implements UserStorageProvider,
    ...
    public SSSDFederationProvider(PassportSession session, UserStorageProviderModel model, SSSDFederationProviderFactory sssdFederationProviderFactory) {
    ...
    protected UserModel findOrCreateAuthenticatedUser(RealmModel realm, String username) {
        ...
        if (user != null) {
            logger.debug("SSSD authenticated user " + username + " found in Passport storage");
        ...
        logger.debug("SSSD authenticated user " + username + " not in Passport storage. Creating...");
        return importUserToPassport(realm, username);
    }

    protected UserModel importUserToPassport(RealmModel realm, String username) {
        ...
        logger.debugf("Creating SSSD user: %s to local Passport storage", username);
        ...
        for (String s : sssd.getGroups()) {
            GroupModel group = PassportModelUtils.findGroupByPath(session, realm, "/" + s);
            if (group == null) {
